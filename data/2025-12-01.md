...
TL;DR: 本文提出了一种新的无分类器引导方法——鞍点自由引导（SFG），它利用对数密度估计的正曲率来引导基于分数的生成模型，无需额外训练或标记数据，在无条件图像生成中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的引导方法如无分类器引导（CFG）需要标记数据并训练额外的无条件模型，而自动引导（Auto-Guidance）需要训练较小的模型。这些方法在缺乏标记数据或无法训练新模型时面临挑战。本文发现对数密度估计在鞍点区域的正曲率可以提供有效的引导。

Method: 提出了鞍点自由引导（SFG）方法，该方法维护对数密度的最大正曲率估计来引导单个基于分数的模型。SFG具有与无分类器引导相同的计算成本，不需要额外训练，可与现成的扩散模型和流匹配模型配合使用。

Result: SFG在无条件ImageNet-512生成中实现了最先进的FID和FD-DINOv2指标。当SFG与自动引导结合时，其无条件样本在FD-DINOv2得分上达到了总体最先进水平。在FLUX.1-dev和Stable Diffusion v3.5上的实验表明，SFG在保持优秀提示遵循和图像保真度的同时，提高了输出图像的多样性。

Conclusion: SFG是一种高效的无分类器引导方法，利用对数密度的正曲率特性，无需额外训练或标记数据，在多种生成任务中表现出色，为基于分数的生成模型提供了新的引导策略。
...
<details>
  <summary>Details</summary>
Motivation: 手工创建可动3D物体成本高且难以扩展，现有方法多为多阶段流程，需要统一表示来简化可动3D物体的生成。

Method: 提出统一潜在表示联合编码多个属性；引入可逆关节到体素嵌入，将运动特征与几何对齐；将关节类型预测作为开放集问题处理。

Result: 在PartNet-Mobility基准测试中达到最先进的网格质量和关节精度，能够泛化到新关节类别和未见物体类型。

Conclusion: UniArt提供了一种端到端的可动3D物体生成方法，通过统一表示和开放集关节预测，显著提升了生成质量和泛化能力。
...
TL;DR: 提出了一种灵活的多模态原型框架，整合全切片图像和不完整基因组学数据用于精准肿瘤学，通过生物原型、多视图对齐、二分融合和语义基因组学插补四个关键组件解决模态异质性和数据缺失问题。


<details>
  <summary>Details</summary>
Motivation: 组织学和基因组学的多模态整合在精准肿瘤学中具有巨大潜力，但表型和基因型异质性限制了模态内表示质量并阻碍了有效的模态间整合。此外，现有方法大多忽略了真实临床场景中基因组学数据可能部分缺失或完全不可用的情况。
...
Conclusion: 该框架有效解决了多模态整合中的异质性和数据缺失问题，为精准肿瘤学提供了灵活且强大的解决方案。
...
TL;DR: AmodalGen3D：一种从稀疏遮挡视图重建完整3D对象的生成框架，通过结合2D模态补全先验和多视角立体几何条件，实现遮挡区域的合理推断。


<details>
  <summary>Details</summary>
Motivation: 现实场景中，从少量无位姿、部分遮挡的视图重建3D对象是一个常见但具有挑战性的问题，传统多视角或修复方法在遮挡严重时难以产生完整且几何一致的3D重建。

Method: 提出AmodalGen3D生成框架，集成2D模态补全先验与多视角立体几何条件，包含视图间交叉注意力机制用于稀疏视图特征融合，以及立体条件交叉注意力模块用于未观测结构的推断。
...
TL;DR: 论文提出了TAPVid-360任务，要求预测视频序列中查询场景点的3D方向，即使这些点远在观察视频的狭窄视野之外，旨在促进学习全景场景表示。


<details>
  <summary>Details</summary>
Motivation: 人类能够构建周围环境的全景心理模型，保持对象恒常性并推断可见区域之外的场景结构。相比之下，当前的人工视觉系统在持久、全景理解方面存在困难，通常以自我为中心逐帧处理场景。这种限制在Track Any Point (TAP)任务中尤为明显，现有方法无法跟踪视野之外的2D点。

Method: 引入TAPVid-360任务，利用360度视频作为监督源，将其重新采样为窄视野视角，同时使用2D流程通过跟踪完整全景中的点来计算地面真实方向。提出了TAPVid360-10k数据集和基准，包含10k个带有地面真实方向点跟踪的视角视频。基线方法改编CoTracker v3来预测每点的旋转以进行方向更新。

Result: 提出的基线方法在TAPVid-360任务上超越了现有的TAP和TAPVid 3D方法，展示了在预测视野外点方向方面的有效性。

Conclusion: TAPVid-360任务能够促进学习全景场景表示，而无需动态4D地面真实场景模型进行训练。通过利用360度视频作为监督源，该方法为开发具有持久、全景理解能力的视觉系统提供了新途径。
...
Method: WalkCLIP通过多模态学习整合三种数据源：1) 从GPT-4o生成的图像描述中学习步行性感知的视觉-语言表示；2) 使用空间聚合模块融入邻域上下文；3) 与人口动态基础模型的特征进行融合。

Result: 在明尼阿波利斯-圣保罗的4,660个地点进行评估，WalkCLIP在预测准确性和空间对齐方面均优于单模态和多模态基线方法。

Conclusion: 视觉和行为信号的整合能够可靠地预测步行环境，多模态方法比单源方法更全面地捕捉城市步行性的复杂特征。
...
TL;DR: 本文对包含4000张内窥镜图像的新型胃肠道医学影像数据集进行了全面的比较模型分析，涵盖四种关键疾病类别。VGG16和MobileNetV2模型取得了96.5%的最佳测试准确率，同时采用Grad-CAM可视化增强临床可解释性。


<details>
  <summary>Details</summary>
Motivation: 该研究旨在解决内窥镜图像分析中的常见挑战，如可变光照、波动的摄像头角度和频繁的成像伪影，通过深度学习技术为胃肠道疾病分类建立可靠的基准和基线。

Method: 研究采用最先进的深度学习技术，在包含4000张内窥镜图像的数据集上进行比较分析，涵盖四种疾病类别。使用VGG16、MobileNetV2和Xception等模型，并集成Grad-CAM可视化技术提供可解释的AI分析。

Result: VGG16和MobileNetV2模型均取得了96.5%的最佳测试准确率，Xception模型达到94.24%的准确率。研究建立了强大的基准线，并通过Grad-CAM可视化成功识别了对模型预测最具影响力的图像区域。

Conclusion: 该工作展示了在复杂现实条件下进行稳健、准确且可解释的医学图像分析的潜力，为胃肠道计算机辅助诊断领域贡献了原始基准、比较见解和视觉解释，强调了多样化临床相关数据集和模型可解释性在医学AI研究中的重要性。
...
Method: 结合视觉语言模型生成3D物体并推断空间关系，构建层次化场景树，通过可微分刚体模拟器确保重力下的真实交互，采用模拟循环优化保证物理稳定性和非交错

Result: PAT3D在物理合理性、语义一致性和视觉质量方面显著优于现有方法，并能生成模拟就绪的3D场景，支持场景编辑和机器人操作等下游任务

Conclusion: PAT3D首次实现了物理增强的文本到3D场景生成，通过集成视觉语言模型与物理模拟，生成了物理合理、稳定且无交错的模拟就绪场景，为下游应用开辟了新可能
...
### [9] [Simultaneous Image Quality Improvement and Artefacts Correction in Accelerated MRI](https://arxiv.org/abs/2511.23274)
...
<details>
  <summary>Details</summary>
Motivation: MRI采集时间长，欠采样可加速但会降低图像质量；现有方法要么处理欠采样重建，要么校正伪影，无法同时解决这两个同时存在的退化因素

Method: 提出USArt模型，采用双子模型架构，专门针对笛卡尔采样的2D脑解剖图像，同时进行欠采样重建和噪声/运动伪影校正

Result: 显著提高信噪比和对比度，梯度欠采样策略效果最佳，实现5倍加速且同时校正伪影而无明显质量下降
...
TL;DR: 本文提出了一个用于指针仪表读数识别的大规模基准数据集RPM-10K和基于物理关系注入的视觉语言模型MRLM，通过显式编码指针与刻度间的几何和因果关系来提升读数精度。


<details>
  <summary>Details</summary>
Motivation: 智能电力系统中指针仪表的精确读数识别至关重要，但现有方法因反射、遮挡、动态视角以及指针与刻度标记过细等挑战而脆弱。该领域目前缺乏大规模数据集来支持鲁棒算法的发展。

Method: 首先构建了包含10730张仪表图像的大规模基准数据集RPM-10K。在此基础上，提出了基于物理关系注入的视觉语言模型MRLM，该模型显式编码指针与刻度间的几何和因果关系，通过跨注意力融合和自适应专家选择来学习解释仪表配置并生成精确数值读数。

Result: 在新提出的基准数据集上进行了大量实验，充分验证了所提出框架的有效性。数据集和源代码将在GitHub上发布。

Conclusion: 本文通过构建大规模数据集和提出基于物理关系注入的视觉语言模型，有效解决了指针仪表读数识别中的关键挑战，为智能电力系统中的精确仪表读数识别提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在对话能力上有进步，但缺乏实时交互式逐步指导能力，无法检测指令执行成功与否、识别用户错误并及时提醒，这是未来AI助手的关键能力。

Method: 基于CaptainCook4D数据集构建了Qualcomm Interactive Cooking基准测试和数据集，包含用户执行任务时的错误记录；提出了LiveMamba模型，这是一个为交互式教学指导设计的流式多模态LLM。

Result: 评估了现有最先进的多模态LLM在Qualcomm Interactive Cooking基准上的表现，并展示了LiveMamba模型作为强基线模型的效果。
...
TL;DR: 提出针对Rectified Flow模型的全面加速管道，通过新方法实现611%的图像生成加速，远超现有方法的18%加速效果


<details>
  <summary>Details</summary>
Motivation: Rectified Flow和Flow Matching等新技术显著提升了生成模型的性能，但由于理论、设计等方面与现有扩散模型的差异，现有加速方法无法直接应用于Rectified Flow模型

Method: 从理论、设计和推理策略三个方面构建全面加速管道，包括：使用新速度场的批处理方法、异构时间步批处理的向量化、以及针对新方法的动态TensorRT编译

Result: 实验证明新方法可将512*512图像生成速度加速至611%，远超现有公开方法通常只能达到的18%加速效果
...
<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习的视觉语言模型在医学诊断中存在局限性，它们倾向于强化表面连贯但临床不准确的推理路径，缺乏对临床医生渐进式视觉聚焦和迭代推理过程的模拟。

Method: 提出MedEyes框架，包含：1) Gaze-guided Reasoning Navigator (GRN)模拟诊断过程，采用双模式探索策略；2) Confidence Value Sampler (CVS)平衡专家模仿和自主发现；3) 双流GRPO优化框架分离策略内和策略外学习信号。

Result: 在多个医学VQA基准测试中平均性能提升+8.5%，验证了MedEyes在构建可解释医学AI系统方面的潜力。

Conclusion: MedEyes通过动态建模临床医生风格的诊断推理过程，结合策略外专家指导，显著提升了医学视觉问答的性能和临床对齐性，为构建可解释的医学AI系统提供了有效途径。
...
TL;DR: 提出一种无需训练的后处理不确定性估计方法，用于检测对比式视觉语言模型的错误预测，通过测量类内视觉特征一致性来实现


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（如CLIP）在开放词汇分类中表现良好，但对错误预测会给出高置信度分数，这在安全关键应用中限制了其可靠性

Method: 使用特征投影结合多元高斯分布创建类特定概率嵌入，测量类内视觉特征一致性，无需微调，对分布偏移具有鲁棒性

Result: 在ImageNet、Flowers102、Food101、EuroSAT和DTD数据集上展示了最先进的错误检测性能，显著优于确定性和概率性VLM基线

Conclusion: 提出了一种VLM无关、无需训练的不确定性估计方法，能有效检测错误预测，即使在每类只有10个训练图像的情况下也能有效工作
...
TL;DR: 该研究探索直接从音频进行物体定位的可能性，绕过传统的语音转文本中间步骤，在某些情况下甚至优于基于转录的方法。


<details>
  <summary>Details</summary>
Motivation: 当前基于转录的物体定位方法存在效率和鲁棒性问题，研究者质疑这种依赖文本中间表示的管道，希望探索直接音频-视觉对齐的可能性。

Method: 简化任务为基于单词语音指令的定位，引入新的音频定位数据集涵盖多种物体和口音，适配并基准测试多种音频-视觉模型。

Result: 直接从音频进行定位不仅可行，在某些情况下甚至优于基于转录的方法，特别是在应对语言多样性方面表现出更强的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的无监督域自适应方法过于复杂，依赖于具有挑战性的对抗训练策略或复杂的架构设计，需要辅助模型进行特征蒸馏和伪标签生成。作者希望开发一种更简单实用的方法。

Method: 在频域中学习适应图像风格以减少源域和目标域之间的差异。方法仅引入轻量级预处理模块进行训练，在推理时完全丢弃该模块，不产生额外计算开销。该方法特别适用于域自适应目标检测任务。

Result: 在多个域自适应目标检测基准测试中进行了广泛实验，证明了该方法能够实现显著的性能提升，突显了其实用性和有效性。

Conclusion: 提出了一种简单而有效的无监督域自适应方法，通过频域图像风格适应来减少域差异，具有轻量级、无额外推理开销的优点，在多个基准测试中表现出色。
...
TL;DR: 提出了一种新颖的轨迹条件3D场景占用预测架构，通过端到端方式直接从原始图像特征预测多帧未来占用，避免了离散token化和BEV投影的限制，在nuScenes基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖变分自编码器生成离散占用token，限制了表示能力；同时基于BEV投影的方法需要显式几何先验。需要一种更直接、更有效的时空依赖建模方法。

Method: 采用稀疏占用表示，绕过BEV投影和显式几何先验，直接从原始图像特征端到端预测多帧未来占用。基于Transformer架构，受GPT和VGGT等基础模型的启发，有效捕捉时空依赖关系。

Result: 在nuScenes基准的1-3秒占用预测任务中取得最先进性能，显著优于现有方法。展示了强大的场景动态理解能力，在任意未来轨迹条件下保持高精度。

Conclusion: 提出的端到端Transformer架构通过避免离散token化和BEV表示的限制，实现了更有效的时空依赖建模，为轨迹条件3D场景占用预测提供了新的解决方案。
...
TL;DR: 本文提出图像条件流形正则化(ICM)方法，通过使用颜色图和Canny边缘的结构信息作为条件，解决真实世界图像超分辨率任务中传统文本条件流形正则化的概念错位和生成先验缺陷问题。


<details>
  <summary>Details</summary>
Motivation: 现有真实世界图像超分辨率方法通常使用文本条件扩散模型的生成先验进行正则化，但这存在两个关键问题：概念上与任务不匹配（超分辨率应生成与低质量图像直接相关的高质量图像），实践中教师模型常产生颜色失真和边缘模糊的图像，表明其生成先验对此任务存在缺陷。

Method: 提出图像条件流形正则化(ICM)方法，通过将输出正则化到基于稀疏但关键结构信息（颜色图和Canny边缘的组合）的条件流形上，避免直接使用原始输入图像的高信息密度导致数值不稳定性，提供任务对齐且稳定的正则化信号。

Result: 实验证实提出的正则化方法显著提升了超分辨率性能，特别是在感知质量方面，证明了其在真实世界应用中的有效性。

Conclusion: ICM方法通过使用图像结构信息作为条件，解决了传统文本条件流形正则化在真实世界图像超分辨率任务中的概念错位和数值不稳定性问题，显著提升了超分辨率质量。
...
TL;DR: OralGPT-Omni是首个专门用于牙科的多模态大语言模型，通过TRACE-CoT数据集和四阶段训练范式，在牙科影像分析和临床任务中表现出色，显著超越GPT-5等通用模型。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在医学多个领域展现出巨大潜力，但牙科领域仍未被充分探索，主要原因是缺乏领域特定数据、牙科专家标注稀缺、模态特定建模不足以及可靠性挑战。

Method: 1. 构建TRACE-CoT数据集，模仿牙科放射科医生的诊断推理过程；2. 提出四阶段训练范式；3. 开发OralGPT-Omni模型；4. 创建MMOral-Uni基准测试，包含2,809个开放性问题-答案对，涵盖5种模态和5种任务。

Result: OralGPT-Omni在MMOral-Uni基准测试中获得51.84分，在MMOral-OPG基准测试中获得45.31分，显著超越GPT-5等现有模型。

Conclusion: OralGPT-Omni是首个牙科专业的多模态大语言模型，通过临床推理监督和系统训练，显著提升了牙科影像理解和分析能力，推动了智能牙科的发展，为未来牙科影像分析奠定了基础。
...
### [20] [GazeTrack: High-Precision Eye Tracking Based on Regularization and Spatial Computing](https://arxiv.org/abs/2511.22607)
*Xiaoyin Yang*

Main category: cs.CV

TL;DR: 提出GazeTrack高精度眼动追踪数据集和新方法，通过形状误差正则化提升瞳孔定位精度，采用类似纸张展开的坐标变换方法预测注视向量，最终构建低计算复杂度的注视向量生成模型。
...
Result: GazeTrack成为首个包含多种族、年龄和视力条件的高精度眼动数据集；提出的方法在瞳孔定位和注视向量预测方面取得更高精度，且计算复杂度低于其他方法。

Conclusion: 通过高质量数据集和创新的形状误差正则化、坐标变换方法，显著提升了眼动追踪的精度，为虚拟/增强现实应用提供了更可靠的注视追踪解决方案。
...
TL;DR: WorldWander是一个基于上下文学习的视频生成框架，专门用于在第一人称（自我中心）和第三人称（外部中心）视角之间进行视频转换，通过创新的视角对齐和位置编码技术实现跨视角同步。


<details>
  <summary>Details</summary>
Motivation: 虽然视频扩散模型在真实性和可控性方面取得了显著进展，但在不同视角（如第一人称和第三人称）之间实现无缝视频转换的研究仍然不足。这种视角转换对于电影制作、具身AI和世界模型至关重要。

Method: 基于先进的视频扩散变换器，WorldWander集成了两个关键技术：(1) 上下文视角对齐，用于有效建模跨视角同步；(2) 协作位置编码。此外，还构建了EgoExo-8K数据集，包含来自合成和真实场景的同步第一人称-第三人称三元组数据。

Result: 实验表明，WorldWander在视角同步、角色一致性和泛化能力方面表现优异，为第一人称-第三人称视频转换设定了新的基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注点级别的图像-基因匹配，但未能充分利用ST数据的完整层次结构，特别是在基因表达方面，导致图像-基因对齐不完整。同时，存在固有的信息不对称性：基因表达谱包含更多分子细节，可能在组织学图像中缺乏明显的视觉对应物，需要复杂的表示学习方法来弥合这种模态差距。

Method: 1. 设计多级表示提取器，从每个模态捕获点级别和生态位级别的表示，提供超出单个点级别图像-基因对的上下文感知信息。2. 引入分层双曲对齐模块，统一这些表示，在执行空间对齐的同时分层结构化图像和基因嵌入，丰富图像表示的分子语义。

Result: HyperST在来自不同组织的四个公共数据集上实现了最先进的性能，为更可扩展和准确的空间转录组学预测铺平了道路。

Conclusion: 通过在双曲空间中建模数据的固有层次结构，HyperST能够学习多层次的图像-基因表示，有效弥合组织学图像和基因表达之间的模态差距，显著提高跨模态预测能力。
...
TL;DR: 提出一种耦合提示学习框架，通过引入外部监督提示来缓解视觉语言模型在视频任务微调中的语义空间收缩问题，提升泛化性能。


<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在视频任务微调过程中出现的泛化能力下降问题。现有方法通过正则化手工提示与软提示之间的差距来缓解遗忘效应，但这会削弱软提示的学习能力。

Method: 提出插拔式耦合提示学习框架：1）文本提示方面，引入其他数据集的预训练提示作为硬提示标记，与软提示标记拼接并通过可学习映射层耦合，形成竞争性提示机制；2）引入精心设计的无关视频集和负提示作为通用属性锚点，保持预训练语义空间中属性的通用相关性。
...
TL;DR: DualVLA通过双重数据剪枝和双教师自适应蒸馏策略，解决了通用视觉-语言-动作模型中动作性能退化问题，在保持推理能力的同时提升动作执行精度。


<details>
  <summary>Details</summary>
Motivation: 现有的通用视觉-语言-动作模型在整合机器人演示数据和多模态数据以恢复推理能力时，往往会出现动作性能相比专家模型退化的现象，即"动作退化"问题。

Method: 提出DualVLA框架：1）双重数据剪枝方法去除冗余的具身推理数据，防止其对动作学习产生负面影响；2）双教师自适应蒸馏策略，为不同数据域分配不同的监督信号，在保持推理能力的同时增强动作生成。

Result: DualVLA在SimplerEnv中达到61.0%的平均成功率，在八个竞争性多模态基准测试中获得65.4的平均分，展示了在精确动作执行和多模态理解之间更强的平衡能力。

Conclusion: DualVLA通过精心设计的后训练策略有效解决了通用VLA模型中的动作退化问题，实现了动作性能和推理能力的更好平衡，同时提出了VLA Score评估框架来更细粒度地评估VLA能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的手语生成方法过于关注语义准确性而忽视了情感表达，导致生成的手语视频缺乏自然性和表现力，无法充分传达交流中的情感信息。

Method: 提出EASL（情感感知手语）多情感引导生成架构，包含情感-语义解耦模块和渐进式训练，分别提取语义和情感特征；在姿态解码阶段，情感表示引导语义交互，生成带有7类情感置信度评分的手语姿态，实现情感表达识别。

Result: 实验结果表明，EASL通过整合多情感信息，在手语姿态准确性上优于所有基线方法，并能有效适应扩散模型生成富有表现力的手语视频。

Conclusion: EASL成功解决了现有手语生成方法中情感表达不足的问题，通过情感-语义解耦和渐进式训练实现了更自然、富有表现力的手语生成，为聋人社区提供了更高质量的交流工具。
...
TL;DR: 本文提出了一种基于堆叠集成模型的膝关节骨关节炎自动分级系统，使用微调CNN模型进行二元分类（检测KOA存在）和多类分类（KL分级），在测试集上分别达到87.5%和73%的平衡准确率。


<details>
  <summary>Details</summary>
Motivation: 膝关节骨关节炎（KOA）的诊断通常依赖X光图像和Kellgren-Lawrence分级系统，这种方法需要专家经验、耗时且存在主观性，可能导致诊断不准确。因此需要开发自动化的、客观的KOA检测和分级系统。

Method: 开发了一个堆叠集成模型，包含MobileNetV2、YOLOv8和DenseNet201作为基础学习器，以及Categorical Boosting（CatBoost）作为元学习器。该模型执行两个分类任务：二元分类（检测KOA存在）和多类分类（精确的KL分级）。

Result: 提出的堆叠集成模型在多类分类任务中达到73%的平衡测试准确率，在二元分类任务中达到87.5%的平衡测试准确率，这些结果优于现有文献中的先前工作。

Conclusion: 该堆叠集成模型能够有效自动化膝关节骨关节炎的检测和分级，减少对专家主观判断的依赖，提高诊断效率和准确性，为临床实践提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射（3DGS）作为主流3D重建技术，已被广泛应用，但近期研究发现其存在关键漏洞，计算成本攻击会导致恶意资源占用甚至拒绝服务，阻碍3DGS的可靠部署。

Method: 提出RemedyGS防御框架，包含两个关键组件：1）检测器识别带有中毒纹理的攻击输入图像；2）净化器从受攻击图像中恢复良性图像，减轻攻击影响。此外，在净化器中加入对抗训练，强制恢复图像与原始自然图像之间的分布对齐。

Result: 实验结果表明，该框架能有效防御3DGS系统中的白盒、黑盒和自适应攻击，在安全性和实用性方面均达到最先进性能。

Conclusion: RemedyGS是首个有效且全面的黑盒防御框架，能够保护3DGS重建系统和服务免受计算成本攻击的影响。
...
<details>
  <summary>Details</summary>
Motivation: 现有说话人脸生成方法依赖显式光流和局部扭曲，无法建模复杂全局运动并导致身份漂移问题，需要更有效的方法来保持身份一致性并处理复杂运动。

Method: 1. 用跨注意力机制替代传统流基扭曲，在统一潜在空间中隐式建模运动差异和身份对齐；2. 引入身份自适应模块，将运动潜在向量投影到个性化空间，确保运动与身份的清晰解耦；3. 使用轻量级流匹配运动生成器从音频、姿态和注视线索生成生动可控的隐式运动向量。

Result: IMTalker在运动准确性、身份保持和音频-唇部同步方面超越先前方法，达到最先进质量，在RTX 4090 GPU上实现视频驱动40 FPS和音频驱动42 FPS的卓越效率。

Conclusion: IMTalker通过隐式运动转移框架有效解决了说话人脸生成中的身份漂移和复杂运动建模问题，在保持高效率的同时实现了高质量生成，为相关应用和未来研究提供了有力工具。
...
TL;DR: 该研究针对东亚地区复杂地形下PM浓度长期预报的挑战，构建了实时观测数据集并提出了基于操作成本对齐的强化学习框架，显著降低了误报率。


<details>
  <summary>Details</summary>
Motivation: 东亚地区复杂地形和强大气动力学使得PM浓度长期预报困难，现有基础模型缺乏区域特异性且依赖非实时输入，无法满足公共卫生预警系统的实际需求。同时，标准点对点目标函数无法反映操作成本的不对称性（误报损害公众信任，漏报危及人群健康）。

Method: 1) 构建并发布了东亚地区的实时观测和高分辨率CMAQ-OBS数据集；2) 提出了Group-Relative Policy Optimization (GRPO)框架，包含类别奖励和课程式rollout，使预测与操作优先级对齐。

Result: 1) 新数据集将区域误差降低了59.5%，实现了对公共卫生预警至关重要的48-120小时实时预报；2) GRPO框架相比仅使用SFT的基线，将误报率降低了47.3%，同时保持了有竞争力的F1分数。

Conclusion: 该研究通过构建区域特定数据集和引入操作成本对齐的强化学习框架，显著提高了东亚地区PM浓度长期预报的可靠性，为实际空气质量预报系统在长提前期场景中的有效应用提供了解决方案。
...
TL;DR: PS-CBM提出部分共享概念瓶颈模型，通过多模态概念生成器、部分共享概念策略和概念效率准确率指标，解决现有CBM的视觉基础差、概念冗余和缺乏平衡指标三大挑战，在11个数据集上实现更高准确率和概念效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM/VLM的自动概念生成方法面临三个核心挑战：视觉基础差（概念与视觉特征关联弱）、概念冗余（生成过多相似概念）、缺乏平衡预测准确率和概念简洁性的原则性指标。

Method: 提出PS-CBM框架，包含三个核心组件：1) 多模态概念生成器，整合LLM语义和基于示例的视觉线索；2) 部分共享概念策略，基于激活模式合并概念以平衡特异性和简洁性；3) 概念效率准确率(CEA)指标，联合捕捉预测准确率和概念简洁性。

Result: 在11个多样化数据集上的实验表明，PS-CBM持续优于最先进的CBM方法，分类准确率提升1.0%-7.4%，CEA提升2.0%-9.5%，同时所需概念数量显著减少。

Conclusion: PS-CBM通过解决现有CBM的三大挑战，有效实现了高准确率和强可解释性的平衡，为概念瓶颈模型的发展提供了新方向。
...
TL;DR: GRiP是一个两阶段训练框架，通过引导模型的感知焦点和逻辑路径，实现稳健灵活的视觉基础推理，在多个挑战性基准测试中取得最先进结果。
...
Result: 基于Qwen2.5-VL-7B模型初始化的GRiP在多个挑战性基准测试中表现显著提升，在TreeBench和V* Bench上达到开源模型中的最先进水平，证明了其在复杂视觉推理中的有效性。
...
TL;DR: MTR-VP：一种基于视觉的自动驾驶轨迹规划方法，使用ViT编码器从原始图像和过去运动状态学习上下文嵌入，替代传统地图特征，在Waymo数据集上评估表明多轨迹预测优于单轨迹预测


<details>
  <summary>Details</summary>
Motivation: 传统轨迹规划方法依赖地图特征，本文旨在探索使用学习到的视觉表示替代地图特征的可能性，将图像上下文与运动预测框架对齐，实现基于视觉的轨迹规划

Method: 提出MTR-VP方法：使用ViT编码器处理原始图像和过去运动状态，生成上下文嵌入；借鉴MTR编码器思想，用视觉表示替代地图特征；在解码器中用交叉注意力机制处理意图和上下文嵌入，而非可学习的意图查询

Result: 在Waymo端到端驾驶数据集上评估，发现基于Transformer的方法在结合视觉特征和运动特征方面效果有限，即使使用CLIP和DINOv2等基础模型增强意图嵌入；但预测多个未来轨迹分布比单轨迹预测能显著提升规划性能

Conclusion: 视觉特征与运动特征的Transformer融合效果不佳，但多轨迹预测策略对规划性能有显著提升，为基于视觉的自动驾驶轨迹规划提供了重要启示
...
<details>
  <summary>Details</summary>
Motivation: 足部接触对人类与世界的互动至关重要，但现有方法通常使用零速度约束近似接触，只关注关节级接触，无法捕捉足部与世界的详细交互。从单张RGB图像预测密集足部接触仍未被充分探索，面临鞋款外观多样性和地面外观单调两大挑战。
...
TL;DR: 本文提出两种创新方法改进3D对象生成：使用现成图像适配器避免文本反转训练，以及深度条件预热策略增强3D一致性，在保持生成质量的同时提高控制能力和效率。


<details>
  <summary>Details</summary>
Motivation: 当前3D对象生成主要依赖文本到图像扩散模型配合文本反转技术，但这种方法需要额外训练时间且缺乏控制能力。为了解决这些问题，作者提出了更高效的替代方案。

Method: 提出两种创新方法：1) 使用现成的图像适配器，无需文本反转即可生成3D对象，提供对深度、姿态和文本等条件的增强控制；2) 深度条件预热策略，用于增强3D一致性。

Result: 实验结果显示，该方法在定性和定量上都与现有基于文本反转的方法性能相当，同时改善了3D一致性。用户研究表明，该模型在匹配输入图像和保持3D一致性方面优于现有替代方案。

Conclusion: 提出的方法通过避免文本反转训练和引入深度条件预热策略，在保持生成质量的同时提高了3D对象生成的效率和可控性，为3D生成任务提供了更优的解决方案。
...
TL;DR: 提出一种无需训练的多视图一致性扩散框架，用于3D场景编辑，通过一致性损失确保不同视角的编辑结果在几何和光度上保持一致。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的图像编辑方法在多视图场景中会产生几何和光度不一致的问题，这对于3D表示（如NeRF或高斯泼溅模型）的编辑尤为严重。

Method: 提出训练免费的扩散框架，假设未编辑图像中的对应点在编辑后应经历相似的变换，通过引入一致性损失来引导扩散采样实现一致的编辑。

Result: 实验结果表明，该方法相比现有多视图编辑方法显著提高了3D一致性，并能实现高质量的高斯泼溅编辑，具有清晰的细节和与文本提示的强保真度。

Conclusion: 该框架灵活且无需训练，可与多种图像编辑方法结合，支持密集和稀疏多视图编辑设置，显著改善了3D场景编辑的一致性。
...
TL;DR: M3LLM：利用生物医学文献中的复合图像开发医疗多图像多模态大语言模型，解决临床多图像分析的数据瓶颈
...
Method: 提出利用生物医学文献中许可允许的复合图像作为数据源，设计五阶段上下文感知指令生成范式，采用分治策略将多图像分析分解为可管理子任务。解析超过23.7万张复合图像及其上下文文本进行指令生成。

Result: 开发出M3LLM模型，在PMC-MI-Bench基准测试中显著优于通用和专业医疗MLLM，在单图像、纯文本和多选场景也表现优异。在MIMIC数据集上展示了对纵向胸部X光分析的强大泛化能力。

Conclusion: 该工作建立了可扩展、高效的医疗MLLM开发范式，使模型能够进行复合推理，弥合生物医学文献与真实临床应用之间的差距。
...
TL;DR: IE-SRGS提出了一种新的3D高斯溅射超分辨率方法，通过结合外部2D超分辨率先验和内部3DGS特征来解决现有方法中的3D高斯模糊问题，实现高保真重建。
...
Method: 提出IE-SRGS范式，联合利用外部2D超分辨率先验和内部3DGS特征的互补优势。使用2D超分辨率和深度估计模型生成高分辨率图像和深度图作为外部知识，采用多尺度3DGS模型生成跨视图一致、领域自适应的对应物作为内部知识，引入掩码引导融合策略整合两者。

Result: 在合成和真实世界基准测试上的广泛实验表明，IE-SRGS在定量准确性和视觉保真度方面始终优于最先进的方法。

Conclusion: IE-SRGS通过有效结合外部和内部知识，解决了3D高斯溅射超分辨率中的模糊问题，实现了高质量的重建效果。
...
TL;DR: VessQC是一个开源工具，通过整合不确定性地图指导用户修正3D显微镜图像分割错误，显著提高错误检测召回率。
...
Method: 开发了VessQC工具，整合不确定性地图来引导用户关注最可能包含生物学意义错误的区域，实现不确定性指导的3D分割修正。

Result: 初步用户研究表明，不确定性指导的修正将错误检测召回率从67%显著提高到94.0%（p=0.007），而总修正时间没有显著增加。

Conclusion: VessQC实现了高效的人机协同体积分割精修，填补了不确定性估计与实际人机交互之间的关键空白，为3D生物图像分析提供了实用工具。
...
### [39] [RobotSeg: A Model and Dataset for Segmenting Robots in Image and Video](https://arxiv.org/abs/2511.22950)
...
<details>
  <summary>Details</summary>
Motivation: 准确的机器人分割是机器人感知的基础能力，对VLA系统的精确视觉伺服、可扩展的机器人中心数据增强、准确的真实到仿真转换以及动态人机环境中的可靠安全监控都至关重要。尽管现代分割模型能力强大，但由于机器人形态多样性、外观模糊性、结构复杂性和快速形状变化，机器人分割仍然具有挑战性。

Method: 基于SAM 2基础模型构建RobotSeg，通过三个创新解决机器人分割的局限性：1) 结构增强记忆关联器，适应关节式机器人；2) 机器人提示生成器，减少对人工提示的依赖；3) 标签高效训练策略，减少每帧训练掩码标注需求。还构建了包含2.8k视频（138k帧）的视频机器人分割数据集。

Result: 广泛实验表明，RobotSeg在图像和视频上都实现了最先进的性能，为机器人感知的未来发展建立了坚实基础。

Conclusion: RobotSeg通过解决SAM 2在机器人分割上的三个关键局限性，提供了一个结构感知、自动化和标签高效的解决方案，在机器人分割任务上取得了突破性进展。
...
Method: 引入对抗扰动使SAM无法"看见"图像内容，将图像转化为神经模型视角的空白画布；采用频率感知优化策略增强对SAM的欺骗效果

Result: 大量实验结果表明该方法在篡改定位方面具有有效性

Conclusion: 通过使SAM"失明"的新策略，能够有效检测图像篡改区域，为图像伪造检测提供了创新思路
...
### [41] [MrGS: Multi-modal Radiance Fields with 3D Gaussian Splatting for RGB-Thermal Novel View Synthesis](https://arxiv.org/abs/2511.22997)
*Minseong Kweon,Janghyun Kim,Ukcheol Shin,Jinsun Park*

Main category: cs.CV

TL;DR: 提出MrGS，一种基于3D高斯泼溅的多模态辐射场，同时重建RGB和热红外3D场景，通过正交特征提取和物理原理建模热传导与辐射特性。


<details>
  <summary>Details</summary>
Motivation: 现有神经辐射场和3D高斯泼溅方法主要关注RGB场景重建，而结合热红外图像的多模态渲染研究不足，且往往忽略了热传导和朗伯特性等独特的热特性。

Method: 1) 通过正交特征提取从单一外观特征中分离RGB和热相关信息；2) 根据各模态的朗伯反射程度采用视图相关或独立的嵌入策略；3) 集成傅里叶热传导定律建模高斯元间的热传导；4) 应用斯特藩-玻尔兹曼定律和平方反比定律构建深度感知热辐射图。

Result: 实验结果表明，MrGS实现了高保真度的RGB-T场景重建，同时减少了所需的高斯元数量。

Conclusion: MrGS成功地将物理原理融入多模态辐射场，有效建模热红外特性，为RGB-T场景重建提供了高效解决方案。
...
TL;DR: TTSnap框架通过噪声感知剪枝在测试时扩展中高效筛选低质量候选图像，避免完全去噪的计算开销，提升搜索效率。


<details>
  <summary>Details</summary>
Motivation: 现有测试时扩展方法需要完全去噪每个噪声种子才能计算奖励函数，计算成本高昂，限制了在固定预算下可探索的样本数量。

Method: 提出TTSnap框架：1) 通过自蒸馏训练噪声感知奖励模型，使中间估计的奖励与最终干净图像的奖励对齐；2) 采用课程学习策略稳定不同噪声水平下的学习；3) 引入奖励对齐和计算预算利用的新度量标准。

Result: 实验表明，该方法相比现有方法性能提升超过16%，实现了更高效和有效的测试时扩展，并能与训练后技术和局部测试时优化技术正交结合获得额外增益。

Conclusion: TTSnap通过噪声感知剪枝显著提高了文本到图像扩散模型测试时扩展的效率，在有限计算预算下实现了更好的性能表现。
...
### [43] [SimScale: Learning to Drive via Real-World Simulation at Scale](https://arxiv.org/abs/2511.23369)
...
Method: 提出SimScale框架：1) 使用先进神经渲染和反应式环境生成高保真多视角观测，通过扰动自车轨迹控制；2) 开发伪专家轨迹生成机制为新模拟状态提供动作监督；3) 采用真实与模拟数据的简单协同训练策略。

Result: 在挑战性真实世界基准测试中，各种规划方法在鲁棒性和泛化能力上获得显著提升：navhard上提升+6.8 EPDMS，navtest上提升+2.9。仅增加模拟数据（无需额外真实数据）即可平滑扩展策略改进。

Conclusion: SimScale框架通过大规模模拟数据生成有效补充真实世界数据不足，显著提升自动驾驶规划性能，揭示了伪专家设计和不同策略架构的扩展特性等重要发现。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像扩散模型在个性化方面存在挑战：当模型专注于主体保真度时，容易对少量参考图像过拟合；而强调先验保持时，又难以学习新的个性化属性。需要一种平衡主体保真度和语义一致性的方法。

Method: 提出语义锚定策略，将个性化重新定义为通过语义锚定在频繁对应概念的引导下学习稀有概念的过程。该方法将新概念锚定到其对应的预训练分布中，以稳定可控的方式扩展预训练分布。

Result: 与基线方法相比，该方法实现了稳定的适应，并在主体保真度和文本-图像对齐方面都取得了显著改进。大量实验和消融研究证明了该锚定策略的鲁棒性和有效性。

Conclusion: 语义锚定策略能够有效解决扩散模型个性化中的平衡问题，通过将新概念锚定到预训练语义结构中，实现了稳定适应和性能提升。
...
TL;DR: UMind-VL是一个统一的超声医学基础模型，能够同时处理像素级结构理解（分割、定位）和高级临床推理（诊断），在1.2百万超声图像-文本对数据集上训练，性能优于现有通用模型，与专业模型相当或更优。


<details>
  <summary>Details</summary>
Motivation: 超声领域缺乏能够同时处理低级别超声感知（如分割、定位）和高级别超声综合解释（如诊断、推理）的全面解决方案，现有模型无法有效结合像素级结构理解和复杂临床推理。
...
Result: UMind-VL显著优于现有通用多模态模型，在分割、检测、关键点定位和诊断推理基准测试中达到或超过最先进专业模型的性能，同时保持强大的泛化能力。

Conclusion: UMind-VL成功统一了超声领域的像素级结构理解和高级临床推理，为超声医学提供了一个全面的基础模型解决方案，填补了该领域的空白。
...
TL;DR: 该论文提出了GSPure，这是首个专门针对3D高斯泼溅水印表示的净化框架，通过分析视图相关渲染贡献和几何特征聚类，有效分离并去除水印相关的高斯基元，在保持场景完整性的同时显著降低水印PSNR。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅水印方案存在漏洞，传统2D图像水印去除技术无法有效推广到3DGS场景，因为3DGS具有专门的渲染管道和独特的高斯基元属性。需要专门针对3DGS水印表示的净化框架来保护3D资产。

Method: 提出GSPure框架，通过分析视图相关渲染贡献和利用几何精确的特征聚类，精确分离并有效去除水印相关的高斯基元，同时保持场景完整性。

Result: GSPure实现了最佳的水印净化性能，将水印PSNR降低高达16.34dB，同时原始场景保真度退化小于1dB PSNR损失。在有效性和泛化性方面均优于现有方法。

Conclusion: 该研究首次系统探索并验证了3DGS水印框架的漏洞，提出的GSPure框架能够有效保护3D资产，为3D高斯泼溅水印保护提供了新的解决方案。
...
TL;DR: DriveVGGT是一个专门为自动驾驶数据设计的尺度感知4D重建框架，通过引入时间视频注意力模块和多摄像头一致性注意力模块，有效利用自动驾驶系统的先验知识，在自动驾驶数据集上优于现有VGGT方法。


<details>
  <summary>Details</summary>
Motivation: 现有的前馈重建方法（如VGGT）直接应用于自动驾驶系统时效果不佳，因为自动驾驶系统具有独特的先验知识：摄像头视野重叠最小、相机内外参数已知、所有摄像头相对位置固定。需要专门设计框架来充分利用这些先验。

Method: 提出DriveVGGT框架，包含三个核心设计：1) 时间视频注意力模块独立处理多摄像头视频，利用单摄像头序列的时空连续性；2) 多摄像头一致性注意力模块使用归一化相对位姿嵌入进行窗口注意力，建立跨摄像头一致性关系；3) 扩展标准VGGT头，增加绝对尺度头和自车姿态头。

Result: 实验表明DriveVGGT在自动驾驶数据集上优于VGGT、StreamVGGT和fastVGGT，广泛的消融研究验证了所提设计的有效性。

Conclusion: 通过专门针对自动驾驶系统先验知识设计的DriveVGGT框架，能够更好地处理自动驾驶数据，实现尺度感知的4D重建，为自动驾驶系统提供了更优的视觉重建解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 观察到图像中某些补丁的实现会降低其他补丁的不确定性，类似于量子力学中的波函数坍缩现象，希望利用这种"补丁坍缩"特性来提高视觉任务的效率

Method: 1. 学习一个自编码器，软选择子集补丁来重建每个目标补丁；2. 为每个补丁的依赖关系图计算PageRank分数；3. 基于PageRank确定最优补丁实现顺序；4. 将该顺序应用于掩码图像建模方法

Result: 1. 提升最先进自回归模型MAR的性能；2. 在图像分类中，仅暴露22%的高排名补丁即可达到高准确率；3. 验证了补丁坍缩顺序对各种掩码图像建模方法的益处

Conclusion: 补丁坍缩为图像建模提供了新视角，通过识别和利用补丁间的依赖关系，能够显著提升视觉任务的效率，促进高效视觉系统的发展
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要处理单个图像或密集采样的视频，缺乏对无结构图像集合（共享视觉元素但视角、拍摄时间和周围内容不同的图像集）进行一致控制生成的能力。

Method: 将任务建模为图结构，每个节点对应一幅图像，每条边触发图像对的联合生成。通过融合跨图像对的内部特征，在密集输入对应关系的指导下，确保局部一致性和全局连贯性，无需掩码或人工监督。

Result: Match-and-Fuse在一致性和视觉质量方面达到最先进水平，能够从图像集合中创建新内容，解锁了内容创作的新能力。

Conclusion: 该方法通过图建模和特征融合实现了无结构图像集合的一致控制生成，为基于图像集合的内容创作提供了新范式。
...
TL;DR: MVMAE是一种利用放射学多视图结构的自监督学习框架，通过掩码图像重建和跨视图对齐学习视图不变和疾病相关表示，MVMAE-V2T进一步整合放射学报告作为文本监督信号，在疾病分类任务中优于监督和视觉语言基线方法。
...
Method: 提出Multiview Masked Autoencoder (MVMAE)自监督框架，结合掩码图像重建和跨视图对齐，将临床投影中的冗余转化为自监督信号。进一步扩展为MVMAE-V2T，整合放射学报告作为文本学习信号，增强语义基础，同时保持纯视觉推理。

Result: 在MIMIC-CXR、CheXpert和PadChest三个大型公共数据集的下游疾病分类任务中，MVMAE持续优于监督学习和视觉语言基线方法。MVMAE-V2T提供额外增益，特别是在低标签情况下，结构化文本监督最为有益。
...
TL;DR: 提出基于语言和分割语义引导的自动视频着色方法，使用扩散模型结合光学流实现时间稳定性，在标准基准测试中达到最先进性能


<details>
  <summary>Details</summary>
Motivation: 现有视频着色方法存在时间闪烁问题或需要大量手动输入，需要一种能够自动实现高保真视频着色的方法

Method: 使用语言条件扩散模型对灰度帧着色，通过自动生成的对象掩码和文本提示提供语义引导，主要自动方法使用通用提示而不需要特定颜色输入。采用RAFT光学流从先前帧扭曲颜色信息实现时间稳定性，并通过校正步骤检测和修复扭曲引入的不一致性

Result: 在标准基准测试（DAVIS30, VIDEVO20）中达到最先进的着色精度（PSNR）和视觉真实感（Colorfulness, CDC），证明了基于自动提示引导的视频着色方法的有效性

Conclusion: 提出的方法通过语言和分割语义引导实现了高质量、时间稳定的自动视频着色，无需特定颜色输入，在多个指标上超越了现有方法
...
TL;DR: 研究发现多模态大语言模型在视觉问答评估中存在对提示格式的敏感性，即使语义中立的格式变化也会显著影响模型表现，现有偏见缓解策略无法解决这些问题。


<details>
  <summary>Details</summary>
Motivation: 先前研究已发现视觉问答基准测试对答案选项顺序敏感，但本研究旨在揭示更多未被探索的提示格式偏见，质疑当前多模态大语言模型评估的可靠性。

Method: 通过大规模研究分析提示格式的三个关键变化因素，涉及7个多模态大语言模型和5个视觉问答数据集，涵盖48种不同的提示格式变化，评估格式变化对模型表现的影响。

Result: 研究发现多项选择视觉问答对微小的提示格式变化高度敏感，即使这些变化在语义上是中立的。这些偏见独立于已知的顺序偏见或模型对正确答案的置信度而持续存在。

Conclusion: 现有偏见缓解策略无法解决新发现的提示格式偏见，这质疑了当前多模态大语言模型评估的可靠性，需要更稳健的评估方法。
...
TL;DR: 提出一种新的归一化流对齐策略，通过将生成过程（反向传播）的中间特征与视觉基础模型对齐，显著提升生成质量和分类精度，训练速度提升3.3倍以上


<details>
  <summary>Details</summary>
Motivation: 标准归一化流（NFs）的生成质量受限于基于对数似然优化的语义表示能力不足，需要改进NFs的语义表示学习能力

Method: 提出创新的对齐策略：利用NFs的可逆性，将生成过程（反向传播）的中间特征与强大的视觉基础模型表示对齐；同时提出无需训练、测试时优化的分类算法来评估NFs的语义知识

Result: 方法使NFs训练速度提升3.3倍以上，在ImageNet 64×64和256×256上实现生成质量和分类准确率的显著提升，建立了新的最先进结果

Conclusion: 通过创新的生成过程特征对齐策略，有效提升了归一化流的语义表示能力和生成质量，同时大幅加速训练过程，为NFs的实际应用提供了新思路
...
TL;DR: INSIGHT是一个用于检测和解释AI生成图像的多模态框架，即使在极低分辨率下也能保持鲁棒性，通过超分辨率、空间定位和语义对齐技术提供透明解释。


<details>
  <summary>Details</summary>
Motivation: 当前深度伪造检测系统在现实世界条件下（如下采样、压缩、跨域分布偏移）性能急剧下降，且大多数检测器作为不透明的分类器运行，无法解释为何将图像标记为合成图像，这削弱了信任并阻碍了在高风险环境中的采用。

Method: INSIGHT结合了分层超分辨率放大细微的取证线索而不引入误导性伪影，Grad-CAM驱动的多尺度定位揭示指示生成模式的空间区域，以及CLIP引导的语义对齐将视觉异常映射到人类可解释的描述符。然后使用结构化的ReAct + Chain-of-Thought协议提示视觉语言模型，通过双阶段G-Eval + LLM-as-a-judge管道验证解释以减少幻觉并确保事实性。
...
TL;DR: AnchorFlow是一种无需训练的3D编辑方法，通过建立全局潜在锚点确保编辑过程中的几何稳定性，无需掩码监督即可保持几何保真度。
...
Method: AnchorFlow基于潜在锚点一致性原则，建立源轨迹和目标轨迹共享的全局潜在锚点，使用松弛锚点对齐损失和锚点对齐更新规则来强制一致性，确保变换在整个编辑过程中保持稳定和语义忠实。

Result: 在Eval3DEdit基准测试中，AnchorFlow在不同编辑类型上始终提供语义对齐和结构鲁棒的编辑效果，能够实现更显著的语义修改，同时有效保持几何保真度。

Conclusion: 通过稳定潜在参考空间，AnchorFlow实现了无需掩码监督的几何稳定3D编辑，在语义对齐和结构鲁棒性方面表现优异。
...
TL;DR: 提出RS-EoT框架解决遥感图像理解中的伪推理问题，通过迭代视觉证据搜寻和SocraticAgent自博弈系统实现真正的证据驱动推理


<details>
  <summary>Details</summary>
Motivation: 现有遥感多模态推理模型存在广泛的伪推理现象，模型只是描述推理过程而非基于视觉证据进行真正推理，这源于"一瞥效应"——对大规模遥感图像的粗略感知导致不完整理解

Method: 提出RS-EoT框架：1) SocraticAgent自博弈多智能体系统，通过推理和视觉检查交替循环合成推理轨迹；2) 两阶段渐进强化学习策略：先在细粒度定位任务上增强RS-EoT能力，后在遥感VQA任务上泛化到更广泛理解场景

Result: RS-EoT在多个遥感VQA和定位基准测试中达到最先进性能，分析显示清晰的推理和证据搜寻迭代循环，证实RS-EoT缓解了一瞥效应并实现了真正的证据驱动推理

Conclusion: RS-EoT框架通过语言驱动的迭代视觉证据搜寻范式，有效解决了遥感多模态推理中的伪推理问题，实现了基于证据的真实推理
...
<details>
  <summary>Details</summary>
Motivation: 在复杂低空环境中准确感知无人机对空域安全和智能系统至关重要，但真实世界无人机数据收集面临空域管制、隐私问题和环境变化等限制，而手动标注3D姿态和跨模态对应关系耗时且昂贵。

Method: 创建UAV-MM3D合成数据集，包含40万帧同步数据，涵盖城市、郊区、森林、沿海等多样场景和不同天气条件，包含多种无人机模型，提供五种模态数据（RGB、红外、激光雷达、雷达、DVS），每帧包含2D/3D边界框、6自由度姿态和实例级标注。同时提出LGFusionNet（激光雷达引导的多模态融合基线）和专用无人机轨迹预测基线。

Result: UAV-MM3D数据集提供了全面的场景覆盖、可控的仿真环境和丰富的标注，为无人机3D感知研究提供了公开基准，支持核心任务如3D检测、姿态估计、目标跟踪和短期轨迹预测。

Conclusion: 通过合成数据方法克服了真实世界数据收集的限制，UAV-MM3D为低空无人机感知和运动理解提供了高质量的多模态基准，有望推动无人机3D感知技术的发展。
...
TL;DR: DiffStyle360：基于扩散模型的3D头部风格化框架，无需逐风格训练即可从单张参考图像生成多视角一致、身份保持的3D头部风格化效果


<details>
  <summary>Details</summary>
Motivation: 现有3D头部风格化方法依赖计算昂贵的优化或领域特定微调来适应新风格，限制了实际应用效率和灵活性

Method: 基于DiffPortrait360架构，引入风格外观模块解耦风格与内容，使用风格融合注意力机制在潜在空间平衡结构保持与风格化保真度，采用3D GAN生成多视角数据集进行鲁棒微调，以及基于温度的关键缩放策略控制风格化强度

Result: 在FFHQ和RenderMe360数据集上的实验表明，DiffStyle360在风格质量上优于最先进的GAN和扩散基风格化方法，能处理具有挑战性的风格领域

Conclusion: DiffStyle360提供了一种高效、灵活的3D头部风格化解决方案，无需逐风格训练即可从单张参考图像生成高质量、多视角一致、身份保持的风格化结果
...
TL;DR: WUKONG是一个无需训练的3D纹理变形框架，通过流式Transformer的生成先验实现高保真3D过渡，避免了传统方法需要手动对应匹配和变形轨迹估计的问题。
...
Method: 1. 利用流式Transformer的生成先验；2. 将变形建模为最优传输重心问题以确保平滑形状过渡；3. 采用序列初始化策略防止几何突变；4. 提出相似性引导的语义一致性机制来保留高频纹理细节。

Result: 广泛的定量和定性评估表明，WUKONG在多样化的几何和纹理变化上显著优于现有最先进方法，实现了卓越的变形效果。

Conclusion: WUKONG提供了一个无需训练的高质量3D纹理变形框架，通过流式Transformer和最优传输理论实现了平滑的形状过渡和精确的纹理保留，在3D变形领域取得了突破性进展。
...
Method: 冻结处理视图匹配的解码器，仅微调负责特征提取的图像编码器。通过轻量级LoRA适配器，从强大的单目教师模型在大规模无标签数据集上提取的精细几何细节来增强编码器。

Result: 在DUSt3R、MASt3R、CUT3R和VGGT等多种模型上验证有效，微调后的模型在单视角和多视角设置下都能提供更清晰的边界、恢复复杂结构，并实现更高的几何精度，同时仅增加微小的LoRA权重，测试时的内存和延迟几乎不变。

Conclusion: Fin3R通过简单的微调步骤有效解决了3D重建模型在精细几何和鲁棒性方面的挑战，显著提升了重建质量，同时保持了模型的效率和轻量性。
...
<details>
  <summary>Details</summary>
Motivation: 少样本多类别工业异常检测面临挑战：视觉语言模型需要同时具备类别适应性和精确判别性，但数据稀缺导致正常与异常状态边界模糊，容易漏检细微缺陷或误拒非典型正常样本

Method: 提出ABounD统一学习框架，包含：1）动态概念融合模块，融合通用先验与类别特定线索生成类别自适应提示；2）对抗边界锻造模块，通过PGD风格扰动生成边界级栅栏特征，塑造精确决策边界；3）单阶段训练，使用概念-边界损失，其中ABF提供主要监督信号，语义-空间正则化器稳定优化
...
TL;DR: 提出了IntentHQ基准，用于分析深度伪造视频背后的意图，从真实性验证转向上下文理解，包含5168个视频和23种细粒度意图类别。


<details>
  <summary>Details</summary>
Motivation: 生成模型的快速发展导致深度伪造视频越来越逼真，带来重大社会和安全隐患。现有检测方法仅关注区分真假视频，但未能解决一个根本问题：操纵视频背后的意图是什么？

Method: 引入IntentHQ基准，包含5168个精心收集和标注的视频，涵盖23种细粒度意图类别。使用监督和自监督的多模态模型进行意图识别，整合时空视频特征、音频处理和文本分析来推断视频背后的动机和目标。

Result: 提出的模型能够有效区分广泛的意图类别，将视频分析从简单的真实性验证提升到对视频背后意图的上下文理解。

Conclusion: IntentHQ基准代表了视频分析范式的转变，从真实性检测转向意图分析，为理解操纵视频背后的动机和目的提供了新的框架和工具。
...
<details>
  <summary>Details</summary>
Motivation: 探索如何在无需额外训练的情况下，通过推理时缩放来提升文本引导3D扩散模型的生成质量，解决现有方法在3D生成中面临的挑战。

Method: 提出ITS3D框架，将任务形式化为优化问题，寻找最有效的高斯噪声输入。采用验证器引导搜索算法迭代优化噪声候选。引入三种关键技术：1）高斯归一化稳定搜索过程；2）基于奇异值分解的压缩技术降低高维搜索空间复杂度；3）奇异空间重置机制防止陷入局部最优。

Result: 大量实验表明ITS3D能够显著提升文本到3D的生成质量，证明了计算高效搜索方法在生成过程中的潜力。

Conclusion: ITS3D展示了通过推理时优化噪声输入可以有效提升3D扩散模型生成质量，为无需额外训练的生成质量改进提供了新思路。
...
TL;DR: 提出一种基于高斯时空表示的方法，仅用三个视角重建动态火焰的3D结构，通过分离静态背景与动态火焰区域，结合多视角立体图像和单目深度先验来解决几何约束不足的问题。


<details>
  <summary>Details</summary>
Motivation: 火焰的动态重建极具挑战性，因其具有易变性、半透明特性和高频特征。现有方法通常需要大量视角，本研究旨在仅用三个视角实现火焰重建，解决几何约束不足的问题。

Method: 1) 结合密集多视角立体图像和单目深度先验分离静态背景与动态火焰区域；2) 通过融合每视角密集光流投影初始化3D流场；3) 使用高斯时空表示，每个3D高斯编码寿命和线性速度以匹配密集光流；4) 采用自定义硬件同步模式确保跨相机亚帧时间对齐。

Result: 在多个重建实验中进行了定量和定性验证，证明该方法在多样且具有挑战性的真实火焰场景中具有鲁棒性能，能够使用经济型硬件实现火焰重建。

Conclusion: 该方法成功实现了仅用三个视角重建动态火焰的3D结构，通过创新的时空表示和硬件同步技术，解决了火焰重建中的几何约束不足和高频特征捕捉难题。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要针对检测或分割等感知任务，忽视了推断道路拓扑和动态场景结构所需的推理能力。理解连接低层感知与高层规划的中层道路语义对于可靠自动驾驶和数字地图构建至关重要。

Method: 提出了RoadSceneBench基准测试，强调关系理解和结构一致性。同时提出了HRRP-T（分层关系奖励传播与时序一致性）训练框架，通过自适应奖励信号促进空间连贯性和语义对齐，使视觉语言模型能够实现几何感知和时序一致的推理。

Result: 该方法在多样化道路配置中实现了最先进的性能。RoadSceneBench为研究中层道路语义和培养结构感知的自主感知提供了紧凑而强大的基础。

Conclusion: RoadSceneBench填补了现有基准的空白，专注于道路场景的视觉推理能力评估。提出的HRRP-T框架能够使模型超越静态识别，实现几何感知和时序一致的推理，为自动驾驶和数字地图构建提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于文本的行人异常检索方法大多依赖复杂的深度学习技术，但如何优化模型以获得更细粒度的特征仍然是一个研究问题。本文旨在探索同时结合细粒度和粗粒度特征的有效性。

Method: 1. 提出局部-全局混合视角（LHP）模块，与视觉语言模型（VLM）集成，探索细粒度和粗粒度特征的结合；2. 研究统一图像-文本（UIT）模型，结合多种目标损失函数（ITC、ITM、MLM、MIM）；3. 提出新颖的迭代集成策略，不同于传统同时使用模型结果的集成方法；4. 基于LHP模型的指导，提出新的特征选择算法。

Result: 在PAB数据集上的广泛实验表明，该方法相比先前工作取得了最先进的性能：R@1提升9.70%，R@5提升1.77%，R@10提升1.01%。

Conclusion: 本文提出的LHP模块、UIT模型和迭代集成策略有效提升了基于文本的行人异常检索性能，通过结合细粒度和粗粒度特征以及创新的集成方法，在PAB数据集上实现了显著的性能改进。
...
TL;DR: 研究发现冻结的视觉基础模型（特别是DINOv3）无需微调就具备强大的跨生成器检测能力，通过简单的token排序策略和轻量级线性探针，能有效选择真实性相关token，提升检测精度。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型日益多样化和强大，跨生成器检测成为新挑战。现有检测方法往往记忆特定生成模型的伪影而非学习可迁移线索，导致对未见生成器检测失败。研究发现基础模型已具备跨生成器检测能力，需要理解其原理并提升性能。

Method: 通过频率、空间和token视角的系统研究，发现DINOv3依赖全局低频结构作为弱但可迁移的真实性线索。引入无需训练的token排序策略，随后使用轻量级线性探针选择一小部分真实性相关token子集。

Result: DINOv3展现出强大的跨生成器检测能力，token子集在所有评估数据集上一致提升检测精度。研究为理解基础模型为何能泛化到多样生成器提供了经验证据和可行假设。

Conclusion: 该研究提供了图像伪造检测的通用、高效且可解释的基线方法，展示了基础模型在跨生成器检测中的潜力，为理解模型泛化机制提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 现有的说话头部生成方法需要同时学习唇部运动、面部表情和头部姿态运动，但现有方法在这些方面的表现仍有改进空间。

Method: 提出条件运动扩散变换器（cMDT）来建模面部运动序列，采用3D表示。cMDT以参考面部图像（确定外观）和音频序列（驱动运动）作为条件输入。

Result: 在VoxCeleb2和CelebV-HQ数据集上的定量、定性实验以及用户研究表明，Dimitra++在生成逼真的说话头部方面优于现有方法，能够更好地模拟唇部运动、面部表情和头部姿态。

Conclusion: Dimitra++框架通过条件运动扩散变换器有效实现了音频驱动的说话头部生成，在唇部同步、面部表情和头部姿态方面表现出色，为相关领域提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着科学论文数量增长，需要有效传达研究发现的方法，海报作为展示论文内容的关键媒介，其布局设计对研究传播和理解至关重要。目前缺乏对论文与对应海报布局关系的大规模数据集。

Method: 引入SciPostGen大规模数据集，分析论文结构与海报布局元素的关系，提出检索增强的海报布局生成框架，该框架检索与给定论文一致的布局作为生成指导。

Result: 分析显示论文结构与海报布局元素数量相关；检索器能估计与论文结构一致的布局；在有无布局约束两种条件下，该框架都能生成满足给定约束的布局。

Conclusion: SciPostGen数据集填补了论文与海报布局对应关系的数据空白，提出的检索增强生成框架能有效生成与论文结构一致且满足约束的海报布局。
...
TL;DR: 本文提出了一种针对密集文本图像去除的贝叶斯优化方法，通过建模灵活掩码轮廓来提升文本去除性能，发现字符级掩码优于最小覆盖区域。


<details>
  <summary>Details</summary>
Motivation: 现有文本去除方法主要针对简单场景文本，缺乏对密集文本图像的研究。研究发现文本去除性能对掩码轮廓扰动敏感，需要精确调整掩码形状。

Method: 开发了建模高度灵活掩码轮廓的方法，并使用贝叶斯优化学习其参数，创建了密集文本去除的基准数据集。

Result: 发现最优掩码轮廓是字符级掩码，且文本区域的最小覆盖并非最优。贝叶斯优化方法能有效学习掩码参数。

Conclusion: 研究为手动掩码提供了用户友好的指导原则，推动了密集文本图像去除的实用化发展。
...
<details>
  <summary>Details</summary>
Motivation: 当前DocVQA系统面临准确性与效率的尖锐权衡：大型教师模型具有强大的定位能力但部署成本高，而紧凑的学生模型在定位性能上大幅下降。需要一种方法将教师的空间推理能力高效转移到学生模型中。

Method: 提出DocVAL验证链式思维蒸馏框架，包含三个关键组件：1) 教师监督与验证时文本检测，过滤和去噪训练信号；2) 多模块验证器(VAL)，确保答案正确性和几何一致性，并提供像素级错误反馈；3) 两阶段学生训练方案，先学习验证后的CoT轨迹，然后通过VAL反馈进行迭代精炼。

Result: 学生模型(Gemma-3 12B)在DocVQA上达到91.4% ANLS和82.4% mAP，作为纯视觉语言模型无需推理时的文本检测或OCR。验证反馈贡献6.3 mAP提升，迭代精炼贡献9.7 mAP改进。发布了95k高质量验证器验证的CoT轨迹。

Conclusion: DocVAL框架成功地将大型教师模型的空间推理能力转移到可部署的学生模型中，显著提升了DocVQA任务的性能，同时发布了高质量数据集促进文档理解中的空间推理研究。
...
Method: 提出CoT4AD框架，通过显式建模感知-问题-预测-动作的思维链来对齐推理空间和动作空间，训练时使用显式CoT，推理时使用隐式CoT。
...
TL;DR: Fast3Dcache：一种无需训练、几何感知的缓存框架，通过预测性缓存调度约束和时空稳定性准则，在保持几何保真度的同时加速3D扩散模型推理，实现最高27.12%的加速和54.8%的FLOPs减少。
...
Result: 显著加速推理，最高实现27.12%的加速和54.8%的FLOPs减少，几何质量退化最小（倒角距离仅增加2.48%，F-Score仅降低1.95%）。
...
Result: Diff-ICMH支持通过单一编解码器和比特流完成多种智能任务，无需任何任务特定适配，同时为人类感知保持高质量的视觉体验。广泛的实验结果表明了该方法的优越性和跨任务的泛化能力。

Conclusion: Diff-ICMH成功协调了机器视觉和人类视觉在图像压缩中的需求，通过生成先验和语义保真度的结合，实现了感知质量和语义信息保存的双重优化，为统一图像压缩框架提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统人工牙科种植体基台设计耗时耗力，不当设计可能导致种植体并发症如种植体周围炎。利用人工智能辅助设计可快速提高效率并增强基台适应性。

Method: 提出文本条件嵌入基台设计框架(TCEAD)，扩展MeshMAE自监督学习框架，引入文本引导定位(TGL)模块，使用CLIP文本编码器描述基台区域，通过口腔扫描数据预训练编码器提升特征提取能力。

Result: 在大型基台设计数据集上验证，TCEAD相比其他主流方法在IoU指标上提升0.8%-12.85%，在自动化牙科基台设计中表现出潜力。

Conclusion: TCEAD框架通过文本条件嵌入和自监督学习有效实现牙科种植体基台的自动化设计，显著提高设计精度和效率，为临床牙科种植提供有力支持。
...
TL;DR: 研究不同思维链设计对视觉语言模型通用视觉推理能力的影响，发现简洁的思维链比冗长的思维链表现更好，呈现"短即是长"效应


<details>
  <summary>Details</summary>
Motivation: 虽然思维链数据（特别是长思维链或视觉思维链）已被广泛用于监督中间推理过程，但尚不清楚为什么特定的思维链设计有效，以及哪些设计真正支持可泛化的推理能力
...
Result: 视觉和较长的思维链主要加速收敛但不提升最终性能上限；仅包含必要接地步骤的简洁思维链优于较长轨迹；仅保留最小接地结果的思维链在不同迷宫大小上泛化效果最好。这些发现在其他视觉中心任务上得到验证
...
TL;DR: HarmoCLIP通过引入细粒度语义监督和区域-语言对齐策略，在保持CLIP全局对齐的同时增强区域级语义理解，解决了全局与局部表示的权衡问题。
...
Method: 提出HarmoCLIP框架，通过引入明确的细粒度语义监督项，直接对齐文本片段与对应的视觉区域，连接图像区域空间和文本空间。同时采用区域-语言对齐监督策略，在不损害全局语义一致性的前提下促进细粒度语义学习。
...
TL;DR: 本文提出AnoRefiner方法，通过异常感知细化器将零样本工业异常检测的粗粒度异常图提升到像素级精度，利用异常分数图的空间线索补充图像特征，并采用渐进式组间测试时训练策略。


<details>
  <summary>Details</summary>
Motivation: 现有零样本工业异常检测方法只能生成粗粒度异常图，因为ViT仅提取补丁级特征。虽然已有方法尝试基于ZSAD特征预测更精细的异常，但由于合成训练异常与真实异常之间的差距，仍难以恢复细粒度异常且存在漏检。作者发现异常分数图提供了互补的空间线索，这一事实之前被忽视。

Method: 提出AnoRefiner异常感知细化器，可插入大多数ZSAD模型：1）设计异常细化解码器，利用异常分数图逐步增强图像特征，减少对合成异常数据的依赖；2）基于大规模生产范式，提出渐进式组间测试时训练策略，在每个产品组中训练ARD用于下一组的细化过程，同时保持与任何ZSAD方法的兼容性。

Result: 在MVTec AD和VisA数据集上的实验表明，AnoRefiner将各种ZSAD模型的像素级AP指标提升了高达5.2%，这一改进在可视化结果中也能直接观察到。

Conclusion: AnoRefiner通过利用异常分数图的互补空间线索，有效解决了零样本工业异常检测中细粒度异常恢复的问题，显著提升了现有方法的性能，且具有良好的通用性和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像编辑模型通常将多模态大语言模型编码器与扩散解码器耦合，但MLLM在训练期间保持冻结状态。作者认为解锁MLLM的推理能力可以进一步提升编辑模型的边界。

Method: 提出基于推理的框架，采用"思考-编辑-反思"循环：思考机制利用MLLM的世界知识解释抽象指令；反思机制审查编辑结果，自动纠正意外操作并确定停止轮次。

Result: 实验表明该方法在多个基准测试上取得显著性能提升：使用Step1X-Edit初始化时，ImgEdit提升4.3%，GEdit提升4.7%，Kris提升8.2%；与Qwen-Image-Edit集成时，在GEdit和Kris上均优于先前开源方法。

Conclusion: 通过解锁MLLM的推理能力并引入思考-编辑-反思循环，可以显著提升图像编辑模型的性能，在多个基准测试上取得优于现有方法的结果。
...
<details>
  <summary>Details</summary>
Motivation: 现有的遥感MLLM通常通过精心策划的思维链数据进行冷启动训练来增强推理能力，这种方法不仅标注成本高昂，而且会引入人为偏见，限制模型推理的多样性。

Method: 提出GeoZero框架，包含两个数据集：GeoZero-Instruct（通过监督微调获取初步地理空间知识）和GeoZero-Hard（在强化学习阶段激发深度推理）。引入答案锚定组相对策略优化（A²GRPO），通过模型自身答案对推理过程进行正则化，鼓励多样而准确的思考。

Result: 在多个遥感视觉语言基准测试上的广泛实验表明，GeoZero不仅超越了现有的最先进方法，还在多样化的地理空间任务中培养了通用的涌现推理能力。

Conclusion: GeoZero框架成功实现了无需预定义思维链监督的地理空间推理，通过创新的数据集构建和策略优化方法，显著提升了多模态大语言模型在地理空间场景理解中的性能。
...
TL;DR: 本文提出注意力交互对齐（AIA）损失函数，旨在缓解多模态模型中理解与生成任务间的冲突，无需模型解耦即可提升性能。


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型在图像生成与理解任务中存在目标冲突，现有方法通过模型解耦（如双编码器、MOE架构等）缓解冲突，但过度解耦会丧失交错生成能力。本文旨在探索不依赖模型解耦的冲突缓解方案。

Method: 通过分析模型解耦如何缓解冲突，发现解耦本质是驱动模型形成任务特定的多模态交互模式。基于此观察，提出注意力交互对齐（AIA）损失函数，在训练中显式学习任务特定的多模态交互模式。在Emu3和Janus-Pro模型的不同训练阶段应用AIA进行验证。

Result: AIA损失不仅细化了跨模态注意力模式，还同时提升了生成和理解任务的性能，证明了其通用性和有效性。

Conclusion: 提出的AIA损失能够在不依赖模型解耦的情况下，通过显式学习任务特定的多模态交互模式来缓解理解与生成任务间的冲突，实现性能提升，为统一多模态模型的发展提供了新思路。
...
TL;DR: VaMP提出了一种变分多模态提示学习框架，通过从学习到的后验分布中采样生成实例条件化提示，实现样本特定、不确定性感知的多模态表示学习，在少样本和领域泛化基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态提示学习方法通常依赖固定、共享的提示和确定性参数，这限制了它们捕捉实例级变化或建模跨不同任务和领域的不确定性的能力。需要一种能够处理样本特定变化和不确定性的方法。

Method: 提出变分多模态提示学习(VaMP)框架，通过从学习到的后验分布中采样生成实例条件化提示；引入基于实例表示和类别原型的类别感知先验；将提示调优公式化为潜在提示表示的变分推断，并通过重参数化采样进行端到端训练。

Result: 在少样本和领域泛化基准测试中，VaMP实现了最先进的性能，证明了建模不确定性和任务结构的好处。

Conclusion: VaMP通过变分推断实现样本特定、不确定性感知的提示学习，能够更好地适应下游任务，在有限监督下展现出优越的泛化能力。
...
TL;DR: 本文挑战了传统对扩散模型蒸馏的理解，揭示了在文本到图像生成等复杂任务中，CFG增强而非分布匹配才是少步蒸馏的主要驱动力，并提出了改进方法。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为Distribution Matching Distillation（DMD）及其变体的优异性能源于学生模型输出分布与教师模型的匹配。本文旨在挑战这一传统理解，深入分析DMD训练目标的本质。

Method: 通过对DMD训练目标进行严格分解，识别出CFG增强（CA）作为蒸馏的核心"引擎"，而分布匹配（DM）则作为"正则化器"。验证了DM项可以被更简单的非参数约束或GAN目标替代，并提出了将引擎和正则化器的噪声调度解耦的改进方法。

Result: 研究发现CFG增强是少步蒸馏的主要驱动力，分布匹配主要起正则化作用。提出的改进方法被Z-Image项目采用，开发出了顶级的8步图像生成模型，验证了发现的泛化性和鲁棒性。

Conclusion: 本文对扩散模型蒸馏机制提供了新的理论理解，揭示了CFG增强的核心作用，并基于此提出了改进蒸馏过程的原理性方法，为高效少步生成器设计提供了新的理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成模型在多人生成场景中存在严重问题，经常出现面部重复、身份合并或人数计数错误，需要专门解决多人生成的可靠方法。

Method: 提出Ar2Can两阶段框架：1) Architect模块预测结构化布局，指定每个人的位置；2) Artist模块合成逼真图像，使用基于匈牙利空间对齐和ArcFace身份相似性的空间接地面部匹配奖励进行指导。开发了两种Architect变体，与基于扩散的Artist模型集成，通过GRPO使用组合奖励进行优化。

Result: 在MultiHuman-Testbench上评估，Ar2Can在计数准确性和身份保持方面取得显著改进，同时保持高感知质量。值得注意的是，该方法主要使用合成数据实现这些结果，无需真实多人生成图像。

Conclusion: Ar2Can通过解耦空间规划和身份渲染，有效解决了多人生成中的核心挑战，为可靠的多人生成提供了创新解决方案，且不依赖真实训练数据。
...
TL;DR: Z-Image是一个6B参数的高效图像生成基础模型，采用S3-DiT架构，挑战了"不计成本扩大规模"的范式，在314K H800 GPU小时内完成训练，性能媲美商业模型。


<details>
  <summary>Details</summary>
Motivation: 当前高性能图像生成模型被Nano Banana Pro、Seedream 4.0等专有系统主导，开源替代品如Qwen-Image、Hunyuan-Image-3.0和FLUX.2参数规模过大（20B-80B），在消费级硬件上推理和微调不实用，需要更高效的解决方案。

Method: 提出Z-Image，基于可扩展单流扩散变换器（S3-DiT）架构的6B参数基础生成模型。通过系统优化整个模型生命周期，包括精选数据基础设施和精简训练课程。采用few-step蒸馏方案和奖励后训练得到Z-Image-Turbo，实现企业级H800 GPU上亚秒级推理延迟，兼容消费级硬件（<16GB VRAM）。还通过全预训练范式训练Z-Image-Edit编辑模型。

Result: 模型在314K H800 GPU小时（约63万美元）内完成完整训练工作流。Z-Image-Turbo在企业级H800 GPU上实现亚秒级推理延迟，兼容消费级硬件。Z-Image-Edit展示出令人印象深刻的指令跟随能力。定性和定量实验表明，模型在各种维度上达到或超越领先竞争对手的性能，特别是在逼真图像生成和双语文本渲染方面媲美顶级商业模型。

Conclusion: Z-Image证明了通过显著减少计算开销也能实现最先进的结果，挑战了"不计成本扩大规模"的范式。公开代码、权重和在线演示，促进可访问、经济实惠且最先进的生成模型发展。
...
<details>
  <summary>Details</summary>
Motivation: 高斯泼溅在渲染任务中表现出潜力，但通常需要密集输入视角的逐场景优化。现有方法虽然通过多视角立体几何先验实现前馈渲染，但仍需要大量重叠的输入视角。本文旨在解决大稀疏性条件下的渲染问题。

Method: 提出两阶段学习策略：第一阶段通过迭代亲和力学习将点图转换到真实空间，便于相机控制；第二阶段将两个输入视角的点图投影到目标视图平面，通过立体匹配细化几何，并在细化平面上锚定高斯基元进行高质量图像渲染。

Result: 收集了多视角以人为中心的数据，实验表明该方法提高了点图重建的稳定性和自由视点渲染的视觉质量。

Conclusion: Splat-SAP方法通过像素级点图重建和两阶段学习策略，成功解决了大稀疏性条件下以人为中心场景的新视图渲染问题，提高了渲染质量和稳定性。
...
TL;DR: ReAG是一种新颖的推理增强多模态RAG方法，通过粗粒度与细粒度检索结合批判模型过滤无关段落，采用强化学习增强推理能力，显著提升了知识密集型视觉问答的准确性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在处理领域特定或知识密集型查询时表现不佳，因为这些信息在预训练数据中代表性不足。现有的基于检索的VQA方法存在检索精度低、噪声段落多和推理能力有限的问题。
...
Conclusion: ReAG通过推理增强的多模态检索方法有效解决了知识密集型视觉问答中的挑战，为MLLMs在需要外部知识的任务中提供了更可靠的解决方案。
...
TL;DR: 提出DIPT方法，通过域不变提示调优增强病理图像领域泛化能力，在知识蒸馏过程中学习域不变提示，提升模型在异构数据源上的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 计算病理学中存在由染色协议、扫描设备和成像设置差异导致的域偏移问题，现有视觉语言模型的零样本性能受提示词变化影响较大，且病理图像缺乏自然图像中的语义描述符，需要数据驱动的方法学习域不变提示。

Method: 提出域不变提示调优(DIPT)方法，为每个域学习多个输入标记，分别训练后跨域平均得到域不变提示，学生模型利用这些提示从PLIP文本编码器蒸馏知识，使视觉特征与域不变嵌入对齐。

Result: 在病理数据集上的领域泛化任务中，DIPT方法显著提升了平均F1分数，优于现有最先进的知识蒸馏方法。

Conclusion: DIPT方法通过域不变提示学习增强了计算病理学模型在异构临床数据源上的泛化能力，有助于在实际临床问题中部署鲁棒的病理模型。
...
TL;DR: 本研究开发并评估了用于合成单侧乳腺双视图乳腺X线摄影的三通道去噪扩散概率模型，探讨了不同通道表示对图像保真度和跨视图一致性的影响。


<details>
  <summary>Details</summary>
Motivation: 开发能够生成真实且解剖学一致的双视图乳腺X线摄影图像的合成方法，以解决医学影像数据稀缺问题，为数据集增强提供技术支持。

Method: 使用预训练的三通道DDPM模型，在11020张乳腺X线摄影的私有数据集上进行微调，评估三种第三通道编码方式（求和、绝对差、零通道），生成500对合成图像，通过IoU、DSC、EMD、KS测试和视觉图灵测试进行定量和定性评估。

Result: 合成图像与真实图像在IoU和DSC分布上具有可比性（EMD=0.020，KS=0.077）；求和和绝对差编码模型在IoU和DSC方面表现更优（p<0.001）；生成的CC和MLO视图保持跨视图一致性，6-8%的合成图像存在与训练数据一致的伪影。
...
TL;DR: 本研究评估了可见光(VIS)和热红外(TIR)图像融合在自动检测大蓝鹭个体和巢穴中的效果，比较了早期融合和晚期融合方法，发现融合方法相比单可见光模型能提高检测性能。


<details>
  <summary>Details</summary>
Motivation: 野生动物监测对生物多样性保护至关重要，可见光和热红外图像的互补使用可以提供比单一图像源更多的信息，提高自动检测的准确性，但两种图像的视场和分辨率差异给对齐和融合带来了挑战。

Method: 以大蓝鹭为案例研究，使用YOLO11n模型自动检测个体和巢穴。测试了两种VIS-TIR融合方法：早期融合（使用主成分分析融合图像对）和晚期融合（基于分类回归树处理VIS-only和TIR-only模型的检测结果）。使用深度学习模型自动对齐VIS和TIR图像。

Result: 所有类别中，晚期和早期融合都比VIS-only模型提高了F1分数。对于主要类别"被占用的巢穴"，晚期融合将F1分数从90.2%提高到93.0%。该模型还能以90%的召回率识别两种源中的误报。

Conclusion: 虽然融合方法能获得更好的结果，但受限于TIR视场和对齐约束会损失数据。使用机载超高分辨率可见光传感器可能是实现可操作调查的有趣选择。
...
TL;DR: 提出一个结合CNN和Vision Transformer的深度学习模型，利用MRI扫描和多时间点数据预测轻度认知障碍向阿尔茨海默病的转化，准确率达95.05%


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病是不可逆的神经退行性疾病，早期预测对及时干预至关重要。轻度认知障碍是AD的前期阶段，但并非所有MCI患者都会转化为AD，因此准确区分稳定型MCI和进展型MCI是临床诊断的关键挑战。

Method: 提出端到端的深度学习模型：1) 结合CNN和Vision Transformer的混合架构，从MRI扫描中提取局部空间特征和全局上下文依赖；2) 使用双向LSTM处理四个连续时间点的MRI特征和其他非影像生物标志物；3) 预测患者在48个月时的认知状态，区分sMCI和pMCI。

Result: 模型在区分稳定型MCI和进展型MCI方面达到95.05%的平均预测准确率，优于现有AD预测研究，展示了在纵向AD预测中的最先进性能。

Conclusion: 该研究证明了结合空间和时间建模在阿尔茨海默病早期检测中的有效性，为临床AD预测提供了高性能的深度学习解决方案，有助于及时干预和管理。
...
TL;DR: 该研究构建了CultureMix基准测试，评估大型视觉语言模型在文化混合场景中的表现，发现模型存在背景依赖性和文化身份识别不一致的问题，并通过监督微调提出了改进方案。


<details>
  <summary>Details</summary>
Motivation: 在全球化的世界中，来自不同文化背景的元素经常出现在同一视觉场景中（文化混合场景），但大型视觉语言模型如何感知这些场景尚未得到充分研究。研究者认为文化混合是LVLMs面临的关键挑战，需要系统评估模型在多文化元素共存时的表现。

Method: 构建了CultureMix基准测试，包含23k张扩散生成、人工验证的文化混合图像，涵盖四个子任务：1)仅食物；2)食物+食物；3)食物+背景；4)食物+食物+背景。评估了10个LVLMs，并探索了三种鲁棒性策略，包括使用多样文化混合数据集进行监督微调。

Result: 评估发现模型在混合场景中无法保持个体文化身份的一致性，表现出强烈的背景依赖性：当在仅食物基准上添加文化背景时，准确率下降14%。模型在不同上下文中对相同食物产生不一致的预测。监督微调显著提高了模型的一致性和减少了背景敏感性。

Conclusion: 文化混合场景是LVLMs发展的关键挑战，需要更多关注。通过使用多样文化混合数据集进行监督微调可以有效提高模型在文化混合场景中的鲁棒性，这对于开发能够在文化多样化的现实环境中可靠运行的LVLMs至关重要。
...
TL;DR: 该论文提出了CogIP-Bench基准测试，用于评估多模态大语言模型在图像认知属性（如记忆性、趣味性、美学价值等）上与人类感知的对齐程度，并展示了通过后训练可以显著提升这种对齐，进而改善图像生成任务。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型擅长识别图像中的物体和描述场景，但缺乏对人类主观认知属性的理解能力，如判断图像是否令人难忘、有趣、美观或具有情感感染力。这种差距限制了模型在需要人类感知判断的任务中的应用。

Method: 1. 引入CogIP-Bench基准测试，系统评估MLLMs在图像认知属性上的表现；2. 通过后训练阶段增强模型与人类感知的对齐；3. 将认知对齐的MLLM集成到图像生成流程中，指导合成过程产生具有特定认知属性的图像。

Result: 评估显示当前模型在图像认知属性上与人类感知存在显著差距；后训练能有效弥合这一差距，显著提升模型与人类判断的对齐程度；学习到的认知对齐不仅具有预测能力，还能迁移到下游创意任务中，生成更具记忆性或视觉吸引力的图像。

Conclusion: 该工作提供了衡量人类感知的基准测试、增强对齐的后训练流程，并证明这种对齐能够实现更以人为本的人工智能，为MLLMs在需要人类主观判断的认知任务中的应用开辟了新途径。
...
TL;DR: 提出LC4-DViT框架，结合文本引导的生成数据增强和变形感知视觉Transformer，用于高分辨率土地覆盖分类，在AID数据集上达到95.72%的总体准确率。


<details>
  <summary>Details</summary>
Motivation: 及时准确的土地覆盖地图对环境管理至关重要，但遥感土地覆盖分类面临标注稀缺、类别不平衡以及高分辨率场景几何失真等挑战。

Method: LC4-DViT框架包含两个核心组件：1）使用GPT-4o生成场景描述和超分辨率示例的文本引导扩散管道，合成类别平衡的高保真训练图像；2）DViT模型，将DCNv4可变形卷积骨干与Vision Transformer编码器结合，共同捕捉细粒度几何特征和全局上下文。

Result: 在AID数据集的8个类别上，DViT达到0.9572总体准确率、0.9576宏观F1分数和0.9510 Cohen's Kappa，优于ViT基线（0.9274 OA）和ResNet50等模型。跨数据集实验在SIRI-WHU子集上获得0.9333总体准确率，显示良好可迁移性。GPT-4o评估的Grad-CAM热图显示DViT注意力与水文结构对齐最佳。
...
<details>
  <summary>Details</summary>
Motivation: 现有世界引擎在激进6-DoF轨迹和复杂户外布局下存在长距离几何一致性丢失、偏离目标路径或运动过于保守的问题，需要新的方法来生成可交互探索的3D一致视频。

Method: 提出姿态条件世界引擎Captain Safari，通过维护动态局部记忆并使用检索器获取姿态对齐的世界标记，这些标记沿着轨迹条件化视频生成，从而保持稳定的3D结构并准确执行挑战性相机操作。
...
TL;DR: 该论文研究了多模态大语言模型在模态冲突情况下的鲁棒性问题，发现现有模型在多模态对齐和误导信息方面存在脆弱性，并提出了一种模态对齐调优策略来改善这一问题。
...
Method: 1) 构建MMA-Bench基准测试，包含视频和任务来探测模型对特定模态的依赖；2) 使用黑盒和白盒可解释性技术分析模型脆弱性；3) 提出模态对齐调优策略，教导模型何时优先考虑、利用或忽略特定模态线索。

Result: 研究发现当前MLLMs在音频-视觉对不匹配和简单误导文本的情况下表现不佳，缺乏鲁棒的多模态推理能力。提出的对齐调优策略能够显著增强多模态基础能力，使模型具有更可靠的跨模态推理能力。

Conclusion: 这项工作不仅提供了可解释性工具，还为开发具有内在可靠跨模态推理能力的MLLMs指明了清晰路径。通过模态对齐调优，可以显著提升模型在多模态冲突情况下的鲁棒性和可靠性。
...
TL;DR: RETINA基准测试解决了现有MKB-VQA数据集中存在的"视觉捷径"问题，通过使用LLM驱动的流程自动构建包含12万训练样本和2千人工标注测试集，其中查询引用次要实体并配对相关实体图像。提出的MIMIR方法通过增强多个相关实体图像来丰富文档嵌入，有效处理RETINA挑战。


<details>
  <summary>Details</summary>
Motivation: 现有多模态知识库视觉问答基准存在"视觉捷径"问题，即查询图像通常与目标文档的主要主题实体匹配，导致模型可以仅利用视觉线索获得可比结果，无法真正测试多模态理解能力。

Method: 1. 提出RETINA基准：使用LLM驱动的流程自动构建，包含120k训练样本和2k人工标注测试集，查询引用次要主题实体并配对相关实体图像，消除视觉捷径。
2. 提出MIMIR方法：通过增强多个相关实体图像来丰富文档嵌入，不同于先前工作每个文档仅使用单张图像。

Result: 1. 在RETINA基准上评估现有模型，性能显著下降，证实了它们对视觉捷径的依赖。
2. MIMIR方法有效处理RETINA挑战，验证了RETINA基准的有效性和MIMIR方法的优越性。

Conclusion: RETINA基准成功揭示了现有MKB-VQA基准的局限性，MIMIR方法通过多图像增强有效解决了视觉捷径问题，为多模态知识库视觉问答提供了更可靠的评估框架。
...
TL;DR: SLEUTH是一个多智能体框架，通过分层细化处理长文档理解问题，使用检索器和四个协作智能体在粗到细的过程中识别关键线索、筛选视觉证据、分析查询策略，最终生成证据密集的多模态上下文进行预测。
...
Method: 提出SLEUTH多智能体框架，协调检索器和四个协作智能体进行粗到细处理：1) 识别检索页面中的关键文本和视觉线索；2) 筛选表格和图表等显著视觉证据；3) 分析查询以制定推理策略；4) 合成蒸馏后的证据密集多模态上下文生成最终预测。

Result: SLEUTH与先进的视觉语言模型主干结合时，在多个长文档基准测试中持续提升性能，达到了最先进的结果。消融研究验证了每个模块的有效性，并确认了分层细化范式的优势。
...
TL;DR: GLOW是一个全局光照感知的逆向渲染框架，专门解决室内场景中反射率和光照的歧义问题，特别是在共置光源-相机设置下的强互反射、动态阴影、近场光照和移动高光等复杂情况。


<details>
  <summary>Details</summary>
Motivation: 室内场景的逆向渲染面临反射率与光照之间的歧义问题，尤其是在多物体互反射的情况下。虽然自然光照方法难以解决这一歧义，但共置光源-相机设置虽然能通过运动结构校准光照，却引入了强互反射、动态阴影、近场光照和移动高光等新挑战，现有方法无法有效处理。

Method: GLOW框架整合了神经隐式表面表示和神经辐射缓存来近似全局光照，通过精心设计的正则化和初始化联合优化几何和反射率。引入了动态辐射缓存来适应近场运动产生的尖锐光照不连续性，以及表面角度加权的辐射度损失来抑制闪光灯捕捉中常见的镜面伪影。

Result: 实验表明，GLOW在自然光照和共置光照条件下，在材料反射率估计方面显著优于先前的方法。

Conclusion: GLOW通过全局光照感知的逆向渲染框架，有效解决了共置光源-相机设置下的复杂光照挑战，在室内场景材料反射率估计方面取得了显著改进。
...
TL;DR: CoordSpeaker框架通过手势标注生成和分层控制扩散模型，实现了语音与文本描述协调控制的手势生成，解决了现有方法中语义先验缺失和多模态协调控制的难题。
...
Method: 提出CoordSpeaker框架：1) 通过新颖的手势标注框架，利用运动-语言模型生成多粒度描述性标注，弥合语义先验差距；2) 基于此构建条件潜在扩散模型，采用统一跨数据集运动表示和分层控制去噪器，实现高度控制的协调手势生成。

Result: 实验表明，该方法能生成高质量手势，既与语音节奏同步，又与任意标注语义一致，相比现有方法实现了更优性能且效率更高。首次探索了手势理解与标注来解决手势生成中的语义差距问题。

Conclusion: CoordSpeaker通过双向手势-文本映射的新视角，实现了协调的标注赋能语音伴随手势合成，为多模态协调控制的手势生成提供了创新解决方案。
...
Result: ClearGCD能够无缝集成到参数化GCD方法中，在多个基准测试中持续优于最先进的方法
...
<details>
  <summary>Details</summary>
Motivation: 多模态多目标跟踪对自动驾驶系统至关重要，但可见光和热红外模态之间存在显著的非线性分布差异，传统的拼接或相加等简单融合策略会导致模态冲突，降低跟踪精度

Method: 提出DM³T框架，将多模态融合重新定义为迭代特征对齐过程：1）跨模态扩散融合模块进行迭代跨模态协调；2）可插拔扩散精炼器增强统一特征表示；3）分层跟踪器自适应处理置信度估计；统一检测、状态估计和数据关联到在线跟踪框架

Result: 在VT-MOT基准上达到41.7 HOTA，相对现有最优方法提升1.54%，无需复杂后处理，代码和模型已开源

Conclusion: DM³T通过扩散模型的迭代细化机制有效解决多模态特征融合中的非线性分布差异问题，实现了更准确、时间一致的目标轨迹生成，为多模态多目标跟踪提供了新思路
...
TL;DR: P2C框架通过将提示学习重构为动态去噪任务，用语义云代替静态点表示，提升视觉语言模型的泛化能力


<details>
  <summary>Details</summary>
Motivation: 当前多模态提示学习方法局限于优化单一静态点表示，这种范式存在脆弱性、在基类上容易过拟合、对新颖或模糊类别泛化能力差的问题。作者认为稳健的泛化需要学习语义云（嵌入空间上的分布）

Method: 提出Points-to-Clouds（P2C）框架，受扩散模型启发，将提示学习重构为动态去噪任务。核心是双重去噪机制：动态提示去噪（DPD）机制用退火噪声扰动文本提示以学习更平滑的语义景观；辅助的V-L映射器去噪损失将映射器重新任务化为去噪自编码器

Result: 在11个数据集上的广泛实验表明P2C始终优于强基线。在基类到新颖类泛化基准上，该方法实现了79.7%的调和平均值，相对于基线有1.4%的相对改进

Conclusion: P2C通过将提示学习重构为动态去噪任务，用语义云代替静态点表示，显著提升了视觉语言模型的泛化能力，为解决当前多模态提示学习方法的局限性提供了新思路
...
TL;DR: CORTEX是一个结合视觉和文本信息的图像变化描述框架，通过视觉语言模型提取场景级文本知识来增强对细微变化的识别能力。
...
Method: CORTEX包含三个核心模块：图像级变化检测器识别低级视觉差异；推理感知文本提取模块使用视觉语言模型生成组合推理描述；图像-文本双重对齐模块对齐视觉和文本特征进行细粒度关系推理。
...
TL;DR: ViGG是一种鲁棒的RGB-D点云配准方法，通过视觉-几何相互引导策略解决传统方法对图像信息利用不足的问题，在多个数据集上优于现有方法。
...
Method: 提出ViGG方法，采用相互引导策略：1) 在视觉-几何组合形式中解决团对齐问题，使用几何引导设计抑制模糊团；2) 提出视觉引导的几何匹配方法，利用视觉先验确定搜索空间，提取高质量、对噪声不敏感的对应关系。

Result: 在3DMatch、ScanNet和KITTI数据集上的实验表明，该方法在无学习和基于学习的设置中都优于最近的最先进方法。

Conclusion: 相互引导策略使ViGG具有优越的鲁棒性，适用于各种RGB-D配准任务，解决了现有方法对图像信息利用不足的问题。
...
TL;DR: 该研究评估了2025年三种视觉语言模型在识别艺术品情感表达方面的能力，发现模型能较好识别图像内容和情感，但在抽象或象征性图像上表现不佳，且存在答案不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 情感是艺术表达的基本方面，但由于其抽象性，艺术品中的情感实现形式多样且受历史变迁影响，分析需要艺术史专业知识。研究旨在探索当前视觉语言模型能够检测情感表达的哪些方面。

Method: 采用案例研究方法，测试三种视觉语言模型（Llava-Llama和两个Qwen模型），向这些模型提出四组复杂度递增的问题（一般内容、情感内容、情感表达、情感符号），并进行定性专家评估。

Result: 视觉语言模型在识别图像内容方面表现惊人地好，通常也能识别描绘的情感和表达方式。模型在具体图像上表现最佳，但在高度抽象或高度象征性图像上失败。可靠识别符号仍然存在根本性困难，且模型继续表现出已知的大语言模型弱点——对相关问题提供不一致的答案。

Conclusion: 当前视觉语言模型在识别艺术品情感表达方面取得了一定进展，能够处理具体图像的内容和情感识别，但在抽象象征性图像和符号识别方面仍有局限，且存在答案一致性问题，需要进一步改进。
...
Method: 提出神经矩阵计算框架（NeuMatC），通过无监督学习从参数到对应矩阵运算结果的低秩连续映射。训练完成后，NeuMatC仅需少量基本操作（如矩阵乘法和非线性激活）即可在任意参数上高效计算。

Result: 在合成和真实数据集上的实验表明，NeuMatC在保持可接受精度的同时，相比广泛使用的NumPy基线，在参数化矩阵求逆上实现3倍以上加速，在参数化SVD上实现10倍以上加速。

Conclusion: NeuMatC通过利用参数维度上的低秩性和连续性，为参数化矩阵运算任务提供了一种高效解决方案，显著减少了冗余计算，在无线通信等应用中展现出优越性能。
...
TL;DR: ReImage是一个基于神经水印的图像自恢复框架，通过将图像的洗牌版本作为水印嵌入自身，实现篡改区域的准确恢复。


<details>
  <summary>Details</summary>
Motivation: 随着AIGC的快速发展，数字媒体的真实性受到关注。图像自恢复技术可以从被篡改版本重建原始内容，有助于理解攻击者意图并恢复可信数据。然而现有方法在准确恢复篡改区域方面存在不足。

Method: 提出ReImage框架：1）将目标图像的洗牌版本作为水印嵌入图像自身；2）设计生成器为神经水印优化水印；3）引入图像增强模块细化恢复图像；4）分析并解决洗牌水印的关键限制。

Result: ReImage在多种篡改场景下实现了最先进的性能，能够持续生成高质量的恢复图像。

Conclusion: ReImage通过神经水印和洗牌水印技术有效解决了图像自恢复的挑战，代码和预训练模型将在发表后发布。
...
TL;DR: 本研究深入评估YOLOv8算法在条形码和二维码识别中的效率，通过不同模型规模（Nano、Small、Medium）的对比，展示了模型缩放对检测精度的影响。
...
Method: 使用YOLOv8算法，在Kaggle数据集上进行大规模训练和微调，专门针对条形码和二维码检测任务。研究评估了三个不同规模的模型迭代版本：Nano、Small和Medium，重点关注精度、召回率和F1分数等评估指标。

Result: 实验结果显示随着模型规模的增加，检测精度显著提升：Nano模型达到88.95%准确率，Small模型达到97.10%准确率，Medium模型达到94.10%准确率。Small模型表现最佳，展示了模型缩放带来的增量改进。

Conclusion: YOLOv8在计算机视觉领域取得了重要进展，特别是在条形码和二维码检测任务上。模型缩放对物体识别性能有显著影响，这项研究为基于深度学习的计算机视觉技术发展提供了重要见解。
...
TL;DR: DenoiseGS首次利用3D高斯泼溅的高效性进行连拍去噪，通过高斯自一致性损失和log加权频率损失解决噪声输入下的几何退化和细节丢失问题，在去噪效果和推理速度上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的连拍去噪方法在处理大运动时效果不佳，或者计算成本过高。3D高斯泼溅在3D重建中表现出高效性，但将其应用于噪声输入时面临高斯点云退化和细节丢失的挑战。

Method: 提出DenoiseGS框架，包含两个关键技术：1) 高斯自一致性损失，用干净输入生成的高质量高斯点云来正则化噪声输入预测的几何；2) log加权频率损失，在频谱域进行自适应加权监督，强调高频细节。

Result: DenoiseGS在噪声条件下的连拍去噪和新视角合成任务上显著超越最先进的NeRF方法，同时实现250倍的推理速度提升。

Conclusion: DenoiseGS成功将3D高斯泼溅的高效性应用于连拍去噪，通过创新的损失函数解决了噪声输入下的几何退化和细节保留问题，在性能和效率上都取得了显著提升。
...
TL;DR: One-to-All Animation是一个统一的角色动画框架，能够处理任意布局的参考图像，解决了现有方法只能处理空间对齐参考姿势对的问题。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在角色动画方面取得了进展，但仅限于处理空间对齐的参考姿势对，无法处理参考姿势不对齐的情况。需要开发一个能够处理任意布局参考图像的通用框架。

Method: 1) 将训练重新定义为自监督的外绘任务，将不同布局的参考转换为统一的遮挡输入格式；2) 设计参考提取器提取全面的身份特征；3) 集成混合参考融合注意力处理不同分辨率和动态序列长度；4) 引入身份鲁棒的姿势控制解耦外观与骨骼结构；5) 使用令牌替换策略实现连贯的长视频生成。

Result: 大量实验表明，该方法在角色动画和图像姿势迁移方面优于现有方法，能够处理任意布局的参考图像。

Conclusion: One-to-All Animation是一个统一的框架，能够处理参考姿势不对齐的问题，实现了高质量的角色动画和图像姿势迁移，代码和模型将开源。
...
TL;DR: FLEX-Seg框架将扩散模型生成数据中的图像-语义掩码不对齐问题转化为鲁棒学习机会，通过多尺度边界特征捕获、不确定性边界强调和难度感知采样实现优越的域泛化性能。


<details>
  <summary>Details</summary>
Motivation: 语义分割的域泛化面临域偏移挑战，特别是在恶劣条件下。虽然基于扩散的数据生成方法有潜力，但它们引入了生成图像与语义掩码之间的固有不对齐问题。

Method: FLEX-Seg包含三个关键组件：(1) 多尺度粒度自适应原型，捕获跨多个尺度的边界特征；(2) 不确定性边界强调，基于预测熵动态调整学习重点；(3) 难度感知采样，逐步聚焦于具有挑战性的样本。

Result: 在五个真实世界数据集上的实验显示，相比最先进方法取得了一致的改进，在ACDC和Dark Zurich数据集上分别实现了2.44%和2.63%的mIoU提升。

Conclusion: 通过利用固有不对齐而非强制严格对齐，FLEX-Seg学习鲁棒表示同时捕获丰富的风格变化。处理不完美合成数据的自适应策略能实现优越的域泛化。
...
TL;DR: 该论文提出了一种新颖的分层多模态表示方法，通过多视角图像和文本描述在输入空间显式对齐大型视觉语言模型，以改善3D场景理解性能。
...
Method: 提出分层多模态表示方法：1) 使用包含检测物体3D坐标的文本描述捕获空间关系；2) 采用多视角图像（俯视视角和四个方向视角）确保全面场景覆盖；3) 引入分层特征表示，将图像块级特征聚合为视图级和场景级表示。

Result: 在situated 3D Q&A和general 3D Q&A基准测试上的实验结果表明该方法有效。

Conclusion: 通过显式对齐输入空间的多视角图像和文本描述，结合分层特征表示，能够显著提升3D场景推理性能。
...
<details>
  <summary>Details</summary>
Motivation: 极端光照条件会同时降低3D地图重建和语义分割的准确性，这对紧耦合系统特别有害。现有系统缺乏对光照变化的鲁棒性，需要一种能够实现光照不变性的解决方案。

Method: 提出了两个核心设计：1）内在外观归一化模块主动解耦场景的内在属性（如反照率）和瞬时光照，学习标准化的光照不变外观模型；2）动态辐射平衡损失仅在图像曝光不佳时激活，直接在辐射场层面进行针对性优化，防止极端光照下的误差累积。

Result: 在公开数据集上的评估表明，该系统在相机跟踪、地图质量以及语义和几何精度方面达到了最先进的性能，展现出前所未有的鲁棒性。

Conclusion: 通过主动的光照不变性设计和被动的极端光照校正机制的协同作用，该语义SLAM框架实现了对极端光照条件的高度鲁棒性，为光照变化环境下的视觉SLAM提供了有效解决方案。
...
TL;DR: BlockVid是一个用于生成长达一分钟视频的块扩散框架，通过语义感知稀疏KV缓存、块强制训练策略以及专门的噪声调度和混洗技术，解决了现有半自回归视频生成方法中的长时误差累积和缺乏细粒度评估基准的问题。


<details>
  <summary>Details</summary>
Motivation: 生成长视频是开发世界模型的关键步骤，但现有的半自回归（块扩散）方法面临两个持久挑战：1）KV缓存导致的长时误差累积；2）缺乏细粒度的长视频基准和连贯性感知指标。

Method: 提出了BlockVid框架，包含：1）语义感知稀疏KV缓存；2）块强制训练策略；3）专门的块级噪声调度和混洗技术。同时引入了LV-Bench，一个用于分钟级视频的细粒度基准，包含评估长程连贯性的新指标。

Result: 在VBench和LV-Bench上的大量实验表明，BlockVid在生成长达一分钟的高质量、连贯视频方面始终优于现有方法。在LV-Bench上，相比最先进方法，VDE Subject指标提升22.2%，VDE Clarity指标提升19.4%。

Conclusion: BlockVid通过创新的块扩散框架和专门的训练策略，有效解决了长视频生成中的误差累积问题，并提供了全面的评估基准，为开发更先进的世界模型和AI模拟器奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成模型已具备多参考生成和编辑能力，但现有基准数据集主要关注单参考或少量参考图像的生成，无法全面评估模型在不同多参考条件下的性能进展和弱点。同时，现有任务定义模糊，未能捕捉多参考设置的内在难度。

Method: 提出了MultiBanana基准，通过广泛覆盖五种多参考特定问题来系统评估模型能力：(1) 参考图像数量变化，(2) 参考图像间的领域不匹配（如照片vs动漫），(3) 参考与目标场景间的尺度不匹配，(4) 包含罕见概念的参考图像，(5) 多语言文本参考。

Result: 通过对多种文本到图像模型的分析，揭示了它们在多参考生成任务上的优越性能、典型失败模式和改进空间。MultiBanana将作为开放基准发布，推动多参考图像生成领域的发展并建立公平比较的标准基础。

Conclusion: MultiBanana基准填补了多参考图像生成评估的空白，通过系统设计的多参考特定问题全面评估模型能力，为领域发展提供了标准化评估框架和公平比较基础。
...
TL;DR: MIMM-X框架通过最小化互信息来解耦因果特征与多个伪相关性，解决医学影像中的捷径学习问题，提高模型在新环境中的泛化能力。
...
Result: 在三个数据集（UK Biobank、NAKO、CheXpert）和两种成像模态（MRI和X射线）上评估，结果显示MIMM-X能有效缓解多个伪相关性的捷径学习。
...
TL;DR: 提出频谱弱化框架用于视觉自回归模型，无需重新训练或架构修改，通过频谱域构建可控弱模型，实现高质量无条件生成和强条件对齐。


<details>
  <summary>Details</summary>
Motivation: 现有无分类器引导方法主要针对扩散模型，缺乏适用于视觉自回归模型的通用引导机制。需要一种无需重新训练、特定条件或架构修改的方法，能在自回归模型中实现类似的无条件生成增强和条件对齐效果。

Method: 提出频谱弱化框架：1）利用可逆频谱变换保留信息，通过选择部分频谱实现可控信息减少；2）在内部表示的通道维度进行频谱选择，避免扩散模型的结构约束；3）引入两种频谱重归一化策略确保弱化过程的数值稳定性。

Result: 在离散和连续自回归模型上进行广泛实验，包括文本或类别条件。结果显示该方法能够实现高质量的无条件生成，同时在条件生成中保持强大的提示对齐能力。

Conclusion: 频谱弱化框架为视觉自回归模型提供了一种有效的引导机制，无需重新训练或架构修改，既支持高质量无条件生成，又能在条件生成中保持强对齐，突破了现有方法对扩散模型的依赖。
...
Method: 评估不同优化器对ViT在虹膜识别中准确性和稳定性的影响

Result: 提供了关于如何增强生物特征识别模型鲁棒性的见解

Conclusion: 优化器选择对基于ViT的虹膜识别系统性能有重要影响，需要系统评估以提升生物特征认证的鲁棒性
...
TL;DR: 本文提出ViRL框架，将视觉动作重构为核心推理原语而非可选工具，通过端到端强化学习训练模型基于视觉证据进行推理，在感知、幻觉和推理基准上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言推理框架将视觉动作视为可选工具，导致推理缺乏视觉基础，产生"用图像思考的幻觉"——模型看似基于视觉证据，实则依赖与上下文无关的动作，既不能改进感知，也无法引导推理得出正确答案。

Method: 提出视觉理性学习（ViRL）框架，将视觉动作重构为视觉理性化（视觉版的思维链）。ViRL包含三个核心组件：1）使用真实理性进行过程监督；2）通过步骤级奖励塑造实现目标对齐；3）细粒度信用分配以区分正确、冗余和错误的动作。

Result: 通过纯端到端强化学习训练，ViRL在感知、幻觉和推理等多个基准测试中取得了最先进的结果，证明了视觉理性化作为任务无关、过程基础范式的有效性。

Conclusion: 视觉理性化是一种任务无关、过程基础的新范式，能够构建透明、可验证且可信赖的视觉语言模型，确保每个动作都对推理链有实质性贡献，使模型"基于正确的视觉理由得出正确答案"。
...
Method: 1) 引入动态一致性检查策略减少跨时空MVS估计不确定性；2) 提出全局-局部深度正则化方法从单目深度中提取时空一致的几何信息，增强4D体积内的连贯几何和外观学习。

Result: 在N3DV和Technicolor数据集上的实验验证了GC-4DGS的有效性。在PSNR指标上，比RF-DeRF（最新的稀疏输入动态视图合成方法）高2.62dB，比原始4DGS高1.58dB，同时保持效率。

Conclusion: GC-4DGS成功将几何一致性融入4D高斯泼溅，解决了稀疏输入视图下的动态场景渲染问题，实现了高质量实时渲染，并可在资源受限的IoT边缘设备上无缝部署。
...
Method: 提出基于命中层级概念的遮挡感知纹理框架，通过多视角光线投射量化网格面的相对深度，将网格面划分为从最外层到最内层的可见性层级。采用两阶段可见性控制策略：先逐步揭示具有结构一致性的内部区域，然后使用预训练扩散模型为每层纹理化。提出软UV空间混合技术，基于视图相关可见性置信度加权融合各层纹理。

Result: 实验结果表明，GOATex在可见和被遮挡表面都能生成无缝、高保真度的纹理，一致优于现有方法。该方法无需对预训练扩散模型进行昂贵的微调，并允许为外部和内部网格区域分别提供提示，实现对分层外观的细粒度控制。

Conclusion: GOATex通过创新的遮挡感知纹理框架，成功解决了3D网格内部纹理生成的挑战，实现了完整、无缝的纹理生成，为3D内容创作提供了更精细的控制能力。
...
TL;DR: 提出一种量化每张图像对NeRF重建贡献度的方法，通过重建质量指标评估贡献，并验证移除低贡献图像对重建效果的影响


<details>
  <summary>Details</summary>
Motivation: 在XR和数字媒体领域，数据估值和货币化日益重要。对于从图像集进行3D场景重建，并非所有输入图像对最终输出贡献相等。NeRF虽然能实现逼真的3D重建，但野外场景图像质量参差不齐，存在遮挡和瞬态物体，导致输入图像的效用不均

Method: 提出一种量化每张图像对NeRF重建贡献度的方法，基于PSNR和MSE等重建质量指标评估贡献，通过移除低贡献图像来验证方法有效性

Result: 验证了通过移除低贡献图像可以测量其对重建保真度的影响，表明该方法能有效识别对NeRF重建贡献较小的图像

Conclusion: 该方法能够量化图像对NeRF重建的贡献度，为数据估值和优化3D重建过程提供实用工具
...
TL;DR: 生成式基础模型虽能通过逼真图像修复去除视觉伪影，但其对医学AI性能影响不确定。研究发现修复儿科手部X光片中的非解剖标记会显著降低骨龄和性别预测性能，表明视觉逼真的修复可能掩盖临床相关特征并引入潜在偏差。
...
Method: 使用RSNA骨龄挑战数据集，选取200张原始X光片，使用gpt-image-1生成语言提示修复非解剖伪影，共生成600张修复图像。通过深度学习集成模型评估骨龄估计和性别分类的下游性能，使用平均绝对误差(MAE)和ROC曲线下面积(AUC)作为指标，并通过像素强度分布检测结构变化。

Result: 修复显著降低了模型性能：骨龄MAE从6.26个月增加到30.11个月，性别分类AUC从0.955下降到0.704。修复图像显示像素强度偏移和不一致性，表明存在结构修改且无法通过简单校准纠正。

Conclusion: 尽管视觉上逼真，基于基础模型的修复可能掩盖细微但临床相关的特征，即使编辑仅限于非诊断区域也会引入潜在偏差。这强调了在将此类生成工具整合到临床AI工作流之前，需要进行严格的任务特定验证。
...
<details>
  <summary>Details</summary>
Motivation: 尽管英语场景文本识别已相当成熟，但印度语言场景文本识别仍面临巨大挑战，主要原因是脚本多样性、非标准字体、不同书写风格，以及缺乏高质量数据集和开源模型。

Method: 创建了Bharat场景文本数据集(BSTD)，包含来自印度各地6500多张场景图像的10万+单词，涵盖11种印度语言和英语，并进行了精细标注以支持多种场景文本任务。

Result: 通过将最先进的英语模型适配到印度语言进行评估，结果突显了印度语言场景文本识别的挑战和机遇，表明该领域仍有很大改进空间。

Conclusion: BSTD数据集是推动印度语言场景文本识别研究的重要一步，所有模型和数据都已开源，为后续研究提供了宝贵资源。
...
TL;DR: SpaceMind是一个专门为空间推理设计的多模态大语言模型，仅从RGB输入就能实现3D空间理解，通过相机引导的模态融合模块取代浅层特征融合，在多个3D空间推理基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有的大视觉语言模型在多模态理解方面表现良好，但在3D空间推理（如距离估计、尺寸比较和跨视角一致性）方面仍有困难。现有的3D感知方法要么依赖辅助3D信息，要么通过浅层特征融合增强RGB-only模型，缺乏真正的空间基础智能。

Method: SpaceMind采用双编码器架构，集成VGGT作为空间理解编码器和InternViT作为2D视觉编码器。核心创新是将相机表示作为主动引导模态而非被动元数据，引入轻量级的相机引导模态融合模块，应用相机条件偏置到空间标记，分配反映几何重要性的查询无关权重，并使用相机嵌入来门控融合表示。

Result: SpaceMind在VSI-Bench、SQA3D和SPBench基准测试中建立了新的最先进结果，在VSI-Bench和SPBench上大幅超越开源和专有系统，在SQA3D上达到最先进性能。

Conclusion: 相机引导的模态融合是为视觉语言模型配备真正空间基础智能的有效且实用的归纳偏置，证明了仅从RGB输入就能实现强大的3D空间推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 韩国数字医疗市场增长和AI医疗数据使用增加，但特应性皮炎等皮肤疾病缺乏客观诊断方法，现有研究使用高质量图像不适用于临床实际，需要兼顾准确性和快速响应

Method: 提出基于集成学习的皮肤病变检测系统ENSEL，通过集成多种深度学习模型来提高诊断准确性

Result: ENSEL在大多数图像中实现了高召回率，处理速度小于1秒

Conclusion: ENSEL系统有助于皮肤病变的客观诊断，促进数字医疗的进步
...
<details>
  <summary>Details</summary>
Motivation: 基于文本指令的图像编辑虽然直观，但缺乏对编辑强度的精确控制。现有方法难以实现细粒度的属性调整，需要更精确的数值控制机制。

Method: 提出NumeriKontrol框架，通过Numeric Adapter编码数值编辑尺度，以即插即用方式注入扩散模型。采用任务分离设计支持零样本多条件编辑，使用Common Attribute Transform (CAT)数据集提供高质量监督训练。

Result: 实验表明NumeriKontrol能够在广泛的属性编辑场景中实现准确、连续、稳定的尺度控制，支持多种指令的任意顺序组合。

Conclusion: NumeriKontrol通过引入数值控制机制，推进了基于指令的图像编辑技术，实现了精确、可扩展、用户可控的图像操作。
...
TL;DR: MathSight是一个大学级多模态数学推理基准，旨在分离和量化视觉输入对推理的贡献。研究发现，随着问题难度增加，视觉信息的贡献逐渐减少，甚至无图像输入的模型表现优于多模态变体。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在多模态数学推理方面取得了显著进展，但视觉信息在推理中真正起到多大作用尚不清楚。现有基准测试报告了强大的整体性能，但很少分离图像模态的作用，无法确定模型是否真正利用视觉理解还是仅仅依赖语言先验。

Method: 提出了MathSight基准，每个问题包含多个视觉变体（原始图像、手绘图、照片拍摄）和纯文本条件用于控制比较。通过在不同视觉条件下测试最先进的视觉语言模型，分离和量化视觉输入对推理的影响。

Result: 实验显示一个一致趋势：随着问题难度增加，视觉信息的贡献逐渐减少。令人惊讶的是，没有任何图像输入的Qwen3-VL模型超越了其多模态变体和GPT-5，这表明当前模型可能过度依赖语言先验而非真正的视觉理解。

Conclusion: 需要像MathSight这样的基准来推动未来模型实现真正的视觉基础推理。当前视觉语言模型在多模态数学推理中可能并未充分利用视觉信息，而是主要依赖语言能力。
...
TL;DR: 提出db-SP方法解决扩散变换器序列并行中的负载不平衡问题，通过双级分区实现近乎完美的工作负载平衡，相比现有方法获得1.25倍端到端加速


<details>
  <summary>Details</summary>
Motivation: 扩散变换器（DiT）推理的序列并行对于降低视觉生成延迟至关重要，但在应用于采用块级稀疏注意力的模型时，由于注意力头间稀疏度变化和稀疏掩码中密集块的不规则分布，导致严重的负载不平衡问题

Method: 提出db-SP方法，包含：1）形式化稀疏不平衡比来量化不平衡问题；2）双级分区方法在头和块级别实现近乎完美的工作负载平衡；3）动态确定并行度以适应去噪步骤和层间不断变化的稀疏模式

Result: 实验结果显示，db-SP相比最先进的序列并行方法平均获得1.25倍端到端加速和1.40倍注意力特定加速

Conclusion: db-SP有效解决了扩散变换器序列并行中的负载不平衡问题，通过智能分区和动态并行度调整实现了显著性能提升
...
<details>
  <summary>Details</summary>
Motivation: 传统基于预训练视觉模型的方法受限于"情感鸿沟"，难以有效应用于图像情感分类任务。心理学研究表明语言具有高度可变性、包含丰富信息，能够有效消除情感鸿沟，因此探索基于文本的方法。

Method: 提出ACIEC方法：1) 设计分层多级对比损失检测图像中的情感概念；2) 提出情感属性链式思维推理生成情感句子；3) 利用预训练语言模型综合情感概念和情感句子进行情感分类；4) 设计基于语义相似度采样的对比损失解决情感数据集中类内差异大、类间差异小的问题；5) 考虑包含嵌入文本的图像。

Result: 在多个基准测试上的广泛实验表明，该方法能有效桥接情感鸿沟，并取得优于现有方法的结果。

Conclusion: 基于纯文本的ACIEC方法通过情感描述生成和语言模型分析，成功解决了图像情感分类中的情感鸿沟问题，为这一领域提供了新的有效解决方案。
...
TL;DR: DualCamCtrl是一个端到端的扩散模型，通过双分支框架同时生成相机一致的RGB和深度序列，使用语义引导互对齐机制融合两种模态，显著提升相机控制视频生成的几何一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基于光线表示的相机姿态控制视频生成方法缺乏足够的场景理解和几何感知能力，导致生成的视频与指定相机轨迹的一致性不足。

Method: 提出双分支框架同时生成相机一致的RGB和深度序列，引入语义引导互对齐机制进行RGB-深度融合，通过语义引导和相互增强的方式协调两种模态。

Result: 相比先前方法，相机运动误差减少超过40%，实现了更一致的相机控制视频生成。分析发现深度和相机姿态在不同去噪阶段有不同影响，早期和后期阶段在形成全局结构和细化局部细节方面起互补作用。

Conclusion: DualCamCtrl通过双分支框架和语义引导互对齐机制，能更好地解耦外观和几何建模，生成更忠实遵循指定相机轨迹的视频，显著提升了相机控制视频生成的几何一致性。
...
Result: InstanceV在实例级视频生成控制方面表现显著，在定性和定量评估中，在通用质量和实例感知指标上都优于现有最先进模型。

Conclusion: InstanceV框架成功解决了文本到视频生成中实例级控制不足的问题，实现了细粒度可控的视频生成，并通过新的评估基准为领域提供了更全面的评估方法。
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中文档矫正面临相机视角变化和物理变形的极端挑战，现有方法难以处理复杂的复合失真

Method: 多阶段渐进式框架：1) 全局仿射变换纠正透视失真；2) 几何变形纠正处理纸张卷曲折叠；3) 内容感知迭代过程消除细粒度内容失真。同时提出新的评估指标：布局对齐OCR指标(AED/ACER)和掩码AD/AAD(AD-M/AAD-M)

Result: 在多个具有挑战性的基准测试中建立了新的SOTA性能，AAD指标显著降低14.1%-34.7%，在现实应用中表现出优越效果

Conclusion: 提出的多阶段渐进式框架能有效处理复杂文档失真，新的评估指标提供了更准确的性能评估，代码将开源
...
TL;DR: 提出RA-RFT方法解决视频时序定位中的硬无关查询拒绝问题，构建HI-VTG数据集，在多种场景下验证有效性


<details>
  <summary>Details</summary>
Motivation: 现有VTG模型假设查询总是相关的，即使查询无关也会预测时间段。现有方法只能拒绝完全无关的查询，无法处理语义相似但实际不相关的硬无关查询

Method: 提出Refusal-Aware Reinforcement Fine-Tuning (RA-RFT)方法，基于GRPO框架，整合格式、拒绝-IoU、解释和查询修正四个奖励目标，提升相关性判别和细粒度语义推理能力

Result: 构建了HI-VTG数据集，在硬无关VTG、简单重排RA-VTG和人工标注RA-VTG等多种场景下验证了方法的有效性，并展示了该方法可扩展到多种LVLM-based VTG模型

Conclusion: RA-RFT方法能有效拒绝硬无关查询，提升VTG模型在相关性感知场景下的性能，具有可扩展性和实用性
...
TL;DR: 提出了REVEAL-Bench基准和REVEAL框架，通过证据链和多专家模型实现可解释的AI生成图像检测


<details>
  <summary>Details</summary>
Motivation: 随着生成模型的快速发展，AI生成的逼真图像越来越难与真实图像区分，对社会信任和信息完整性构成严重威胁，急需高效且真正可解释的图像取证方法。现有方法主要依赖事后合理化或视觉判别，缺乏可验证的证据链，限制了因果解释的生成和泛化能力。

Method: 1) 引入REVEAL-Bench基准，这是首个基于多专家模型证据链构建的推理增强多模态AI生成图像检测基准；2) 提出REVEAL框架，集成检测与专家基础的强化学习，奖励机制专门优化检测准确性、解释保真度和基于明确取证证据的逻辑连贯性。

Result: 大量实验结果表明，REVEAL显著提高了检测准确性、解释保真度和跨模型鲁棒泛化能力，为可解释图像取证设定了新的最先进水平。

Conclusion: REVEAL框架通过证据链和推理增强方法，解决了现有可解释取证方法的局限性，实现了细粒度、可解释且可验证的检测结果，为AI生成图像检测提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言预训练框架（如CLIP）在零样本任务上表现出色，但将单个文本标记与特定图像块或区域对齐的方法难以捕捉跨多个图像区域的组合语义。为了解决这一局限性，需要开发能够有效处理多区域组合语义的方法。

Method: 提出PowerCLIP框架，采用幂集对齐方法，通过最小化图像区域幂集与文本解析树之间的损失来优化区域到短语的对齐。为了解决朴素幂集构造带来的指数级计算成本，引入了高效的非线性聚合器（NLAs），将复杂度从O(2^M)降低到O(M)，同时以任意精度近似精确损失值。

Result: 广泛的实验表明，PowerCLIP在零样本分类和检索任务上优于现有最先进方法，证明了该方法的组合性和鲁棒性。代码将公开提供。

Conclusion: PowerCLIP通过幂集对齐有效解决了跨多个图像区域的组合语义理解问题，同时通过非线性聚合器实现了计算效率的提升，为视觉语言预训练提供了更强大的组合理解能力。
...
TL;DR: ViP3DE利用预训练视频生成模型的时间一致性先验，通过单次前向传播实现多视角一致的3D编辑，避免传统迭代方法的耗时和过平滑问题。
...
Method: 1) 利用视频生成模型的时间一致性先验，基于单个编辑视角生成其他一致编辑视角；2) 提出运动保持噪声混合技术，使视频模型能在预定义相机位姿生成编辑视角；3) 引入几何感知去噪，将3D几何先验整合到视频模型中增强多视角一致性。

Result: ViP3DE能在单次前向传播中实现高质量的3D编辑结果，在编辑质量和速度上都显著优于现有方法。

Conclusion: 通过利用视频生成模型的时间一致性先验，ViP3DE成功解决了传统3D编辑方法的多视角一致性问题，实现了高效高质量的3D编辑。
...
TL;DR: GeoWorld通过整合几何模型和视频生成，从单张图像生成高质量3D场景，解决了现有方法中的几何扭曲和模糊问题
...
Method: 1) 先生成连续视频帧，然后利用几何模型提取全帧几何特征；2) 提出几何对齐损失提供真实世界几何约束；3) 引入几何适配模块确保几何特征的有效利用
...
<details>
  <summary>Details</summary>
Motivation: 当前病理学基础模型参数量巨大（可达数十亿参数），产生高维嵌入，在计算资源有限的研究或临床环境中应用受限，需要开发更高效的模型部署方案。

Method: 采用多教师蒸馏框架，结合RADIO蒸馏和Matryoshka表示学习技术，通过知识蒸馏减少模型大小，同时保持可调节的嵌入维度，实现模型压缩和效率提升。

Result: 在10个公共病理学基准测试中，Pathryoshka在性能相当的情况下将模型大小减少86-92%，相比同等大小的单教师蒸馏模型，准确率中位数提升7.0%。

Conclusion: Pathryoshka通过高效的知识蒸馏技术，在不牺牲准确率或表示丰富性的前提下，实现了病理学基础模型的本地化部署，为更广泛的研究和临床社区提供了先进病理学模型的民主化访问途径。
...
Method: 通过对象检测和已知CAD模型的姿态估计语义描述工业场景，实现实时数字孪生渲染；提供可扩展的分层标注策略，将姿态标注与逻辑和结构缺陷标注统一；使用简单的距离测量方法进行缺陷检测。

Result: 在汽车轴向磁通电机质量检测用例中，与真实掩码相比获得了高达63.3%的交并比(IoU)得分，即使在半控制工业条件下使用简单距离测量方法。
...
TL;DR: 该研究首次探索了指令调优在提升大语言模型表格数据生成能力方面的效果，通过创建高质量指令数据集并在有限计算资源下进行调优，实现了与GPT-4o相当的表格生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有表格指令调优研究主要集中在问答和推理任务上，忽视了表格数据生成这一重要方向。同时，表格指令调优通常需要大量数据和计算资源，本研究旨在探索在有限资源和数据条件下实现有效表格数据生成的可能性。

Method: 首先创建高质量的表格数据指令数据集以提升LLM理解效率，然后使用开源模型Llama3.1-8B-Instruct在该数据集的训练集上进行指令调优。整个训练过程仅使用7K条指令，在A100 GPU上运行不到6小时。

Result: 实验结果显示，通过高质量数据集和有限的指令调优，在表格数据生成任务上达到了与最先进的商业模型GPT-4o相当的性能水平。

Conclusion: 研究表明，即使使用有限的数据和计算资源，通过精心设计的指令调优也能显著提升LLM的表格数据生成能力，为资源受限环境下的表格数据处理提供了可行方案。
...
Conclusion: 相比单纯追求渲染质量，增强光栅化过程对参数误差的鲁棒性对3DGS-SLAM的跟踪精度更为关键，CB-KNN方法为此提供了一种实用有效的解决方案。
...
Conclusion: DAONet-YOLOv8在复杂茶园环境中有效提升了病虫害检测性能，优于主流检测模型，为茶园病虫害智能监测提供了有效解决方案
...
TL;DR: SynthFun3D：首个基于任务的3D场景合成方法，通过家具资产数据库生成可完成特定动作的3D室内环境，自动识别功能元素，为数据稀缺的3D功能理解提供低成本、大规模的数据生成方案。


<details>
  <summary>Details</summary>
Motivation: 3D功能理解需要识别场景中完成特定动作的功能元素（如"打开床头柜第二个抽屉的正确把手"），但真实世界数据收集和标注成本高昂，导致数据稀缺问题。

Method: 提出SynthFun3D方法：基于动作描述，使用带有部件级标注的家具资产数据库生成3D室内环境；通过推理动作自动识别和检索正确功能元素的3D掩码；实现低成本、大规模的高质量标注数据生成。

Result: 用户研究表明SynthFun3D相比其他方法提高了场景-提示一致性；定量结果显示生成的数据可以替代真实数据（性能损失很小）或补充真实数据（提升性能）。

Conclusion: SynthFun3D为数据稀缺的3D应用提供了经济高效、可扩展的解决方案，能够生成高质量标注数据，有效缓解3D功能理解领域的数据瓶颈问题。
...
TL;DR: MRRE是一种无需训练、在推理时增强多语言推理能力的方法，通过注入预计算的表征向量来提升低资源语言的推理性能，同时保持输入输出语言一致性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型和视觉语言模型在英语上的表现远优于低资源语言，这引发了多语言应用中的公平性问题。现有方法要么依赖昂贵的多语言训练，要么使用外部翻译工具进行提示，这两种方法都资源密集且对翻译质量敏感。

Method: MRRE在推理处理过程中在特定层顺序注入两个预计算向量：跨语言推理增强向量（将非英语推理表征引导至英语空间以解锁多语言推理能力）和目标语言输出锚定向量（恢复目标语言的分布以保持输入输出语言一致性）。

Result: 在六个先进LLM和LVLM上的四个推理基准测试表明，MRRE持续提升非英语推理能力，平均增益5.48%，在低资源语言（泰语和斯瓦希里语）上最高提升7.54%，同时将输入输出语言一致性提高3.78%。

Conclusion: MRRE提供了一种无需额外训练数据或工具的有效方法，通过表征工程增强多语言推理能力，解决了低资源语言与英语之间的性能差距问题。
...
TL;DR: 对比不同合成数据生成方法（领域随机化、领域自适应、生成式AI等）在缩小仿真-现实差距中的效果，发现简单的基于特征的方法（如感知哈希）在准确性和资源效率上优于复杂的生成式AI方法。


<details>
  <summary>Details</summary>
Motivation: 降低数据生成和标注成本是工业与机器人领域机器学习应用的关键挑战。虽然合成渲染是潜在解决方案，但缩小仿真-现实差距通常需要专家干预。本研究旨在评估各种无需手动标注的上下文合成数据生成方法。

Method: 基准测试了多种领域随机化（DR）和领域自适应（DA）技术，包括基于特征的方法、生成式AI（GenAI）和经典渲染方法。重点关注低层和高层特征对齐的有效性和效率，以及基于提示控制的扩散式DA方法。在工业数据集（汽车和物流）和公共机器人数据集上验证方法。

Result: 如果有足够多样性的渲染数据作为种子，简单的基于特征方法（如亮度过滤和感知哈希）在准确性和资源效率上优于复杂的生成式AI方法。感知哈希表现最佳，在工业和机器人数据集上分别达到98%和67%的mAP50分数。生成式AI方法在数据生成上有显著时间开销，且相比简单方法没有明显的仿真-现实性能提升。

Conclusion: 研究结果为高效缩小仿真-现实差距提供了可操作的见解，表明简单的基于特征方法（特别是感知哈希）能够使仅用合成数据训练的模型获得高真实世界性能，而无需依赖计算成本高昂的生成式AI方法。
...
TL;DR: 提出基于单张地面RGB图像估算地上生物量的新方法，通过生成AGB密度图实现密集预测，在合成和真实数据集上分别达到1.22和1.94 kg/m²的中位误差。


<details>
  <summary>Details</summary>
Motivation: 森林在全球生态系统中至关重要，但传统的地上生物量估算方法依赖劳动密集的实地测量或遥感技术，在茂密植被中存在显著局限性。需要一种可扩展、成本效益高的解决方案。

Method: 将AGB估算构建为密集预测任务，引入AGB密度图概念，每个像素表示归一化的树木生物量。利用SPREAD合成3D数据集，通过异速生长方程计算AGB，训练模型预测AGB密度图，然后整合得到整个场景的AGB估计。

Result: 在SPREAD合成数据集上达到1.22 kg/m²的中位AGB估算误差，在真实图像数据集上达到1.94 kg/m²。这是首个直接从单张RGB图像估算地上生物量的方法。

Conclusion: 该方法为森林监测提供了可扩展、可解释且成本效益高的解决方案，同时通过公民科学计划实现了更广泛的参与。开辟了从单张RGB图像直接估算地上生物量的新途径。
...
TL;DR: FACT-GS提出了一种频率对齐的复杂度感知纹理高斯泼溅框架，通过自适应采样密度分配来改善纹理空间利用率，在相同参数预算下恢复更清晰的高频细节。
...
Method: FACT-GS将纹理参数化重新表述为可微分的采样密度分配问题，用可学习的频率感知分配策略替换均匀纹理。该策略通过变形场实现，其雅可比矩阵调制局部采样密度。基于2D高斯泼溅，在固定分辨率纹理网格上执行非均匀采样。

Result: FACT-GS在保持实时性能的同时，在相同参数预算下恢复了更清晰的高频细节，提高了纹理空间利用率。

Conclusion: 通过频率对齐的复杂度感知纹理采样，FACT-GS解决了传统均匀采样方法的效率问题，实现了更高效的高斯泼溅外观建模。
...
TL;DR: 该研究探索大型视觉语言模型在自动驾驶安全监控中的应用，通过构建数据集评估模型对驾驶员和道路双视角视频的处理能力，发现微调后的LVLMs能生成准确的安全驾驶指令，但仍存在检测复杂事件的挑战。


<details>
  <summary>Details</summary>
Motivation: 虽然大型视觉语言模型在视觉任务中表现出色，但现有研究主要关注道路视角监控，缺乏对驾驶员行为的同步监控。为确保驾驶安全，需要同时处理驾驶员视角和道路视角的视频输入，以检测如驾驶时使用手机等风险行为。

Method: 研究构建了包含驾驶员视角和道路视角同步视频的数据集，评估预训练LVLMs的性能，并对模型进行微调以提升其在双视角安全监控任务中的表现。

Result: 实验结果显示，预训练LVLMs在双视角安全监控任务中效果有限，但经过微调的模型能够生成准确且具有安全意识的驾驶指令。然而，模型在检测视频中细微或复杂事件方面仍存在困难。

Conclusion: 微调后的LVLMs在自动驾驶安全监控中具有应用潜力，但需要进一步改进以提升对复杂事件的检测能力。研究中的发现和错误分析为该领域LVLM系统的改进提供了有价值的见解。
...
TL;DR: 提出一种基于人类色彩感知现象学的变分色彩对比度增强方法，满足感知启发的能量函数要求，并开发了计算复杂度从O(N²)降低到O(NlogN)的优化算法。


<details>
  <summary>Details</summary>
Motivation: 现有基于人类色彩视觉现象学的显式色彩校正算法在图像特征（如对比度和分散度）方面的行为难以表征，需要一种更系统的方法来设计感知启发的色彩增强模型。

Method: 提出变分色彩对比度增强公式，定义感知启发能量函数的基本要求，识别出满足所有要求的一类显式泛函，选择三个具有基本意义的泛函，使用梯度下降法计算最小值，并开发将计算复杂度从O(N²)降低到O(NlogN)的通用方法。

Result: 建立了满足感知启发要求的能量函数类别，识别了三个具有基本意义的显式泛函，展示了与现有模型的异同，并实现了计算效率的显著提升。

Conclusion: 该变分方法为设计感知启发的色彩对比度增强算法提供了系统框架，通过数学形式化保证了感知特性，同时通过算法优化实现了计算效率的大幅提升。
...
TL;DR: 提出了GeoSeg-1M百万级遥感指令分割数据集、GeoSeg-Bench评估基准和UniGeoSeg统一框架，解决了现有方法任务碎片化和指令数据有限的问题，在多个基准上实现了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有遥感指令驱动分割方法存在任务表述碎片化和指令数据有限的问题，这阻碍了模型的有效理解和泛化能力，需要构建大规模数据集和统一框架来解决这些问题。

Method: 1) 通过自动掩码过滤和指令生成流水线构建GeoSeg-1M百万级数据集；2) 创建GeoSeg-Bench评估基准；3) 提出UniGeoSeg统一框架，包含任务感知文本增强、潜在知识记忆和渐进式训练策略。

Result: GeoSeg-1M包含590K图像、117个类别和1.1M图像-掩码-指令三元组。UniGeoSeg在GeoSeg-Bench和多个公共基准上实现了最先进的性能，并展现出强大的零样本泛化能力。

Conclusion: 该工作通过构建大规模数据集、评估基准和统一框架，显著推进了遥感指令驱动分割领域的发展，为可访问和可泛化的遥感应用提供了有力支持。
...
TL;DR: 提出Markov-VAR模型，通过将视觉自回归建模重新表述为非全上下文马尔可夫过程，使用滑动窗口压缩历史信息，在保持性能的同时大幅降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 传统VAR模型基于全上下文依赖（使用所有先前尺度预测下一尺度），虽然能促进稳定全面的表示学习，但计算效率低下且开销巨大，严重阻碍了VAR的实际应用和可扩展性。

Method: 将VAR重新表述为非全上下文马尔可夫过程，提出Markov-VAR。采用马尔可夫尺度预测：将每个尺度视为马尔可夫状态，引入滑动窗口将某些先前尺度压缩为紧凑的历史向量，以补偿因非全上下文依赖造成的历史信息损失。将历史向量与马尔可夫状态结合，形成在马尔可夫过程下演化的代表性动态状态。
...
<details>
  <summary>Details</summary>
Motivation: 现有流模型存在采样效率问题：Flow-based模型需要昂贵的ODE数值积分，Rectified Flow需要多次计算密集的reflow迭代才能实现直线路径，MeanFlow在高度弯曲的流上训练时收敛慢且监督噪声大

Method: 提出Rectified MeanFlow框架，仅使用单次reflow步骤建模整流轨迹上的平均速度场，避免了对完美直线轨迹的需求；引入简单的截断启发式方法减少残余曲率

Result: 在ImageNet 64×64、256×256和512×512分辨率上的大量实验表明，Re-MeanFlow在样本质量和训练效率上均优于先前的一步流蒸馏和Rectified Flow方法
...
TL;DR: 该论文提出了DEAL-300K数据集，用于扩散模型编辑图像的像素级定位，并开发了基于视觉基础模型和多频率提示调优的定位框架，在测试集上达到82.56%的F1分数。
...
Conclusion: DEAL-300K数据集填补了扩散模型编辑图像定位的空白，提出的定位框架能有效捕捉编辑特征，为未来扩散图像操作定位研究提供了重要资源和基准。
...
TL;DR: VQRAE提出了一种统一的多模态表示方法，通过向量量化的自编码器同时生成用于图像理解的连续语义特征和用于视觉生成的离散标记。
...
Conclusion: VQRAE首次探索了统一表示方法，能够在单一标记器中同时产生用于理解的连续语义特征和用于生成的离散标记，为构建统一多模态模型提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 水下目标跟踪面临波长相关衰减和散射的挑战，这些物理现象在不同深度和水条件下严重扭曲目标外观。现有的基于陆地数据训练的跟踪器无法泛化到这些物理驱动的退化情况。

Method: 提出了MANTA框架，包含：1）双正对比学习策略，将时间一致性与Beer-Lambert增强相结合，产生对时间和水下失真都鲁棒的特征；2）多阶段管道，通过物理信息的二次关联算法增强基于运动的跟踪，该算法整合了几何一致性和外观相似性，用于遮挡和漂移下的重识别。

Result: 在四个水下基准测试（WebUOT-1M, UOT32, UTB180, UWCOT220）上，MANTA实现了最先进的性能，将Success AUC提高了高达6%，同时确保了稳定的长期泛化水下跟踪和高效的运行时间。

Conclusion: MANTA通过将表示学习与跟踪设计相结合，并引入物理信息增强，成功解决了水下目标跟踪中的物理退化问题，提出了新的评估指标（CSC和GAS）来补充标准IoU度量，为水下跟踪提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本到视频（T2V）和图像到视频（I2V）模型虽然能够从文本描述或初始帧生成动态视频，但往往无法提供与内容分离的显式运动表示，这限制了内容创作者的适用性。这些方法通常在运动保真度和提示遵循之间进行权衡，要么过度拟合源结构，要么偏离描述的动作。

Method: DisMo通过图像空间重建目标直接从原始视频数据学习抽象运动表示。该表示是通用的，独立于外观、对象身份或姿态等静态信息。方法将运动语义与外观解耦，可以通过轻量级适配器与任何现有视频生成器结合使用。

Result: DisMo实现了开放世界的运动迁移，允许在语义无关的实体之间传输运动，即使是在完全不同的类别之间也无需对象对应关系。在Something-Something v2和Jester等基准测试中，该方法在零样本动作分类任务上持续优于V-JEPA等最先进的视频表示模型。

Conclusion: DisMo提供了一种有效的运动表示学习范式，能够解耦运动与内容，支持准确的运动迁移和条件控制。学习到的表示不仅适用于运动迁移任务，还能很好地支持下游运动理解任务，为视频生成和理解提供了新的可能性。
...
Method: 1) 正式定义交互式视频数据概念；2) 开发自动化流程将大规模非结构化文本-视频对转换为因果对齐的交互数据集；3) 基于14B图像到视频混合专家基础模型；4) 引入文本驱动的交互注入机制，实现对相机运动、角色行为和环境动态的细粒度控制。

Result: 1) 创建了交互聚焦的基准测试InterBench；2) 模型能够生成时间连贯且因果基础的交互式游戏视频；3) 能够忠实响应多样化和自由形式的用户指令，如"开门"、"画火炬"或"触发爆炸"。

Conclusion: Hunyuan-GameCraft-2通过指令驱动的交互范式，成功解决了现有生成式世界模型的局限性，实现了灵活、语义丰富的游戏世界交互建模，为生成式游戏环境创建提供了新方向。
...
TL;DR: 论文研究了在对象中心数据（多视角图像或3D模型）有限的情况下，通过四种数据合成方法微调目标检测模型在新类别上的性能，显著提升了模型在真实数据中的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统目标检测模型扩展到新类别需要大量标注数据，成本高且耗时，特别是对于长尾类别。当只有有限的对象中心数据可用时，需要有效的数据合成方法来提升模型性能。

Method: 提出了四种基于对象中心数据的数据合成方法：1) 简单图像处理技术；2) 3D渲染；3) 图像扩散模型。这些方法用于合成具有不同上下文连贯性和复杂度的真实杂乱图像。

Result: 在数据受限的实验设置下，这些方法显著提升了目标检测模型的性能，实现了在真实世界数据中的类别级泛化。

Conclusion: 对象中心数据设置下的数据合成方法能有效解决新类别检测中的数据稀缺问题，为有限数据条件下的目标检测模型扩展提供了实用解决方案。
...
TL;DR: VGT是一种新的视觉生成调优范式，能够激发任何视觉语言模型中的视觉生成潜力，显著降低对齐成本并加速连续空间中的自回归建模（20倍加速）。


<details>
  <summary>Details</summary>
Motivation: 尽管大型视觉语言模型通过预训练获得了与语言对齐的复杂视觉表示，但这些为多模态理解任务优化的表示是否具有视觉生成的潜力尚未充分探索。研究者希望探索如何激发VLMs中固有的视觉生成能力。

Method: 提出VGT（视觉生成调优）范式，通过将预训练VLMs的语义编码器与像素解码器的潜在表示对齐，构建VGT-AE，摒弃了为扩散变换器设计的纠缠像素级VAEs。在预训练良好的VLMs上进行高效的视觉生成调优。

Result: 在图像重建任务中，以28倍压缩比实现了26.67 PSNR和0.50 rFID，优于专用VAEs；在视觉生成任务中，在自回归模型中达到最先进水平，GenEval上0.77，DPG-Bench上78.73。实现了20倍加速收敛。
...
TL;DR: AnyTalker是一个多人生成框架，采用可扩展的多流处理架构，通过身份感知注意力机制迭代处理身份-音频对，实现任意规模的身份驱动，仅需单人视频训练即可学习多人说话模式。


<details>
  <summary>Details</summary>
Motivation: 现有的音频驱动多人说话视频生成方法面临两个主要挑战：1) 多样化的多人数据收集成本高昂；2) 驱动多个身份并保持连贯交互性困难。需要一种既能降低数据成本又能实现身份可扩展性的解决方案。

Method: 提出AnyTalker框架，核心创新包括：1) 扩展Diffusion Transformer的注意力块，引入身份感知注意力机制，迭代处理身份-音频对，实现任意规模的身份驱动；2) 训练流程仅依赖单人视频学习多人说话模式，仅需少量真实多人片段优化交互性；3) 设计了专门的评估指标和数据集来评估生成视频的自然度和交互性。

Result: 大量实验表明，AnyTalker在唇部同步、视觉质量和自然交互性方面表现优异，在数据成本和身份可扩展性之间取得了良好平衡。

Conclusion: AnyTalker通过创新的身份感知注意力机制和高效训练流程，成功解决了多人视频生成中的数据成本和交互性挑战，为音频驱动的多人说话视频生成提供了有效解决方案。
...
TL;DR: Video CoM提出交互式视频推理新范式，通过链式操作让模型主动"用视频思考"，而非被动处理视觉输入，显著提升细粒度时空理解能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视频理解中存在语义瓶颈：视频编码后推理完全在文本空间进行，视觉输入被当作静态上下文，模型无法重新观看、聚焦或验证证据，导致需要细粒度时空理解的任务上视觉推理能力不足。

Method: 1. 提出交互式视频推理新范式，将视频转化为主动认知工作空间；2. 开发Video CoM模型，通过链式操作进行迭代视觉动作以收集和精炼证据；3. 构建包含18K指令的Video CoM Instruct数据集用于多步操作推理微调；4. 采用强化学习优化操作策略，引入推理感知的分组相对策略优化方法，提供步骤级推理奖励而非仅稀疏答案奖励。

Result: 在九个视频推理基准测试中取得优异结果，平均性能比当前最先进模型提升3.6%，仅使用25K监督微调和3K强化学习样本，远少于可比大规模模型。消融研究表明推理感知奖励同时提高了准确性和可解释性。

Conclusion: 交互式视频推理范式通过让模型主动操作视频内容进行推理，突破了传统被动处理模式的限制，显著提升了视频理解的深度和准确性，为细粒度时空推理任务提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在动态视觉内容推理中存在逻辑不一致和视觉证据基础薄弱的问题，模型推理看似合理但实际依赖语言先验而非视觉内容。

Method: 提出强化学习方法：结合时间戳感知的监督微调和基于时序对齐奖励（TAR）的组相对策略优化（GRPO），增强时序精度和推理一致性。

Result: Video R2模型在11个视频推理基准测试中显著提升了思考答案一致性（TAC）、视频注意力分数（VAS）和准确率，实现了更准确可靠的视频理解。

Conclusion: 通过提升时序对齐和推理一致性，可以有效改善视频理解的准确性和可信度，Video R2模型为视频推理提供了更可靠的解决方案。
...
### [166] [Effective Hyper-clutter Artifacts Suppression for Ultrafast Ultrasound Doppler Imaging](https://arxiv.org/abs/2511.21833)
...
TL;DR: PETRIC是首个针对PET图像重建算法的计算速度优化挑战赛，提供了开源软件框架、评估指标和数据集，吸引了4个团队参与，为后续研究奠定了基础。


<details>
  <summary>Details</summary>
Motivation: 尽管医学影像领域已有多个类似挑战赛，但之前从未有针对PET图像重建的专门挑战。PET图像重建算法的计算效率对临床应用至关重要，需要建立标准化的评估框架。

Method: 提供开源软件供参与者实现重建算法；定义目标函数并重建"黄金标准"参考图像；提供量化算法性能的指标；收集并整理不同扫描仪、放射性核素和体模类型的体模数据集，分为训练集和评估集；发布自动化的计算框架作为开源软件。

Result: 4个团队共提交了9种算法参与挑战。这些算法利用了优化理论中的各种工具，包括预处理、随机梯度和人工智能。虽然大多数提交的方法在本质上看起来很相似，但具体实现导致了不同的算法性能表现。

Conclusion: 作为首个PET图像重建挑战赛，PETRIC的坚实基础使研究人员能够重用其框架，在新数据集或现有数据集上评估新的和现有的图像重建方法。挑战赛的变体版本已经并将继续在未来推出。
...
### [168] [The Biologically Effective Particle Number Ratio (BPNR): a new framework to quantify the therapeutic window in SFRT and other modalities](https://arxiv.org/abs/2511.22716)
*Niels Bassler,Giuseppe Schettino,Hugo Palmans,Thomas Friedrich,Kelvin Ng Wei Siang,Emanuele Scifoni,Emanuele Scifoni,Fardous Reaz*

Main category: physics.med-ph

TL;DR: 提出BPNR/RBPNR框架，基于生物结果而非空间剂量指标来量化治疗窗口变化，应用于空间分割放疗的疗效评估。


<details>
  <summary>Details</summary>
Motivation: 传统空间剂量指标（峰值、谷值、平均值）在评估空间分割放疗时存在模糊性和不一致性，无法与生物结果直接关联，难以准确比较不同治疗方式的治疗窗口变化。

Method: 引入生物有效粒子数比（BPNR）框架，定义为达到特定肿瘤控制概率和正常组织并发症概率所需总粒子数（与监测单位成正比）的比值。相对BPNR（RBPNR）用于比较不同治疗方式的治疗窗口。

Result: 将框架应用于临床前质子微束放疗与常规质子治疗的比较，在最高测试MU下质子微束放疗未出现正常组织并发症，RBPNR > 1.4，表明治疗窗口扩大。

Conclusion: BPNR/RBPNR方法提供了一种简洁、实验基础、模式无关的方式来量化治疗窗口变化，可应用于空间分割放疗、FLASH、高LET辐射及联合治疗等领域。
...
### [169] [Design and Construction of a Dedicated Radiolucent 8-element Flexible Radiofrequency (RF) Torso Coil for the 1.0T Australian MRI-Linac System](https://arxiv.org/abs/2511.22879)
*Mingyan Li,Ewald Weber,David E. J. Waddington,Shanshan Shan,Paul Liu,Bin Dong,Paul J. Keall,Feng Liu,Stuart Crozier*

Main category: physics.med-ph

TL;DR: 本文设计并验证了一种用于澳大利亚1.0T MRI-Linac的专用8通道柔性接收线圈阵列，相比现有全身线圈，在中心区域SNR提升约3倍，浅表区域提升8倍，支持GRAPPA加速因子达3的并行成像，且对放射治疗剂量影响小于1%。


<details>
  <summary>Details</summary>
Motivation: 澳大利亚1.0T MRI-Linac目前依赖全身射频线圈，但线圈与患者之间的大间距限制了信噪比，并制约了实时肿瘤追踪所需的快速成像能力。

Method: 设计并验证了专门针对澳大利亚MRI-Linac几何结构和束流通道要求的8通道柔性接收线圈阵列，通过电磁仿真、台架测量、体模成像和体内腹部成像进行验证，并进行剂量测量和动态束流噪声分析。

Result: 相比全身线圈，新线圈阵列在中心区域SNR提升约3倍，浅表区域提升8倍；体内成像显示解剖细节增强、组织边界更清晰；支持GRAPPA加速因子达3的并行成像；剂量差异低于1%，图像噪声无显著增加。

Conclusion: 该工作展示了适用于澳大利亚MRI-Linac患者旋转和束流传输的实用射频解决方案，可实现加速、高信噪比成像。

Abstract: Magnetic resonance imaging-guided linear accelerators (MRI-Linacs) are an emerging treatment technology that enable online soft-tissue visualisation and adaptive radiotherapy. The Australian 1.0 T MRI-Linac employs a fixed, inline beamline, with treatment angles achieved via patient couch rotation, and currently relies on a radiolucent whole-body radiofrequency (RF) coil. However, the large coil-patient separation limits signal-to-noise ratio (SNR) and constrains the fast imaging required for real-time tumour tracking. Here, we show the design and validation of a dedicated, flexible 8-element receive-only RF torso array tailored to the geometry and beam access requirements of the Australian MRI-Linac. Electromagnetic simulations, bench measurements, and phantom imaging demonstrate approximately three-fold SNR improvement at the centre and eight-fold improvement at superficial regions compared with the whole-body coil. In vivo abdominal imaging confirms enhanced anatomical detail and sharper tissue boundaries, and the increased SNR enables parallel imaging with GRAPPA acceleration factors up to three without severe artefacts. Dosimetry measurements and dynamic beam-on noise analysis confirm that the coil array is effectively radiolucent, with dose differences below 1% and no measurable degradation in image noise. This work demonstrates a practical RF solution for accelerated, high-SNR imaging compatible with patient rotation and beam delivery on the Australian MRI-Linac.

</details>


### [170] [Optimization and application of ultra-high field preclinical high-resolution and 3D 1H-MRSI using compressed sensing](https://arxiv.org/abs/2511.23331)
*Brayan Alves,Thanh Phong Lê,Gianna Nossa,Tan Toi Phan,Alessio Siviglia,Bernard Lanz,Wolfgang Bogner,Antoine Klauser,Bernhard Strasser,Cristina Cudalbu*

Main category: physics.med-ph

TL;DR: 本研究探索了压缩感知技术在超高场质子磁共振波谱成像中的应用，旨在通过加速方案实现高分辨率2D和3D/MRSI采集，将3D采集时间控制在30分钟以内。


<details>
  <summary>Details</summary>
Motivation: 超高场质子磁共振波谱成像在临床前研究中应用增加，但面临采集时间长和脑代谢物浓度低的挑战。需要开发加速方案来解决这些问题，特别是针对3D-1H-MRSI。

Method: 采用压缩感知技术作为加速方案，探索参数以实现最高加速比。研究包括2D-FID-MRSI（提高平面内分辨率）和3D-FID-MRSI（增加通过平面编码/切片）。重点优化k空间中心核心采样大小以获得最佳加速效果。

Result: 参数研究表明，通过优化k空间中心核心采样，可实现加速因子为4。在此基础上，实现了亚1μL名义体素尺寸的高矩阵2D-FID-MRSI，并添加9个额外相位编码/切片实现3D-FID-MRSI。与未加速的2D-FID-MRSI相比，在感兴趣切片内，光谱质量和代谢图具有足够准确性。

Conclusion: 本研究提出了一个稳健有效的协议，利用压缩感知技术实现3D-1H-MRSI，将采集时间控制在30分钟以内，同时保持最小技术限制和高质量采集。尽管在不同CS应用中观察到点扩散函数相关问题，但整体方案具有实用价值。
...
### [171] [Bridging Planning and Execution: Multi-Agent Path Finding Under Real-World Deadlines](https://arxiv.org/abs/2511.21886)
...
<details>
  <summary>Details</summary>
Motivation: 传统MAPF问题假设简化的机器人模型进行规划，忽略了执行时的动态约束、通信延迟和控制器变化等因素，这种规划与执行之间的差距对时间敏感应用造成问题。

Method: 提出REMAP执行感知MAPF规划框架，结合ExecTimeNet准确估计执行时间，可轻松集成到主流搜索式MAPF规划器中，用于解决具有现实截止时间的MAPF-RD问题。

Result: 实验显示REMAP在基准地图上（最多300个智能体）比基线方法（如恒定执行速度估计器）在解决方案质量上提升高达20%。

Conclusion: REMAP框架有效桥接了MAPF规划与执行之间的差距，通过准确估计执行时间，在时间敏感应用中显著提升了解决方案质量。
...
### [172] [RSPECT: Robust and Scalable Planner for Energy-Aware Coordination of UAV-UGV Teams in Aerial Monitoring](https://arxiv.org/abs/2511.21957)
*Cahit Ikbal Er,Amin Kashiri,Yasin Yazicioglu*

Main category: cs.RO

TL;DR: 本文提出了一种用于无人机与无人地面车辆协同执行长时间空中监测任务的鲁棒规划方法，通过MIP建模和RSPECT启发式算法解决能量约束下的最小时间路径规划问题。


<details>
  <summary>Details</summary>
Motivation: 无人机在执行长时间空中监测任务时面临能量限制问题，需要无人地面车辆作为移动充电站提供支持。同时，实际环境中存在未知障碍物、地形和风力等不确定性因素，需要鲁棒的规划方案以避免频繁重新规划。

Method: 首先将问题形式化为混合整数规划（MIP）模型，然后提出RSPECT启发式算法来解决这个NP难问题。该算法能够在不确定性环境下生成可执行的鲁棒轨迹规划。

Result: 提供了算法复杂度的理论分析，证明了生成方案的可行性和鲁棒性。通过仿真和实验验证了方法的性能，表明RSPECT算法具有可扩展性和高效性。

Conclusion: 提出的RSPECT方法能够有效解决能量约束下无人机-无人地面车辆协同的鲁棒规划问题，在不确定性环境中实现最小时间任务完成，为实际应用提供了可行的解决方案。
...
### [173] [Constant-Volume Deformation Manufacturing for Material-Efficient Shaping](https://arxiv.org/abs/2511.22042)
*Lei Li,Jiale Gong,Ziyang Li,Hong Wang*

Main category: cs.RO

TL;DR: 提出了一种保持体积的数字模具范式，通过实时体积一致性建模、几何信息变形预测和误差补偿策略，实现塑料材料的高可预测成型，达到98%以上材料利用率。


<details>
  <summary>Details</summary>
Motivation: 增材和减材制造虽然能实现复杂几何形状，但依赖于离散堆叠或局部去除，限制了连续可控变形，导致体积损失和形状偏差。需要一种能保持体积、减少浪费的可持续成型方法。

Method: 集成实时体积一致性建模、几何信息变形预测和误差补偿策略的数字模具范式。通过分析后成型点云的变形模式和误差趋势，校正弹性回弹和累积误差，保持体积一致性和表面连续性。

Result: 在五种代表性几何形状上的实验表明，系统能以高保真度再现目标形状，同时实现超过98%的材料利用率。

Conclusion: 该方法建立了数字化驱动、可重复的可持续零浪费成型路径，连接数字建模、实时传感和自适应成型，推动下一代可持续和可定制制造的发展。
...
### [174] [SwordRiding: A Unified Navigation Framework for Quadrotors in Unknown Complex Environments via Online Guiding Vector Fields](https://arxiv.org/abs/2511.22043)
*Xuchen Liu,Ruocheng Li,Bin Xin,Weijia Yao,Qigeng Duan,Jinqiang Cui,Ben M. Chen,Jie Chen*

Main category: cs.RO

TL;DR: 该论文提出了一种基于引导向量场的四旋翼无人机实时导航框架，通过在线构建GVF实现未知复杂环境中的闭环导航，提高了对外部扰动的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有四旋翼导航框架大多采用开环方式运行，难以应对环境不确定性（如风扰动、外部干扰），在未知复杂环境中的实时适应性存在核心挑战。

Method: 1) 机载感知模块构建环境的ESDF表示；2) 全局规划器生成离散无碰撞路径点；3) 通过均匀B样条参数化生成平滑参考轨迹；4) 从ESDF和优化后的B样条轨迹合成自适应引导向量场；5) 采用闭环导航范式。

Result: 大量仿真和真实世界实验表明，该方法相比传统方法具有更好的抗外部扰动鲁棒性和优越的实时性能。

Conclusion: 提出的基于引导向量场的统一实时导航框架能够有效处理未知复杂环境中的不确定性，通过闭环导航显著增强了系统鲁棒性，且与标准规划算法兼容。
...
### [175] [Design of an Adaptive Modular Anthropomorphic Dexterous Hand for Human-like Manipulation](https://arxiv.org/abs/2511.22100)
*Zelong Zhou,Wenrui Chen,Zeyun Hu,Qiang Diao,Qixin Gao,Yaonan Wang*

Main category: cs.RO

TL;DR: 提出一种由2个驱动器驱动4自由度的仿人手指拓扑结构，并基于此开发自适应模块化灵巧手，平衡驱动复杂性与灵巧性之间的权衡
...
Method: 提出4自由度2驱动器的仿人手指拓扑结构；探索手部协同作用的生物学基础和人类手势分析，将关节级协调和结构属性转化为模块化手指架构；基于这些仿生映射设计五指模块化手并建立其运动学模型
...
### [176] [Bayesian Decentralized Decision-making for Multi-Robot Systems: Sample-efficient Estimation of Event Rates](https://arxiv.org/abs/2511.22225)
*Gabriel Aguirre,Simay Atasoy Bingöl,Heiko Hamann,Jonas Kuckling*

Main category: cs.RO

TL;DR: 提出一种分散式贝叶斯框架，使机器人群体能够在危险环境中识别更安全区域，通过泊松过程建模危险事件，实现样本高效的安全决策。
...
Method: 采用分散式贝叶斯框架，机器人使用共轭先验逐步预测危险事件间隔时间，并推导置信度估计来调整行为。基于泊松过程建模未知危险事件率。

Result: 模拟结果显示，机器人群体能持续选择正确区域，同时通过样本高效性减少暴露于危险事件。相比基准启发式方法，在安全性和收敛速度方面表现更优。

Conclusion: 该框架扩展了群体决策的基准测试集，在危险动态环境中的自适应风险感知采样和探索方面具有应用潜力。
...
### [177] [MLATC: Fast Hierarchical Topological Mapping from 3D LiDAR Point Clouds Based on Adaptive Resonance Theory](https://arxiv.org/abs/2511.22238)
*Ryosuke Ofuchi,Yuichiro Toda,Naoki Masuyama,Takayuki Matsuno*

Main category: cs.RO

TL;DR: 本文提出了一种多层自适应共振理论拓扑聚类（MLATC）方法，通过分层结构加速大规模环境下的全局拓扑地图构建，解决了原始ATC-DT方法因穷举最近邻搜索导致的计算效率问题。


<details>
  <summary>Details</summary>
Motivation: 原始ATC-DT方法在构建全局拓扑地图时，其胜者选择机制需要对所有现有节点进行穷举最近邻搜索，随着地图规模增长会导致严重的可扩展性问题，限制了在大规模动态环境中的实时应用。

Method: 提出多层ATC（MLATC）方法：1）将节点组织成层次结构，实现从粗到细的分辨率搜索；2）采用自适应层添加机制，当较低层饱和时自动加深层次；3）层数不预先固定，保持用户定义超参数数量较少。

Result: 仿真实验显示MLATC相比原始ATC-DT显著加速拓扑地图构建，搜索时间随节点数呈亚线性（近似对数）增长。真实校园规模LiDAR数据集实验证实MLATC保持毫秒级每帧运行时间，实现大规模环境中的实时全局拓扑地图构建。

Conclusion: MLATC通过分层搜索结构和自适应层管理，有效解决了ATC-DT的可扩展性限制，在大规模动态环境中实现了高效的实时全局拓扑地图构建，显著提升了计算效率。
...
### [178] [Soft Fluidic Sheet Transistor for Soft Robotic System Enabling Fluid Logic Operations](https://arxiv.org/abs/2511.22318)
*Yuki Origane,Koya Cho,Hideyuki Tsukagoshi*

Main category: cs.RO

TL;DR: 本文提出了一种软质聚氨酯片状阀门（流体片晶体管FST），仅使用气动信号即可实现逻辑运算，可用于构建软体机器人系统。


<details>
  <summary>Details</summary>
Motivation: 为了在软体机器人系统中同时实现高功能性和灵活性，需要开发能够仅使用气动信号执行逻辑操作的元件，从而简化系统结构并提高适应性。

Method: 设计了一种软质聚氨酯片状阀门，当控制腔加压时，主通道沿中心轴压缩、屈曲并被压紧，实现阻断。这种阀门类似电路中的晶体管，基本类型作为NOT逻辑元件（FST），通过集成多个FST可在单一片材上实现正逻辑、NAND、NOR等逻辑运算。

Result: 成功开发了FST并验证了其工作原理、制造方法和特性。通过集成多个FST实现了多种逻辑运算，并构建了锁存电路（自保持逻辑电路）。结合触觉管作为流体检测器和流体执行器，构建了流体机器人系统原型，仅使用单管气压即可实现撞击障碍物时主动改变姿态的行为。

Conclusion: 提出的流体片晶体管（FST）能够仅使用气动信号执行逻辑运算，为软体机器人系统提供了高功能性和灵活性，验证了该方法的有效性，展示了在软体机器人控制中的实际应用潜力。
...
### [179] [Nonholonomic Narrow Dead-End Escape with Deep Reinforcement Learning](https://arxiv.org/abs/2511.22338)
*Denghan Xiong,Yanzhe Zhao,Yutong Chen,Zichun Wang*

Main category: cs.RO

TL;DR: 针对阿克曼转向车辆的狭窄死胡同逃逸问题，本文提出了一种基于深度强化学习的方法，相比传统规划器在采样效率和连接有效性方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 非完整约束限制了阿克曼转向车辆的可行速度，使其无法执行几何路径，狭窄死胡同逃逸需要复杂的进退序列操作。传统规划器将全局搜索与局部转向解耦，在狭窄通道中采样效率低且对间隙敏感。

Method: 1) 构建生成器采样与阿克曼运动学兼容的多阶段进退轨迹，并膨胀其包络以合成保证至少有一个可行逃逸路径的狭窄死胡同族；2) 构建强制运动学约束的训练环境，使用SAC算法训练策略；3) 与传统规划器进行对比评估。

Result: 在参数化的死胡同族中，学习到的策略解决了更大比例的实例，减少了操作次数，在相同感知和控制限制下保持了可比较的路径长度和规划时间。

Conclusion: 深度强化学习方法在阿克曼车辆的狭窄死胡同逃逸问题上优于传统规划器，特别是在采样效率和连接有效性方面，项目已开源。
...
### [180] [LLM-Based Generalizable Hierarchical Task Planning and Execution for Heterogeneous Robot Teams with Event-Driven Replanning](https://arxiv.org/abs/2511.22354)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的多机器人系统缺乏运行时事件驱动的重规划能力，难以处理任务失败、用户意图变化等动态事件，限制了实际机器人协作的鲁棒性和灵活性

Method: 分层架构：任务管理器LLM解释自然语言目标并分配任务；每个机器人运行本地LLM将技能组合为可执行代码；机载感知持续监控事件并分类；事件触发重规划机制

Result: 硬件实验：自主恢复成功率9/10，协调运输8/8，人机协作恢复5/5；仿真展示意图感知重规划；基准测试在22个场景中平均正确率高达0.91，重规划场景正确率1.0

Conclusion: CoMuRoS首次在物理机器人上实现了运行时事件驱动的重规划，相比现有LLM系统显著提升了多机器人和人机协作的鲁棒性和灵活性
...
### [181] [BINDER: Instantly Adaptive Mobile Manipulation with Open-Vocabulary Commands](https://arxiv.org/abs/2511.22364)
...
<details>
  <summary>Details</summary>
Motivation: 现有开放词汇移动操作方法只在离散时间点更新世界表示（如导航目标、路径点或动作结束时），导致机器人在更新间隔期间"失明"，造成级联失败：遗漏对象、错误检测延迟和重规划延迟。

Method: 提出BINDER双过程框架，包含深思响应模块（DRM，多模态LLM用于任务规划）和即时响应模块（IRM，VideoLLM用于连续监控）。两个模块双向协调：DRM进行战略规划并指导IRM关注点，IRM分析视频流更新记忆、纠正正在进行的动作并在必要时触发重规划。

Result: 在三个真实世界动态物体放置环境中评估，BINDER相比最先进的基线方法实现了显著更高的成功率和效率。

Conclusion: BINDER通过分离战略规划和连续监控，解决了保持环境感知与避免昂贵更新之间的权衡问题，能够在动态条件下实现稳健适应，适合真实世界部署。
...
### [182] [Visual-Geometry Diffusion Policy: Robust Generalization via Complementarity-Aware Multimodal Fusion](https://arxiv.org/abs/2511.22445)
*Yikai Tang,Haoran Geng,Sheng Zang,Pieter Abbeel,Jitendra Malik*

Main category: cs.RO

TL;DR: VGDP是一种多模态模仿学习框架，通过互补感知融合模块平衡使用RGB和点云信息，在视觉和空间扰动下表现出强鲁棒性。
...
Conclusion: 融合潜在空间的表达能力主要来自模态级dropout强制的互补性，交叉注意力主要作为轻量级交互机制而非鲁棒性的主要来源。
...
### [183] [RealD$^2$iff: Bridging Real-World Gap in Robot Manipulation via Depth Diffusion](https://arxiv.org/abs/2511.22505)
*Xiujian Liang,Jiacheng Liu,Mingyang Sun,Qichen He,Cewu Lu,Jianhua Sun*

Main category: cs.RO

TL;DR: 提出RealD²iff框架，通过干净到噪声的范式学习合成真实噪声深度，解决视觉sim2real差距，实现零样本机器人操作


<details>
  <summary>Details</summary>
Motivation: 现实世界机器人操作受视觉sim2real差距限制，模拟中的深度观测无法反映真实传感器的复杂噪声模式

Method: 提出RealD²iff分层扩散框架，将深度噪声分解为全局结构扭曲和局部扰动，采用频率引导监督和差异引导优化策略，构建六阶段模仿学习流程

Result: 能够生成真实世界深度数据构建干净-噪声配对数据集，实现零样本sim2real机器人操作，显著提升真实世界性能

Conclusion: 通过干净到噪声的范式成功桥接视觉sim2real差距，为纯模拟驱动的机器人学习提供有效解决方案
...
### [184] [BUDD-e: an autonomous robotic guide for visually impaired users](https://arxiv.org/abs/2511.22541)
...
TL;DR: 开发了一款名为BUDD-e的新型导盲机器人原型，并在真实医院场景中通过视障志愿者进行了测试，展示了出色的性能和用户接受度。


<details>
  <summary>Details</summary>
Motivation: 为视障用户开发新型导盲机器人，帮助他们在复杂环境中导航，提高独立性和安全性。

Method: 设计了BUDD-e导盲机器人原型，并在米兰Niguarda大都会医院的真实场景中，通过视障志愿者进行实地测试。
...
### [185] [Beyond Success: Refining Elegant Robot Manipulation from Mixed-Quality Data via Just-in-Time Intervention](https://arxiv.org/abs/2511.22555)
*Yanbo Mao,Jianlong Fu,Ruoxuan Zhang,Hongxia Xie,Meibao Yao*

Main category: cs.RO

TL;DR: 提出LIBERO-Elegant基准和解耦精炼框架，通过优雅执行概念和即时干预机制提升VLA模型执行质量，无需重新训练基础策略


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在机器人操作中表现出执行质量不稳定的问题，这源于人类演示数据的质量参差不齐，其中控制动作执行方式的隐式原则仅得到部分满足

Method: 1) 提出LIBERO-Elegant基准，包含明确的执行质量评估标准；2) 将优雅执行定义为隐式任务约束的满足；3) 通过离线校准Q学习训练优雅评判器；4) 在推理时使用即时干预机制，仅在决策关键时刻进行选择性精炼

Result: 在LIBERO-Elegant基准和真实世界操作任务上的实验表明，学习的优雅评判器显著提升了执行质量，即使在未见任务上也能有效工作

Conclusion: 该模型实现了不仅关注任务是否成功，还重视任务执行方式的机器人控制，通过解耦精炼框架在不修改基础VLA策略的情况下提升了执行质量
...
### [186] [Deadlock-Free Hybrid RL-MAPF Framework for Zero-Shot Multi-Robot Navigation](https://arxiv.org/abs/2511.22685)
*Haoyi Wang,Licheng Luo,Yiannis Kantaros,Bruno Sinopoli,Mingyu Cai*

Main category: cs.RO

TL;DR: 提出一种混合框架，将基于强化学习的反应式导航与按需多智能体路径规划相结合，通过检测并解决拓扑死锁来提升多机器人在密集环境中的导航性能。


<details>
  <summary>Details</summary>
Motivation: 多机器人在密集环境（如狭窄通道）中导航时，强化学习方法在遇到超出学习分布的新配置时容易产生死锁，现有方法在未见环境中的泛化能力有限。

Method: 提出混合框架：1) 安全层监控智能体进度检测死锁；2) 检测到死锁时触发协调控制器；3) 通过MAPF构建全局可行轨迹；4) 调节航点进度以减少导航中的智能体冲突。

Result: 在密集多智能体基准测试中，任务完成率从边际提升到接近普遍成功，显著减少了死锁和碰撞。与分层任务规划结合时，能为异构机器人实现协调导航。

Conclusion: 将反应式RL导航与选择性MAPF干预相结合，能够实现鲁棒的零样本性能，有效解决多机器人在密集环境中的导航挑战。
...
### [187] [Mechanistic Finetuning of Vision-Language-Action Models via Few-Shot Demonstrations](https://arxiv.org/abs/2511.22697)
...
TL;DR: Robotic Steering是一种基于机制可解释性的微调方法，通过少量演示识别并选择性微调与机器人任务物理、视觉和语言要求对齐的特定注意力头，在机器人任务适应中优于LoRA方法。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言动作模型微调方法缺乏特异性，无论任务的视觉、语言和物理特性如何，都调整相同的参数集。受神经科学中功能特异性的启发，作者认为针对特定任务微调稀疏模型表示会更有效。

Method: 提出Robotic Steering方法，基于机制可解释性，利用少量演示来识别并选择性微调与机器人任务物理、视觉和语言要求对齐的任务特定注意力头。

Result: 通过Franka Emika机器人臂的综合评估，Robotic Steering在任务变化下表现出优于LoRA的鲁棒性，同时减少了计算成本，并增强了将视觉语言动作模型适应到多样化机器人任务的可解释性。

Conclusion: Robotic Steering通过选择性微调任务特定注意力头，为视觉语言动作模型在机器人任务中的适应提供了一种更有效、更高效且更具可解释性的方法。
...
### [188] [A Two Degrees-of-Freedom Floor-Based Robot for Transfer and Rehabilitation Applications](https://arxiv.org/abs/2511.22705)
...
<details>
  <summary>Details</summary>
Motivation: 坐立动作能力对提高功能活动性和降低再住院风险至关重要。现有的升降辅助设备和部分体重支撑康复设备都无法根据不同的活动能力水平调整坐立训练。

Method: 开发了一种坐立训练设备，允许配置不同的阻抗和垂直/前向力，以适应多种训练需求，同时保持商业升降辅助设备的转移能力。

Result: 对健康成年人（男女均有，不同身高体重）的实验表明：1）设备对自然坐立运动学影响小；2）能在患者质心提供精确的减重；3）能在坐立动作开始时增加虚拟前向弹簧，辅助体重转移到脚部以便离座。

Conclusion: 该设备能够适应不同训练需求，有效辅助坐立动作训练，同时保持商业升降辅助功能，具有临床应用潜力。
...
TL;DR: 本文提出了一种用于复杂驾驶场景（如自主变道）的新型轨迹规划框架，通过将风险感知规划与保证碰撞避免集成到统一的优化框架中，实现了安全高效的轨迹生成。


<details>
  <summary>Details</summary>
Motivation: 在动态和交互式交通场景中，传统轨迹规划方法往往难以同时保证安全性、效率和舒适性。特别是在复杂驾驶场景如自主变道时，需要同时考虑静态和动态碰撞风险，并确保轨迹的平滑性和可行性。

Method: 首先构建动态风险场（DRF）来捕捉周围车辆的静态和动态碰撞风险；然后开发严格的时间变化凸可行空间生成策略，确保运动学可行性和安全要求；最后将轨迹规划问题表述为有限时域最优控制问题，并使用约束迭代线性二次调节器（iLQR）算法进行求解，同时优化轨迹平滑度、控制努力和风险暴露。

Result: 仿真结果表明，该方法在安全性和效率方面优于传统方法，实现了无碰撞轨迹，变道距离（28.59米）和时间（2.84秒）更短，同时保持平滑舒适的加速度模式。在密集环岛环境中，规划器表现出更强的鲁棒适应性，与APF、MPC和RRT基线相比，具有更大的安全裕度、更低的急动度和更好的曲率平滑度。

Conclusion: 集成的DRF与凸可行空间以及约束iLQR求解器为动态交互交通场景提供了安全、高效和舒适的轨迹生成平衡解决方案，在复杂驾驶场景中表现出优越性能。
...
### [190] [Beyond Egocentric Limits: Multi-View Depth-Based Learning for Robust Quadrupedal Locomotion](https://arxiv.org/abs/2511.22744)
*Rémy Rahem,Wael Suleiman*

Main category: cs.RO

TL;DR: 本文提出了一种多视角深度感知的腿式机器人运动框架，结合第一人称和第三人称视角，通过师生蒸馏方法提升机器人在动态运动中的环境感知能力和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前腿式机器人的动态运动主要依赖第一人称视角感知，当机器人视野被遮挡时性能受限。需要利用多视角互补信息来增强机器人的环境感知能力。

Method: 采用师生蒸馏方法，学生策略学习融合本体感知与双深度流（第一人称和第三人称视角），引入广泛的域随机化，包括随机远程相机丢失和3D位置扰动，模拟空地协同感知。

Result: 仿真结果表明，多视角策略在跨越间隙、台阶下降等动态动作上优于单视角基线，且在第三人称相机部分或完全不可用时仍能保持稳定性。适度的视角偏差在训练中被良好容忍。

Conclusion: 异构视觉反馈显著提升了四足机器人运动的鲁棒性和敏捷性。该研究为多视角感知在动态腿式运动中的应用提供了有效框架，相关实现已公开以支持可复现性。
...
### [191] [CAPE: Context-Aware Diffusion Policy Via Proximal Mode Expansion for Collision Avoidance](https://arxiv.org/abs/2511.22773)
*Rui Heng Yang,Xuan Zhao,Leo Maxime Brunswic,Montgomery Alban,Mateo Clemente,Tongtong Cao,Jun Jin,Amir Rasouli*

Main category: cs.RO

TL;DR: CAPE提出了一种上下文感知的扩散策略框架，通过先验种子迭代引导精炼过程扩展轨迹分布模式，在未见环境中实现更好的碰撞避免和泛化性能。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在机器人模仿学习中能捕捉多模态轨迹，但需要大规模数据集，这在碰撞避免等挑战性任务中成本高昂。测试时需要泛化到多种障碍物类型和空间配置，纯数据获取不现实。

Method: 提出CAPE框架，通过上下文感知先验和引导扩展轨迹分布模式。采用先验种子迭代引导精炼过程：生成初始轨迹计划，执行短前缀轨迹，将剩余轨迹扰动到中间噪声水平形成上下文感知先验，然后通过上下文感知引导去噪迭代扩展模式支持。

Result: 在杂乱未见模拟和真实世界设置中评估CAPE，相比SOTA方法分别获得高达26%和80%的更高成功率，在未见环境中表现出更好的泛化能力。

Conclusion: CAPE通过上下文感知先验和引导扩展轨迹分布模式，能够在未见环境中采样无碰撞轨迹同时保持目标一致性，解决了扩散模型在机器人模仿学习中需要大规模数据集的问题。
...
### [192] [Improving Robotic Manipulation Robustness via NICE Scene Surgery](https://arxiv.org/abs/2511.22777)
*Sajjad Pakdamansavoji,Mozhgan Pourkeshavarz,Adam Sigal,Zhiyuan Li,Rui Heng Yang,Amir Rasouli*

Main category: cs.RO

TL;DR: NICE框架通过自然图像修复增强视觉多样性，减少模仿学习中的分布外差距，无需额外数据收集即可提升机器人操作的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的视觉干扰会显著降低机器人操作的性能和安全性，现有方法需要额外数据收集或模拟器访问，限制了可扩展性。

Method: 提出NICE框架，利用图像生成框架和大型语言模型对现有演示数据进行三种编辑操作：物体替换、重新风格化和移除干扰物体，保持空间关系和动作标签一致性。

Result: 在高度杂乱场景中，空间可预测性准确率提升超过20%；在含干扰物的环境中，操作成功率平均提高11%；目标混淆率降低6%，碰撞率减少7%。

Conclusion: NICE框架能有效减少分布外差距，提升视觉鲁棒性和安全性，且无需额外数据收集或模型训练，可直接应用于现有机器人数据集。
...
### [193] [Distracted Robot: How Visual Clutter Undermine Robotic Manipulation](https://arxiv.org/abs/2511.22780)
*Amir Rasouli,Montgomery Alban,Sajjad Pakdamansavoji,Zhiyuan Li,Zhanguang Zhang,Aaron Wu,Xuan Zhao*

Main category: cs.RO

TL;DR: 提出了一种从心理物理学角度评估机器人操作策略在杂乱场景中性能的方法，使用统一的杂乱度量标准，并在仿真和真实世界中进行实验，发现场景杂乱会显著降低VLA模型性能达34%。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法在机器人操作策略面对杂乱场景时存在不足，需要从心理物理学角度建立统一的杂乱度量标准，以更全面地评估策略性能。

Method: 提出统一的杂乱度量方法，考虑环境因素、干扰物数量、特征和排列；在超真实仿真和真实世界中系统构建评估场景；对视觉-语言-动作模型进行广泛实验。

Result: 场景杂乱显著降低策略性能达34%；不同VLA策略具有独特的脆弱性和较低的成功场景一致性；杂乱度量是性能下降的有效指标；微调增强数据不能同等缓解所有杂乱负面影响。

Conclusion: 需要更全面的评估协议来理解机器人操作策略在杂乱场景中的性能，统一的杂乱度量标准为系统评估提供了有效工具，揭示了当前VLA模型的局限性。
...
### [194] [Threat-Aware UAV Dodging of Human-Thrown Projectiles with an RGB-D Camera](https://arxiv.org/abs/2511.22847)
*Yuying Zhang,Na Fan,Haowen Zheng,Junning Liang,Zongliang Pan,Qifeng Chen,Ximin Lyu*

Main category: cs.RO

TL;DR: 提出基于RGB-D相机和人体姿态估计的无人机实时躲避系统，通过预测攻击者动作和抛射物轨迹实现高效躲避


<details>
  <summary>Details</summary>
Motivation: 无人机在执行运输、航拍等任务时容易受到人为抛射物攻击，需要超低延迟响应和敏捷机动能力来躲避突然快速的攻击

Method: 受棒球运动启发，利用RGB-D相机结合人体姿态估计与深度信息预测攻击者动作轨迹和抛射物轨迹，并引入不确定性感知躲避策略

Result: 感知系统达到高预测精度，在有效距离和延迟方面优于基线；躲避策略处理时空不确定性确保无人机安全；真实实验证明系统在不同场景下具有可靠躲避能力和出色鲁棒性

Conclusion: 提出的实时躲避系统能有效应对突然攻击，在各种场景下表现出可靠的躲避能力和优秀的鲁棒性
...
### [195] [MARVO: Marine-Adaptive Radiance-aware Visual Odometry](https://arxiv.org/abs/2511.22860)
...
Method: 1. 前端：基于Transformer的特征匹配器结合物理感知辐射适配器补偿颜色通道衰减和对比度损失；2. 后端：因子图融合惯性、压力和视觉测量，使用GTSAM库进行关键帧估计；3. 强化学习姿态图优化器学习SE(2)上的最优回缩动作。

Result: 实现了实时全状态最大后验估计，能够产生几何一致的特征对应关系，并在局部最小值问题上超越传统最小二乘求解器。

Conclusion: MARVO通过融合物理建模、可微分匹配和强化学习优化，有效解决了水下视觉定位的关键挑战，实现了鲁棒的水下定位性能。
...
### [196] [SUPER-AD: Semantic Uncertainty-aware Planning for End-to-End Robust Autonomous Driving](https://arxiv.org/abs/2511.22865)
*Wonjeong Ryu,Seungjun Yu,Seokha Moon,Hojun Choi,Junsung Park,Jinkyu Kim,Hyunjung Shim*

Main category: cs.RO

TL;DR: 提出了一种基于相机端到端自动驾驶框架，通过估计BEV空间中的偶然不确定性并将其融入规划，结合车道跟随正则化，在NAVSIM基准上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶系统缺乏对不确定性的显式建模，假设感知输出完全可靠，这在模糊或观测不良的场景中存在安全隐患。需要开发能够处理感知不确定性的规划框架。

Method: 1) 提出相机端到端框架，在BEV空间中直接估计偶然不确定性；2) 生成密集的、不确定性感知的可行驶性地图，捕捉语义结构和几何布局；3) 引入车道跟随正则化，编码车道结构和交通规则，在正常条件下稳定轨迹规划，同时保留超车、变道等操作的灵活性。

Result: 在NAVSIM基准测试中达到最先进性能，在具有挑战性的NAVHARD和NAVSAFE子集上取得显著提升，证明了偶然不确定性建模与驾驶先验结合能显著提高相机端到端自动驾驶的安全性和可靠性。

Conclusion: 通过将偶然不确定性直接建模到BEV空间并融入规划，结合车道结构先验，实现了鲁棒且可解释的轨迹规划，显著提升了自动驾驶系统在不确定性条件下的安全性和可靠性。
...
### [197] [Seeing before Observable: Potential Risk Reasoning in Autonomous Driving via Vision Language Models](https://arxiv.org/abs/2511.22928)
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶系统在罕见复杂场景中面临安全挑战，特别是对"潜在风险"（风险尚未可观测但可从细微前兆推断）的识别能力不足。现有数据集缺乏此类案例和因果推理链标注，限制了系统的语义理解和推理能力

Method: 1. 引入PotentialRiskQA视觉语言数据集，包含结构化场景描述、语义前兆和推断风险结果标注；2. 提出PR-Reasoner框架，基于视觉语言模型进行车载潜在风险推理；3. 在数据集上微调模型提升性能

Result: 实验结果表明，在PotentialRiskQA上微调使PR-Reasoner在潜在风险推理任务上的性能显著优于基线视觉语言模型

Conclusion: 该数据集和模型为开发具有更强预见性和主动安全能力的自动驾驶系统奠定了基础，推动更智能、更具韧性的自动驾驶车辆发展
...
### [198] [Commanding Humanoid by Free-form Language: A Large Language Action Model with Unified Motion Vocabulary](https://arxiv.org/abs/2511.22963)
*Zhirui Liu,Kaiyang Ji,Ke Yang,Jingyi Yu,Ye Shi,Jingya Wang*

Main category: cs.RO

TL;DR: Humanoid-LLA：一种大型语言动作模型，可将自由形式语言指令映射为人形机器人可执行的全身动作，解决了语言条件全身控制的挑战。
...
Method: 提出Humanoid-LLA模型，包含三个核心组件：1）统一运动词汇表，将人类和人形机器人运动基元对齐到共享离散空间；2）从特权策略蒸馏的词汇导向控制器，确保物理可行性；3）使用强化学习和动态感知奖励的物理感知微调阶段，增强鲁棒性和稳定性。

Result: 在仿真和真实Unitree G1人形机器人上的广泛评估表明，Humanoid-LLA在保持高物理保真度的同时实现了强大的语言泛化能力，在运动自然性、稳定性和执行成功率方面优于现有语言条件控制器。

Conclusion: Humanoid-LLA成功解决了人形机器人语言条件全身控制的挑战，实现了从自由形式语言指令到物理可执行动作的有效映射，为人机交互和具身智能提供了有前景的解决方案。
...
### [199] [Analytical Inverse Kinematic Solution for "Moz1" NonSRS 7-DOF Robot arm with novel arm angle](https://arxiv.org/abs/2511.22996)
*Ke Chen*

Main category: cs.RO

TL;DR: 该论文提出了7自由度Moz1机械臂的逆运动学解析解，采用新颖的臂角概念解决传统SEW角无法定义的问题，提供完整解空间（16个解）


<details>
  <summary>Details</summary>
Motivation: 解决7自由度机械臂逆运动学问题，特别是存在手腕偏移的情况。传统SEW角在某些姿态下无法定义，需要新的表示方法来解决冗余度并处理算法奇异点

Method: 提出基于新颖臂角的封闭形式解析解，能够完全处理自运动，解决工作空间内的算法奇异点问题。通过新的臂角表示方法解决冗余度问题

Result: 获得了简单、快速且精确的解析解，能够为每个姿态提供完整的解空间（16个解）。解决了传统SEW角无法定义的情况，并有效处理了奇异点

Conclusion: 该方法为7自由度Moz1机械臂提供了有效的逆运动学解决方案，解决了手腕偏移带来的挑战，通过新颖的臂角表示克服了传统方法的局限性
...
### [200] [Adaptive Factor Graph-Based Tightly Coupled GNSS/IMU Fusion for Robust Positionin](https://arxiv.org/abs/2511.23017)
*Elham Ahmadi,Alireza Olama,Petri Välisuo,Heidi Kuusniemi*

Main category: cs.RO

TL;DR: 提出了一种基于因子图的鲁棒自适应GNSS/IMU融合框架，采用Barron损失函数处理非高斯噪声和异常值，在GNSS信号受限的城市场景中显著提升定位精度。


<details>
  <summary>Details</summary>
Motivation: GNSS信号受限环境中的可靠定位是导航系统的关键挑战。紧耦合GNSS/IMU融合虽然提高了鲁棒性，但仍容易受到非高斯噪声和异常值的影响，需要更强大的鲁棒性解决方案。

Method: 提出基于因子图的鲁棒自适应融合框架，直接集成GNSS伪距测量和IMU预积分因子，并引入Barron损失函数（一种通过单个可调参数统一多种M估计器的通用鲁棒损失函数），自适应降低不可靠GNSS测量的权重。

Result: 在UrbanNav数据集上的评估显示，相比标准因子图优化方法，定位误差减少了高达41%；相比扩展卡尔曼滤波器基线，在城市峡谷环境中获得了更大的改进。

Conclusion: Barron损失函数能有效增强GNSS/IMU导航在城市场景和信号受限环境中的鲁棒性，为GNSS挑战环境中的可靠定位提供了有效解决方案。
...
### [201] [DiskChunGS: Large-Scale 3D Gaussian SLAM Through Chunk-Based Memory Management](https://arxiv.org/abs/2511.23030)
*Casimir Feldmann,Maximum Wilder-Smith,Vaishakh Patil,Michael Oechsle,Michael Niemeyer,Keisuke Tateno,Marco Hutter*

Main category: cs.RO

TL;DR: DiskChunGS是一个可扩展的3D高斯泼溅SLAM系统，通过外存方法将场景分割成空间块，只在GPU内存中保留活动区域，将非活动区域存储在磁盘上，解决了内存限制问题。


<details>
  <summary>Details</summary>
Motivation: 现有的3D高斯泼溅（3DGS）SLAM系统面临GPU内存容量的基本可扩展性限制，只能重建小规模环境，无法处理大规模场景。

Method: 采用外存方法，将场景分割成空间块，只在GPU内存中维护活动区域，非活动区域存储在磁盘上，并与现有SLAM框架无缝集成进行位姿估计和闭环检测。

Result: 在室内场景（Replica、TUM-RGBD）、城市驾驶场景（KITTI）和资源受限的Nvidia Jetson平台上验证，能够完成所有11个KITTI序列而不会出现内存故障，同时实现卓越的视觉质量。
...
### [202] [LatBot: Distilling Universal Latent Actions for Vision-Language-Action Models](https://arxiv.org/abs/2511.23034)
*Zuolei Li,Xingyu Gao,Xiaofan Wang,Jianlong Fu*

Main category: cs.RO

TL;DR: 提出通用潜在动作学习框架，通过结合未来帧重建和动作序列预测来学习可迁移的潜在动作表示，在模拟和真实机器人任务中实现强泛化能力
...
Method: 提出通用潜在动作学习框架，输入任务指令和多帧图像，同时优化未来帧重建和动作序列预测。将潜在动作分解为可学习的运动和场景标记，区分机器人主动运动和环境变化。通过将学习的潜在动作蒸馏到最新VLA模型中实现迁移

Result: 在模拟环境（SIMPLER和LIBERO）和真实机器人设置中均取得强性能。在Franka机器人上，每个任务仅需10条真实轨迹就能成功完成所有五个挑战性任务，展示了强大的少样本迁移能力
...
### [203] [Automated Generation of MDPs Using Logic Programming and LLMs for Robotic Applications](https://arxiv.org/abs/2511.23143)
...
<details>
  <summary>Details</summary>
Motivation: 旨在通过结合语言模型和形式化方法，使机器人领域的概率规划更加易用和可扩展，减少手动工作量
...
Conclusion: 该工作展示了语言模型与形式化方法结合在实现更易用、可扩展的概率规划方面的潜力
...
### [204] [Obstruction reasoning for robotic grasping](https://arxiv.org/abs/2511.23186)
*Runyu Jiao,Matteo Bortolon,Francesco Giuliari,Alice Fasoli,Sergio Povoli,Guofeng Mei,Yiming Wang,Fabio Poiesi*

Main category: cs.RO

TL;DR: UNOGrasp是一个基于学习的视觉语言模型，专门用于机器人抓取中的障碍物推理，通过多步推理过程识别并规划清除障碍物的动作序列，以提高在杂乱环境中的抓取成功率。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言具身推理模型在空间理解方面有所进展，但在障碍物推理和可达性规划方面仍存在局限，无法有效处理杂乱环境中的机器人抓取任务。

Method: 提出基于目标物体障碍路径的多步推理过程，使用障碍感知视觉线索锚定每个推理步骤，结合监督学习和强化学习微调，并构建了包含10万+障碍路径标注的大规模数据集UNOBench。

Result: 实验表明UNOGrasp在合成和真实世界环境中显著提升了障碍物推理能力和抓取成功率，优于通用模型和专有替代方案。

Conclusion: UNOGrasp通过专门的障碍物推理框架有效解决了杂乱环境中机器人抓取的挑战，为具身智能在复杂操作任务中的应用提供了新思路。
...
### [205] [Fault-Tolerant MARL for CAVs under Observation Perturbations for Highway On-Ramp Merging](https://arxiv.org/abs/2511.23193)
*Yuchen Shi,Huaxin Pei,Yi Zhang,Danya Yao*

Main category: cs.RO

TL;DR: 本文提出了一种用于协同匝道车辆容错的多智能体强化学习方法，通过对抗性故障注入智能体和具有自诊断能力的容错车辆智能体，解决MARL在自动驾驶中观测故障容错性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习在协同自动驾驶中具有重要应用前景，但实际部署面临观测故障容错性不足的关键限制。观测故障表现为车辆感知数据中的扰动，会严重影响MARL驾驶系统的性能。需要解决两个主要挑战：生成有效挑战策略的对抗性扰动，以及使车辆具备减轻损坏观测影响的能力。

Method: 提出了一种容错MARL方法，包含两个关键智能体：1）对抗性故障注入智能体，通过协同训练生成主动挑战和强化车辆策略的扰动；2）容错车辆智能体，具有自诊断能力，利用车辆状态序列固有的时空相关性来检测故障并重建可信观测，从而保护策略免受误导输入影响。

Result: 在模拟高速公路匝道合并场景中的实验表明，该方法显著优于基线MARL方法，在各种观测故障模式下实现了接近无故障水平的安全性和效率。

Conclusion: 提出的容错MARL方法通过对抗性故障注入和自诊断机制，有效解决了协同自动驾驶中观测故障容错性的关键问题，为MARL在实际自动驾驶系统中的可靠应用提供了重要解决方案。
...
### [206] [Field-programmable dynamics in a soft magnetic actuator enabling true random number generation and reservoir computing](https://arxiv.org/abs/2511.23215)
...
TL;DR: 软体机器人中复杂动力学行为的研究，展示了混沌动力学在软体机器人中的优势，包括真随机数生成、随机计算和时间序列预测等新功能


<details>
  <summary>Details</summary>
Motivation: 传统机电系统通常避免复杂和混沌动力学，担心磨损和可控性问题。但研究表明，在软体机器人中，复杂动力学可能特别有利，能够提供传统驱动方法难以实现的新功能

Method: 设计并实现了具有可调动态机制的弹性磁软体驱动器，能够在数万次循环中运行而不疲劳。通过实验验证了这些驱动器在真随机数生成、随机计算以及作为物理储层进行Mackey-Glass时间序列预测的应用

Result: 成功展示了软体机器人作为物理储层的能力，能够执行时间序列预测。证明了复杂动力学在软体机器人中的实际应用价值，包括生物模拟眨眼和随机化语音调制等演示

Conclusion: 探索软体机器人中的复杂动力学将扩展其在软计算、人机交互和协作机器人等领域的应用场景，为软体机器人开辟了新的功能可能性
...
### [207] [Incorporating Ephemeral Traffic Waves in A Data-Driven Framework for Microsimulation in CARLA](https://arxiv.org/abs/2511.23236)
...
<details>
  <summary>Details</summary>
Motivation: 传统的微观仿真器校准过程难以重现交通波等短暂现象，特别是在大规模仿真中面临挑战。现有研究多关注局部跟车行为或抽象几何结构，缺乏对完整时空图保真度的验证。

Method: 采用协同仿真方法，将真实交通数据作为边界条件注入仿真环境。自动生成1英里高速公路段对应I-24，以经验数据中的自我车辆为中心，在其纵向范围内自动生成"可见"交通，超出范围则使用上游和下游的幽灵单元进行边界控制。

Result: 该框架在低拥堵和高拥堵场景下成功模拟了交通波的形成和消散，产生的涌现行为与真实交通高度一致，实现了对经验交通波现象的首次感知真实、边界驱动的仿真。

Conclusion: 该工作将微观建模与物理实验数据相结合，提供了一个新颖的协同仿真框架，可用于评估交通控制策略、感知驱动的自动驾驶以及未来交通波缓解方案的部署。
...
### [208] [SafeHumanoid: VLM-RAG-driven Control of Upper Body Impedance for Humanoid Robot](https://arxiv.org/abs/2511.23300)
...
TL;DR: SafeHumanoid：基于视觉语言模型和检索增强生成的仿人机器人阻抗控制管道，通过语义理解场景上下文来调节刚度和速度，提高人机交互安全性


<details>
  <summary>Details</summary>
Motivation: 安全可信的人机交互需要机器人不仅完成任务，还要根据场景上下文和人类接近程度调节阻抗和速度参数。当前需要一种能够语义理解场景并自动调整控制参数的解决方案。

Method: 提出SafeHumanoid系统：1）使用以自我为中心的视觉帧；2）通过结构化视觉语言模型提示处理图像；3）嵌入并与验证场景数据库匹配（检索增强生成）；4）通过逆运动学映射到关节级阻抗命令

Result: 在桌面操作任务（擦拭、物体交接、液体倾倒）中测试，系统能够根据上下文自适应调整刚度、阻尼和速度曲线，保持任务成功率的同时提高安全性。当前推理延迟最高1.4秒，在高度动态场景中响应性有限

Conclusion: SafeHumanoid证明了阻抗控制的语义基础是通向更安全、符合标准的仿人机器人协作的可行路径，尽管当前延迟限制了高度动态环境中的响应性
...
### [209] [From CAD to POMDP: Probabilistic Planning for Robotic Disassembly of End-of-Life Products](https://arxiv.org/abs/2511.23407)
*Jan Baumgärtner,Malte Hansjosten,David Hald,Adrian Hauptmannl,Alexander Puchta,Jürgen Fleischer*

Main category: cs.RO

TL;DR: 论文提出将产品拆卸建模为部分可观测马尔可夫决策过程（POMDP），开发了基于CAD数据、机器人能力和检测结果的概率规划框架，通过强化学习和贝叶斯滤波处理EOL产品的不确定性。


<details>
  <summary>Details</summary>
Motivation: 支持循环经济需要机器人系统能够拆卸报废产品，但现有拆卸序列规划方法通常假设确定性和完全可观测的产品模型，而实际报废产品常因磨损、腐蚀或未记录的维修而偏离原始设计，存在不确定性。

Method: 将拆卸建模为POMDP，隐藏变量表示不确定的结构或物理属性；提出任务和运动规划框架，从CAD数据、机器人能力和检测结果自动推导POMDP模型；采用强化学习近似求解，基于检测先验处理随机动作结果，执行期间使用贝叶斯滤波持续更新对潜在EOL条件的信念。

Result: 在两种机器人系统上的三个产品测试表明，该概率规划框架在平均拆卸时间和方差方面优于确定性基线方法，能够跨不同机器人设置泛化，并成功适应CAD模型的偏差（如缺失或卡住的零件）。

Conclusion: 将拆卸建模为POMDP并采用概率规划框架能够有效处理报废产品的不确定性，提高机器人拆卸的效率和鲁棒性，支持循环经济中对多样化报废产品的处理需求。
...
### [210] [LAYER: A Quantitative Explainable AI Framework for Decoding Tissue-Layer Drivers of Myofascial Low Back Pain](https://arxiv.org/abs/2511.21767)
...
<details>
  <summary>Details</summary>
Motivation: 肌筋膜疼痛是慢性腰痛的主要原因，但其组织层面的驱动因素仍不明确，缺乏可靠的影像生物标志物。现有研究主要关注肌肉而忽略了筋膜、脂肪等软组织在生物力学中的重要作用。

Method: 开发了LAYER（层间分析产生可解释相关性组织）框架，这是一个基于解剖学的可解释人工智能框架，分析三维超声中的六个组织层，并量化它们对肌筋膜疼痛预测的贡献。使用了包含超过4000次扫描的最大多模态三维超声队列。

Result: 在B模式成像中，深筋膜膜显示出最高的显著性（0.420）；在B模式和剪切波图像结合分析中，非肌肉层的集体显著性（0.316）几乎与肌肉层（0.317）相当，挑战了肌筋膜疼痛研究中传统的肌肉中心范式。

Conclusion: LAYER建立了一个定量、可解释的框架，将特定层解剖结构与疼痛生理学联系起来，揭示了新的组织靶点，并为软组织影像的可解释分析提供了通用方法。
...
### [211] [Attention-Guided Fair AI Modeling for Skin Cancer Diagnosis](https://arxiv.org/abs/2511.21775)
*Mingcheng Zhu,Mingxuan Liu,Han Yuan,Yilin Ning,Zhiyao Luo,Tingting Zhu,Nan Liu*

Main category: eess.IV

TL;DR: LesionAttn算法通过整合临床知识（关注病灶区域）和帕累托前沿优化，在保持高诊断性能的同时显著减轻皮肤科AI中的性别偏见。


<details>
  <summary>Details</summary>
Motivation: 皮肤科AI虽然显示出巨大潜力，但现有研究主要关注肤色偏见，而性别偏见问题尚未充分探索，导致不平等医疗护理和加剧现有性别差异。

Method: 开发了LesionAttn算法，将临床知识融入模型设计，引导注意力聚焦病灶区域（模仿临床医生的诊断焦点），并结合帕累托前沿优化进行双目标模型选择，平衡公平性和预测准确性。

Result: 在两个大规模皮肤科数据集上验证，LesionAttn显著减轻了性别偏见，同时保持高诊断性能，优于现有的偏见缓解算法。

Conclusion: 研究强调了将临床知识嵌入AI开发的重要性，既能提升模型性能和公平性，又能促进临床医生与AI开发者之间的跨学科合作。
...
### [212] [Comparing SAM 2 and SAM 3 for Zero-Shot Segmentation of 3D Medical Data](https://arxiv.org/abs/2511.21926)
*Satrajit Chakrabarty,Ravi Soni*

Main category: eess.IV

TL;DR: SAM 3在医学图像零样本分割中表现优于SAM 2，特别是在复杂解剖结构和稀疏用户交互场景下，成为更优的通用分割选择。


<details>
  <summary>Details</summary>
Motivation: 虽然SAM系列模型在自然图像上表现良好，但在医学数据上的行为特征尚未充分研究。需要评估SAM 3是否能作为SAM 2的直接替代品用于医学图像分割。

Method: 在16个公共医学数据集上进行控制性比较，涵盖CT、MRI、3D超声等多种模态，评估54种解剖结构、病理和手术器械。使用四种提示模式（单点、多点、边界框、密集掩码），限制在第一帧，标准化预处理和评估流程。

Result: SAM 3在点击提示下提供显著更强的初始化效果，在复杂、血管和软组织解剖结构上保持优势。SAM 2仅在紧凑、刚性器官和强空间引导下保持竞争力，但在挑战性目标上经常失败。

Conclusion: SAM 3是大多数医学分割任务的更优默认选择，特别是在涉及稀疏用户交互或复杂解剖拓扑的场景中，可以作为SAM 2的直接替代品。
...
### [213] [When Do Domain-Specific Foundation Models Justify Their Cost? A Systematic Evaluation Across Retinal Imaging Tasks](https://arxiv.org/abs/2511.22001)
*David Isztl,Tahm Spitznagel,Gabor Mark Somfai,Rui Santos*

Main category: eess.IV

TL;DR: 该研究挑战了视网膜疾病分类需要大型领域特定基础模型的假设，证明紧凑的通用模型在大多数任务上表现接近最优，只有少数精细分类任务才需要大型专业模型。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决两个关键问题：1）视网膜疾病分类是否必须使用大型领域特定基础模型，还是紧凑的通用架构就足够？2）专门的视网膜预训练是否值得其计算成本？

Method: 在四个视网膜成像分类任务上对初始化策略进行基准测试：8类OCT分类、3类糖尿病性黄斑水肿、5类糖尿病视网膜病变和3类青光眼检测。评估12-13种模型配置，包括ViT、Swin Transformers、ConvNeXt和领域特定的RETFound模型，在相同训练条件下比较。

Result: 1）预训练带来普遍好处（5.18-18.41%提升），与任务难度成正比；2）紧凑架构（27-29M参数）主导帕累托前沿，SwinV2-tiny在三个数据集上达到top-1性能；3）RETFound（303M）仅在具有挑战性的DR分级任务中值得其计算成本（准确率71.15%），其他任务ImageNet预训练已足够（DME准确率99.24%，OCT准确率97.96%）。

Conclusion: 紧凑的通用模型在大多数视网膜分类任务中提供接近最优的性能；只有在极端类别不平衡下的精细判别任务才需要专门的基础模型。CFP任务比OCT任务从预训练中获益更大。
...
### [214] [GACELLE: GPU-accelerated tools for model parameter estimation and image reconstruction](https://arxiv.org/abs/2511.22094)
*Kwok-Shing Chan,Hansol Lee,Yixin Ma,Berkin Bilgic,Susie Y. Huang,Hong-Hsi Lee,José P. Marques*

Main category: eess.IV

TL;DR: GACELLE是一个开源的GPU加速定量MRI分析框架，通过随机梯度下降优化器和随机采样器实现快速参数映射，相比CPU方法加速高达451倍（优化器）和14,380倍（采样器），同时提高参数精度和可重复性。


<details>
  <summary>Details</summary>
Motivation: 定量MRI（qMRI）能够提供组织特异性生物标志物，但其参数估计的计算需求巨大，阻碍了临床研究中的应用。高空间分辨率图像或多参数拟合需要冗长的处理时间，限制了常规流程使用，减缓了方法创新和临床转化。

Method: GACELLE是一个开源GPU加速框架，提供随机梯度下降优化器和随机采样器。用户只需提供前向信号模型，框架后端管理计算并行化、自动参数更新和内存批处理。随机求解器在CPU和GPU上执行完全向量化的马尔可夫链蒙特卡洛，确保硬件间的可重复性。

Result: 基准测试显示，随机梯度下降求解器加速高达451倍，随机采样加速高达14,380倍，且不损失精度。在三个代表性qMRI模型和图像重建任务中，GACELLE提高了参数精度、增强了测试-重测可重复性，并减少了定量图中的噪声。

Conclusion: GACELLE通过结合速度、可用性和灵活性，为医学图像分析提供了通用的优化框架，降低了qMRI的计算障碍，为可重复的生物标志物开发、大规模成像研究和临床转化铺平了道路。
...
### [215] [ColonAdapter: Geometry Estimation Through Foundation Model Adaptation for Colonoscopy](https://arxiv.org/abs/2511.22250)
...
<details>
  <summary>Details</summary>
Motivation: 现有3D几何基础模型主要在自然场景数据集上训练，在结肠镜临床场景中性能下降，因为结肠镜图像具有镜面反射和均匀纹理等特性，导致几何估计不准确。

Method: 提出ColonAdapter框架，通过自监督微调将预训练的几何基础模型适配到结肠镜数据。引入细节恢复模块（DRM）改善低纹理区域性能，使用几何一致性损失确保尺度一致性，采用置信度加权光度损失增强临床环境中的训练稳定性。

Result: 在合成和真实数据集上的实验表明，该方法在相机姿态估计、单目深度预测和密集3D点云重建方面达到了最先进的性能，且不需要真实的内参标定。

Conclusion: ColonAdapter成功将几何基础模型适配到结肠镜场景，有效解决了临床环境中的特殊挑战，为结肠镜3D几何估计提供了有效的解决方案。
...
### [216] [TokCom-UEP: Semantic Importance-Matched Unequal Error Protection for Resilient Image Transmission](https://arxiv.org/abs/2511.22859)
*Kaizheng Zhang,Zuolin Jin,Zhihang Cheng,Ming Zeng,Li Qiao,Zesong Fei*

Main category: eess.IV

TL;DR: 该论文提出了TokCom-UEP框架，通过语义重要性匹配的不等差错保护机制优化基于大语言模型的Token通信系统，提升图像传输的恢复性能。


<details>
  <summary>Details</summary>
Motivation: 现有Token通信设计假设所有token重要性相同，采用等差错保护方案，但压缩的一维token序列实际上具有异质的语义重要性层次结构，这使得EEP方案次优。

Method: 提出TokCom-UEP框架，将无速率UEP编码与token的非均匀语义重要性相结合，通过将源token划分为嵌套扩展窗口，为包含关键token的窗口分配更高选择概率，确保其优先恢复。

Result: 仿真结果表明，TokCom-UEP在低开销条件下，在三个核心语义恢复指标和频谱效率方面均优于EEP方案。

Conclusion: TokCom-UEP通过语义重要性匹配的不等差错保护机制，有效解决了现有Token通信中均匀保护假设的局限性，提升了图像传输的恢复性能和频谱效率。
...
### [217] [Two-Dimensional Tomographic Reconstruction From Projections With Unknown Angles and Unknown Spatial Shifts](https://arxiv.org/abs/2511.22890)
*Shreyas Jayant Grampurohit,Satish Mulleti,Ajit Rajwade*

Main category: eess.IV

TL;DR: 提出了一种用于二维断层扫描的新方法，能够同时估计未知投影角度和空间偏移，改进了现有仅处理角度未知的方法


<details>
  <summary>Details</summary>
Motivation: 在工业成像和生物医学成像中，投影几何参数（如角度和空间偏移）常常未知或部分未知。现有二维未知视角断层扫描算法大多假设投影是居中的，忽略了空间偏移问题，这在实际应用中限制了重建质量。

Method: 首先改进现有的基于图拉普拉斯算子的二维未知视角断层扫描算法以纳入空间偏移，然后将其作为初始化，提出三向交替最小化算法，联合估计二维结构、投影角度和相应的空间偏移。

Result: 在核糖体图像的噪声投影上评估了该方法，证明相比忽略空间偏移的基线方法，该方法能够实现更优的重建效果。

Conclusion: 该方法能够有效处理二维断层扫描中未知投影角度和空间偏移的问题，为工业成像和生物医学成像中几何参数未知的情况提供了更准确的解决方案。
...
### [218] [MICCAI STS 2024 Challenge: Semi-Supervised Instance-Level Tooth Segmentation in Panoramic X-ray and CBCT Images](https://arxiv.org/abs/2511.22911)
...
TL;DR: STS 2024挑战赛展示了半监督学习在牙齿分割任务中的显著优势，在标注数据稀缺的情况下，半监督方法相比全监督基线取得了44-61个百分点的性能提升。


<details>
  <summary>Details</summary>
Motivation: 牙齿分割需要大量标注数据，但手动标注成本高昂。本研究旨在通过半监督学习解决牙科影像（OPG和CBCT）分割中的数据稀缺问题。

Method: 在MICCAI 2024组织STS挑战赛，提供包含90,000+ 2D图像和3D切片的标注数据集，评估参与者提交的半监督学习算法。对OPG和CBCT赛道分别评估前10和前5名团队的开源方法。

Result: 所有成功提交均为基于深度学习的半监督方法。OPG赛道最佳方法将Instance Affinity分数提升44个百分点以上；CBCT赛道最佳方法将Instance Dice分数提升61个百分点。最有效的方法结合了SAM等基础模型与多阶段粗到精细化流程。

Conclusion: 半监督学习在标注数据稀缺的复杂实例级医学图像分割任务中具有显著优势。挑战数据集和参与者代码已公开，确保透明性和可复现性。
...
### [219] [When Are Reactive Notebooks Not Reactive?](https://arxiv.org/abs/2511.21994)
...
<details>
  <summary>Details</summary>
Motivation: 计算笔记本虽然方便，但由于增量编辑能力容易变得混乱和不一致。现有的反应式笔记本系统（如Ipyflow、Marimo和Observable）各有不同的反应性定义，且简单的笔记本修改就可能破坏这些系统，使得用户难以构建对反应式笔记本实现的心理模型。

Method: 提出Rex，一个细粒度的测试套件，用于讨论和评估反应式笔记本系统的反应能力。在三个现有的反应式笔记本系统上进行评估，并对它们的失败进行分类。

Result: 评估了三个反应式笔记本系统，发现了各种失败情况，并对这些失败进行了分类，旨在帮助程序员理解反应性何时会失效。

Conclusion: Rex测试套件有助于评估反应式笔记本系统的反应能力，帮助程序员理解反应性失效的情况，同时帮助笔记本实现改进。
...
### [220] [EAST: Environment-Aware Stylized Transition Along the Reality-Virtuality Continuum](https://arxiv.org/abs/2511.22056)
*Fengze Li,Jieming Ma,Kan Liu,Xiaohan Zhang,Yangle Liu,Yue Li*

Main category: cs.HC

TL;DR: EAST框架使用风格迁移的3D高斯泼溅技术，将现实世界中断无缝融入虚拟环境，解决VR游戏中风格化过渡的美学不连续问题。


<details>
  <summary>Details</summary>
Motivation: VR游戏行业在现实世界中断期间保持沉浸感存在挑战，特别是在现实-虚拟连续体过渡时。现有方法依赖数字副本或简单视觉过渡，忽视了现实与虚拟环境之间的美学不连续性，尤其是在高度风格化的VR游戏中。

Method: 提出环境感知风格化过渡（EAST）框架，采用新颖的风格迁移3D高斯泼溅（3DGS）技术，将现实世界中断转移到虚拟环境中，实现无缝美学一致性。该方法不是简单地将现实世界转换为游戏化视觉效果，而是通过整合现实世界元素来最小化中断的破坏性影响。

Result: 定性用户研究表明在过渡期间认知舒适度和情感连续性显著增强；定量实验显示EAST能够跨不同VR风格保持视觉连贯性。

Conclusion: EAST框架通过风格迁移的3DGS技术有效解决了VR游戏中现实中断的美学不连续问题，提升了过渡体验的沉浸感和连续性。
...
### [221] [HandyLabel: Towards Post-Processing to Real-Time Annotation Using Skeleton Based Hand Gesture Recognition](https://arxiv.org/abs/2511.22337)
*Sachin Kumar Singh,Ko Watanabe,Brian Moser,Andreas Dengel*

Main category: cs.HC

TL;DR: HandyLabel是一个实时标注工具，利用手势识别技术进行数据标注，相比传统后处理标注方法更高效准确。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习标注工具需要后处理，耗时耗力且容易出错，特别是标注认知活动时记忆困难。需要一种实时标注工具来提高标注效率和质量。

Method: 开发HandyLabel实时标注工具，基于手势识别技术，允许用户通过网页界面自定义手势映射。评估了多种手势识别模型在HaGRID数据集上的性能，采用骨架预处理技术。

Result: ResNet50结合骨架预处理图像在HaGRID数据集上达到0.923的F1分数。用户研究显示88.9%的参与者更喜欢HandyLabel而非传统标注工具。

Conclusion: HandyLabel通过实时手势识别显著提高了数据标注的效率和准确性，为机器学习数据标注提供了创新解决方案。
...
### [222] [Engineering Trustworthy Automation: Design Principles and Evaluation for AutoML Tools for Novices](https://arxiv.org/abs/2511.22352)
*Jarne Thys,Davy Vanacken,Gustavo Rovelo Ruiz*

Main category: cs.HC

TL;DR: 该研究针对面向新手的AutoML系统存在的可用性问题，提出了一个抽象管道框架，并通过用户研究验证其效果，最终提出了四项设计原则来改进面向新手的AutoML系统设计。


<details>
  <summary>Details</summary>
Motivation: 当前面向新手的AutoML系统过于注重算法自动化而忽视了可用性，导致用户存在理解不足、信任缺失以及端到端工作流程支持不够的问题。

Method: 提出了一个覆盖数据输入、引导配置、训练、评估和推理的抽象管道框架，并通过24名参与者的用户研究来评估原型实现的信任度、可理解性和用户体验。

Result: 所有参与者都成功构建了自己的模型，用户体验问卷评分积极，但有经验的用户比新手报告了更高的信任度和理解度。

Conclusion: 提出了四项设计原则来改进面向新手的AutoML系统：支持首次模型成功以增强用户自我效能感；提供解释以帮助形成正确心智模型和适当依赖度；提供抽象和上下文感知辅助以保持用户在最近发展区内；确保可预测性和安全措施以增强用户控制感。
...
### [223] [Epistemic Fragility in Large Language Models: Prompt Framing Systematically Modulates Misinformation Correction](https://arxiv.org/abs/2511.22746)
*Sekoul Krastev,Hilary Sweatman,Anni Sternisko,Steve Rathje*

Main category: cs.HC

TL;DR: 研究发现大语言模型在纠正错误信息时存在"认知脆弱性"：提示语的框架会系统性影响纠错效果，特别是创造性意图、专家角色和封闭式框架会显著降低纠错可能性和策略有效性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型快速取代传统专业知识，它们纠正错误信息的能力成为核心关切。本研究旨在探索提示语框架如何系统性调节错误信息纠正，这一现象被称为"认知脆弱性"。

Method: 通过操纵提示语的开放性、用户意图、用户角色和复杂性四个维度，在十个错误信息领域生成了320个提示，从四个前沿LLM中获取了2,560个响应，并对错误信息纠正强度和纠正策略使用进行了编码分析。

Result: 分析显示：创造性意图、专家角色和封闭式框架导致纠正可能性和使用策略有效性显著降低。模型间存在显著差异：Gemini 2.5 Pro的强纠正几率比Claude Sonnet 4.5低74%。

Conclusion: 认知脆弱性是LLMs的重要结构特性，挑战了当前的防护机制，强调了需要优先考虑认知完整性而非对话合规性的对齐策略。
...
### [224] [Learning Programming in Informal Spaces: Using Emotion as a Lens to Understand Novice Struggles on r/learnprogramming](https://arxiv.org/abs/2511.22789)
*Alif Al Hasan,Subarna Saha,Mia Mohammad Imran*

Main category: cs.HC

TL;DR: 该研究分析了新手程序员在非正式在线学习环境中的情感体验，发现困惑、好奇和挫败感是最常见的情感，并识别了情感触发因素和五个关键支持领域。


<details>
  <summary>Details</summary>
Motivation: 新手程序员在非正式在线学习环境中经历情感困难，困惑和挫败感会阻碍学习动机和成果。研究旨在了解他们的情感体验、识别情感挣扎的原因，并探索情感感知支持系统的设计机会。

Method: 研究手动标注了r/learnprogramming子论坛的1500个帖子，使用学习中心情感框架进行分析，并进行了聚类和轴向编码。

Result: 困惑、好奇和挫败感是最常见的情感，经常同时出现并与早期学习阶段相关。积极情感相对较少。主要情感触发因素包括模糊的错误信息、不明确的学习路径和不对齐的学习资源。

Conclusion: 识别了新手程序员在非正式学习空间中需要的五个关键支持领域：压力缓解和韧性动机、主题解释和资源推荐、战略决策和学习指导、技术支持、以及对他们挑战的认可。研究强调了需要智能、情感敏感的机制来提供与学习者情感状态相符的及时支持。
...
### [225] [AI summaries in online search influence users' attitudes](https://arxiv.org/abs/2511.22809)
*Yiwei Xu,Saloni Dash,Sungha Kang,Wang Liao,Emma S. Spiro*

Main category: cs.HC

TL;DR: AI生成的搜索摘要能显著影响用户对议题的态度、行为意图和政策支持，且摘要位置、议题熟悉度和AI信任度会调节这种影响


<details>
  <summary>Details</summary>
Motivation: 随着AI生成的摘要越来越频繁地出现在搜索结果中，研究这些摘要如何影响用户对争议性议题的认知和态度变得尤为重要

Method: 采用预注册的随机对照实验，2004名参与者查看包含不同AI摘要（有无、位置、立场）的模拟搜索结果页面，涵盖四个公共辩论话题

Result: AI摘要显著影响用户态度、行为意图和政策支持，使其与摘要立场更一致；顶部位置对态度影响更强；议题熟悉度和AI信任度有调节作用；强调健康危害的摘要被认为更有用
...
### [226] [Body Management Information Practices on a Female-dominant Platform](https://arxiv.org/abs/2511.22942)
*Na Li,Chuhao Wu,Hongyang Zhou,Huiran Yi,Xuefei Wang,Jie Cai,Xinyi Fu,John Carroll*

Main category: cs.HC

TL;DR: 研究中国女性主导社交媒体平台上用户如何浏览和评估身体管理信息，以及这些信息如何影响他们的生活方式选择


<details>
  <summary>Details</summary>
Motivation: 随着人们对长期健康和健康的意识增强，日常身体管理已成为普遍实践。虽然HCI研究广泛关注技术介导的健康干预，但用户主动浏览和评估身体管理信息的实践仍未被充分探索。

Method: 通过对18名用户进行半结构化访谈，研究中国女性主导社交媒体平台，分析用户如何寻求身体管理信息。

Result: 识别出影响决策的因素包括消费主义、发布者受欢迎程度和感知真实性，同时面临辨别可靠方法和管理社交媒体引发的身体焦虑等挑战。
...
### [227] [Robust In-the-Wild Exercise Recognition from a Single Wearable: Data-Side Fusion, Sensor Rotation, and Feature Engineering](https://arxiv.org/abs/2511.23173)
...
<details>
  <summary>Details</summary>
Motivation: 自动化系统在个人健康监测中日益重要，但传感器放置位置的变化和无约束运动限制了其有效性。本研究旨在开发一种能够应对这些挑战的单传感器运动活动识别方法。

Method: 从原始加速度数据中提取角度和信号幅度向量特征，然后提取统计特征、分形/频谱特征和高阶差分特征。创建融合左右肢体数据的综合数据集，并通过传感器旋转和轴反转进行数据增强。使用软投票模型结合Hist梯度提升（带平衡权重）和极限梯度提升（不带权重）。

Result: 在组5折评估下，模型整体宏F1分数达到58.83%（手臂61.72%，腿部55.95%）。ANOVA F分数分析显示，分形/频谱特征对于手臂识别最重要，但对于腿部识别最不重要。

Conclusion: 该方法在2ndWEAR数据集挑战中表现良好，证明了特征提取和数据增强策略的有效性，特别是在处理传感器位置变化和运动多样性方面。代码已公开供复现实验。
...
### [228] [Can Intelligent User Interfaces Engage in Philosophical Discussions? A Longitudinal Study of Philosophers' Evolving Perceptions](https://arxiv.org/abs/2511.23188)
*Yibo Meng,Lyumanshan Ye,Eve He,Zhe Yan,Zhiming Liu,Yipeng Yu,Yan Guan,Xiaolan Ding*

Main category: cs.HC

TL;DR: 哲学学者对生成式AI智能用户界面参与哲学讨论的态度经历了三阶段演变：从最初抵制到工具性接受，最终发展为对AI是否具备真正哲学思考能力的根本性质疑。


<details>
  <summary>Details</summary>
Motivation: 研究哲学学者对生成式AI智能用户界面参与哲学讨论的态度演变，了解AI在哲学领域中的角色定位和人类哲学思考的独特性。

Method: 采用混合方法纵向研究（2023-2025年），对16位哲学学者和学生进行年度访谈收集定性数据，同时通过盲评实验让参与者评估匿名的人类和AI哲学答案。

Result: 态度演变呈现三阶段：初期抵制不熟悉→中期工具性接受→后期对AI哲学思考能力的根本性质疑。盲评显示AI在形式逻辑和知识复制方面熟练，但在辩证推理、原创性和具身理解方面存在显著不足。

Conclusion: 参与者不将AI视为哲学同行，而是看作一面复杂镜子，其能力和局限性促使人们反思人类哲学探究的独特不可替代维度，如直觉、价值承诺和质疑基本前提的勇气。
...
### [229] [Is Passive Expertise-Based Personalization Enough? A Case Study in AI-Assisted Test-Taking](https://arxiv.org/abs/2511.23376)
*Li Siyan,Jason Zhang,Akash Maharaj,Yuanming Shi,Yunyao Li*

Main category: cs.HC

TL;DR: 研究探讨了在面向任务的对话系统中，基于用户专业水平的个性化是否真的能提升用户体验和任务表现。通过构建具有被动个性化功能的企业AI助手并进行用户研究，发现被动个性化有助于减轻任务负荷、改善助手感知，但也存在任务特定限制，需要结合主动个性化来优化。


<details>
  <summary>Details</summary>
Motivation: 新手和专家用户在面向任务的对话中有不同的系统性偏好，但针对这些偏好进行个性化是否真正改善用户体验和任务表现尚未得到充分研究。本研究旨在探讨基于专业水平的个性化效果。

Method: 首先构建了一个具有被动个性化功能的企业AI助手版本，然后进行用户研究，参与者在两种版本的AI助手帮助下完成限时考试任务，比较不同个性化策略的效果。

Result: 初步结果显示，被动个性化有助于减轻任务负荷并改善助手感知，但也揭示了任务特定的局限性，这些局限性可以通过提供更多用户自主权来解决。

Conclusion: 在面向任务的企业环境中，结合主动和被动个性化对于优化用户体验和有效性至关重要。研究强调了在个性化设计中平衡系统自动化和用户控制的重要性。
...
### [230] [AugGen: Augmenting Task-Based Learning in Professional Creative Software with LLM-Generated Scaffolded UIs](https://arxiv.org/abs/2511.23379)
*Yimeng Liu,Misha Sra*

Main category: cs.HC

TL;DR: AugGen：一种生成脚手架式用户界面的方法，通过简化界面复杂度、提供任务导向指导来增强用户学习体验，在Blender 3D建模软件中验证有效。


<details>
  <summary>Details</summary>
Motivation: 专业创意软件通常学习曲线陡峭，原因包括复杂界面、缺乏结构化任务感知指导以及不熟悉的领域术语，这阻碍了用户的学习和使用效率。

Method: AugGen方法生成脚手架式UI：1) 根据用户任务筛选相关工具减少干扰功能；2) 按任务工作流程阶段组织工具提供执行指导；3) 将工具与领域概念连接促进学习参与；4) 渐进式披露高级功能管理学习进度。使用LLM辅助管道生成两个任务特定脚手架UI并在Blender中部署。

Result: 邀请了32名初学者和8名专家用户评估实现的界面。结果显示：脚手架界面显著降低了用户感知的任务负载，通过嵌入式指导增强了任务性能，并在任务执行过程中增强了概念学习。

Conclusion: AugGen方法能有效生成脚手架式用户界面，简化专业创意软件的复杂性，提供任务导向指导，增强用户学习体验和概念掌握。
...
### [231] [Improving motor imagery decoding methods for an EEG-based mobile brain-computer interface in the context of the 2024 Cybathlon](https://arxiv.org/abs/2511.23384)
...
TL;DR: 为参加Cybathlon 2024竞赛，研究人员开发了模块化在线EEG脑机接口系统，使用三种心理和运动想象任务控制五个信号，采用S4D深度学习模型，离线准确率达84%，实时游戏成功率73%，为便携式BCI提供框架。


<details>
  <summary>Details</summary>
Motivation: 为Cybathlon 2024竞赛开发脑机接口系统，旨在提高严重运动障碍患者的可访问性，弥合实验室研究与日常应用之间的差距。

Method: 采用模块化设计：数据采集、预处理、分类和传输函数四个模块。使用三种心理和运动想象任务控制五个控制信号。采用基于S4D层的深度学习分类器，开发训练游戏和移动Web应用提供实时反馈，采用以人为本的设计方法。

Result: 离线分析中S4D模型达到84%分类准确率；竞赛环境下完成一项任务；后续验证中原始飞行员和新增参与者在实时游戏中达到73%成功率；S4D模型优于传统机器学习模型，但EEGEncoder训练更慢但性能更高。

Conclusion: 该研究提供了一个集成模块化设计、实时数据处理、用户中心反馈和低成本硬件的便携式BCI框架，解决了当前BCI应用的关键差距，为实验室到日常生活的过渡提供了桥梁。
