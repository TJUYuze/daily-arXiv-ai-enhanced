...
Motivation: 当前视频生成模型在实时交互、长时程一致性和动态场景持久记忆方面存在局限，阻碍了它们发展成为实用的世界模型。需要一种能够统一视频生成、动态场景重建和长期记忆的框架。

Method: 提出生成-重建-引导范式：生成的视频流被连续重建为动态4D时空表示，该表示反过来引导后续生成以保持一致性。采用自回归扩散视频模型，增强宏观-微观规划（MMPL）层次规划方法减少误差积累，结合高效分布匹配蒸馏（DMD）实现实时合成。

Result: TeleWorld在静态和动态世界理解、长期一致性和实时生成效率方面表现优异，实现了动态对象建模和静态场景表示在统一4D框架中的无缝集成。

Conclusion: TeleWorld是向实用、交互式、支持记忆的世界模型迈出的实际步骤，为多模态生成和具身智能提供了可行的解决方案。
...
TL;DR: 本文提出通过噪声优化解决文本到图像模型中的模式崩溃问题，相比传统方法，该方法能提升生成多样性同时保持模型保真度


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型存在显著的模式崩溃问题，即在相同文本提示下生成多个图像时缺乏多样性。传统方法通过引导机制或生成大量候选再筛选来解决，但本文探索不同的方向

Method: 采用噪声优化方法，通过简单的噪声优化目标来缓解模式崩溃，同时保持基础模型的保真度。分析噪声的频率特性，并探索具有不同频率分布的替代噪声初始化策略

Result: 实验表明噪声优化方法在生成质量和多样性方面均优于现有方法，能够有效缓解模式崩溃问题

Conclusion: 噪声优化是一种简单有效的解决文本到图像模型模式崩溃的方法，通过优化噪声初始化策略可以进一步提升生成多样性和质量
...
Method: 提出了Spatial4D-Bench基准测试，这是一个大规模、多任务的评估基准，包含约40,000个问答对，涵盖18个明确定义的任务。这些任务被系统地组织成六个认知类别：物体理解、场景理解、空间关系理解、时空关系理解、空间推理和时空推理。

Result: 对多种最先进的开源和专有MLLMs进行了基准测试，揭示了它们在多种4D空间推理方面的显著局限性，如路径规划、动作识别和物理合理性推理等。

Conclusion: Spatial4D-Bench为评估MLLMs的空间认知能力提供了一个结构化、全面的基准测试，覆盖了与人类空间智能多样性平行的广泛任务。研究结果为社区提供了有价值的见解，该基准测试有望促进开发更具能力的MLLMs，实现人类水平的4D空间智能。
...
TL;DR: CMP框架通过压缩历史遍历数据构建空间先验，显著提升3D目标检测性能，存储需求仅为32KB/km²，比密集存储减少20倍


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶视觉系统通常将每个位置视为首次访问，忽略了大多数部署区域已被多次遍历的历史信息。人类驾驶员很少去从未有人去过的地方，但自动驾驶系统却缺乏利用历史空间先验的能力。

Method: 提出压缩地图先验（CMP）框架，从历史遍历数据中学习空间先验。使用二值化哈希图存储空间信息，存储密度仅为32KB/km²。该方法可轻松集成到主流3D感知系统中，几乎不增加额外计算成本。

Result: 在nuScenes数据集上，CMP框架显著且一致地提升了多种架构的3D目标检测性能。存储效率比密集存储提高了20倍。

Conclusion: 压缩地图先验是一种简单有效的框架，能够利用历史遍历数据中的空间先验信息，显著提升自动驾驶3D感知系统的性能，同时保持极低的存储和计算开销。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态AI在信贷风险评估和文档审查中的广泛应用，迫切需要领域特定的基准测试来反映金融信贷应用中的实际文档和工作流程，包含信贷特定理解能力和真实世界鲁棒性，同时保持隐私合规性而不牺牲实用性。

Method: 通过封闭式合成-采集流水线构建所有样本：手动合成带有虚拟内容的文档模板，并在内部采集场景感知图像。评估框架包括三个维度：感知（3个基础任务）、推理（4个信贷特定任务）和鲁棒性（10种真实世界采集伪影类型）。
...
Conclusion: FCMBench能够有效区分现代视觉语言模型的性能差异和鲁棒性，为金融信贷领域的多模态AI评估提供了专门、实用且隐私合规的基准测试工具。
...
<details>
  <summary>Details</summary>
Motivation: 面部分析中多属性自然语言描述生成与识别问题未被充分探索，系统关注个体面部区域能带来更好的理解和控制能力

Method: 构建新的多属性面部区域描述数据集，提出基于Qwen2.5-VL的Focal-RegionFace模型，通过多个渐进微调阶段逐步聚焦局部面部特征

Result: Focal-RegionFace在新基准测试中取得最佳性能，在传统指标和新提出的指标上都表现优异，验证了其在细粒度多属性面部区域聚焦分析场景中的有效性和多功能性

Conclusion: 该方法成功解决了面部区域聚焦的多属性描述问题，通过渐进式聚焦策略实现了可解释的面部状态分析，为细粒度面部分析提供了新思路
...
Method: 提出Morphing Cross-Attention（MCA）融合源和目标SLAT特征以保持结构连贯性，Temporal-Fused Self-Attention（TFSA）增强时间一致性，并引入方向校正策略解决姿态模糊问题。

Result: 实验表明该方法能生成最先进的变形序列，即使在具有挑战性的跨类别情况下也表现优异，并支持解耦变形和3D风格迁移等高级应用。

Conclusion: MorphAny3D是一个无需训练的高质量3D变形框架，通过创新的注意力机制有效解决了跨类别3D变形的关键挑战，并可推广到其他基于SLAT的生成模型。
...
<details>
  <summary>Details</summary>
Motivation: 农田作物精确计数对农业管理至关重要，但户外环境中部分遮挡和作物聚集造成的视觉模糊给基于图像的2D分割方法带来巨大挑战。

Method: 利用多视角2D图像，结合独立实例掩码进行NeRF视图合成，引入作物可见性和掩码一致性评分，结合NeRF的3D信息实现3D实例分割。

Result: 在棉花、苹果、梨三个农业数据集上验证，尽管作物颜色、形状、尺寸差异大，仍能保持一致的计数性能，优于现有先进方法。

Conclusion: 提出的3D实例分割框架能有效解决户外作物计数问题，无需作物特定参数调优，并贡献了棉花植物数据集促进后续研究。
...
TL;DR: IntraStyler：一种无需先验知识的基于范例的风格合成方法，用于增强无监督域适应的域内多样性


<details>
  <summary>Details</summary>
Motivation: 当前无监督域适应方法主要关注源域和目标域之间的域偏移，而域内变异性研究不足。现有方法需要预先指定域内变化进行风格合成，这在实践中不切实际。

Method: 提出IntraStyler方法：1）使用范例图像指导风格合成，使输出风格匹配范例风格；2）引入基于对比学习的风格编码器来提取纯风格特征

Result: 在最大的跨模态域适应公共数据集CrossMoDA 2023上评估，实验证明该方法在可控风格合成方面的有效性，以及多样化合成数据对下游分割任务的益处

Conclusion: IntraStyler能够无需先验知识捕获多样化的域内风格，通过范例引导的风格合成增强域适应性能，代码已开源
...
TL;DR: 本文提出使用强化学习来提升多模态大语言模型的视觉推理能力，通过设计针对不同推理方面的奖励函数，使模型能够生成更长的结构化推理链，避免视觉信息被绕过。
...
Method: 采用奖励驱动的强化学习方法，设计了六个针对不同推理方面的奖励函数，包括图像理解、思考步骤和答案准确性。使用组相对策略优化（GRPO）来明确激励更长、结构化的推理，并减轻视觉信息被绕过的问题。

Result: 在Qwen-2.5-VL-7B模型上的实验显示，相比基础模型获得了5.56%的性能提升，在领域内和领域外设置下都取得了稳定的增益。将图像转换为文本描述的方法为Claude 3.5和Claude 3.7分别带来了26.7%和23.6%的性能提升。

Conclusion: 强化学习是解锁开源多模态大语言模型中长视觉推理能力的有效机制，无需昂贵的监督。通过设计合适的奖励函数和优化策略，可以显著提升模型在需要视觉感知任务上的性能。
...
Method: LooC采用低维码本组合量化：1) 将码向量重构为特征向量内的低维组合单元，通过组合而非单独匹配来扩展解空间；2) 引入参数无关的插值外推机制，在量化过程中增强和平滑特征；3) 设计确保码本完全使用，避免崩溃问题。

Result: 在不同任务、数据集和架构上的广泛评估表明，LooC显著优于现有VQ方法，在码本规模大幅减小的情况下实现了最先进的性能。

Conclusion: LooC成功解决了向量量化中容量与紧凑性的冲突，通过低维码本组合量化和插值外推机制，实现了高性能的紧凑量化，可作为通用模块应用于各种基于VQ的下游任务。
...
TL;DR: 本文提出SynDR-IQA框架，通过重塑合成数据分布来提升盲图像质量评估的泛化能力，解决了合成数据训练模型泛化能力有限的问题。


<details>
  <summary>Details</summary>
Motivation: 盲图像质量评估面临大规模标注数据稀缺的挑战，合成数据是潜在解决方案，但现有合成数据集训练的模型泛化能力有限。研究发现合成数据学习到的表示呈现离散聚类模式，阻碍回归性能：高质量图像特征围绕参考图像聚类，低质量图像特征按失真类型聚类。这一问题源于合成数据分布而非模型架构。
...
Conclusion: 通过重塑合成数据分布，SynDR-IQA框架显著提升了盲图像质量评估的泛化能力，为解决合成数据训练模型的泛化问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 红外(IR)数据稀缺是PCB缺陷检测的关键瓶颈，传统方法依赖配对监督数据不足

Method: 使用CycleGAN进行非配对图像到图像转换，将丰富的可见光PCB图像映射到红外域，生成高质量伪红外样本；构建异构训练策略，融合生成的伪红外数据和有限真实红外样本训练轻量级YOLOv8检测器

Result: 该方法有效增强低数据条件下的特征学习，增强后的检测器显著优于仅使用有限真实数据训练的模型，接近完全监督训练的性能基准

Conclusion: 伪红外合成作为工业检测的稳健增强策略具有显著效果，为解决红外数据稀缺问题提供了有效方案
...
<details>
  <summary>Details</summary>
Motivation: 传统害虫管理方法依赖人工田间检查和化学农药，成本高、耗时、劳动密集且对环境有害。需要为资源有限的小农户开发高效、环保的解决方案。

Method: 框架包含两个模块：1) 害虫检测模块使用轻量级CNN结合原型元学习，即使在少量训练样本下也能准确识别害虫；2) 农药推荐模块结合作物类型和生长阶段等环境因素，推荐安全环保的农药。使用综合害虫图像数据集进行训练和评估。

Result: 轻量级CNN达到与最先进模型相当的高精度，同时显著降低计算复杂度。决策支持系统减少了对传统化学农药的依赖，促进了可持续实践。

Conclusion: 该框架在精准农业中具有实时应用潜力，特别适合资源有限的小农户，通过高效害虫检测和环保农药推荐实现可持续农业管理。
...
<details>
  <summary>Details</summary>
Motivation: 放射学基础模型在处理3D-CT容积数据时面临计算成本限制的挑战，需要平衡计算效率与表征能力。
...
Conclusion: 器官分离学习框架为3D-CT基础模型的实际实现提供了现实有效的设计指南，展示了在临床评估环境中的高泛化性能。
...
Method: 1. 从250万篇开放获取科学论文中收集1550万高质量图像-文本对，涵盖物理、生物、工程等多个学科；2. 引入AI就绪的语义增强管道，利用Qwen-VL多模态大模型系列，通过合成论文摘要和引用上下文来重新描述图像，改善原始科学标题的弱对齐问题。

Result: 技术验证显示增强显著提升数据质量：基于SciBERT的伪困惑度指标显示语义模糊性降低，CLIP分数表明图像-文本对齐改善了18.21%。数据集已公开在Hugging Face平台。

Conclusion: S1-MMAlign为推进科学推理和跨模态理解提供了基础资源，有助于在AI for Science时代促进科学发现。该数据集公开可用，支持科学多模态学习研究。
...
TL;DR: 提出ActErase：一种无需训练的概念擦除方法，通过激活差异区域识别和动态替换，在保持模型生成能力的同时高效移除敏感概念
...
Method: 基于模型激活主要由通用概念组成、只有极小部分能表示目标概念的观察，通过提示对分析识别激活差异区域，提取目标激活并在前向传播中动态替换输入激活
...
<details>
  <summary>Details</summary>
Motivation: 家庭环境中机器人需要全面理解周围环境以与未经训练的人类有效交互，传统语义分割方法使用固定预定义类别，缺乏灵活性和细粒度理解

Method: 提出DVEFormer方法，使用Alpha-CLIP的教师嵌入指导高效学生模型学习细粒度像素级嵌入，基于RGB-D Transformer架构，通过知识蒸馏实现文本对齐视觉嵌入预测

Result: 在室内数据集上表现竞争力，满足实时要求：完整模型26.3 FPS，小型变体77.0 FPS（NVIDIA Jetson AGX Orin），支持传统语义分割（通过线性探测）、灵活文本查询和3D建图应用

Conclusion: DVEFormer可作为传统分割方法的直接替代，同时支持灵活自然语言查询和无缝集成到移动机器人3D建图流程中，为家庭环境机器人提供更直观的交互能力
...
TL;DR: RGS-SLAM是一个鲁棒的基于高斯泼溅的SLAM框架，使用训练免费的对应关系到高斯初始化替代了传统的残差驱动致密化阶段，通过一次性三角化稠密多视角对应关系生成结构感知的高斯种子，加速收敛约20%，保持实时性能。


<details>
  <summary>Details</summary>
Motivation: 传统GS-SLAM使用残差驱动致密化逐步添加高斯，可能导致早期映射不稳定和收敛缓慢。RGS-SLAM旨在通过更好的初始化策略提高映射稳定性和收敛速度。

Method: 1. 使用DINOv3描述符提取稠密多视角对应关系；2. 通过置信感知内点分类器精炼对应关系；3. 一次性三角化对应关系生成结构感知的高斯种子；4. 在优化前提供良好分布的高斯先验；5. 完全兼容现有GS-SLAM流程。

Result: 在TUM RGB-D和Replica数据集上评估：1. 收敛速度提升约20%；2. 在纹理丰富和杂乱场景中获得更高的渲染保真度；3. 定位和重建精度达到或优于最先进的高斯和基于点的SLAM系统；4. 保持实时映射性能，最高达925 FPS。

Conclusion: RGS-SLAM通过训练免费的对应关系到高斯初始化策略，显著提高了高斯泼溅SLAM的稳定性和效率，在保持实时性能的同时实现了更好的重建质量，为高斯泼溅SLAM提供了更鲁棒的初始化方法。
...
Method: 提出SV-GS框架，利用粗略骨架图和初始静态重建作为输入，优化骨架驱动变形场，包括粗粒度骨架关节姿态估计器和细粒度变形模块，仅使关节姿态估计器随时间变化以实现平滑运动插值

Result: 在合成数据集上，相比现有方法在稀疏观测下PSNR提升达34%；在真实数据集上，使用显著更少的帧数即可达到与密集单目视频方法相当的性能；输入要求可放宽，可用扩散生成先验替代初始静态重建

Conclusion: SV-GS在稀疏观测条件下实现了有效的动态目标重建，通过骨架驱动变形场和运动估计，提高了重建质量，同时放宽了输入要求，增强了实际应用可行性
...
TL;DR: 开发基于Swin Transformer的深度学习模型，用于皮肤疾病图像分类，在ISIC2019数据集上对8种皮肤病变类别达到87.71%的准确率。
...
TL;DR: TimeColor是一个基于草图的视频着色模型，支持使用异质、可变数量的参考图像，通过显式的每参考区域分配和时空对应掩码注意力来提升颜色保真度、身份一致性和时间稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有着色模型通常只基于单帧参考（通常是场景的第一帧），忽略了其他条件数据源，如角色设定图、背景图像或任意着色帧。这限制了模型的灵活性和着色质量。

Method: 1. 将参考图像编码为额外的潜在帧，在时间维度上拼接，使模型能在每个扩散步骤中并行处理参考图像；2. 使用显式的每参考区域分配；3. 采用时空对应掩码注意力来加强主体-参考绑定；4. 使用模态分离的RoPE索引。这些机制防止了捷径学习和跨身份调色板泄漏。

Result: 在SAKUGA-42M数据集上的实验表明，TimeColor在单参考和多参考协议下，相比先前基线模型，在颜色保真度、身份一致性和时间稳定性方面都有显著提升。

Conclusion: TimeColor通过支持异质、可变数量的参考图像，结合显式区域分配和时空对应注意力机制，有效提升了视频着色的质量，解决了传统单参考方法的局限性。
...
TL;DR: VisNet是一种计算高效的ReID模型，通过多尺度特征融合、语义聚类、动态权重平均和FIDI损失函数等技术，在保持高精度的同时显著降低计算成本，适合实时监控和移动应用部署。


<details>
  <summary>Details</summary>
Motivation: 当前行人重识别方法虽然准确率高但计算成本大，难以在计算资源有限的实时监控和移动应用中部署。需要一种既保持高精度又具有低计算复杂度的实用解决方案。

Method: 1. 多尺度特征融合：融合ResNet50的1-4阶段特征，无需并行路径；2. 语义聚类：基于解剖学身体分区的规则伪标注引入空间约束；3. 动态权重平均：平衡分类语义正则化；4. FIDI损失函数：改进度量学习任务。

Result: 在Market-1501数据集上达到87.05% Rank-1和77.65% mAP，仅需32.41M参数和4.601 GFLOPs计算量，在保持高精度的同时显著降低了计算复杂度。

Conclusion: VisNet提出了一种计算高效且有效的行人重识别模型，通过创新的多尺度融合、语义聚类和优化技术，为计算资源有限的实时监控和移动应用提供了实用的部署方案。
...
TL;DR: 该论文提出了OmniVaT框架，首次成功解决了单域泛化多模态视觉触觉学习（SDG-VTL）任务，通过统一嵌入频率空间和分层树结构来缓解模态差异和领域差距问题。
...
Result: 大量实验证明，OmniVaT在SDG-VTL任务上表现出优异的跨领域泛化性能，成功解决了单域泛化多模态视觉触觉学习问题。

Conclusion: OmniVaT框架首次成功解决了单域泛化多模态视觉触觉学习任务，通过统一嵌入频率空间和分层树结构有效缓解了模态差异和领域差距，为VTL的实际应用提供了有效解决方案。
...
TL;DR: 提出概率双流框架，通过可靠性建模和多模态集成，统一处理骨架和RGB模态，提升细粒度动作识别性能，特别是在手部精细动作识别方面。


<details>
  <summary>Details</summary>
Motivation: 现有基于骨架的动作识别方法主要关注身体大尺度运动，忽略了手部精细动作对于细粒度识别的重要性。同时，现有方法在多模态融合和可靠性建模方面存在不足，特别是在噪声和异构条件下的鲁棒性问题。

Method: 提出概率双流框架，包含三个关键组件：1）无需校准的预处理管道，直接从原生坐标学习；2）概率Noisy-OR融合，实现无需显式置信度监督的可靠性感知双流学习；3）从骨架模态（关节、骨骼、关节运动、骨骼运动）到RGB表示的跨模态集成，在统一框架中结合结构和视觉运动线索。

Result: 在多个基准测试（NTU RGB+D~60/120、PKU-MMD、N-UCLA）和新定义的手部中心基准上进行了全面评估，显示出在噪声和异构条件下的一致改进和鲁棒性。

Conclusion: 该框架通过概率建模和多模态集成，有效解决了细粒度动作识别中的手部精细动作识别问题，并在多种条件下表现出优越的性能和鲁棒性，为动作识别领域提供了新的解决方案。
...
TL;DR: NeoVerse是一个多功能4D世界模型，能够进行4D重建、新轨迹视频生成和丰富的下游应用。它解决了现有4D世界建模方法在可扩展性方面的限制，通过免姿态前馈4D重建、在线单目退化模式模拟等技术，实现了对多样化单目视频的扩展性。


<details>
  <summary>Details</summary>
Motivation: 当前4D世界建模方法存在可扩展性限制，主要原因是需要昂贵且专业的多视角4D数据，或者繁琐的训练预处理。这使得现有方法难以扩展到多样化的真实世界单目视频。

Method: NeoVerse采用免姿态前馈4D重建、在线单目退化模式模拟以及其他对齐良好的技术。这些设计使整个流程能够扩展到多样化的真实世界单目视频，无需昂贵的多视角数据或繁琐的预处理。

Result: NeoVerse在标准重建和生成基准测试中达到了最先进的性能。该模型具有多功能性和对不同领域的泛化能力。

Conclusion: NeoVerse是一个可扩展的4D世界模型，通过创新的设计解决了现有方法的可扩展性限制，在4D重建和视频生成任务上表现出色，并支持丰富的下游应用。
...
<details>
  <summary>Details</summary>
Motivation: 当前路边垃圾监测依赖劳动密集型调查和公众报告，空间覆盖有限。现有的视觉检测数据集主要针对街景静态图像、航拍场景或水环境，无法反映行车记录仪视频中垃圾目标极小、稀疏且嵌入杂乱路侧背景的独特特征。

Method: 创建了RoLID-11K数据集，包含超过1.1万张标注图像，涵盖英国多样的驾驶条件，呈现明显的长尾分布和小目标分布特征。对多种现代检测器进行了基准测试，包括精度导向的transformer架构和实时YOLO模型。

Result: CO-DETR及相关transformer模型实现了最佳定位精度，但实时模型受限于粗糙的特征层次结构。该数据集为动态驾驶场景中的极端小目标检测建立了具有挑战性的基准。

Conclusion: RoLID-11K是首个针对行车记录仪路边垃圾检测的大规模数据集，支持开发可扩展、低成本的路边垃圾监测系统，为动态驾驶场景中的极端小目标检测提供了重要基准。
...
Conclusion: CPPO通过创新的对比感知损失和基于熵的感知标记检测，有效解决了多模态强化学习中感知与推理分离的难题，提供了一种更高效、可扩展的视觉语言模型微调方法。
...
<details>
  <summary>Details</summary>
Motivation: 准确模拟3D物体和多种材料通常需要专家知识和耗时的物理参数调整才能达到期望的动态行为，现有方法依赖真实轨迹或标注视频指导，限制了应用范围。

Method: 1) 使用多模态大语言模型估计材料参数值，并约束在合理范围内；2) 提出可学习的运动蒸馏损失，从预训练视频扩散模型中提取鲁棒的运动先验，同时最小化外观和几何归纳偏差来指导模拟。

Result: 在30多个场景中评估，包括真实世界、人工设计和AI生成的3D物体，涵盖弹性固体、金属、泡沫、沙子、牛顿和非牛顿流体等多种材料，证明MotionPhysics能生成视觉逼真的动态模拟，超越现有技术并自动确定物理合理的参数。

Conclusion: MotionPhysics通过自然语言指导生成视觉逼真的动态模拟，无需真实轨迹或标注视频，自动确定物理合理的参数，为3D物理模拟提供了更便捷有效的方法。
...
Method: FreeText将问题分解为"在哪里写"和"写什么"两个部分。对于"在哪里写"，通过读取token级别的空间归因来定位书写区域，使用sink-like tokens作为稳定的空间锚点，并通过拓扑感知细化产生高置信度掩码。对于"写什么"，引入频谱调制字形注入(SGMI)，通过注入噪声对齐的字形先验，在频域进行带通调制，以增强字形结构并抑制语义泄漏。

Result: 在Qwen-Image、FLUX.1-dev和SD3变体上的广泛实验表明，在LongText-Benchmark、CVTG和自建的CLT-Bench上，FreeText在文本可读性方面取得了一致的提升，同时很大程度上保持了语义对齐和美学质量，推理开销适中。

Conclusion: FreeText是一个无需训练、即插即用的框架，能够有效改进文本到图像扩散模型的文本渲染能力，特别是在处理复杂布局和长尾脚本时表现出色，为文本渲染问题提供了一种灵活且高效的解决方案。
...
TL;DR: VNS-SAM改进了SAM在视觉非显著场景下的分割性能，通过Mask-Edge Token交互解码器和非显著特征挖掘模块，在保持零样本泛化能力的同时提升对低对比度场景的分割效果。
...
Method: 提出VNS-SAM，包含两个核心设计：1) Mask-Edge Token交互解码器，有效利用SAM的低层特征；2) 非显著特征挖掘模块。这些设计以少量参数和计算成本增强对非显著特征的理解。同时构建了VNS-SEG数据集，包含超过35K图像，涵盖多种VNS场景。

Result: VNS-SAM在多种VNS分割任务中表现出优越性能，特别是在零样本设置下。额外参数可在4小时内优化完成，证明了方法的可行性和实用性。

Conclusion: VNS-SAM成功增强了SAM在视觉非显著场景下的分割能力，同时保持了其零样本泛化性，具有广泛的现实应用潜力。公开了代码和数据集。
...
TL;DR: DynaDrag提出了一种基于预测-移动框架的拖拽式图像编辑方法，通过迭代执行运动预测和运动监督来避免传统方法的跟踪问题。
...
Method: 提出DynaDrag方法，采用预测-移动框架，迭代执行运动预测（预测控制点应移动的位置）和运动监督（实际拖动控制点），并动态调整有效控制点以提升性能。

Result: 在人脸和人体数据集上的实验表明，该方法在性能上优于先前的工作。

Conclusion: DynaDrag通过创新的预测-移动框架有效解决了传统拖拽式图像编辑方法的局限性，实现了更高质量的像素级图像操控。
...
TL;DR: SlingBAG Pro是一种基于点云迭代概念的高级光声成像重建算法，扩展了对任意阵列几何形状的兼容性，在保持高质量重建的同时减少所需换能器数量，并通过分层优化策略显著缩短重建时间。


<details>
  <summary>Details</summary>
Motivation: 临床应用中高质量3D光声成像需求增加，但传统不规则几何换能器阵列面临挑战：传统迭代重建算法处理不规则阵列配置时存在计算复杂度高、内存需求大、重建时间长等问题。

Method: 基于Sliding ball adaptive growth (SlingBAG)方法的点云迭代概念，扩展对任意阵列几何形状的兼容性，采用分层优化策略，结合零梯度滤波和迭代中逐步增加的时间采样率，快速去除冗余空间点云并加速收敛。

Result: 与原始SlingBAG算法相比，SlingBAG Pro在不规则阵列几何形状下实现了点云基3D光声重建速度提升高达2.2倍，通过仿真和活体小鼠实验验证了方法的有效性。

Conclusion: SlingBAG Pro算法能够有效解决不规则阵列光声成像重建的挑战，在保持重建质量的同时显著提高计算效率，为临床应用中高质量3D光声成像提供了实用解决方案。
...
TL;DR: MS COCOAI是一个包含96000个真实和合成图像的数据集，用于AI生成图像检测，支持两个任务：图像真实性分类和生成模型识别。


<details>
  <summary>Details</summary>
Motivation: 随着Stable Diffusion、DALL-E、MidJourney等多模态生成AI系统的普及，合成图像越来越难以与真实照片区分，这导致了误导性内容、虚假信息和操纵媒体的传播，因此检测AI生成图像变得迫切重要。

Method: 基于MS COCO数据集构建MS COCOAI数据集，包含96000个数据点。使用五种生成器创建合成图像：Stable Diffusion 3、Stable Diffusion 2.1、SDXL、DALL-E 3和MidJourney v6。提出了两个检测任务：图像真实性分类和生成模型识别。

Result: 创建了一个公开可用的数据集（https://huggingface.co/datasets/Rajarshi-Roy-research/Defactify_Image_Dataset），为AI生成图像检测研究提供了标准化的基准资源。

Conclusion: MS COCOAI数据集为解决AI生成图像检测问题提供了重要工具，有助于应对合成媒体带来的挑战，促进该领域的研究发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试在评估统一多模态模型的世界知识应用能力方面存在不足，它们通常只提供孤立的单任务评估，诊断能力有限。需要更全面的多任务基准来准确评估模型在不同任务中应用世界知识的能力。

Method: 提出了AEGIS基准，包含1050个具有挑战性的手动标注问题，涵盖视觉理解、生成、编辑和交错生成等任务，涉及21个主题（STEM、人文、日常生活等）和6种推理类型。同时提出了确定性检查表评估协议，用原子化的"是/否"判断替代模糊的提示式评分，提高评估可靠性。

Result: 实验表明大多数统一多模态模型存在严重的世界知识缺陷，且随着推理复杂度增加，性能显著下降。简单的插件式推理模块可以部分缓解这些弱点，这为未来研究指明了有前景的方向。
...
TL;DR: 提出GranAlign框架解决零样本视频时刻检索中的语义粒度不匹配问题，通过粒度感知查询重写和查询感知字幕生成，在三个基准测试中达到SOTA性能。
...
Method: 提出Granularity-Aware Alignment (GranAlign)训练免费框架，包含两种互补技术：1) 基于粒度的查询重写，生成不同语义粒度的查询；2) 查询感知字幕生成，将查询意图嵌入视频内容。通过将多粒度查询与查询无关和查询感知的字幕配对，有效解决语义不匹配问题。

Result: 在三个主要基准测试(QVHighlights, Charades-STA, ActivityNet-Captions)上均达到新的最先进性能，特别是在具有挑战性的QVHighlights数据集上实现了3.23% mAP@avg的显著提升。

Conclusion: GranAlign框架通过解决语义粒度不匹配问题，显著提升了零样本视频时刻检索的性能，证明了平衡不同模态语义粒度的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本到运动生成方法存在安全隐患，基于离散VQ-VAE码本替换的方法有两个关键缺陷：1）替换被良性提示重用的码本条目会导致日常任务漂移，降低模型良性性能；2）离散标记方法引入量化和平滑度损失，导致伪影和抖动过渡。此外，现有文本到运动数据集天然包含不安全意图和对应运动，不适合安全驱动的机器学习。

Method: 提出SafeMo框架，集成最小化运动遗忘（MMU）策略，这是一种两阶段机器学习遗忘策略，能够在连续空间中实现安全人体运动生成，保持连续运动学特性而无需码本损失。基于DiP构建，有效生成具有自然过渡的安全人体运动。同时创建了首个安全文本到运动数据集SafeMoVAE-29K，包含重写的安全文本提示和连续精炼运动。

Result: 实验显示SafeMo具有有效的遗忘性能，在不安全提示上表现出强化的遗忘效果，在HumanML3D和Motion-X数据集上分别达到之前最先进方法LCR的2.5倍和14.4倍遗忘集FID。在安全提示上的良性性能优于或与现有方法相当。

Conclusion: SafeMo通过连续空间中的最小化运动遗忘策略解决了现有文本到运动生成方法的安全问题，避免了离散码本替换方法的缺陷，实现了强大的安全-效用权衡，为可信的人体运动生成提供了有效解决方案。
...
TL;DR: 本文提出了一种模态主导感知框架（MDACL），通过量化RGB-红外模态不对称性并调节跨模态优化，解决多模态感知中的优化偏差问题。


<details>
  <summary>Details</summary>
Motivation: RGB-红外多模态感知在复杂物理环境中的嵌入式多媒体系统中至关重要。现有跨模态融合方法虽然有所进展，但由于模态特征不对称（信息密度和特征质量差异）导致的优化偏差问题尚未得到充分研究，这种偏差会导致训练过度强调主导模态，阻碍有效融合。

Method: 1. 提出模态主导指数（MDI），通过联合建模特征熵和梯度贡献来量化模态主导程度；2. 基于MDI开发模态主导感知跨模态学习框架（MDACL），包含分层跨模态指导（HCG）增强特征对齐，以及对抗均衡正则化（AER）平衡融合过程中的优化动态。

Result: 在三个RGB-红外基准测试上进行广泛实验，结果表明MDACL有效缓解了优化偏差，并实现了最先进的性能。

Conclusion: 通过量化模态主导现象并设计相应的调节机制，MDACL框架能够有效解决RGB-红外多模态感知中的优化偏差问题，提升跨模态融合效果。
...
<details>
  <summary>Details</summary>
Motivation: 当前通用目标检测方法在小目标检测上存在性能差距，小目标对标注噪声高度敏感，优化严格定位目标容易导致噪声过拟合

Method: 提出Tiny Object Localization with Flows (TOLF)框架：1) 使用归一化流进行灵活的错误建模，捕捉复杂的非高斯预测分布；2) 不确定性感知的梯度调制机制，抑制高不确定性噪声样本的学习

Result: 在三个数据集上的实验验证了方法的有效性，特别是在AI-TOD数据集上将DINO基线提升了1.2% AP

Conclusion: TOLF通过流基误差建模和不确定性引导优化，有效解决了小目标检测中的噪声鲁棒性问题，提升了小目标定位性能
...
Method: 1. 提出统一的端到端实时人体姿态估计与运动分析流程，使用多摄像头RGB视频输入；2. 针对多人干扰的医疗康复场景提出快速跟踪方法，单帧跟踪时间小于1ms；3. 改进SmoothNet用于实时姿态估计，减少姿态估计误差并恢复患者的真实运动状态；4. 使用Unity平台进行实时监测和评估，并显示肌肉应力情况。

Result: 该方法能够实时监测和评估康复训练中的患者运动，提供即时反馈和指导，快速跟踪方法在多人干扰场景下表现高效，改进的姿态估计方法使运动状态更平滑准确。

Conclusion: RePose是一个有效的实时3D人体姿态估计与运动分析系统，特别适用于康复训练场景，能够帮助患者正确执行康复动作，加速恢复过程。
...
TL;DR: HyperPriv-EPN：基于超图的学习使用特权信息框架，通过双流蒸馏让学生图仅从视觉特征中"幻觉"出语义社区结构，实现术前胶质瘤预后预测


<details>
  <summary>Details</summary>
Motivation: 术前胶质瘤预后对治疗规划至关重要，但MRI缺乏术后手术报告中的语义信息。现有多模态方法无法在推理时利用这些不可用的特权文本数据。

Method: 提出HyperPriv-EPN框架，采用割裂图策略：共享编码器处理教师图（含术后特权信息）和学生图（仅术前数据），通过双流蒸馏让学生学习仅从视觉特征中"幻觉"语义社区结构。

Result: 在311名患者的多中心队列中验证，达到最先进的诊断准确率和生存分层效果，有效将专家知识转移到术前环境。

Conclusion: 该方法解锁了历史术后数据的价值，可在推理时不需文本的情况下指导新患者的诊断，实现了特权知识向术前环境的有效转移。
...
TL;DR: 基于图像的深度学习为马铃薯储存期间的质量监测提供了非侵入式、可扩展的解决方案，通过预训练模型实现了发芽检测、重量损失估计和保质期预测，其中DenseNet在发芽检测中达到98.03%准确率。
...
Method: 在200天控制温湿度条件下收集图像和重量数据，利用ResNet、VGG、DenseNet和Vision Transformer等预训练架构，设计两个专门模型：高精度二分类发芽检测器和高级多分类预测器用于重量损失估计和保质期预测。

Result: DenseNet在发芽检测中达到98.03%准确率；保质期预测在粗分类（2-5类）时准确率超过89.83%，细分类（6-8类）时准确率下降；证明了图像模型集成到自动化系统的可行性。

Conclusion: 图像深度学习为马铃薯质量评估提供了经济高效的非破坏性方法，支持储存和分销的效率和可持续性；未来需开发适用于不同品种和储存条件的通用模型以增强适应性和可扩展性。
...
TL;DR: CRoPS框架通过选择性移除关键文本标记构建幻觉模型，结合广义对比解码，无需训练即可显著减少大型视觉语言模型的幻觉生成


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型存在生成幻觉内容的严重问题，影响实际应用的可靠性。现有免训练方法存在两个主要局限：1）对幻觉来源的假设过于狭窄；2）在生成后期效果下降，而这时幻觉最可能发生

Method: 提出CRoPS框架：1）通过选择性移除关键文本标记构建幻觉模型，更好地捕捉幻觉效应；2）引入广义对比解码，整合多个幻觉模型以代表不同的幻觉来源

Result: CRoPS将CHAIR分数提高了20%，在六个基准测试和三个LVLM家族中均取得一致提升，性能优于最先进的免训练方法

Conclusion: CRoPS通过创新的幻觉模型构建方法和广义对比解码机制，有效解决了LVLM的幻觉问题，无需额外训练即可显著提升模型可靠性
...
<details>
  <summary>Details</summary>
Motivation: 现有单图像视频生成方法虽然改善了时间一致性和3D一致性，但缺乏鲁棒的用户可控性（如修改相机路径），且现有相机控制方法在准确建模相机运动、保持时间一致性和几何完整性方面存在不足。虽然使用中间3D表示的方法有前景，但现有的两步流程（先渲染场景后添加物体运动）仍无法实现完全的时间一致性。

Method: 提出新颖框架，在单次前向传播中同时构建3D高斯场景表示并采样合理的物体运动。该方法不需要通过迭代去噪来将物体运动注入渲染帧，实现了快速的相机引导视频生成。

Result: 在KITTI、Waymo、RealEstate10K和DL3DV-10K数据集上的广泛实验表明，该方法在视频质量和推理效率方面达到了最先进的水平。

Conclusion: 该方法通过单次前向传播同时构建3D场景表示和采样物体运动，实现了高效、可控的单图像视频生成，在保持时间一致性和几何完整性的同时提供了精确的相机路径控制。
...
TL;DR: 该研究针对病理学视觉语言模型在数据分布偏移下的性能退化检测问题，开发了DomainSAT工具箱并提出了基于输出置信度的无标签退化指示器，通过结合输入级数据偏移检测和输出级预测行为监控，实现了更可靠的模型性能退化检测。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在医疗图像分析和疾病诊断中表现出强大潜力，但部署后当输入数据分布发生变化时，模型性能可能退化。检测这种性能退化对临床可靠性至关重要，但对于没有标签数据的大型预训练VLMs来说仍然具有挑战性。

Method: 1. 开发DomainSAT轻量级工具箱，集成代表性偏移检测算法并提供图形界面，用于系统分析输入数据偏移；2. 研究输出级预测行为监控，引入基于模型预测置信度的无标签退化指示器；3. 在大型病理学数据集上进行肿瘤分类实验，结合输入数据偏移检测和输出置信度指标。

Result: 输入数据偏移检测能有效识别分布变化并提供早期诊断信号，但并不总是与实际性能退化相关；基于输出置信度的退化指示器与性能退化有密切关系，可作为输入偏移检测的有效补充；结合两种方法能在数据偏移下实现更可靠的VLMs性能退化检测和解释。

Conclusion: 研究为数字病理学中基础模型的可靠性监控提供了一个实用且互补的框架，通过同时监控输入数据分布变化和模型预测置信度变化，能够更全面地检测和解释视觉语言模型在数据偏移下的性能退化问题。
...
Method: 开发端到端工作流，使用多模态LLMs处理扫描手写试卷；教师提供手写参考解决方案和评分规则；采用多阶段设计包括格式检查、独立评分器集成、监督聚合和确定性验证模板。

Result: 使用GPT-5.2和Gemini-3 Pro后端，完整流程与教师评分平均绝对差异约8分，偏差低，手动审查触发率约17%；消融实验显示结构化提示和参考解决方案对准确性至关重要。

Conclusion: 该工作流能够可靠地自动批改手写工程测验，保持标准考试流程，为扩展手写STEM考试批改提供了可行方案。
...
TL;DR: 本文探索了将自监督学习作为辅助任务来优化深度伪造检测主任务的方法，通过融合自监督任务的特征表示，在跨数据集评估中实现了更好的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 深度伪造检测面临泛化能力不足的问题，特别是在面对未见过的伪造方法时。作者试图通过自监督学习作为辅助任务来增强主任务的表示能力，从而提高检测器的跨数据集泛化性能。

Method: 研究了自监督学习与深度伪造检测主任务的不同训练方案组合，探索了如何最有效地融合自监督辅助任务的特征表示。通过特征融合技术，将自监督任务和主任务的特征表示结合起来，形成更强大的特征表示。

Result: 在多个数据集（DF40、FaceForensics++、Celeb-DF、DFD、FaceShifter、UADFV）上进行了实验，结果显示该方法在跨数据集评估中相比当前最先进的检测器具有更好的泛化性能。

Conclusion: 融合自监督辅助任务的特征表示是一种强大的方法，能够充分利用自监督学习和主任务的潜力，为深度伪造检测问题带来独特的特征表示，从而提升主任务的性能。
...
TL;DR: 本文提出LNU-Net和IBU-Net两种深度学习架构用于心脏MRI图像的左心室分割，通过不同的归一化策略改进U-Net，在805张MRI图像上验证了优于现有方法的效果。


<details>
  <summary>Details</summary>
Motivation: 左心室分割对于心脏图像的临床量化和诊断至关重要，需要更准确的分割方法来支持医疗决策。

Method: 基于U-Net架构，提出LNU-Net（使用层归一化）和IBU-Net（结合实例和批量归一化），采用下采样特征提取和上采样精确定位的结构，并应用仿射变换和弹性变形进行数据增强。
...
Method: 1）自适应Gabor表示：通过可学习频率权重和自适应能量补偿扩展高斯函数，平衡细节捕捉和稳定性；2）时序连续性：采用三次Hermite样条和时序曲率正则化确保平滑运动演化；3）自适应初始化：结合深度估计、点跟踪和前景掩码在早期训练中建立稳定的点云分布。

Result: 在Tap-Vid DAVIS数据集上取得SOTA性能（PSNR 35.49，SSIM 0.9433，LPIPS 0.0723），在帧插值、深度一致性、视频编辑和立体视图合成等任务上表现出强大的泛化能力。

Conclusion: AdaGaR通过统一解决频率自适应性和时序连续性，在动态场景建模中实现了更好的细节捕捉和运动平滑性，为单目视频3D重建提供了有效的解决方案。
...
### [50] [Zero-Dimensional Cardiovascular Modeling: A Personalized Approach to Non-Invasive Measurement and Sensitivity Analysis](https://arxiv.org/abs/2601.00027)
*Pranav Kumar Sasikumar*

Main category: physics.med-ph

TL;DR: 研究比较了简化单心室和详细四腔室两种零维心血管模型，发现模型复杂度显著影响生理参数的敏感性排序，支持基于敏感性的模型简化方法。


<details>
  <summary>Details</summary>
Motivation: 零维心血管模型计算效率高，但模型复杂度对参数敏感性的影响尚不明确。本研究旨在探究不同复杂度的心血管模型中生理参数敏感性的变化规律。

Method: 使用两种集总参数心血管模型：简化单心室配置和详细四腔室表示，采用时变弹性函数表示心脏动力学，使用Sobol和Morris方法进行全局敏感性分析，量化关键生理参数（静脉回流、心肌收缩力、总外周阻力、动脉顺应性等）的影响。

Result: 两种模型的敏感性排序存在显著差异，表明模型粒度和参数相互作用在塑造心血管响应中起重要作用。

Conclusion: 研究结果支持基于敏感性的模型简化方法，为可扩展的非侵入性心血管模拟框架提供了基础。

Abstract: Zero-dimensional cardiovascular models provide a computationally efficient framework for studying global hemodynamic behavior, yet the influence of model complexity on parameter sensitivity remains insufficiently understood. This work investigates two lumped-parameter cardiovascular models, a simplified single-ventricle configuration and a detailed four-chamber representation, to examine how physiological parameter sensitivities vary with model structure. Time-varying elastance functions are used to represent cardiac dynamics, and global sensitivity analysis is performed using Sobol and Morris methods to quantify the impact of key physiological parameters, including venous return, myocardial contractility, total peripheral resistance, and arterial compliance. The results demonstrate that sensitivity rankings differ substantially between the two models, highlighting the role of model granularity and parameter interactions in shaping cardiovascular responses. These findings support sensitivity-driven model reduction and provide a foundation for scalable, non-invasive cardiovascular simulation frameworks.

</details>


### [51] [Human-like AI-based Auto-Field-in-Field Whole-Brain Radiotherapy Treatment Planning With Conversation Large Language Model Feedback](https://arxiv.org/abs/2601.00685)
*Adnan Jafar,An Qin,Gavin Atkins,Xiaoyu Hu,Yin Gao,Xun Jia*

Main category: physics.med-ph

TL;DR: 该研究开发了一个自动化全脑放疗规划流程，结合深度学习预测患者特定参数和基于大语言模型的对话界面进行交互式规划优化，显著提高了规划效率。


<details>
  <summary>Details</summary>
Motivation: 全脑放疗规划虽然已有自动功能，但仍需手动调整患者特定参数和基于质量反馈进行规划优化，过程耗时且依赖经验。研究旨在开发一个完全自动化的规划流程，提高效率和一致性。

Method: 研究提出自动化WBRT规划流程，包括：1）深度学习超参数预测模型，基于55个病例的CTV和OAR几何特征训练，预测RayStation中Auto-FiF最优设置；2）基于LLM的对话界面，当规划不理想时，通过语音输入质量反馈，使用Whisper转录并由GPT-4o解释以调整规划设置。

Result: 在15个独立病例评估中，14个DL生成的规划达到临床可接受标准。与临床规划相比，在保持相同CTV D95%剂量下，DL规划在眼睛、晶状体剂量及CTV D1%、D99%指标上无统计学显著差异。DL规划计算时间小于1分钟，整个流程约7分钟（单次点击），而手动规划需15分钟。对话模块成功改善了剂量适形性和热点减少。

Conclusion: 该自动化WBRT规划流程结合深度学习和LLM技术，能够高效生成高质量的放疗规划，减少手动工作量，提高规划一致性，为临床实践提供了有前景的自动化解决方案。
...
### [52] [Reinforcement learning with timed constraints for robotics motion planning](https://arxiv.org/abs/2601.00087)
*Zhaoan Wang,Junchao Li,Mahdi Mohammad,Shaoping Xiao*

Main category: cs.RO

TL;DR: 提出了一种基于自动机的强化学习框架，用于在MDP和POMDP中合成满足MITL时序约束的策略，通过将MITL公式转换为定时LDGBA并与决策过程同步构建产品模型，在三个仿真研究中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 机器人在动态不确定环境中需要满足复杂时序约束的任务规划，MITL提供了形式化表达框架，但将MITL与强化学习结合面临随机动态和部分可观测性的挑战。

Method: 将MITL公式转换为定时限确定广义Büchi自动机(Timed-LDGBA)，与底层决策过程同步构建产品定时模型，采用简单而富有表达力的奖励结构，使用Q-learning进行策略学习。

Result: 在三个仿真研究中验证：5×5网格世界(MDP)、10×10网格世界(POMDP)和办公室服务机器人场景。框架能够学习满足严格时间约束的策略，可扩展到更大状态空间，在部分可观测环境中保持有效。

Conclusion: 提出的统一自动机强化学习框架能够可靠地合成满足MITL时序约束的策略，适用于随机动态和部分可观测环境，展示了在时间关键和不确定环境中可靠机器人规划的潜力。
...
### [53] [Compositional Diffusion with Guided search for Long-Horizon Planning](https://arxiv.org/abs/2601.00126)
*Utkarsh A Mishra,David He,Yongxin Chen,Danfei Xu*

Main category: cs.RO

TL;DR: CDGS通过将搜索嵌入扩散去噪过程，解决了组合生成模型中的模式平均问题，在机器人操作、全景图像和长视频生成中实现局部可行性和全局一致性。


<details>
  <summary>Details</summary>
Motivation: 组合生成模型在处理长时域任务分布时面临模式平均问题：当局部分布是多模态时，现有组合方法会平均不兼容的模式，导致生成的计划既不可行也不连贯。

Method: CDGS将搜索直接嵌入扩散去噪过程，通过基于种群的采样探索局部模式的多样化组合，使用基于似然的过滤剪枝不可行候选，并通过迭代重采样在重叠段之间强制执行全局一致性。

Result: CDGS在七个机器人操作任务上匹配了oracle性能，优于缺乏组合性或需要长时域训练数据的基线方法。该方法跨领域泛化，通过有效的局部到全局消息传递实现了连贯的文本引导全景图像和长视频生成。

Conclusion: CDGS通过将搜索与扩散过程相结合，有效解决了组合生成模型中的模式平均问题，实现了局部可行性和全局一致性的平衡，为长时域任务规划提供了有效的解决方案。
...
### [54] [SLEI3D: Simultaneous Exploration and Inspection via Heterogeneous Fleets under Limited Communication](https://arxiv.org/abs/2601.00163)
*Junfeng Chen,Yuxiao Zhu,Xintong Zhang,Bing Luo,Meng Guo*

Main category: cs.RO

TL;DR: 提出SLEI3D框架，用于异构机器人群体在未知环境中的协同3D探索、自适应检测和实时通信，支持大规模任务执行。


<details>
  <summary>Details</summary>
Motivation: 现有机器人舰队主要用于已知环境的例行检查，但在许多应用中，感兴趣区域未知且需要在线识别。同时需要考虑异构传感器、间歇性通信等实际约束。

Method: 提出SLEI3D规划协调框架，集成在线协同3D探索、自适应检测和及时通信策略。采用多层多速率规划机制处理特征数量和位置的不确定性，支持间歇或主动通信协议。

Result: 通过高保真仿真验证，支持多达48个机器人和38.4万立方米的大规模任务。同时进行了7个机器人的硬件实验验证。

Conclusion: SLEI3D框架有效解决了异构机器人在未知环境中的协同探索、检测和通信问题，为大规模实际应用提供了可行解决方案。
...
### [55] [SLAP: Slapband-based Autonomous Perching Drone with Failure Recovery for Vertical Tree Trunks](https://arxiv.org/abs/2601.00238)
*Julia Di,Kenneth A. W. Hoffmann,Tony G. Chen,Tian-Ao Ren,Mark R. Cutkosky*

Main category: cs.RO

TL;DR: 本文提出了一种适用于较大无人机的垂直树干停栖系统SLAP，通过视觉检测、IMU故障检测、姿态控制和弹性抓爪，实现了轻柔停栖和故障恢复，室内实验达到75%停栖成功率。


<details>
  <summary>Details</summary>
Motivation: 无人机在垂直表面停栖可降低能耗、进行表面采样或稳定观测，但现有方案多关注轻量化机械设计，系统集成度低，且常需高速危险着陆操作，不适合携带敏感电子设备的大型调查无人机。

Method: 提出SLAP系统，包含：视觉停栖点检测器、IMU停栖故障检测器、轻柔停栖姿态控制器、光学近距离检测系统，以及由商用slapbands制成的带微刺快速主动弹性抓爪。在1.2公斤商用四旋翼无人机上进行验证。

Result: 室内人机协同自主飞行实验显示，在真实橡树段上进行20次飞行，停栖成功率达75%；在2次诱导故障飞行中，停栖故障恢复率达100%。

Conclusion: SLAP系统为较大无人机提供了安全可靠的垂直树干停栖方案，实现了轻柔停栖和故障恢复能力，为无人机长期监测和采样任务提供了实用解决方案。
...
### [56] [Vehicle Painting Robot Path Planning Using Hierarchical Optimization](https://arxiv.org/abs/2601.00271)
...
<details>
  <summary>Details</summary>
Motivation: 车辆生产工厂中，多机械臂同时为传送带上的车身喷漆，路径设计目前依赖工程师手动完成，耗时且需要自动化来减少设计时间。传统机器人路径规划技术（如焊接）无法直接应用于喷漆过程的独特约束。

Method: 将喷漆路径设计建模为分层优化问题：上层子问题类似于车辆路径问题（VRP），负责分配车身区域给各机械臂；下层子问题进行详细路径规划。通过设计变量表示、约束条件、修复算子和初始化过程，灵活处理喷漆过程的特定约束。

Result: 在三种商用车型上的实验表明，该方法能够自动设计出满足所有喷漆约束的路径，其质量与工程师手动设计的路径相当。

Conclusion: 提出的分层优化方法成功实现了车辆喷漆路径的自动化设计，解决了传统手工设计的效率问题，为工业生产中的类似复杂路径规划问题提供了有效解决方案。
...
### [57] [Pure Inertial Navigation in Challenging Environments with Wheeled and Chassis Mounted Inertial Sensors](https://arxiv.org/abs/2601.00275)
...
<details>
  <summary>Details</summary>
Motivation: 在GNSS信号受限或光照条件恶劣的室内外环境中，自动驾驶车辆和轮式机器人仅依赖惯性传感器进行导航时，会因惯性测量误差而产生时间漂移问题。

Method: 提出WiCHINS系统，将轮载惯性传感器与底盘惯性传感器相结合。开发了一个三阶段框架，每个阶段使用专用的扩展卡尔曼滤波器，充分利用轮子和车身不同位置传感器的优势进行状态估计。

Result: 使用5个惯性测量单元，总记录时间228.6分钟的数据集进行测试。与四种其他惯性基线方法相比，使用两个轮子和一个车身惯性测量单元时，平均位置误差为11.4米，占平均行驶距离的2.4%。
...
### [58] [Replaceable Bit-based Gripper for Picking Cluttered Food Items](https://arxiv.org/abs/2601.00305)
*Prashant Kumar,Yukiyasu Domae,Weiwei Wan,Kensuke Harada*

Main category: cs.RO

TL;DR: 提出一种可更换夹具头的抓取系统，用于处理杂乱食品的重量分拣，针对不同形状食品设计专用夹具头，实现高精度重量控制抓取


<details>
  <summary>Details</summary>
Motivation: 食品包装行业面临快速变化的食品种类和重量需求，需要处理从单件到柔性、长条状、杂乱等多种食品的抓取挑战

Method: 开发可更换夹具头的抓取系统，配备专用食品夹具头（bits）增强抓取能力，采用皮带更换系统在不同食品间快速切换，针对鲑鱼子和意大利面设计专用夹具头

Result: 抓取系统成功抓取意大利面和鲑鱼子，重量分拣精度分别超过80%和95%，能够快速切换不同夹具头，处理多种食品

Conclusion: 可更换夹具头系统有效解决了杂乱食品的重量分拣问题，为食品包装自动化提供了灵活高效的解决方案
...
### [59] [Calling for Backup: How Children Navigate Successive Robot Communication Failures](https://arxiv.org/abs/2601.00754)
...
<details>
  <summary>Details</summary>
Motivation: 虽然已有研究探讨成人对连续机器人错误的反应，但儿童对此类错误的反应尚不明确。本研究旨在填补这一空白，探索儿童对机器人社交错误和性能错误的反应，特别是重复对话错误。

Method: 复制Liu等人的连续机器人失败范式，让59名8-10岁儿童与机器人互动。机器人连续三次无法理解儿童提示，研究人员通过视频记录和分析儿童的行为反应。

Result: 儿童与成人反应既有相似性也有差异：相似之处包括调整提示、改变语调、情绪性非语言反应增加；差异在于儿童表现出更多脱离行为（如暂时忽略机器人或寻求成人帮助）。错误不影响儿童对机器人的感知。

Conclusion: 儿童对机器人错误表现出更灵活的对话期望，这些发现有助于为年轻用户设计更有效、更符合发展特点的人机交互系统。
...
### [60] [LLM-Based Agentic Exploration for Robot Navigation & Manipulation with Skill Orchestration](https://arxiv.org/abs/2601.00555)
*Abu Hanif Muhammad Syarubany,Farhan Zaki Rahmani,Trio Widianto*

Main category: cs.RO

TL;DR: 本文提出了一个基于LLM的端到端智能探索系统，用于室内购物任务，在Gazebo仿真和真实走廊环境中进行评估。系统通过检测路标构建轻量级语义地图，使用AprilTags作为定位锚点，LLM根据自然语言指令生成决策，ROS控制器执行模块化运动原语。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够在复杂室内环境中执行多商店导航和物品获取任务的智能机器人系统，实现从用户自然语言指令到实际物理操作的端到端执行，同时保持系统的模块化和可调试性。

Method: 系统采用增量式构建轻量级语义地图的方法，在路口检测标志牌并存储方向-POI关系及估计的路口位姿。AprilTags提供可重复的定位锚点。LLM根据自然语言购物请求在每个路口生成约束的离散动作（方向和是否进入商店），ROS有限状态主控制器通过门控模块化运动原语执行决策，包括基于局部代价地图的避障、AprilTag接近、商店进入和抓取。

Result: 定性结果表明，集成系统能够从用户指令到多商店导航和物品检索执行端到端任务执行，同时通过其基于文本的地图和记录决策历史保持模块化和可调试性。

Conclusion: 该系统展示了基于LLM的智能探索在室内购物任务中的可行性，实现了从自然语言理解到物理执行的完整流程，同时保持了系统的模块化架构和可调试性，为实际应用提供了有价值的参考。
...
### [61] [Priority-Aware Multi-Robot Coverage Path Planning](https://arxiv.org/abs/2601.00580)
*Kanghoon Lee,Hyeonjun Kim,Jiachen Li,Jinkyoo Park*

Main category: cs.RO

TL;DR: 提出优先级感知的多机器人覆盖路径规划问题，通过两阶段框架显著降低优先级加权延迟，同时保持竞争性的总完成时间


<details>
  <summary>Details</summary>
Motivation: 传统多机器人覆盖路径规划方法假设环境区域具有均匀重要性，但在实际应用中某些区域需要优先覆盖，现有方法无法有效处理这种优先级需求

Method: 提出可扩展的两阶段框架：1) 贪心区域分配结合局部搜索和基于生成树的路径规划；2) Steiner树引导的剩余覆盖

Result: 实验表明该方法相比标准MCPP基线显著降低优先级加权延迟，同时保持竞争性的总完成时间，方法能有效扩展到不同机器人数量，通过调整优先级权重可有效控制区域覆盖行为

Conclusion: 提出的优先级感知MCPP框架能有效处理区域优先级差异，在保持整体覆盖效率的同时优先覆盖重要区域，为实际应用中的多机器人覆盖任务提供了更实用的解决方案
...
### [62] [Vision-based Goal-Reaching Control for Mobile Robots Using a Hierarchical Learning Framework](https://arxiv.org/abs/2601.00610)
*Mehdi Heydari Shahna,Pauli Mustalahti,Jouni Mattila*

Main category: cs.RO

TL;DR: 提出一个用于大型机器人的安全目标到达控制框架，通过模块化设计结合强化学习规划、深度学习建模和鲁棒自适应控制，确保在易打滑地形上的稳定性和安全性。


<details>
  <summary>Details</summary>
Motivation: 强化学习在机器人应用中需要大量探索状态-动作空间，期间行为可能不安全，这限制了其在大型复杂机器人上的应用。需要设计一个安全的目标到达控制框架，使大型机器人能在不稳定地形上安全操作。

Method: 将系统分解为紧密耦合的功能模块：1)实时视觉姿态估计提供准确机器人状态；2)强化学习运动规划器生成平滑运动指令；3)监督深度学习模型捕捉机器人复杂动力学；4)基于模型的鲁棒自适应控制器确保轮子跟踪运动指令；5)数学安全监督器监控机器人安全。

Result: 提出的框架保证了执行系统的均匀指数稳定性和整个操作的安全性。在6000公斤机器人上的不同场景实验证实了该框架的有效性。

Conclusion: 通过模块化设计结合强化学习、深度学习和鲁棒控制，成功实现了大型机器人在不稳定地形上的安全目标到达控制，减少了人工干预并确保了操作安全性。
...
### [63] [From 2D to 3D terrain-following area coverage path planning](https://arxiv.org/abs/2601.00614)
*Mogens Plessen*

Main category: cs.RO

TL;DR: 提出了一种3D地形跟随的区域覆盖路径规划算法，生成相邻路径使其在水平方向上保持机械工作宽度距离，同时在垂直方向上保持特定工作高度跟随地形起伏。


<details>
  <summary>Details</summary>
Motivation: 在农业等实际应用中，需要机械在复杂3D地形上执行覆盖任务，传统2D路径规划无法适应地形起伏，需要开发能够同时考虑水平间距和垂直高度的3D地形跟随路径规划方法。

Method: 使用逆距离加权方法生成均匀间距的高程数据，结合局部搜索算法，生成相邻路径使其在水平方向保持机械工作宽度距离，同时在垂直方向保持特定工作高度跟随地形起伏。

Result: 算法在真实农业3D地形数据上进行了验证，能够成功生成满足工作宽度和工作高度要求的3D地形跟随覆盖路径，相比2D算法具有更高的复杂性和适应性。

Conclusion: 提出的3D地形跟随区域覆盖路径规划算法有效解决了复杂地形下的机械作业路径规划问题，为农业等领域的实际应用提供了可行的技术方案。
...
### [64] [RoboReward: General-Purpose Vision-Language Reward Models for Robotics](https://arxiv.org/abs/2601.00675)
*Tony Lee,Andrew Wagenmaker,Karl Pertsch,Percy Liang,Sergey Levine,Chelsea Finn*

Main category: cs.RO

TL;DR: 该研究提出了RoboReward——一个基于大规模真实机器人数据的奖励数据集和基准，以及相应的视觉语言奖励模型，用于改进机器人强化学习中的奖励设计。


<details>
  <summary>Details</summary>
Motivation: 在真实机器人领域，获取有效的奖励函数通常需要费时的人工标注或脆硬的手工设计目标。虽然视觉语言模型（VLMs）有潜力作为自动奖励模型，但其在真实机器人任务中的有效性尚不清楚。

Method: 1) 构建RoboReward数据集和基准，基于Open X-Embodiment和RoboArena的大规模真实机器人数据；2) 提出负样本数据增强流程，通过反事实重标注和时间裁剪生成校准的负样本和接近成功的样本；3) 训练4B和8B参数的通用视觉语言奖励模型。

Result: 1) 评估显示现有VLMs在所有任务中表现不均，有较大改进空间；2) 训练的4B/8B参数模型在短时程机器人任务奖励分配上优于更大的VLMs；3) 8B参数奖励VLM在真实机器人强化学习中大幅优于Gemini Robotics-ER 1.5，显著缩小了与人工奖励的差距。

Conclusion: RoboReward为机器人奖励学习提供了系统化的数据集和基准，训练出的视觉语言奖励模型在真实机器人强化学习中表现出色，为自动奖励设计提供了有效解决方案。
...
### [65] [DefVINS: Visual-Inertial Odometry for Deformable Scenes](https://arxiv.org/abs/2601.00702)
...
<details>
  <summary>Details</summary>
Motivation: 传统VIO基于刚性假设，在可变形场景中会过度拟合局部非刚性运动或产生严重漂移，需要新的框架来处理场景变形问题。

Method: DefVINS将刚性IMU锚定状态与非刚性变形图分离，使用标准VIO初始化后渐进激活非刚性自由度，基于可观测性分析采用条件激活策略防止病态更新。

Result: 消融研究表明，结合惯性约束和可观测性感知的变形激活策略，在非刚性环境下显著提高了系统的鲁棒性。

Conclusion: DefVINS通过显式分离刚性和非刚性运动，结合惯性约束和智能激活策略，有效解决了可变形场景中的VIO问题，提高了系统在非刚性环境下的性能。
...
### [66] [Deep Learning Approach for the Diagnosis of Pediatric Pneumonia Using Chest X-ray Imaging](https://arxiv.org/abs/2601.00041)
*Fatemeh Hosseinabadi,Mohammad Mojtaba Rohani*

Main category: eess.IV

TL;DR: 本研究评估了三种CNN架构（ResNetRS、RegNet、EfficientNetV2）在儿科肺炎X光图像自动分类中的性能，RegNet表现最佳，准确率达92.4%，灵敏度90.1%。
...
Method: 从公开数据集中提取1000张儿科胸部X光图像，预处理并进行二元分类标注。使用预训练的ImageNet权重对ResNetRS、RegNet和EfficientNetV2三种CNN架构进行微调，基于准确率和灵敏度进行评估。

Result: RegNet获得最高分类性能：准确率92.4%，灵敏度90.1%；ResNetRS次之：准确率91.9%，灵敏度89.3%；EfficientNetV2：准确率88.5%，灵敏度88.1%。

Conclusion: 基于迁移学习的CNN架构在儿科肺炎X光图像自动分类中表现出色，特别是RegNet模型，为资源有限环境下的儿科肺炎诊断提供了有前景的自动化解决方案。
...
### [67] [Hear the Heartbeat in Phases: Physiologically Grounded Phase-Aware ECG Biometrics](https://arxiv.org/abs/2601.00170)
*Jintao Huang,Lu Leng,Yi Zhang,Ziyuan Yang*

Main category: eess.IV

TL;DR: 提出HPAF框架用于ECG身份认证，通过三阶段设计避免特征纠缠：IPR提取各心脏相位特征，PGHF融合相关相位，GRF整合全局表示。同时提出HAM注册策略减少心跳噪声影响。


<details>
  <summary>Details</summary>
Motivation: 现有ECG身份认证方法将心跳视为同质信号，忽略了心脏周期内相位特异性特征，导致特征纠缠问题。

Method: 提出HPAF三阶段框架：1) IPR独立提取各心脏相位表示；2) PGHF分层融合生理相关相位；3) GRF自适应平衡各相位贡献。同时提出HAM多原型注册策略。

Result: 在三个公开数据集上的实验表明，HPAF在闭集和开集设置下均达到最先进性能。

Conclusion: HPAF框架通过显式处理心脏相位特异性特征，有效避免了特征纠缠问题，结合HAM注册策略，显著提升了ECG身份认证的准确性和鲁棒性。
...
### [68] [Let Distortion Guide Restoration (DGR): A physics-informed learning framework for Prostate Diffusion MRI](https://arxiv.org/abs/2601.00226)
...
TL;DR: DGR是一个物理信息混合CNN-扩散框架，用于前列腺单次激发EPI扩散加权成像中严重磁化率畸变的采集无关校正，通过训练来反转真实的畸变前向模型。


<details>
  <summary>Details</summary>
Motivation: 前列腺DWI中的严重磁化率畸变（如髋关节假体和直肠扩张）会降低图像质量和诊断信心，现有方法如FSL TOPUP和FUGUE效果有限，需要开发不依赖额外采集的畸变校正方法。

Method: DGR结合CNN几何校正模块和条件扩散细化，在T2加权解剖指导下工作。使用410个多机构研究的无畸变前列腺DWI和配准T2加权图像，结合11个金属植入病例的实测B0场图，通过前向模拟器生成低b值、高b值DWI和ADC畸变数据。

Result: 在34个合成验证集上，DGR比FSL TOPUP和FUGUE获得更高的PSNR和更低的NMSE。在34个真实临床病例（包括髋关节假体和明显直肠扩张）中，DGR改善了几何保真度，提高了放射科医生评定的图像质量和诊断信心。

Conclusion: 学习物理模拟前向过程的逆过程为前列腺DWI畸变校正提供了一种实用的采集无关替代方案，优于传统采集依赖的校正流程。
...
### [69] [The Impact of Lesion Focus on the Performance of AI-Based Melanoma Classification](https://arxiv.org/abs/2601.00355)
*Tanay Donde*

Main category: eess.IV

TL;DR: 研究分析了黑色素瘤分类模型中病变区域关注度与诊断性能的关系，发现模型对病变区域关注度越高，诊断性能越好。


<details>
  <summary>Details</summary>
Motivation: 黑色素瘤是最致命的皮肤癌亚型，早期准确检测可改善患者预后。虽然机器学习模型（特别是CNN）在自动化黑色素瘤分类方面显示出巨大潜力，但由于对病变区域关注不一致，其诊断可靠性仍存在问题。

Method: 使用掩码图像、边界框检测和迁移学习，通过多种可解释性和敏感性分析方法，研究模型注意力与病变区域的对齐程度，以及这种对齐与精确率、召回率和F1分数的相关性。

Result: 结果显示，对病变区域关注度更高的模型取得了更好的诊断性能，表明可解释性AI在医学诊断中的潜力。
...
### [70] [Physics-Guided Dual-Domain Plug-and-Play ADMM for Low-Dose CT Reconstruction](https://arxiv.org/abs/2601.00669)
*Sayantan Dutta,Sudhanya Chatterjee,Ashwini Galande,K. S. Shriram,Bipul Das*

Main category: eess.IV

TL;DR: 提出一种基于Plug-and-Play模型迭代重建框架，结合两阶段自监督噪声到噪声训练的深度卷积去噪器，用于超低剂量CT重建，在降低70-80%辐射剂量的同时保持诊断质量。


<details>
  <summary>Details</summary>
Motivation: 超低剂量CT成像能显著减少患者辐射暴露，但会导致严重的结构性和随机噪声，降低图像质量。现有方法难以在极低剂量下保持图像质量和诊断准确性。

Method: 提出PnP-MBIR框架，交替执行正弦图域数据保真和图像域深度卷积去噪。采用两阶段自监督训练：第一阶段从噪声数据完全自监督训练，第二阶段使用高剂量数据进行微调，确保在超低剂量下的鲁棒性。

Result: 方法在降低70-80%辐射剂量的情况下，重建质量与标准全剂量扫描相当。GLCM纹理特征分析（对比度、同质性、熵、相关性）显示优于独立深度学习和监督PnP基线，能有效减少条纹和结构伪影，同时保持细微组织对比度。

Conclusion: 提出的PnP-MBIR框架结合两阶段自监督训练，在超低剂量CT重建中表现出色，能显著降低辐射剂量同时保持诊断质量，是ULDCT重建的有前景工具。
...
### [71] [KDPhys: An Attention Guided 3D to 2D Knowledge Distillation for Real-time Video-Based Physiological Measurement](https://arxiv.org/abs/2601.00714)
...
TL;DR: 提出KDPhys框架，通过注意力知识蒸馏将3D CNN教师模型的时间特征迁移到轻量2D CNN学生模型，用于从面部视频提取rPPG信号，显著降低计算复杂度并提高心率估计精度。


<details>
  <summary>Details</summary>
Motivation: 随着COVID-19疫情推动远程医疗需求增长，需要实时、非接触的生理监测技术。基于摄像头的rPPG技术面临计算复杂度高、实时性不足的挑战，需要开发轻量高效的模型。

Method: 提出KDPhys框架：1) 使用注意力机制的知识蒸馏，将3D CNN教师模型的全局时间特征迁移到2D CNN学生模型；2) 引入DILATE损失函数，同时考虑rPPG信号的形态和时间特性；3) 实现3D到2D特征的有效蒸馏。

Result: 在三个基准数据集上评估：1) 计算复杂度大幅降低，参数量仅0.23M（现有方法一半），运行速度提升56.67%；2) 平均MAE为1.78 bpm，相比SOTA方法降低18.15%；3) 在不同环境和活动场景下表现出鲁棒性和适应性。

Conclusion: KDPhys是rPPG领域首个知识蒸馏应用，通过3D到2D特征蒸馏实现了高效轻量的rPPG信号提取，在保持精度的同时显著提升计算效率，为实时远程生理监测提供了实用解决方案。
...
### [72] [Augmented Reality Indoor Wayfinding in Hospital Environments An Empirical Study on Navigation Efficiency, User Experience, and Cognitive Load](https://arxiv.org/abs/2601.00001)
...
<details>
  <summary>Details</summary>
Motivation: 医院是认知需求最高的室内环境之一，尤其对不熟悉布局的患者和访客。研究旨在比较AR导航系统与传统纸质地图在大型医院环境中的有效性

Method: 采用混合方法实验，32名参与者使用AR手持导航系统或纸质地图完成导航任务，测量导航性能、认知负荷（NASA-TLX）、情境焦虑（STAI-State）、空间行为和用户满意度

Result: AR用户导航任务完成时间显著更快，错误更少，焦虑和工作负荷更低；但纸质地图用户在基于草图的回忆任务中表现出更强的空间记忆，揭示了实时效率与长期空间学习之间的权衡

Conclusion: AR导航系统在医院环境中具有明显优势，但需要考虑空间记忆的权衡。研究讨论了包容性AR设计、空间认知和医疗可及性的影响，为自适应室内导航工具提供了可行的设计策略
...
### [73] [Effects of Limited Field of View on Musical Collaboration Experience with Avatars in Extended Reality](https://arxiv.org/abs/2601.00333)
*Suibi Che-Chuan Weng,Torin Hopkins,Shih-Yu Ma,Amy Banic,Ellen Yi-Luen Do*

Main category: cs.HC

TL;DR: 研究显示，在XR音乐协作中，受限视野会降低共在感、手势识别速度和整体享受度，但通知系统可以改善反应时间。


<details>
  <summary>Details</summary>
Motivation: 在音乐协作中，视觉线索对音乐家之间的沟通至关重要。XR应用（如AR眼镜）会限制玩家的视野范围，本研究旨在探究受限视野对音乐协作的影响。

Method: 首先观察经验丰富的音乐家在有无视觉遮挡情况下的非正式协作，然后进行19名参与者的组内研究，比较无限制视野的HoloJam全息设置与52°受限视野的Nreal AR眼镜。在AR设置中测试了两种条件：标准AR和名为Mini Musicians的改进AR通知系统。

Result: HoloJam提供了更高的共在感、更快的手势识别和更大的享受度。Mini Musicians应用相比标准AR设置减少了反应时间并保持了享受度。
...
### [74] [Unseen Risks of Clinical Speech-to-Text Systems: Transparency, Privacy, and Reliability Challenges in AI-Driven Documentation](https://arxiv.org/abs/2601.00382)
*Nelly Elsayed*

Main category: cs.HC

TL;DR: 该研究分析了AI语音转文本系统在临床应用中面临的社会技术风险，提出了一个多层治理框架和实施路线图，以支持安全、公平的医疗技术整合。


<details>
  <summary>Details</summary>
Motivation: AI语音转文本系统在临床环境中的快速部署已超过对其相关社会技术风险的理解，包括透明度、可靠性、患者自主权、工作流程协调和组织治理等方面。需要更清晰的风险分析来支持安全、公平的医疗实践整合。

Method: 研究综合了来自技术性能研究、监管和伦理标准、临床工作流程分析以及组织政策指导的跨学科证据，用于开发一个多层社会技术概念框架来评估和治理STT系统。

Result: 研究发现STT系统在紧密耦合的社会技术环境中运行，模型性能、临床医生监督、患者权利、工作流程设计和机构治理相互依存。研究提供了一个结构化的社会技术治理框架和实施路线图。

Conclusion: 该框架强调保护患者自主权、文档完整性和机构信任的保障措施，同时实现STT技术的高效和有益使用，为医疗组织负责任、公平地采用STT系统提供了可操作的指导。
...
### [75] [User Perceptions of an LLM-Based Chatbot for Cognitive Reappraisal of Stress: Feasibility Study](https://arxiv.org/abs/2601.00570)
*Ananya Bhattacharjee,Jina Suh,Mohit Chandra,Javier Hernandez*

Main category: cs.HC

TL;DR: 基于GPT-4o的聊天机器人提供单次认知重评干预，显著降低职场压力感知强度并改善压力心态，但存在脚本化与AI同理心等设计张力


<details>
  <summary>Details</summary>
Motivation: 现有数字心理健康工具因脚本僵化难以支持用户自然描述压力源，研究探索LLM在职场压力认知重评干预中的可行性

Method: 对100名科技公司员工进行GPT-4o聊天机器人提供的结构化认知重评单次干预，采用前后测设计测量压力强度、压力心态、感知需求和资源，使用Wilcoxon符号秩检验分析，通过RoBERTa分类器和LLM压力评分器分析对话情感和压力轨迹，并进行主题分析

Result: 感知压力强度显著降低，压力心态显著改善；感知资源和需求变化符合预期但未达统计显著性；自动化分析显示负面情感和压力在对话过程中持续下降；质性分析发现参与者重视结构化提示的组织、视角获取和认可感，但也报告脚本化、交互时长偏好和AI同理心反应等张力

Conclusion: LLM整合到数字心理健康干预在职场环境中具有前景，但需解决脚本化、交互设计和AI同理心接受度等设计约束
...
### [76] [The AI Invisibility Effect: Understanding Human-AI Interaction When Users Don't Recognize Artificial Intelligence](https://arxiv.org/abs/2601.00579)
*Obada Kraishan*

Main category: cs.HC

TL;DR: 大规模分析发现AI功能应用评分更低，但控制变量后关系反转，用户对AI的认知而非AI本身导致负面评价


<details>
  <summary>Details</summary>
Motivation: 研究AI在移动应用中快速集成对用户感知的影响，目前对此了解不足

Method: 分析148万+应用评论，涵盖422个应用（200个含AI功能，222个对照），使用情感分类、主题建模和关注-收益分类方法

Result: 仅11.9%评论提及AI，AI应用评分显著更低；但控制变量后关系反转；隐私是主要担忧（34.8%），效率是主要收益（42.3%）；不同类别效果差异大

Conclusion: AI功能常低于用户意识阈值，是AI的显性认知而非其存在本身驱动负面评价，挑战了AI系统技术接受度的基本假设
...
### [77] [Wave2Word: A Multimodal Transformer Framework for Joint EEG-Text Alignment and Multi-Task Representation Learning in Neurocritical Care](https://arxiv.org/abs/2601.00670)
*Argha Kamal Samanta,Deepak Mewada,Monalisa Sarma,Debasis Samanta*

Main category: cs.HC

TL;DR: 提出一个多模态EEG表征学习框架，整合信号域建模与结构化临床语言监督，通过对比学习和文本重建损失提升EEG表征的临床意义，而不仅仅是分类准确性。


<details>
  <summary>Details</summary>
Motivation: 当前EEG建模实践的核心局限在于学习到的表征与临床工作流程中EEG发现解释和总结方式之间的对应关系较弱。有害EEG活动表现出重叠模式、专家分级共识和时间持续性，这些特征无法仅通过分类目标很好地捕捉。

Method: 1. 将原始EEG转换为纵向双极导联和时间频率表示；2. 使用双Transformer编码器建模互补的时间依赖性和频率中心依赖性，通过自适应门控机制融合；3. 通过对比目标将EEG嵌入与结构化专家共识描述对齐；4. 引入EEG条件文本重建损失作为表征级约束，与标准分类损失结合。

Result: 在受控的训练-验证-测试划分下，六类测试准确率达到0.9797。消融分析显示，移除对比对齐会使跨模态检索性能从Recall@10的0.3390降至0.0045，尽管分类准确性变化很小。

Conclusion: 研究结果表明，判别准确性并不能可靠地反映具有临床意义的EEG建模的表征质量。多模态监督方法能够学习到与临床解释更一致的EEG表征，这对于神经重症监护中的EEG监测具有重要意义。
...
### [78] [The Effect of Transparency on Students' Perceptions of AI Graders](https://arxiv.org/abs/2601.00765)
*Joslyn Orgill,Andra Rice,Max Fowler,Seth Poulsen*

Main category: cs.HC

TL;DR: 研究测试透明度对学生对自动评分器态度的影响，发现透明性提高了学生对评分器准确性的感知和讨论意愿，但未改善其他相关态度


<details>
  <summary>Details</summary>
Motivation: 虽然基于NLP的自动评分系统对开放式问题反馈有益，但学生并不总是喜欢、理解或信任自动评分器，因此研究透明度如何影响学生对自动评分器的态度

Method: 通过实验测试透明度对学生态度的影响，比较透明自动评分器与无透明度控制组的效果

Result: 透明自动评分器提高了学生对评分器准确性的感知和在调查评论中讨论的意愿，但未改善其他相关态度（如愿意在测试中被评分），这可能是因为本研究中学生对自动评分器的信任度高于先前研究

Conclusion: 透明度对改善学生对自动评分器态度有部分积极效果，但整体影响有限，需要进一步探讨学生信任度变化的原因
