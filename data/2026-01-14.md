...
TL;DR: 本文提出了一种基于边缘AI的实时路边感知节点，用于多类交通违规分析和安全事件传播，在印度等新兴经济体的快速机动化背景下解决执法不对称问题。


<details>
  <summary>Details</summary>
Motivation: 印度等新兴经济体的快速机动化导致严重的执法不对称问题，2023年记录了超过1100万起违规行为，而警力密度仅为每4000辆车一名警察。传统的监控和人工罚单无法应对这种规模，需要自主、协作且节能的边缘AI感知基础设施。

Method: 系统集成了YOLOv8 Nano进行高精度多目标检测，DeepSORT实现时间一致的车辆跟踪，以及基于规则的光学字符识别后处理引擎，能够识别符合MoRTH AIS 159和ISO 7591视觉对比度标准的退化或多语言车牌。系统部署在NVIDIA Jetson Nano上，通过TensorRT FP16量化优化，无需手动校准感兴趣区域。

Result: 系统在9.6W功耗下维持28-30帧/秒的推理速度，实现97.7%的违规检测准确率和84.9%的OCR精度，涵盖信号灯闯行、斑马线违规、逆向行驶、非法掉头和超速五类违规。相比YOLOv4 Tiny、PP YOLOE S和Nano DetPlus，平均精度提升10.7%，每瓦特精度提升1.4倍。

Conclusion: 该路边边缘AI分析节点不仅能够增强执法能力，还能通过V2X协议向联网车辆和智能交通系统后端发布标准化的CAM和DENM类型安全事件，证明路边边缘AI分析可以增强协作感知和主动道路安全管理，符合IEEE智能车辆生态系统。
...
TL;DR: 该论文提出了ROAD基准测试，用于评估LiDAR点云分类在同时面临域偏移和标签演化（类分裂、类扩展、类插入）时的持续学习能力，填补了3D感知中持续学习研究的空白。


<details>
  <summary>Details</summary>
Motivation: 3D感知系统在实际应用中需要适应不断变化的对象定义和传感器域，但目前3D点云感知中的持续学习和迁移学习研究相对不足，特别是在同时面临域偏移和标签演化的情况下。

Method: 提出ROAD基准测试，基于大规模数据集（Waymo、NuScenes、Argoverse2），评估零样本迁移、线性探测和持续学习方法，并分析骨干架构、训练目标和持续学习方法的影响。

Result: 研究发现现有方法在现实偏移下存在局限性，为未来稳健3D感知研究建立了强基线。

Conclusion: 该工作填补了3D点云感知中持续学习研究的空白，为自动驾驶等实际应用中的3D感知系统稳健性评估提供了重要基准。
...
TL;DR: 该研究探讨了Gabor特征和GLCM纹理特征在建模早期视觉搜索行为中的关系，提出了两种特征组合管道来预测人类注视区域，并在模拟数字乳腺断层合成图像上验证了有效性。
...
Method: 提出了两种特征组合管道来整合Gabor和GLCM特征，以缩小可能的人类注视区域范围。这些管道在模拟数字乳腺断层合成图像上进行评估，并与基于阈值的模型观察者进行比较。

Result: 结果显示，提出的管道预测的注视候选区域与基于阈值的模型观察者在定性上一致。观察到GLCM均值与Gabor特征响应之间存在强相关性，表明这些特征编码了相关的图像信息。人类观察者的眼动追踪数据进一步表明预测的注视区域与早期注视行为一致。

Conclusion: 这些发现强调了结合结构和纹理特征对于建模视觉搜索的价值，并支持开发感知信息驱动的观察者模型。
...
### [4] [Zero-Shot Distracted Driver Detection via Vision Language Models with Double Decoupling](https://arxiv.org/abs/2601.08467)
...
<details>
  <summary>Details</summary>
Motivation: 分心驾驶是交通事故的主要原因，现有基于视觉语言模型的方法在真实场景中表现不佳，主要问题是模型将驾驶员特定外观特征（如服装、年龄、性别）与行为线索纠缠在一起，导致分类决策基于"驾驶员是谁"而非"驾驶员在做什么"。

Method: 提出主题解耦框架：1) 提取驾驶员外观嵌入表示；2) 在零样本分类前从图像嵌入中移除外观影响，强调与分心相关的证据；3) 通过Stiefel流形上的度量投影正交化文本嵌入，在保持原始语义的同时提高可分离性。

Result: 实验表明该方法在多个基准测试中相比先前基线方法取得了一致的性能提升，证明了该框架在实际道路安全应用中的潜力。

Conclusion: 通过解耦驾驶员外观特征与行为线索，提出的框架能够显著提升基于视觉语言模型的分心驾驶检测性能，为解决真实场景中的分心驾驶检测问题提供了有前景的解决方案。
...
Method: 结合两种关键技术：1）基于3D高斯泼溅的变形引导实现一致的几何修改；2）扩散引导用于内容校正和视觉质量提升；采用渐进式编辑策略支持激进拖拽编辑
...
TL;DR: 本文提出AKT模型，通过PKAN模块和PAA注意力机制解决无人机图像中玉米点级定位的挑战，在PML数据集上取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 无人机高分辨率摄影测量在精准农业中至关重要，但玉米点级定位面临三大挑战：1) 物体与像素比极低(<0.1%)；2) 超高分辨率图像(>3000x4000)的二次注意力计算成本过高；3) 农业场景特有的稀疏物体分布和环境变化问题

Method: 提出Additive Kolmogorov-Arnold Transformer (AKT)：1) 用Pade Kolmogorov-Arnold Network (PKAN)模块替代传统MLP，增强小物体特征提取的功能表达能力；2) 引入PKAN Additive Attention (PAA)建模多尺度空间依赖关系，降低计算复杂度；3) 构建Point-based Maize Localization (PML)数据集，包含1,928张高分辨率无人机图像和约501,000个点标注
...
TL;DR: 开发了新的统计理想观察者模型，通过设置最小可提取图像特征阈值来执行整体视觉搜索处理，减少自由参数，缩小系统规模


<details>
  <summary>Details</summary>
Motivation: 开发一个更高效的视觉搜索处理模型，通过减少自由参数来简化系统，为医学图像感知、计算机视觉、目标检测等领域提供更好的性能评估框架

Method: 开发统计理想观察者模型，在最小可提取图像特征上设置阈值，实现整体视觉搜索处理，通过参数缩减来优化系统结构

Result: 创建了一个参数更少、系统更紧凑的视觉搜索处理框架，能够有效应用于多个领域

Conclusion: 该新型统计理想观察者模型为医学图像感知、计算机视觉、目标检测等应用提供了优化的性能评估和特征选择框架
...
TL;DR: CASHEW框架通过推理时聚合多个候选轨迹来稳定视觉语言模型的多步推理，CASHEW-RL变体通过强化学习将聚合行为内化到单一模型中，显著提升多模态理解任务的性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在多模态理解和推理任务中表现优异，但其多步推理过程不稳定，相同输入下多次采样会产生不同的推理轨迹和不一致的最终预测，需要解决这一稳定性问题。

Method: 提出两种互补方法：1) CASHEW推理时框架，通过迭代聚合多个候选轨迹形成更高质量的推理轨迹，利用显式视觉验证过滤幻觉步骤并将推理基于视觉证据；2) CASHEW-RL学习变体，使用组序列策略优化(GSPO)训练，采用复合奖励鼓励基于最小充分视觉证据的正确答案，并根据任务难度自适应分配推理努力。

Result: 在13个图像理解、视频理解和视频推理基准测试中显示显著性能提升，包括ScienceQA上提升23.6个百分点，EgoSchema上提升8.1个百分点。

Conclusion: CASHEW框架通过推理时轨迹聚合和视觉验证有效稳定了视觉语言模型的多步推理，CASHEW-RL进一步将这种能力内化到模型中，实现了更鲁棒的自聚合推理。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于文本条件的扩散编辑器在单个对象替换方面表现良好，但当需要同时引入新对象和新风格时存在困难。需要一种能够同时处理内容和外观控制的编辑方法。

Method: 提出TP-Blend框架，包含两个互补的注意力处理器：1) 交叉注意力对象融合(CAOF)，通过熵正则化最优传输问题重新分配多头特征向量；2) 自注意力风格融合(SASF)，通过细节敏感实例归一化和Key/Value矩阵交换注入风格。

Result: 实验表明TP-Blend能够生成高分辨率、照片级真实的编辑结果，在内容和外观控制方面具有精确性，在定量保真度、感知质量和推理速度方面超越现有基线方法。

Conclusion: TP-Blend是一个轻量级、无需训练的双提示融合框架，能够同时实现对象融合和风格注入，为图像编辑提供了更精确的内容和外观控制能力。
...
TL;DR: 提出一种无需训练、仅使用视觉的零样本异常检测框架DIVAD，通过预训练DDIM模型的反向过程重构图像，利用输入图像与重构图像之间的差异来检测和定位异常，无需依赖细粒度提示或其他模态辅助。


<details>
  <summary>Details</summary>
Motivation: 当前零样本异常检测方法大多依赖语言等额外模态来生成细粒度提示进行定位，而纯视觉方法仅限于图像级分类，缺乏空间精度。需要开发一种简单有效的无需训练、仅使用视觉的零样本异常检测框架，避免对细粒度提示的依赖。

Method: 提出DIVAD框架：1）给定输入图像和通用文本描述（如"一张[物体类别]的图像"）；2）将图像反演为潜在表示；3）从固定的中间时间步开始去噪过程重构图像；4）由于基础扩散模型仅在正常数据上训练，重构过程会产生正常外观的图像；5）通过输入图像与重构图像之间的差异来检测和定位异常。

Result: 在VISA数据集上实现了最先进的性能，展示了强大的定位能力，无需辅助模态，促进了零样本异常检测研究从提示依赖的转变。

Conclusion: 提出的DIVAD框架是一种简单有效的无需训练、仅使用视觉的零样本异常检测方法，通过扩散模型反演重构图像来检测异常，避免了细粒度提示的依赖，在VISA数据集上表现出色，为纯视觉零样本异常检测提供了新方向。
...
TL;DR: FigEx2是一个视觉条件框架，用于从科学复合图中定位面板并生成面板级描述，通过噪声感知门控融合模块和强化学习策略提升性能，在跨学科科学领域表现出色。


<details>
  <summary>Details</summary>
Motivation: 科学复合图将多个标记面板组合成单个图像，但实际管道中的标题常常缺失或仅提供图级摘要，这使得面板级理解变得困难。

Method: 提出FigEx2视觉条件框架，包含噪声感知门控融合模块来过滤标记级特征以稳定检测查询空间，并采用结合监督学习和强化学习的阶段性优化策略，使用CLIP对齐和BERTScore语义奖励来确保严格的多模态一致性。

Result: FigEx2在检测方面达到0.726 mAP@0.5:0.95的优异性能，在METEOR和BERTScore上分别显著优于Qwen3-VL-8B模型0.51和0.24分，并且在未经微调的情况下对分布外科学领域表现出卓越的零样本迁移能力。

Conclusion: FigEx2框架有效解决了科学复合图的面板级理解和描述问题，通过创新的融合模块和优化策略实现了高性能和良好的跨领域泛化能力。
...
TL;DR: 本文提出了首个视觉语言引导的生物医学图像伪造生成与检测框架，包括Rescind大规模基准数据集和Integscan检测模型，在伪造检测和定位方面达到最先进性能。
...
Method: 1. 提出结合扩散合成与视觉语言提示的方法，实现真实且语义可控的生物医学图像操纵（复制、拼接、区域移除）；2. 引入Rescind大规模基准数据集，包含细粒度标注和模态特定划分；3. 提出Integscan结构化状态空间建模框架，集成注意力增强的视觉编码与提示条件语义对齐；4. 采用基于视觉语言模型的验证循环，确保生成伪造的语义保真度。

Result: 在Rescind和现有基准上的大量实验表明，Integscan在伪造检测和定位方面均达到最先进的性能，为自动化科学完整性分析奠定了坚实基础。

Conclusion: 该研究建立了首个视觉语言引导的生物医学图像伪造生成与检测框架，通过创新的数据集、模型架构和验证机制，显著提升了生物医学图像伪造检测能力，为维护科学研究的完整性提供了有效工具。
...
TL;DR: DINO-AugSeg：基于DINOv3特征的小样本医学图像分割框架，通过小波特征增强和上下文融合提升性能
...
Result: 实现了高效构建高质量、可部署数据集的能力，减少了真实世界数据收集的依赖

Conclusion: 提出的自动化流水线能够有效生成领域特定的合成数据集，解决分布偏移问题，为实际部署提供高质量数据支持
...
TL;DR: PathoGen是一种基于扩散模型的生成模型，能够在良性组织病理图像中可控地、高保真地修复病变区域，解决医学AI中专家标注数据稀缺的问题。
...
Method: PathoGen是一种基于扩散模型的生成模型，利用扩散模型的迭代细化过程，在良性组织病理图像中合成具有自然组织边界、保留细胞结构和真实染色特征的病变。该方法能够同时生成真实的形态和像素级标注。

Result: 在肾脏、皮肤、乳腺和前列腺病理四个不同诊断挑战的数据集上验证，PathoGen在图像保真度和分布相似性方面优于条件GAN和Stable Diffusion等最先进的生成基线。使用PathoGen合成的病变增强训练集，相比传统几何增强，特别是在数据稀缺情况下，能提高下游分割性能。
...
TL;DR: 本文提出SSP框架，通过光学流和文本提示增强音频-视觉语义分割，解决移动和静止声源的分割挑战


<details>
  <summary>Details</summary>
Motivation: 音频-视觉语义分割（AVSS）需要在像素级别识别发声物体的基础上，进一步理解音频-视觉场景的语义。现有方法将任务分解为两个子任务，但面临移动物体和静止发声物体的分割挑战。

Method: 提出SSP协作框架：1）使用光学流捕捉运动动态，为移动声源提供时间上下文；2）引入两种文本提示（物体类别和场景描述）处理静止发声物体；3）实现视觉-文本对齐模块促进跨模态整合；4）采用后掩码技术训练模型学习光学流图。

Result: 实验结果表明，SSP框架在音频-视觉分割任务上优于现有AVS方法，实现了高效且精确的分割结果。

Conclusion: SSP框架通过整合光学流和文本提示，有效解决了音频-视觉语义分割中移动和静止声源的挑战，实现了更准确和语义丰富的分割性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的测试时适应方法严重依赖零样本预测作为伪标签进行自训练，但在分布偏移下这些预测不可靠，主要受两个基本限制：模态差距（分布偏移导致视觉和文本模态间的差距）和视觉干扰（视觉嵌入编码了丰富但与任务无关的噪声）。

Method: SubTTA通过对齐两种模态的语义子空间来增强零样本预测。首先提取两种模态的主子空间，通过最小化弦距离将视觉流形对齐到文本语义锚点以弥合模态差距；然后将对齐的视觉特征投影到任务特定的文本子空间，通过将视觉嵌入约束在有效语义范围内来过滤任务无关噪声；最后在净化空间上执行标准TTA以细化决策边界。

Result: 在多个基准测试和VLM架构上的广泛实验表明，SubTTA相比最先进的TTA方法平均提升了2.24%。

Conclusion: SubTTA通过语义子空间对齐有效解决了分布偏移下VLM测试时适应的模态差距和视觉干扰问题，显著提升了适应性能。
...
TL;DR: 本文提出了一种基于BLIP-2的轻量级多模态任务平台，集成YOLO-World和YOLOv8-Seg模型，用于无人机边缘设备的实时视觉理解与交互，解决了大视觉语言模型高计算成本与无人机有限计算资源之间的矛盾。


<details>
  <summary>Details</summary>
Motivation: 无人机在复杂场景中需要实时视觉理解与交互能力，但大型视觉语言模型的高计算成本与无人机边缘设备的有限计算资源之间存在矛盾，需要轻量级解决方案。

Method: 1. 深度集成BLIP-2与YOLO模型，利用YOLO的精确感知结果进行物体检测和实例分割；2. 设计基于K-Means聚类的内容感知关键帧采样机制，包含智能帧选择和时序特征拼接；3. 实现统一的多任务适应提示优化方案，将YOLO模型的结构化事件日志作为上下文信息注入BLIP-2输入。
...
TL;DR: 本文通过层级的掩码分析揭示了多模态大语言模型中视觉-文本融合的演化模式，发现融合发生在特定层而非均匀分布，并提出了一种无需训练的关注对比框架来提升多模态推理性能。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在视觉-语言理解方面取得了显著进展，但其内部如何整合视觉和文本信息仍然缺乏深入理解。为了填补这一研究空白，需要对模型内部的融合机制进行系统性分析。

Method: 采用层级的掩码分析方法，对多种架构进行系统性研究，揭示视觉-文本融合在模型内部的演化过程。同时分析层级注意力的演化，并基于这些洞察提出了一个无需训练的对比注意力框架，该框架建模早期融合层和最终层之间的转换，以突出有意义的注意力转移。

Result: 研究发现：1）融合发生在几个特定层而非均匀分布；2）某些模型在输出生成前会出现视觉信号的"回顾"现象；3）观察到对无关区域的持续高注意力噪声，以及对文本对齐区域逐渐增强的注意力。提出的对比注意力框架在多种MLLM和基准测试中验证了分析的有效性，并提升了多模态推理性能。

Conclusion: 本文通过系统性分析揭示了MLLM内部视觉-文本融合的演化模式，发现了融合的层级特性和注意力演化规律。基于这些洞察提出的训练免费对比注意力框架能够有效提升多模态推理能力，为理解和改进MLLM的内部工作机制提供了重要洞见。
...
<details>
  <summary>Details</summary>
Motivation: 尽管视频生成模型发展迅速，但数据如何影响运动质量仍不清楚。现有方法主要关注视觉外观而非时间动态，需要专门的运动归因框架来理解数据对运动的影响。

Method: 提出Motive框架：1) 使用运动加权损失掩码分离时间动态与静态外观；2) 基于梯度的数据归因方法，可扩展到大规模高质量视频数据集和模型；3) 识别对运动有强影响的微调片段。

Result: 在文本到视频模型中，Motive能有效识别影响运动的片段，指导数据筛选。使用Motive选择的高影响力数据，在VBench上提升了运动平滑度和动态程度，相比预训练基础模型获得74.1%的人类偏好胜率。

Conclusion: Motive是首个专门针对视频生成模型中运动而非视觉外观的归因框架，可用于筛选微调数据以提升时间一致性和物理合理性，为理解数据对运动的影响提供了新工具。
...
<details>
  <summary>Details</summary>
Motivation: 当前可解释视频异常检测方法缺乏空间定位能力，特别是在多实体交互场景中，现有方法产生的解释往往不完整或视觉不对齐，降低了可信度。需要一种能够验证的解释框架。

Method: 提出实例对齐描述框架，将每个文本声明链接到具有外观和运动属性的特定对象实例。框架捕捉异常由谁引起、每个实体在做什么、影响谁以及解释在哪里定位，实现可验证和可操作的推理。

Result: 标注了8个广泛使用的VAD基准数据集，并扩展了360度第一人称数据集VIEW360，新增868个视频、8个位置和4种新异常类型，创建了VIEW360+综合测试平台。实验表明实例级空间定位描述揭示了当前基于LLM和VLM方法的显著局限性。

Conclusion: 提出的实例对齐空间定位描述方法为可信和可解释的异常检测提供了鲁棒基准，解决了现有可解释VAD方法在空间定位和多实体交互中的不足，推动了该领域的研究进展。
...
<details>
  <summary>Details</summary>
Motivation: 火箭发射时极端成像条件（密集颗粒雾霾、超过120dB的亮度变化）严重阻碍关键机械参数（羽流场、冲击波结构、喷嘴振荡）的光学测量，传统图像数据质量差，影响后续摄影测量和速度分析

Method: 1) 定制空间变化曝光传感器单次拍摄获取多曝光数据，实现鲁棒雾霾评估；2) 物理感知去雾算法动态估计雾霾密度；3) 区域自适应光照优化；4) 多尺度熵约束融合有效分离雾霾与场景辐射

Result: 在真实发射图像和受控实验中验证，框架在恢复羽流和发动机区域物理准确视觉信息方面表现优异，为提取粒子速度、流动不稳定频率和结构振动等关键机械参数提供可靠图像基础

Conclusion: 该硬件-算法协同设计框架能有效解决极端航空航天环境下的成像挑战，支持精确的定量分析，为火箭发射关键机械参数的光学测量提供可靠解决方案
...
TL;DR: 提出SISTA框架，通过图像-报告和补丁-单词两级对齐解决医学VLP中未配对样本被错误视为负样本的问题，提升下游任务性能


<details>
  <summary>Details</summary>
Motivation: 传统医学对比学习将所有未配对的图像-报告样本都视为负样本，但医学数据中不同患者的图像或报告可能存在高度相似性，这种刚性处理会破坏语义结构，影响学习表示的质量

Method: 提出SISTA多级对齐框架：1）改进传统对比学习，引入报告间相似性来消除假负样本；2）在图像补丁和相关单词标记之间进行有效对齐

Result: 实验表明该框架在不同数据集的下游任务（图像分类、图像分割、目标检测）上提升了迁移性能，在有限标注数据的细粒度任务中取得了显著改进

Conclusion: SISTA框架通过语义感知的实例和稀疏标记对齐，解决了医学VLP中的假负样本问题，提高了表示学习质量，代码和预训练模型将公开
...
<details>
  <summary>Details</summary>
Motivation: 解决跨平台3D目标检测中的领域泛化问题，缩小不同平台数据之间的领域差距

Method: 在PVRCNN++框架基础上，采用定制化数据增强和基于伪标签的自训练策略来提升跨平台泛化能力

Result: 在挑战赛中取得第三名，第一阶段目标域Car类别3D AP达到62.67%，第二阶段Car和Pedestrian类别分别达到58.76%和49.81%

Conclusion: 通过数据增强和自训练策略有效提升了3D目标检测的跨平台泛化性能，在RoboSense2025挑战赛中取得了优异成绩
...
TL;DR: CogniMap3D是一个受人类认知启发的动态3D场景理解与重建框架，通过持久记忆库存储静态场景，结合运动检测、认知映射和优化策略，实现跨多次访问的连续场景理解。


<details>
  <summary>Details</summary>
Motivation: 受人类认知过程启发，旨在开发一个能够像人类一样理解动态3D场景的系统，通过记忆存储和检索机制，实现高效的空间知识管理和跨时间访问的场景理解。

Method: 1) 多阶段运动线索框架识别动态对象；2) 认知映射系统存储、回忆和更新多次访问中的静态场景；3) 因子图优化策略精炼相机位姿；4) 利用深度和相机位姿先验识别动态区域；5) 匹配静态元素与记忆库。

Result: 在视频深度估计、相机位姿重建和3D映射任务中达到最先进性能，能够有效支持跨长时间序列和多次访问的连续场景理解。

Conclusion: CogniMap3D成功模拟人类认知过程，通过持久记忆机制实现了高效的动态3D场景理解与重建，为跨时间访问的连续环境理解提供了有效解决方案。
...
TL;DR: 提出一个基于文本指令驱动的3D面部表情生成框架，能够从任意起始表情平滑过渡到目标表情，并生成完整的面部表情序列。


<details>
  <summary>Details</summary>
Motivation: 传统3D虚拟形象通常只有六种基本面部表情，缺乏真实的情感变化模拟。需要能够根据文本指令生成任意表情之间的平滑过渡，以增强虚拟形象的表达能力和真实感。

Method: 1. 提出指令驱动的面部表情分解器（IFED）模块，用于多模态数据学习并捕捉文本描述与面部表情特征之间的相关性。2. 提出指令到面部表情转换（I2FET）方法，利用IFED和顶点重建损失函数来优化潜在向量的语义理解，根据给定指令生成面部表情序列。3. 提出面部表情转换模型，生成面部表情之间的平滑过渡。

Result: 在CK+和CelebV-HQ数据集上的广泛评估表明，所提模型优于最先进的方法。结果显示该框架能够根据文本指令生成面部表情轨迹，且文本提示允许对人类情感状态进行多样化描述，大大扩展了面部表情及其过渡的库。

Conclusion: 该框架能够根据文本指令生成面部表情之间的平滑过渡，扩展了虚拟形象的表情表达能力，有望在各种实际应用中找到用途。
...
TL;DR: 本文针对Zhang等人使用简单角点模型的理论缺陷，提出基于二阶高斯方向导数滤波的高分辨率角点检测方法，能准确检测相邻角点，在定位精度、图像匹配和3D重建方面优于现有方法。
...
Method: 使用二阶高斯方向导数滤波器对两种典型高分辨率角点模型进行平滑处理，推导其SOGDD表示，发现高分辨率角点的多种特征，提出基于高斯滤波尺度选择的新检测方法。

Result: 实验验证表明，该方法在定位误差、图像模糊变换鲁棒性、图像匹配和3D重建方面均优于现有最先进方法，能准确检测相邻角点。

Conclusion: 提出的基于SOGDD的高分辨率角点检测方法解决了相邻角点相互影响的问题，在多个性能指标上优于现有方法，为图像匹配和3D重建等应用提供了更准确的角点检测方案。
...
TL;DR: 多模态大语言模型在胃肠内镜工作流中表现出潜力，在诊断推理方面优于实习生并与初级内镜医师相当，但在空间定位和事实准确性方面存在瓶颈。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在胃肠病学中显示出潜力，但其在完整临床工作流程中的表现以及与人类基准的比较尚未得到验证，需要系统评估其在胃肠内镜工作流中的临床实用性。

Method: 构建GI-Bench基准测试，涵盖20个细粒度病变类别。评估12个MLLM在五阶段临床工作流程中的表现：解剖定位、病变识别、诊断、发现描述和管理。使用Macro-F1、平均交并比和多维度Likert量表将模型性能与3名初级内镜医师和3名住院医师进行比较。

Result: Gemini-3-Pro达到最先进性能。在诊断推理方面，顶级模型（Macro-F1 0.641）优于实习生（0.492），与初级内镜医师相当（0.727；p>0.05）。但存在关键的"空间定位瓶颈"：人类病变定位（mIoU >0.506）显著优于最佳模型（0.345；p<0.05）。定性分析显示"流畅性-准确性悖论"：模型生成的报告语言可读性优于人类（p<0.05），但事实准确性显著较低（p<0.05），存在"过度解释"和视觉特征幻觉。

Conclusion: MLLM在胃肠内镜诊断推理方面显示出临床潜力，但在空间定位和事实准确性方面仍需改进。GI-Bench建立了动态排行榜以跟踪MLLM在临床内镜中的演进表现，为未来研究提供基准。
...
<details>
  <summary>Details</summary>
Motivation: 多站点脑MRI数据聚合可以增强深度学习模型训练，但也会引入站点特异性变化（如扫描仪厂商、采集参数和成像协议差异）导致的非生物异质性，这会削弱模型的泛化能力。现有方法通常依赖有限的配对数据，难以有效分离风格与解剖结构，且大多只处理单序列协调，限制了在实际多序列MRI场景中的应用。
...
Conclusion: MMH提供了一个有效的多站点多序列脑MRI协调框架，能够在不依赖配对数据的情况下实现风格与解剖结构的有效分离，显著提升了跨站点数据的协调效果和下游任务的性能。
...
TL;DR: MobiDiary是一个从IMU和Wi-Fi信号生成自然语言活动描述的系统，避免了视觉系统的隐私问题，通过统一传感器编码器将物理信号转换为可读文本。


<details>
  <summary>Details</summary>
Motivation: 智能家居中的人类活动识别对健康监测至关重要，但基于视觉的系统存在隐私问题和环境限制（如遮挡）。需要一种从物理信号直接生成自然语言描述的方法。

Method: 提出统一传感器编码器，利用运动诱导信号的共享归纳偏差，通过基于补丁的机制捕捉局部时间相关性，集成异构放置嵌入统一空间上下文，然后使用基于Transformer的解码器自回归生成文本描述。

Result: 在多个公开基准测试（XRF V2、UWash、WiFiTAD）上评估，MobiDiary在跨模态泛化方面表现优异，在字幕指标（BLEU@4、CIDEr、RMC）上达到最先进性能，在连续动作理解方面优于专门基线。

Conclusion: MobiDiary成功地从异构物理信号生成自然语言活动描述，解决了隐私问题，通过统一编码器有效弥合了物理信号与语言描述之间的语义鸿沟。
...
TL;DR: 该研究比较了基于Vision Transformer(EVA-ViT)图像编码器与LlaMA或ChatGPT LLM的多模态方法，用于减少幻觉问题并检测胸部X光图像中的疾病。使用NIH Chest X-ray数据集，比较了图像检索增强生成、文本检索增强生成和基线方法。


<details>
  <summary>Details</summary>
Motivation: 解决多模态医学图像分析中的幻觉问题，提高胸部X光图像疾病检测的准确性和可靠性。通过比较不同检索增强生成方法和LLM模型，寻找减少幻觉并改善预测置信度校准的有效方案。

Method: 使用EVA-ViT作为图像编码器，结合LlaMA或ChatGPT LLM构建多模态系统。在NIH Chest X-ray数据集上训练模型，比较三种方法：1) 图像检索增强生成(RAG)使用KNN方法；2) 文本检索增强生成(RAG)利用外部知识信息；3) 基线方法。

Result: 文本RAG能有效减少幻觉问题，图像RAG通过KNN方法提高了预测置信度和校准。GPT LLM相比LlaMA LLM表现出更好的性能、更低的幻觉率和更优的预期校准误差(ECE)。

Conclusion: 该研究揭示了数据不平衡和复杂多阶段结构的挑战，但提出了大规模实验环境和平衡使用示例的建议。文本RAG在减少幻觉方面有效，图像RAG在改善校准方面有优势，GPT LLM整体表现优于LlaMA LLM。
...
TL;DR: 本文提出了一种基于事件分割的零样本ADL识别方法，通过改进LLM的上下文对齐和引入置信度估计，在复杂数据集上超越了传统时间分割方法和监督学习方法。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的零样本ADL识别方法存在两个主要问题：1) 依赖时间分割，与LLM的上下文推理能力不匹配；2) 缺乏预测置信度估计方法。这些问题限制了零样本方法在实际智能家居应用中的可靠性和性能。

Method: 提出了事件分割方法替代传统时间分割，更好地与LLM的上下文推理能力对齐。同时开发了一种新的预测置信度估计方法，能够有效区分正确和错误的预测结果。

Result: 实验评估显示，事件分割方法在复杂现实数据集上持续优于基于时间的LLM方法，甚至超越了监督数据驱动方法。即使使用相对较小的LLM（如Gemma 3 27B）也能取得良好效果。提出的置信度度量方法能有效区分正确和错误预测。

Conclusion: 事件分割和置信度估计显著提升了零样本ADL识别性能，使LLM方法在实际智能家居应用中更加可靠和实用，为医疗保健、安全和能源管理等应用提供了更好的支持。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频-LLM的推测解码方法主要关注剪枝冗余视觉token，但无法达到文本LLM的加速效果。研究发现这主要源于两个限制：1）现有剪枝策略无法充分保留视觉语义token，降低了草稿质量和接受率；2）即使大幅剪枝，草稿模型的剩余推理成本仍限制了整体加速效果。

Method: HIPPO框架包含两个核心创新：1）语义感知的token保留方法，融合全局注意力分数和局部视觉语义，在高剪枝率下保留语义信息；2）视频并行推测解码算法，将草稿生成和目标验证阶段解耦并重叠执行。

Result: 在4个视频-LLM和6个基准测试上的实验表明，HIPPO相比原始自回归解码实现了最高3.51倍的加速效果，证明了其有效性。

Conclusion: HIPPO通过语义感知的token保留和并行化设计，解决了现有视频-LLM推测解码方法的局限性，显著提升了推理速度，为视频-LLM的高效推理提供了有效解决方案。
...
TL;DR: KidVis基准测试揭示多模态大语言模型在基础视觉能力上远不及6-7岁儿童，存在"缩放定律悖论"：单纯增加参数无法提升这些核心视觉能力。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在高级推理任务上表现出色，但尚不清楚它们是否具备人类直觉般的基础视觉原语能力。本研究旨在通过人类视觉发展理论来评估MLLMs的基础视觉能力。

Method: 引入KidVis基准，将视觉智能解构为6种原子能力（专注、追踪、辨别、记忆、空间、闭合），包含10类低语义依赖的视觉任务。评估20个最先进的MLLMs，并与人类生理基线（6-7岁儿童）进行对比。

Result: 人类儿童平均得分95.32（接近完美），而最先进的GPT-5仅得67.33。发现"缩放定律悖论"：单纯增加模型参数无法线性提升这些基础视觉能力。

Conclusion: 当前MLLMs尽管在推理能力上强大，但缺乏实现广义视觉智能所需的基本生理感知原语，表明需要新的方法来培养这些基础视觉能力。
...
TL;DR: M3SR：一种用于光谱重建任务的多尺度、多感知Mamba架构，通过多感知融合块增强特征理解能力，在U-Net结构中实现多尺度特征提取与融合，在计算成本更低的情况下超越现有SOTA方法。


<details>
  <summary>Details</summary>
Motivation: 尽管Mamba架构已应用于光谱重建任务，但仍面临两个主要挑战：1）单一空间感知限制了全面理解高光谱图像的能力；2）单尺度特征提取难以捕捉高光谱图像中的复杂结构和精细细节。

Method: 提出M3SR（多尺度、多感知Mamba架构），设计多感知融合块以增强模型对输入特征的全面理解和分析能力。将该块集成到U-Net结构中，使M3SR能够有效提取和融合全局、中间和局部特征，从而实现多尺度的高光谱图像准确重建。

Result: 广泛的定量和定性实验表明，提出的M3SR在计算成本更低的情况下，性能优于现有的最先进方法。

Conclusion: M3SR通过多尺度、多感知的Mamba架构设计，成功解决了现有方法在光谱重建任务中的局限性，实现了更准确的重建效果和更高的计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 虽然大型多模态模型在图像质量评估任务中表现出色，但达到最先进性能通常需要计算昂贵的微调方法。作者希望开发一种无需训练的资源高效替代方案。

Method: 提出IQARAG框架，包含三个关键阶段：检索特征提取、图像检索、集成与质量分数生成。通过检索语义相似但质量不同的参考图像及其平均意见分数，为输入图像提供视觉感知锚点，使用特定提示整合这些信息。
...
TL;DR: 提出YOLOBirDrone架构，通过AELAN、MPDA和RMPDA等组件提升无人机与鸟类的检测分类准确率，并引入BirDrone数据集，检测准确率达85%。


<details>
  <summary>Details</summary>
Motivation: 商用和军用无人机应用广泛，但也常被用于针对性攻击，构成安全隐患。现有视觉检测系统在区分无人机和小型鸟类方面存在精度限制。
...
Conclusion: YOLOBirDrone架构能有效提高无人机与鸟类的检测分类精度，为解决空中目标识别挑战提供了有效方案。
...
Conclusion: UM-Text是一个有效的统一多模态模型，能够通过自然语言指令进行上下文理解和视觉文本编辑，生成与参考图像风格一致的视觉文本，在多个基准测试上表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有Transformer方法在SISR任务中虽然性能优秀，但注意力计算成本高。为提升效率，多数方法采用固定分组注意力，但忽略了token相似性的固有非对称性，无法实现灵活的自适应注意力计算。

Method: 提出个体化探索Transformer（IET），引入个体化探索注意力（IEA）机制，让每个token自适应地选择其内容感知且独立的注意力候选，实现token自适应和非对称设计。
...
TL;DR: 该研究系统分析了视觉语言模型在自动驾驶等安全关键应用中，面对上游视觉感知退化时的语义对齐失效问题，发现即使分割指标下降不大，也会导致严重的下游VLM行为故障。


<details>
  <summary>Details</summary>
Motivation: 虽然当前视觉语言模型在多模态基准测试中表现良好，但其在真实感知退化情况下的鲁棒性尚未得到充分理解。在自动驾驶和具身AI等安全关键系统中，可靠的感知对于语义推理和决策至关重要，因此需要研究VLM在感知退化下的语义对齐可靠性。

Method: 使用Cityscapes数据集上的语义分割作为代表性感知模块，引入感知现实的退化方法，这些退化仅导致传统分割指标适度下降。然后分析下游VLM行为，提出一套语言层面的错位指标来量化幻觉、关键遗漏和安全误判，并分析这些指标与分割质量的关系。

Result: 研究发现，即使像素级分割指标下降不大，也会导致下游VLM出现严重故障，包括幻觉对象提及、安全关键实体遗漏和不一致的安全判断。对比性和生成性VLM都存在这一问题，揭示了像素级鲁棒性与多模态语义可靠性之间的明显脱节。

Conclusion: 当前基于VLM的系统存在关键局限性，像素级鲁棒性不能保证多模态语义可靠性。研究强调了在安全关键应用中需要建立明确考虑感知不确定性的评估框架，以确保系统的安全性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有野外新视角合成方法虽然表现良好，但缺乏对复杂表面的几何基础，有时会产生不一致的结果。需要开发一个既能保持高质量渲染，又能确保几何一致性的方法。

Method: 基于有符号距离函数(SDF)的几何表示来指导渲染过程，并引入新颖的几何保持损失来确保精细结构细节的保留。

Result: 实现了具有竞争力的渲染性能，同时相比类似方法能耗降低了4-5倍，能够生成具有锐利、几何一致细节的逼真结果。

Conclusion: Geo-NVS-w是一个鲁棒的野外新视角合成方法，通过几何感知框架实现了高质量、几何一致且节能的渲染效果。
...
<details>
  <summary>Details</summary>
Motivation: 地理空间点云语义分割面临区域间地理模式差异和数据采集策略不同导致的域偏移问题，现有域适应方法通常需要源域数据，但由于数据隐私、监管政策和传输限制等实际约束，源域数据往往不可用，因此需要研究无源无监督域适应方法

Method: 提出LoGo框架：1) 局部类平衡原型估计模块，采用类内独立锚点挖掘策略而非传统全局阈值过滤，确保尾类也能生成鲁棒特征原型；2) 基于最优传输的全局分布对齐模块，将伪标签分配建模为全局优化问题，通过全局分布约束纠正头类过度主导；3) 双重一致性伪标签过滤机制，仅保留局部多增强集成预测与全局最优传输分配一致的高置信度伪标签用于自训练

Result: 未在摘要中明确给出具体实验结果，但方法设计旨在有效缓解长尾分布导致的特征崩溃问题，防止模型预测过度偏向多数类，提高地理空间点云语义分割在域适应场景下的性能

Conclusion: LoGo框架为地理空间点云的无源无监督域适应提供了一种有效的解决方案，通过局部-全局双重共识机制解决了长尾分布带来的挑战，在数据隐私和传输限制的实际约束下具有重要应用价值
...
Conclusion: 该系统在全景X光评估中显示出强大的AI辅助潜力，具有可解释的AI特征，支持临床信心
...
TL;DR: SPARK是一个自标定的实时多相机点云重建框架，能联合处理点云融合和相机外参不确定性，适用于动态场景和大规模相机系统。


<details>
  <summary>Details</summary>
Motivation: 实时多相机3D重建对于3D感知、沉浸式交互和机器人技术至关重要，但现有方法在多视图融合、相机外参不确定性和大规模相机系统可扩展性方面存在困难。

Method: SPARK包含两个核心模块：(1) 几何感知的在线外参估计模块，利用多视图先验并强制跨视图和时间一致性；(2) 置信度驱动的点云融合策略，在像素和点级别建模深度可靠性和可见性。

Result: 在真实世界多相机系统上的广泛实验表明，SPARK在外参精度、几何一致性、时间稳定性和实时性能方面优于现有方法，展示了其有效性和可扩展性。
...
TL;DR: 提出MMLGNet多模态框架，使用CLIP等视觉语言模型将高光谱成像和激光雷达等异构遥感模态与自然语言语义对齐，通过双向对比学习在共享潜在空间中对齐视觉特征和文本嵌入。
...
TL;DR: 提出基于局部补丁的动画重定向方法，将源视频的面部动画转移到风格化3D角色上，保持语义表达


<details>
  <summary>Details</summary>
Motivation: 在数字动画时代，为虚拟角色生成逼真面部动画的需求催生了各种重定向方法。然而，当重定向应用于风格化或夸张的3D角色时，由于这些角色的面部结构与人类面部结构差异显著，现有方法面临挑战。需要同时考虑目标角色的面部结构和运动范围，以保持原始面部动作的语义含义。

Method: 提出基于局部补丁的重定向方法，包含三个模块：1) 自动补丁提取模块从源视频帧中提取局部补丁；2) 重演模块生成相应的目标局部补丁重演；3) 权重估计模块计算每一帧的目标角色动画参数，生成完整的面部动画序列。

Result: 大量实验表明，该方法能够成功将源面部表情的语义含义转移到面部特征比例变化显著的风格化角色上。

Conclusion: 该方法有效解决了风格化3D角色面部动画重定向的挑战，通过局部补丁方法保持了原始动画的语义表达，适用于面部特征比例差异显著的情况。
...
Result: 方法在15种复杂心脏病的多视图超声心动图诊断中提升48%，在CardiacNet-PAH上提升5%，用户研究显示93.33%的临床医生同意其推理逻辑。

Conclusion: 通过引入类似心脏病专家的思维模式，CardiacMind有效提升了多模态大语言模型在超声心动图诊断中的推理能力，为医学AI系统提供了更可靠的诊断支持。
...
TL;DR: 提出NAR方法，一种针对遥感多标签分类中标签噪声的自适应正则化方法，能区分加性、减性和混合噪声，通过置信度机制选择性处理标签，提升噪声鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 遥感多标签分类中，由于使用专题产品或众包标注来降低成本，常引入部分错误的标签噪声（加性、减性或混合噪声）。现有方法大多忽视这种区别，将噪声标注直接作为监督信号，缺乏针对不同噪声类型自适应调整学习行为的机制。
...
Result: 在加性、减性和混合噪声场景下的实验表明，NAR相比现有方法能持续提升鲁棒性。在减性和混合噪声下性能提升最为显著，表明自适应抑制和选择性校正噪声监督为遥感多标签分类中的噪声鲁棒学习提供了有效策略。

Conclusion: NAR方法通过区分噪声类型并自适应调整学习行为，有效解决了遥感多标签分类中的标签噪声问题，特别是在减性和混合噪声场景下表现出色，为噪声鲁棒学习提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 少样本类增量学习（FSCIL）需要在有限数据下持续识别新类别，这面临着关键的稳定性-可塑性困境：如何在保留旧知识的同时获取新知识。现有方法难以在这两者之间取得良好平衡。

Method: 提出静态-动态协作（SDC）框架，将FSCIL分为两个阶段：1）静态保留阶段（SRS）：在基础会话中使用充足数据训练初始模型，并保留关键部分作为静态记忆；2）动态学习阶段（DLS）：引入额外的动态投影器与之前的静态记忆联合训练，通过两个阶段的协作实现稳定性与可塑性的平衡。

Result: 在三个公共基准数据集和一个真实世界应用数据集上的广泛实验表明，该方法相比其他竞争方法取得了最先进的性能，有效提高了旧知识的保留能力，同时持续适应新类别。

Conclusion: 通过将FSCIL任务分解为静态保留和动态学习两个阶段，并采用静态-动态协作框架，能够更好地解决稳定性-可塑性困境，在少样本类增量学习中实现旧知识保留和新知识获取的良好平衡。
...
TL;DR: 该研究开发了一个结合稳健特征选择的放射组学框架，用于预测高级别浆液性卵巢癌患者对新辅助化疗的治疗反应，通过整合不同特征选择方法提高了预测准确性。
...
Method: 采用自动化随机化算法模拟观察者间变异，平衡特征稳健性和预测准确性。使用四种反应指标（CRS、RECIST、体积减少、直径减少），分析不同解剖部位病灶。基于治疗前后CT扫描提取特征并训练模型，在一个队列上训练，在独立队列上进行外部测试。

Result: 所有病灶组合用于体积减少预测时获得最佳性能（AUC 0.83）。网膜病灶对CRS预测效果最好（AUC 0.77），盆腔病灶对直径减少预测最佳（AUC 0.76）。
...
<details>
  <summary>Details</summary>
Motivation: RGB-T检测在融合跨模态互补信息方面有优势，但在极端条件下（某一模态质量差）需要模态分离以减少噪声影响。现有方法难以平衡互补与分离的需求

Method: 提出模态解耦的RGB-T检测框架MDQF：1）使用类似DETR的检测器作为RGB和TIR图像的独立分支；2）在每个细化阶段通过查询选择与适应进行跨分支查询融合；3）将高质量查询从一个分支馈送到另一个分支，排除退化模态影响

Result: 实验表明MDQF优于现有RGB-T检测器，实现了更好的模态独立性，且解耦框架允许使用非配对RGB或TIR图像优化各分支

Conclusion: MDQF框架有效平衡了RGB-T检测中的模态互补与分离需求，在极端条件下通过查询融合排除退化模态干扰，同时支持非配对数据训练，提高了检测鲁棒性
...
Conclusion: 该研究为数据驱动的建筑设计提供了重要数据集和基准框架，展示了视觉语言模型在建筑体量生成中的应用潜力，为未来研究指明了方向
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型越来越多地部署在自动驾驶车辆和移动系统中，但现有基准测试无法充分覆盖多样化的危险情况，特别是具有时空动态的异常场景。需要评估这些模型在复杂环境中支持更安全决策的能力。
...
Result: 实验表明，在包含异常物体的条件下，视觉语言模型的性能显著下降，特别是在需要细微运动理解的场景中下降最大。

Conclusion: HazardForge能够有效生成具有时空动态的危险场景，MovSafeBench基准测试揭示了视觉语言模型在异常场景下的性能局限，特别是在运动理解方面的不足。
...
TL;DR: 本文提出了一种基于物联网的智能鱼缸系统，通过ESP32微控制器集成多种传感器和执行器，实现水质实时监测与自动化控制，显著提升鱼缸维护效率。


<details>
  <summary>Details</summary>
Motivation: 传统鱼缸水质管理依赖人工监测，效率低下、劳动密集且易出错，导致水生环境不稳定，需要更智能、自动化的解决方案来保障水生生物健康。

Method: 系统采用ESP32微控制器集成pH、TDS、温度、浊度传感器，配合伺服喂食器和水泵执行器，结合边缘处理、Blynk物联网平台云连接，并设计了带可配置冷却期的智能警报机制。

Result: 在10升鱼缸环境中测试，系统传感器平均准确率达96%，异常检测响应时间1.2秒，自动喂食和水循环模块运行可靠性达97%，显著减少人工干预。

Conclusion: 研究表明，经济高效的物联网解决方案能够革新鱼缸维护，使水生生态系统管理更加可靠、高效，适用于住宅和商业应用。
...
TL;DR: EfficientFSL是一个针对Vision Transformers的查询式微调框架，在少样本分类任务中通过极少量可训练参数实现高性能，显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 大型模型如Vision Transformers在少样本分类中表现出色，但微调需要大量GPU内存和训练时间，不适用于低资源场景。需要开发高效的微调方法。

Method: 提出EfficientFSL框架：1) 轻量级可训练前向块合成任务特定查询，从预训练模型中间表示中提取特征；2) 组合块融合多层输出增强特征表示；3) 支持-查询注意力块通过调整原型对齐查询集分布。

Result: 在四个域内少样本数据集和六个跨域数据集上达到最先进性能，仅需极少量可训练参数，显著降低计算开销。

Conclusion: EfficientFSL通过查询式微调框架有效解决了大型ViT模型在少样本分类中的计算效率问题，为实际应用提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 少样本类增量学习面临灾难性遗忘问题，现有方法通常使用外部存储器存储先前知识并平等处理增量类，无法适当保留先前关键知识。

Method: 提出CD²框架，包含数据集蒸馏模块（DDM）和蒸馏约束模块（DCM）。DDM在分类器指导下合成高度压缩的样本，迫使模型从少量增量样本中学习紧凑的关键类相关线索；DCM引入设计的损失函数来约束先前学习的类分布，更充分地保留蒸馏知识。
...
TL;DR: VideoHEDGE是一个用于视频问答中幻觉检测的模块化框架，通过语义熵等方法评估视频-语言模型的可靠性，在SoccerChat基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 视频-语言模型在视频问答中经常产生高置信度的幻觉，而现有的不确定性度量方法往往无法与正确性对齐，需要更有效的幻觉检测方法。

Method: VideoHEDGE框架从干净视频片段和经过光度、时空扰动的变体中生成基线答案和多个高温生成结果，然后使用自然语言推理或嵌入方法将文本输出聚类为语义假设，通过聚类级概率质量计算三种可靠性分数：语义熵、RadFlag和视觉增强语义熵。

Result: 在三个7B视频-语言模型上评估，视觉增强语义熵在ROC-AUC方面表现最佳，特别是在较大失真预算下；嵌入聚类与自然语言推理聚类性能相当但计算成本更低；领域微调减少了幻觉频率但校准改进有限。

Conclusion: VideoHEDGE提供了一个有效的视频问答幻觉检测框架，视觉增强语义熵是最可靠的度量方法，嵌入聚类提供了计算效率高的替代方案，领域微调对校准改善有限。
...
Method: 提出MoCha框架，仅需单帧任意掩码；引入条件感知RoPE适应多模态输入条件并增强面部身份；采用基于RL的后训练阶段；构建三个专门数据集：UE5渲染的高保真数据集、肖像动画技术合成的表情驱动数据集、现有视频-掩码对增强的数据集。

Result: 大量实验表明，该方法在性能上显著优于现有最先进方法，在复杂场景中表现出更好的泛化能力和视觉质量。

Conclusion: MoCha通过简化输入要求、引入条件感知机制和构建综合数据集，有效解决了视频角色替换中的泛化问题，为可控视频角色替换提供了更实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 膝关节MRI自动评估需要兼顾准确性和可解释性。传统放射组学特征在人群层面预定义，虽然可解释但过于限制，无法捕捉患者特异性变异，性能不如端到端深度学习

Method: 1. 放射组学指纹：动态构建患者特异性特征集，从候选特征池中预测特征相关性，为每个患者选择最具预测性的特征；2. 健康人格：使用扩散模型为每个患者合成无病理基线MRI，通过对比病理图像与健康人格的特征差异来突出异常

Result: 在三个临床任务中，两种方法单独或组合使用均达到或超越最先进深度学习模型性能，同时支持多层次可解释性。案例研究展示了这些方法如何促进人类可解释的生物标志物发现和病理定位

Conclusion: 放射组学指纹和健康人格两种互补策略为膝关节MRI自动评估提供了准确且可解释的解决方案，平衡了深度学习性能与传统放射组学可解释性的优势
...
<details>
  <summary>Details</summary>
Motivation: 现有无源域自适应方法在感知场和计算效率之间存在权衡，而Visual Mamba在捕捉通道频率特征和保持空间鲁棒性方面存在局限

Method: 提出SfMamba框架，包含通道视觉状态空间块进行通道序列扫描，以及语义一致打乱策略来破坏背景补丁序列同时保持预测一致性

Result: 在多个基准测试中，SfMamba相比现有方法获得了一致更强的性能，同时保持了良好的参数效率

Conclusion: SfMamba为无源域自适应提供了一个实用的解决方案，通过选择性扫描机制实现了域不变特征提取和计算效率的平衡
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型在医疗、自动驾驶等关键决策系统中的广泛应用，其不确定性估计的校准变得至关重要。然而，当前VLM测试时提示调优文献主要关注提升判别性能，而校准维度被严重忽视。现有最先进方法通过强制文本提示嵌入的完全正交性来增强可分离性，但这会导致语义相关类别被过度推离，使模型过度自信。

Method: 提出语义正交校准（SoC）方法，使用Huber-based正则化器，在保持语义邻近性的同时实现平滑原型分离。该方法避免了完全正交约束的缺点，能够更好地平衡类别分离与语义关系保留。

Result: 通过全面的实证验证，SoC方法在多个数据集上一致地改善了校准性能，同时保持了有竞争力的判别能力。相比之前基于正交性的方法，SoC在保持语义相关性的同时实现了更好的校准效果。

Conclusion: 语义正交校准（SoC）通过平衡原型分离与语义邻近性，有效改善了视觉语言模型在测试时提示调优中的校准性能，为关键决策系统中的不确定性估计提供了更可靠的解决方案。
...
Result: 实验表明在融合可控性和分割准确性方面达到最先进结果，适应后的任务分支甚至超越了原始分割模型

Conclusion: 通过并行分割和融合分支的协同优化，实现了任务性能与融合质量的相互增强，为智能无人系统提供全天气感知能力
...
TL;DR: 本文提出基于四大支柱的深度伪造检测可靠性评估框架，分析五种SOTA方法发现显著进展与关键局限


<details>
  <summary>Details</summary>
Motivation: 深度伪造技术既有教育创意等正面应用，也有欺诈、虚假信息、隐私侵犯等严重负面影响。尽管检测技术已有进展，但超越分类性能的综合评估方法仍然缺乏。

Method: 提出基于四大支柱的可靠性评估框架：可迁移性、鲁棒性、可解释性和计算效率。对五种最先进的深度伪造检测方法进行系统性分析。

Result: 分析揭示了深度伪造检测方法的显著进展，同时也发现了关键局限性，特别是在评估框架的四个维度上存在不足。
...
TL;DR: Salience-SGG提出了一种新的场景图生成框架，通过迭代显著性解码器强调具有显著空间结构的三元组，解决了长尾分布和空间理解不足的问题。
...
Method: 提出了Salience-SGG框架，包含迭代显著性解码器（ISD），该解码器强调具有显著空间结构的三元组。同时提出了语义无关的显著性标签来指导ISD的学习。
...
Conclusion: Salience-SGG通过强调空间显著性，有效解决了场景图生成中的长尾分布问题，同时保持了良好的空间理解能力，超越了现有的无偏SGG方法。
...
<details>
  <summary>Details</summary>
Motivation: 目前大多数急性缺血性卒中病变分割模型基于U-Net框架，但在损失函数、深度监督、残差连接和注意力机制的选择上存在差异，且许多实现不公开，最优配置不明确，需要开发更鲁棒且可复现的解决方案。

Method: 提出ISLA模型，系统优化损失函数、卷积架构、深度监督和注意力机制，使用三个多中心数据库（超过1500名AIS参与者）进行训练，并研究无监督域适应以提升对外部临床数据集的泛化能力。

Result: ISLA在外部测试集上优于两种最先进的AIS病变分割方法，展示了更好的分割性能。

Conclusion: ISLA为急性缺血性卒中病变分割提供了一个鲁棒且可复现的深度学习框架，代码和训练模型将公开以促进重用和可复现性。
...
Method: 采用Cycle-Consistent Adversarial Network (CycleGAN)方法进行非配对图像到图像转换，将C01和C02荧光通道组合成RGB，学习荧光和H&E域之间的双向映射。使用基于ResNet的生成器（带残差块）和PatchGAN判别器，通过对抗损失、循环一致性和身份损失进行训练。
...
Conclusion: 通过利用口腔结构信息来缓解X光片中的视觉模糊问题，DentalX能够有效检测细微的牙科疾病，两个任务之间的相互受益在模型优化过程中自然产生。
...
TL;DR: 本文提出了一种自动合成3D视觉定位数据及其推理过程的管道，并基于此数据微调LLM得到Reason3DVG-8B模型，该模型仅使用1.6%的训练数据就超越了之前的LLM方法3D-GRAND。


<details>
  <summary>Details</summary>
Motivation: 当前3D视觉定位任务面临两大挑战：一是现有模型推理能力有限，二是需要大量标注数据进行监督训练。虽然已有研究尝试通过合成数据训练更强的3D视觉定位LLM，但性能提升有限且与数据收集成本不成比例。

Method: 提出了一个自动化的3D视觉定位数据合成管道，能够生成包含对应推理过程的3D视觉定位数据。利用生成的数据对LLM进行微调，开发了Reason3DVG-8B模型。

Result: Reason3DVG-8B仅使用1.6%的训练数据就超越了之前的LLM方法3D-GRAND，证明了所提出数据的有效性以及推理在3D视觉定位中的重要性。

Conclusion: 通过自动合成包含推理过程的3D视觉定位数据，可以有效提升LLM在3D视觉定位任务上的性能，同时大幅降低数据需求，为3D视觉理解提供了新的解决方案。
...
TL;DR: 3AM是一个训练时增强方法，将MUSt3R的3D感知特征集成到SAM2中，通过轻量级特征融合器结合多级几何特征，实现仅需RGB输入即可在宽基线运动场景中实现几何一致性的视频对象分割。


<details>
  <summary>Details</summary>
Motivation: 现有视频对象分割方法（如SAM2）依赖外观特征，在大视角变化下表现不佳；传统3D实例分割方法需要相机位姿、深度图等预处理，成本高昂。需要一种仅需RGB输入就能实现几何一致性的解决方案。

Method: 1. 集成MUSt3R的3D感知特征到SAM2架构；2. 设计轻量级特征融合器，融合多级MUSt3R特征（编码隐式几何对应关系）和SAM2的外观特征；3. 提出视场感知采样策略，确保帧间观察空间一致的对象区域；4. 推理时仅需RGB输入，无需相机位姿或预处理。
...
Conclusion: 3AM通过融合3D感知几何特征和外观特征，实现了仅需RGB输入的高性能视频对象分割，在大视角变化下保持几何一致性，显著提升了现有方法的性能。
...
TL;DR: 本文提出了一种基于零样本扩散的隐形水印去除方法，将水印去除重新定义为视角合成问题，通过潜在空间的几何变换和视角引导的对应注意力机制，在保持视觉质量的同时有效去除多种水印方案。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成图像水印技术的大规模部署，评估这些方案对抗高级去除攻击的脆弱性变得至关重要。当前水印方案虽然对像素空间和频域攻击具有鲁棒性，但在语义保持的视角变换下可能存在根本性漏洞。

Method: 提出零样本扩散框架，将水印去除重新定义为视角合成问题。核心方法包括：1)在潜在空间应用受控几何变换；2)使用视角引导的对应注意力机制保持结构一致性；3)基于冻结的预训练模型，无需水印检测器或水印知识。

Result: 在15种水印方法上实现了最先进的水印抑制效果，优于14种基线攻击方法，同时在多个数据集上保持了优越的感知质量。

Conclusion: 揭示了隐形水印的根本性脆弱性：即使对像素空间和频域攻击鲁棒的水印，在语义保持的视角变换下仍然易受攻击。这为评估水印方案的可靠性和指导鲁棒设计提供了重要洞见。
...
### [71] [From Tool to Teacher: Rethinking Search Systems as Instructive Interfaces](https://arxiv.org/abs/2601.08035)
*David Elsweiler*

Main category: cs.HC

TL;DR: 论文提出从教学视角审视信息访问系统，将搜索和对话系统视为可教导用户学习技能的教学界面，而非单纯优化检索效率。


<details>
  <summary>Details</summary>
Motivation: 当前信息访问系统（如搜索引擎、生成式AI）主要优化检索效率，但缺乏帮助用户发展更好搜索策略和批判意识的设计。需要将系统重新概念化为教学界面，促进用户学习信息素养技能。

Method: 借鉴教育和行为科学的七个教学框架，分析现有和新兴系统功能（如查询建议、来源标签、对话式AI）如何支持或限制用户学习。通过两个示例搜索任务，展示不同设计选择如何促进批判性评估、元认知反思和策略迁移等技能。

Result: 建立了评估信息访问系统教学价值的概念框架，展示了不同设计特征如何影响用户学习效果，为设计更具教学意义的信息系统提供了实证基础。

Conclusion: 信息访问系统应被设计为教学界面，通过精心设计的功能促进用户发展搜索策略、批判性思维和信息素养技能，培养更有效、反思性和有韧性的信息寻求者。
...
### [72] [The Impact of AI Generated Content on Decision Making for Topics Requiring Expertise](https://arxiv.org/abs/2601.08178)
*Shangqian Li,Tianwa Chen,Gianluca Demartini*

Main category: cs.HC

TL;DR: 研究探讨AI生成内容与领域专业知识在在线决策中的交互作用，发现参与者在专业领域话题上更不愿改变观点，且AI生成信息与人类专家信息在决策支持上效果相当。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI产品如ChatGPT逐渐成为在线信息检索的重要工具，研究需要理解领域特定知识与AI生成内容在在线决策过程中的交互作用，填补这一研究空白。

Method: 采用实验室解释性顺序研究设计，对大学生参与者进行调查。实验设置两组话题：易于理解的通用领域话题和需要化学科学知识的专业领域话题。提供两种决策支持信息：生成式AI生成的内容和专家人工撰写的精选内容。

Result: 参与者在专业领域话题上改变观点的可能性较低。缺乏专业知识的参与者难以基于信息进行深入独立推理，倾向于依赖提供材料中的结论并坚持初始观点。标记为AI生成的信息与标记为人类专家撰写的信息在帮助参与者方面效果相当。

Conclusion: AI生成内容在决策支持方面具有与人类专家信息相当的潜力，但也引发了对AI取代人类专家帮助用户处理专业话题的担忧。研究揭示了用户在专业领域决策中的认知局限和对权威信息的依赖倾向。
...
### [73] [Simulations for Augmented Reality Evaluation for Mass Casualty Incident Triage](https://arxiv.org/abs/2601.08186)
*Cassidy R. Nelson,Joseph L. Gabbard,Jason B. Moats,Ranjana K. Mehta*

Main category: cs.HC

TL;DR: 本文提出了两种渐进式模拟策略，用于在计算机模拟和实际现场响应之间架起桥梁，评估增强现实在重大伤亡事件中的应用。


<details>
  <summary>Details</summary>
Motivation: 重大伤亡事件是高风险敏感领域，增强现实作为辅助工具在高压工作环境和伤员分类中显示出潜力，但由于事件的脆弱性，难以评估新工具。需要在投入实际响应前进行彻底的概念验证评估。

Method: 提出了两种渐进式模拟策略，用于增强现实系统评估，旨在填补计算机模拟和实际现场响应之间的差距。

Result: 通过渐进式模拟策略，可以在更接近真实环境但风险可控的条件下评估增强现实系统，为实际部署提供概念验证。

Conclusion: 渐进式模拟策略为评估增强现实在重大伤亡事件响应中的应用提供了有效方法，有助于在投入实际使用前进行充分验证。
...
### [74] [Data-Induced Groupings and How To Find Them](https://arxiv.org/abs/2601.08256)
*Yilan Jiang,Cindy Xiong Bearfield,Steven Franconeri,Eugene Wu*

Main category: cs.HC

TL;DR: 该研究探讨了可视化设计中数据值如何与视觉设计交互影响用户解读，以点图为例发现用户容易受"数据诱导分组"影响，即使这些分组在名义数据中无关紧要，并开发了预测模型帮助设计师诊断和优化可视化。


<details>
  <summary>Details</summary>
Motivation: 现有可视化研究主要关注设计元素（如颜色、图表类型）的可用性，但视觉设计与底层数据值如何交互影响用户解读和推理尚未充分探索。点图和条形图常用于识别趋势和聚类，但容易产生由空间排列而非数据固有模式形成的"数据诱导分组"，可能导致用户做出次优比较甚至错误结论。

Method: 以点图为案例研究，进行了两项用户研究来理解数据诱导分组的普遍性。基于研究结果，构建了一个预测模型来判断用户是否可能将特定点图点集感知为一个分组。

Result: 研究发现用户在两种条件下都依赖数据诱导分组，尽管趋势性分组在名义数据中无关紧要。开发的模型能够预测用户对点图分组的感知。

Conclusion: 数据诱导分组是可视化解读中的重要问题，可能误导用户。提出的预测模型可帮助可视化设计师诊断潜在的用户感知分组，并通过数据重排提供优化设计，更好地突出期望的分组模式。
...
### [75] [Characterizing Personality from Eye-Tracking: The Role of Gaze and Its Absence in Interactive Search Environments](https://arxiv.org/abs/2601.08287)
*Jiaman He,Marta Micheli,Damiano Spina,Dana McKay,Johanne R. Trippas,Noriko Kando*

Main category: cs.HC

TL;DR: 通过整合眼动追踪数据和注视缺失时段的多模态时间序列模型，能够从搜索相关的注视行为中有效预测人格特质，Big Five五维度预测准确率均超过73%。


<details>
  <summary>Details</summary>
Motivation: 人格特质影响个体的信息寻求过程，但现有研究很少将人格与可观察的搜索行为联系起来。本研究旨在通过整合眼动数据和注视缺失时段来表征人格特质，基于人们在思考时常常移开视线这一观察。

Method: 采用多模态时间序列模型，整合眼动追踪原始数据和注视缺失时段信息。研究包含25名参与者，在iPad上使用交互式应用程序与博物馆数字文物互动。模型最小化预处理以保留行为模式，通过五折交叉验证预测Big Five人格特质。

Result: 模型在所有五个维度上都表现出强预测性能：神经质（Macro F1 = 77.69%）、尽责性（74.52%）、开放性（77.52%）、宜人性（73.09%）和外向性（76.69%）。完整模型比不含时间序列信号和缺失信息的模型在所有特质上准确率和Macro F1分数高出10-15%。

Conclusion: 人格可以从搜索相关的注视行为中推断，并且将缺失注视数据整合到多模态时间序列建模中具有重要价值，注视缺失时段作为认知过程的信号能够提升人格特质预测的准确性。
...
### [76] [Rewriting Video: Text-Driven Reauthoring of Video Footage](https://arxiv.org/abs/2601.08565)
*Sitong Wang,Anh Truong,Lydia B. Chilton,Dingzeyu Li*

Main category: cs.HC

TL;DR: 本文提出了一种文本驱动的视频重编辑方法，通过生成式重建算法将视频反向工程为可编辑的文本提示，并开发了交互式工具Rewrite Kit，使创作者能够像编辑文本一样编辑视频。


<details>
  <summary>Details</summary>
Motivation: 视频是强大的传播和叙事媒介，但现有视频素材的重新编辑仍然具有挑战性，需要专业知识、时间和精心规划，限制了创作者的叙事想象和塑造能力。生成式AI的进步为视频编辑提供了新范式：能否像编辑文本一样简单地编辑视频？

Method: 提出两种技术贡献：(1) 生成式重建算法，将视频反向工程为可编辑的文本提示；(2) 交互式工具Rewrite Kit，允许创作者操作这些文本提示。通过技术评估和12名创作者的探针研究来验证方法。

Result: 技术评估揭示了关键的人机感知差距。探针研究发现了虚拟重拍、合成连续性、美学重风格等新颖用例，同时也凸显了在新范式中围绕连贯性、控制和创意对齐的关键张力。

Conclusion: 本研究为文本驱动视频重编辑的机遇和挑战提供了实证见解，为未来协同创意视频工具的设计提供了启示，推动了视频编辑向更直观、可访问的方向发展。
...
### [77] [Enhancing Financial Literacy and Management through Goal-Directed Design and Gamification in Personal Finance Application](https://arxiv.org/abs/2601.08640)
*Phuong Lien To*

Main category: cs.HC

TL;DR: 该研究采用Alan Cooper的目标导向设计方法，为年轻人开发了一款财务管理应用，通过个性化功能和游戏化设计提升财务素养。


<details>
  <summary>Details</summary>
Motivation: 旨在改善年轻人的财务素养和财务行为，通过技术手段帮助年轻用户建立更好的财务管理习惯。

Method: 采用Alan Cooper的目标导向设计方法，结合访谈、问卷调查和可用性测试来设计和评估应用。

Result: 研究发现游戏化学习和个性化体验在鼓励年轻用户改善财务行为方面具有显著效果。

Conclusion: 目标导向设计方法结合游戏化和个性化功能能有效提升年轻人的财务素养和财务管理行为。
...
### [78] [Tailored Immersive Environments: Advancing Neurodivergent Support Through Virtual Reality](https://arxiv.org/abs/2601.08652)
...
TL;DR: 开发了一个基于虚拟现实的个性化训练系统，帮助自闭症谱系障碍患者通过模拟日常生活场景来练习应对策略，系统能根据个人能力和训练需求自动调整环境难度。


<details>
  <summary>Details</summary>
Motivation: 神经多样性个体，特别是自闭症谱系障碍患者，在日常生活中面临显著挑战，他们具有特定的敏感性，需要针对性的训练方法来帮助他们应对日常困难。

Method: 设计了一个虚拟现实系统，能够根据个体的能力和特定训练需求自动个性化虚拟环境，系统通过四个合成用户配置文件进行评估，并提出了一种评估同一难度级别内特征差异的度量方法。

Result: 该方法能够为不同难度级别生成大量场景，在同一难度级别内，特定配置文件的非约束特征存在广泛差异，表明系统能够提供多样化的训练体验。

Conclusion: 虚拟现实系统能够有效为神经多样性个体提供个性化的日常情境训练，通过自动调整环境参数来满足不同能力和需求，为自闭症谱系障碍患者的日常挑战应对提供了有前景的技术支持。
...
### [79] [Auditing Student-AI Collaboration: A Case Study of Online Graduate CS Students](https://arxiv.org/abs/2601.08697)
*Nifu Dan*

Main category: cs.HC

TL;DR: 研究通过混合方法审计学生-AI协作偏好，分析当前AI能力与学生期望的自动化水平之间的差距，为教育AI系统设计提供依据。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI在高等教育中的普及，虽然提高了效率，但也引发了过度自动化、学生能动性减弱以及AI输出不可靠等担忧，需要了解学生期望的协作边界。

Method: 采用两个顺序互补的调查：第一个调查使用任务框架评估12项学术任务中AI使用偏好与实际使用情况；第二个调查通过开放式问题探讨如何设计AI系统来解决已识别的问题。

Result: 研究发现当前AI能力与学生期望的协作水平之间存在差距，识别了学生的主要关切点和使用原因，为AI系统设计提供了具体方向。

Conclusion: 研究揭示了学生-AI协作的偏好边界，为开发更有效、可信赖的教育AI系统提供了实证基础，有助于弥合现有AI功能与学生规范性期望之间的差距。
...
### [80] [Application of Ideal Observer for Thresholded Data in Search Task](https://arxiv.org/abs/2601.07976)
*Hongwei Lin,Howard C. Gifford*

Main category: eess.IV

TL;DR: 开发了一种基于阈值视觉搜索的拟人化模型观察者，通过选择性处理高显著性特征来提高图像质量评估性能，在噪声环境中特别有效。


<details>
  <summary>Details</summary>
Motivation: 改进基于任务的图像质量评估方法，通过模拟人类视觉系统的选择性注意机制，开发更符合人类视觉搜索行为的模型观察者，提高诊断准确性和计算效率。

Method: 采用两阶段框架：候选区域选择和决策制定。在候选选择阶段使用阈值化数据精确定位感兴趣区域，通过阶段特异性特征处理优化性能。通过模拟实验评估阈值化对特征图、候选定位和多特征场景的影响。

Result: 阈值化通过排除低显著性特征显著提高了观察者性能，特别是在噪声环境中。中间阈值通常优于无阈值处理，表明仅保留相关特征比保留所有特征更有效。模型还能用较少图像进行有效训练，同时保持与人类性能的一致性。

Conclusion: 提出的新型框架能够预测临床现实任务中的人类视觉搜索性能，并为资源有限条件下的模型观察者训练提供解决方案。该方法在计算机视觉、机器学习、国防安全图像分析等领域具有广泛应用前景。
...
### [81] [A Single-Parameter Factor-Graph Image Prior](https://arxiv.org/abs/2601.08749)
*Tianyang Wang,Ender Konukoglu,Hans-Andrea Loeliger*

Main category: eess.IV

TL;DR: 提出了一种具有分段常数局部参数的自适应分段平滑图像模型，使用因子图和NUP先验，通过共轭梯度步和高斯消息传递进行迭代计算，应用于去噪和对比度增强。


<details>
  <summary>Details</summary>
Motivation: 传统的图像模型往往使用固定的全局参数，无法适应图像不同区域的特性。本文旨在开发一种能够自动适应每个图像局部特性的分段平滑模型，以更好地处理图像处理任务。

Method: 使用因子图框架和NUP（参数未知的正态分布）先验来构建模型。通过迭代的共轭梯度步骤和高斯消息传递算法进行相关计算，实现参数的自适应估计。

Result: 提出的模型和算法在去噪和对比度增强应用中得到了验证，展示了其有效性和适应性。

Conclusion: 该分段平滑图像模型能够自动适应图像的局部特性，通过因子图和NUP先验框架实现高效计算，在图像处理任务中表现出良好的性能。
...
### [82] [M3CoTBench: Benchmark Chain-of-Thought of MLLMs in Medical Image Understanding](https://arxiv.org/abs/2601.08758)
*Juntao Jiang,Jiangning Zhang,Yali Bi,Jinsheng Bai,Weixuan Liu,Weiwei Jin,Zhucun Xue,Yong Liu,Xiaobin Hu,Shuicheng Yan*

Main category: eess.IV

TL;DR: 提出了M3CoTBench基准，专门用于评估医疗图像理解中思维链推理的正确性、效率、影响和一致性，填补了现有基准只关注最终答案而忽略推理过程的空白。


<details>
  <summary>Details</summary>
Motivation: 在医疗领域，诊断决策依赖于细微的视觉线索和顺序推理，思维链与临床思维过程自然契合。然而，当前的医疗图像理解基准通常只关注最终答案而忽略推理路径，这种不透明的过程缺乏可靠的判断依据，难以辅助医生诊断。

Method: 设计了M3CoTBench基准，包含：1）覆盖24种检查类型的多样化、多难度级别数据集；2）13个不同难度的任务；3）针对临床推理定制的思维链特定评估指标（正确性、效率、影响和一致性）；4）对多个多模态大语言模型的性能分析。

Result: M3CoTBench系统性地评估了各种医疗成像任务中的思维链推理，揭示了当前多模态大语言模型在生成可靠且临床可解释的推理方面的局限性。

Conclusion: 该基准旨在促进开发透明、可信且诊断准确的医疗AI系统，填补了医疗图像理解中评估推理过程的空白，为未来研究提供了重要工具。
...
### [83] [Contact-aware Path Planning for Autonomous Neuroendovascular Navigation](https://arxiv.org/abs/2601.07945)
*Aabha Tamhankar,Ron Alterovitz,Ajit S. Puri,Giovanni Pittiglio*

Main category: cs.RO

TL;DR: 提出了一种用于神经血管导航的确定性、时间高效的接触感知路径规划器，能够快速计算可行路径，在血管解剖模型中达到100%收敛率，亚毫米级跟踪误差。


<details>
  <summary>Details</summary>
Motivation: 神经血管导航需要精确控制预弯曲被动工具在复杂血管结构中的运动，传统方法难以高效处理工具与解剖结构的接触交互，需要开发能够智能预测和利用这种交互的路径规划算法。

Method: 基于术前和术中血管图像信息，建立运动学模型，采用基于采样的规划器进行树扩展，利用简化的运动基元，智能预测和利用工具与解剖结构的相互作用。

Result: 在最坏情况下算法在22.8秒内达到100%收敛率，跟踪误差小于0.64毫米（亚毫米级），在约94%患者的代表性解剖模型中表现有效。

Conclusion: 该接触感知路径规划器能够快速、准确地计算神经血管导航的可行路径，通过智能利用工具与血管的相互作用，在保持高精度的同时显著提高计算效率。
...
### [84] [Fiducial Exoskeletons: Image-Centric Robot State Estimation](https://arxiv.org/abs/2601.08034)
*Cameron Smith,Basile Van Hoorick,Vitor Guizilini,Yue Wang*

Main category: cs.RO

TL;DR: Fiducial Exoskeletons：一种基于图像的3D机器人状态估计方法，通过单张RGB图像估计机器人各连杆的6D位姿，替代传统繁琐的标定流程，实现低成本硬件上的鲁棒状态估计。


<details>
  <summary>Details</summary>
Motivation: 传统机器人状态估计方法依赖高精度执行器和繁琐的标定流程（如手眼标定），而现代基于学习的机器人控制越来越多地使用低成本硬件上的RGB观测。需要一种简化机器人-相机外参估计和状态估计的方法。

Method: 1. 将机器人状态估计重新定义为从单张RGB图像估计每个连杆的6D位姿；2. 引入"基准外骨骼"：在每个连杆上安装已知几何关系的基准标记的3D打印支架；3. 通过全局优化确保观测到的连杆位姿与运动学一致性，从估计的基连杆位姿直接获得机器人-相机变换。

Result: 在低成本机械臂上验证，基准外骨骼显著简化了设置过程，同时提高了标定精度、状态准确性和下游3D控制性能。即使在断电机器人上也能实现鲁棒的状态估计。

Conclusion: 基准外骨骼提供了一种简单有效的图像驱动机器人状态估计方法，实现了算法-硬件协同设计，为低成本机器人系统提供了实用的状态估计解决方案。
...
### [85] [Efficient Incremental SLAM via Information-Guided and Selective Optimization](https://arxiv.org/abs/2601.08110)
*Reza Arablouei*
...
Method: 结合两种互补思想：1) 信息引导门控(IGG)，基于信息矩阵对数行列式的信息理论准则量化新测量的贡献，仅在观察到显著信息增益时触发全局优化；2) 选择性部分优化(SPO)，执行多迭代高斯牛顿更新但限制每次迭代到受新测量影响最大的变量子集，动态优化活动集直到收敛
...
### [86] [A Pin-Array Structure for Gripping and Shape Recognition of Convex and Concave Terrain Profiles](https://arxiv.org/abs/2601.08143)
*Takuya Kato,Kentaro Uno,Kazuya Yoshida*

Main category: cs.RO

TL;DR: 开发了一种用于极端环境移动机器人的抓取器，能够抓握并识别地形形状，采用针阵列结构同时实现自适应抓取和地形测量


<details>
  <summary>Details</summary>
Motivation: 多肢攀爬机器人在崎岖地形（如悬崖和洞穴墙壁）上有效，但在未知自然环境中可能因误抓表面或失去可抓握点而摔倒或卡住，需要能够自适应抓取不规则地形并准确测量地形形状的抓取器

Method: 开发了一种采用针阵列结构的抓取器，能够抓握凸凹地形并同时测量地形形状，通过原型验证了其抓取和地形识别性能

Result: 该抓取器能够抓握凸凹地形并同时测量地形形状，针阵列设计不仅适用于自适应抓取不规则地形，还能有效进行3D地形映射

Conclusion: 提出的针阵列抓取器解决了极端环境下移动机器人的自适应抓取和地形识别问题，为在未知自然环境中安全操作提供了有效解决方案
...
### [87] [Robust Subpixel Localization of Diagonal Markers in Large-Scale Navigation via Multi-Layer Screening and Adaptive Matching](https://arxiv.org/abs/2601.08161)
...
Method: 采用三层框架：1) 通过光照均衡和结构信息提取进行降维；2) 粗到细的候选点选择策略减少滑动窗口计算成本；3) 为候选点生成自适应模板，通过改进的模板匹配和相关系数极值拟合实现亚像素精度。
...
### [88] [FSAG: Enhancing Human-to-Dexterous-Hand Finger-Specific Affordance Grounding via Diffusion Models](https://arxiv.org/abs/2601.08246)
*Yifan Han,Pengfei Yi,Junyan Li,Hanqing Wang,Gaojing Zhang,Qi Peng Liu,Wenzhao Lian*

Main category: cs.RO

TL;DR: 提出了一种数据高效框架，利用预训练扩散模型的语义先验从人类视频演示中提取抓取信息，实现无需硬件特定数据集的多指灵巧抓取合成


<details>
  <summary>Details</summary>
Motivation: 灵巧抓取合成面临高维度和运动学多样性挑战，现有方法依赖硬件特定的大规模抓取数据集，限制了新灵巧手设计的可扩展性

Method: 1) 从原始人类视频演示中提取时间对齐的细粒度抓取功能；2) 与深度图像的三维场景几何融合推断语义接触目标；3) 运动学感知的重定向模块将功能表示映射到不同灵巧手

Result: 系统产生稳定、功能适当的多接触抓取，在常见物体和工具上可靠成功，在未见物体实例、姿态变化和多种手部实现上表现出强泛化能力

Conclusion: 通过人类演示和预训练生成模型，为可扩展、硬件无关的灵巧操作开辟了新路径，单深度模态结合基础模型语义即可实现高性能抓取合成
...
### [89] [ActiveVLA: Injecting Active Perception into Vision-Language-Action Models for Precise 3D Robotic Manipulation](https://arxiv.org/abs/2601.08325)
...
<details>
  <summary>Details</summary>
Motivation: 现有机器人操作研究大多依赖静态腕部摄像头，缺乏主动感知能力，无法自适应选择最优视角和分辨率，限制了长时程任务和细粒度操作场景的性能

Method: 采用粗到细两阶段范式：1）关键区域定位：将3D输入投影到多视角2D投影，识别关键3D区域；2）主动感知优化：基于定位区域选择最优视角（最大化模态相关性和多样性，最小化遮挡），并对关键区域进行3D放大以提高分辨率

Result: 在三个仿真基准测试中优于现有最先进方法，实现了精确的3D操作，并能无缝迁移到真实世界场景，使机器人能在复杂环境中学习高精度任务

Conclusion: ActiveVLA通过引入主动感知能力，显著提升了机器人细粒度操作的性能，为高精度机器人操作提供了有效的解决方案
...
### [90] [Large Language Models to Enhance Multi-task Drone Operations in Simulated Environments](https://arxiv.org/abs/2601.08405)
*Yizhan Feng,Hichem Snoussi,Jing Teng,Abel Cherouat,Tian Wang*

Main category: cs.RO

TL;DR: 提出了一种将微调CodeT5模型与AirSim无人机模拟器结合的方法，通过自然语言命令高效执行多任务操作，降低无人机操作门槛。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速发展，人机交互迎来了前所未有的机遇。本文旨在利用自然语言命令简化无人机操作，降低用户使用门槛，提高交互效率。

Method: 1) 将微调的CodeT5模型与基于Unreal Engine的AirSim无人机模拟器集成；2) 使用ChatGPT生成的自然语言-程序代码命令执行对数据集和开发者编写的无人机代码作为训练数据；3) 在AirSim中构建视觉逼真的动态环境模拟复杂场景；4) 实现自然语言到无人机任务可执行代码的自动翻译。

Result: 实验结果表明，该方法在模拟环境中表现出优越的任务执行效率和命令理解能力。能够通过自然语言提示或命令描述灵活控制无人机状态。

Conclusion: 该方法有效降低了无人机操作门槛，提高了交互效率。未来计划以模块化方式扩展模型功能，增强对复杂场景的适应性，推动无人机技术在真实环境中的应用。
...
<details>
  <summary>Details</summary>
Motivation: 人们以不同方式回应反馈和指导，机器人需要个性化互动并利用言语和非言语沟通线索。特别关注老年人对机器人运动教练反馈节奏的反应，以改善人机交互设计

Method: 通过在线研究，让老年人观看机器人以不同节奏提供言语和非言语反馈的视频，评估他们对不同节奏模态的反应

Result: 改变一种模态的节奏会影响该模态和另一种模态的感知，表明言语和非言语反馈节奏之间存在相互影响
...
### [92] [Teaching Robots Like Dogs: Learning Agile Navigation from Luring, Gesture, and Speech](https://arxiv.org/abs/2601.08422)
*Taerim Yoon,Dongho Kang,Jin Cheng,Fatemeh Zargarbashi,Yijiang Huang,Minsung Ahn,Stelian Coros,Sungjoon Choi*

Main category: cs.RO

TL;DR: 提出人机交互框架，让四足机器人通过少量人类演示数据学习理解社交线索和手势/语音指令，实现敏捷导航
...
Method: 提出人机交互框架，使用基于物理的仿真重建交互场景并聚合数据以减少分布偏移；采用渐进式目标提示策略，在训练中自适应地提供指令和导航目标

Result: 在6个真实世界敏捷导航场景中测试，包括跳跃或避开障碍物，任务成功率97.15%，总演示数据少于1小时

Conclusion: 提出的框架能够以数据高效的方式让机器人学习导航行为，并通过多模态自然输入控制，实现高成功率导航
...
### [93] [Large Multimodal Models for Embodied Intelligent Driving: The Next Frontier in Self-Driving?](https://arxiv.org/abs/2601.08434)
*Long Zhang,Yuchen Xia*

Main category: cs.RO

TL;DR: 本文提出了一种语义与策略双驱动的混合决策框架，将大型多模态模型与深度强化学习相结合，以解决自动驾驶在开放世界场景中的持续环境理解和逻辑推理问题。


<details>
  <summary>Details</summary>
Motivation: 当前模块化设计的自动驾驶系统在需要持续环境理解和逻辑推理的开放世界场景中存在局限性。虽然大型多模态模型和具身人工智能为增强自动驾驶能力提供了可能，但仅依赖LMMs无法实现联合决策，限制了自动驾驶向具身智能驾驶的发展。

Method: 提出语义与策略双驱动的混合决策框架：1）利用大型多模态模型进行语义理解和认知表征；2）使用深度强化学习进行实时策略优化。该框架确保持续学习和联合决策能力。

Result: 通过车道变换规划任务的案例研究实验验证了该框架的性能优越性。实验结果表明该框架在完成自动驾驶任务方面具有显著优势。

Conclusion: 该混合决策框架为自动驾驶向具身智能驾驶发展提供了有效解决方案，并识别了多个未来研究方向以指导后续工作，包括框架的进一步优化和应用扩展。
...
### [94] [Real2Sim based on Active Perception with automatically VLM-generated Behavior Trees](https://arxiv.org/abs/2601.08454)
*Alessandro Adami,Sebastian Zudaire,Ruggero Carli,Pietro Falco*

Main category: cs.RO

TL;DR: 本文提出了一种自主生成行为树的Real2Sim框架，通过任务特定的物理交互获取仿真所需参数，无需预定义任务模板或专家设计的探索程序。


<details>
  <summary>Details</summary>
Motivation: 传统Real2Sim流水线依赖手动测量或固定的预编程探索程序，限制了其对不同任务和用户意图的适应性。需要一种能够根据具体仿真目标自主获取所需参数的方法。

Method: 使用视觉语言模型进行多模态推理，识别相关对象、推断所需物理参数，并生成由基本机器人动作组成的结构化行为树。通过扭矩控制的Franka Emika Panda执行行为，实现柔顺的接触丰富交互以进行参数估计。

Result: 在真实机械臂上的实验结果表明，该方法能够在多种场景下（包括遮挡对象和不完整先验模型）准确估计物体质量、表面高度和摩擦相关参数。

Conclusion: 所提出的方法实现了可解释、意图驱动且自主的Real2Sim流水线，将高级推理与物理基础的机器人交互相结合。
...
### [95] [AME-2: Agile and Generalized Legged Locomotion via Attention-Based Neural Map Encoding](https://arxiv.org/abs/2601.08485)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：端到端的感知运动模型泛化性和可解释性有限；针对泛化运动的方法又缺乏敏捷性且难以处理视觉遮挡。需要一种既能实现敏捷运动又能良好泛化到未知地形的统一框架。

Method: 1. 注意力机制地图编码器：提取局部和全局地图特征，通过注意力机制聚焦关键区域，为RL控制生成可解释的泛化嵌入。2. 学习型建图管道：将深度观测转换为带不确定性的局部高程图，与里程计融合，提供抗噪声和遮挡的快速地形表示。3. 与并行仿真集成，支持在线建图的控制器训练，促进仿真到现实的迁移。

Result: 在四足和双足机器人上验证了AME-2框架，控制器在仿真和真实世界实验中表现出强大的敏捷性和对未知地形的泛化能力。

Conclusion: AME-2框架通过结合注意力机制的地图编码器和学习型建图管道，成功实现了机器人在复杂地形上的敏捷和泛化运动，解决了现有方法在可解释性、泛化性和处理遮挡方面的局限性。
...
### [96] [AUV Trajectory Learning for Underwater Acoustic Energy Transfer and Age Minimization](https://arxiv.org/abs/2601.08491)
*Mohamed Afouene Melki,Mohammad Shehab,Mohamed-Slim Alouini*

Main category: cs.RO

TL;DR: 本文提出了一种可持续的水下物联网解决方案，通过自主水下航行器同时实现信息上行和声能传输，使设备能够无限期运行，并采用深度强化学习算法优化信息时效性和公平性。
...
Method: 采用自主水下航行器同时进行信息上行和声能传输，引入信息时效性和Jain公平指数作为评估指标，开发了两种深度强化学习算法：高性能的频分双工方案和中等性能的时分双工方案。

Result: 提出的频分双工和时分双工解决方案相比基线方法显著降低了平均信息时效性，提高了能量收集效率，并改善了数据收集的公平性。

Conclusion: 通过自主水下航行器实现的信息上行和声能传输结合深度强化学习优化，为水下物联网提供了一种可持续的解决方案，能够显著提升系统性能并支持设备长期运行。
...
### [97] [Simplifying ROS2 controllers with a modular architecture for robot-agnostic reference generation](https://arxiv.org/abs/2601.08514)
*Davide Risi,Vincenzo Petrone,Antonio Langella,Lorenzo Pagliara,Enrico Ferrentino,Pasquale Chiacchio*

Main category: cs.RO

TL;DR: 提出ROS2的模块化架构，将参考获取、验证和插值与控制律解耦，通过Reference Generator组件处理参考输入，减少控制器中的重复代码，提高跨平台复用性。


<details>
  <summary>Details</summary>
Motivation: 当前ROS2控制器中参考处理代码重复，跨平台复用性差，需要一种架构将参考处理逻辑与控制律分离，提高控制器开发效率和复用性。

Method: 设计Reference Generator组件，接收外部节点（如规划器）的单点或轨迹参考，通过ros2_control链式机制以控制器采样周期写入单点参考；实现关节空间和笛卡尔空间两种参考生成器，以及PD重力补偿、笛卡尔位姿和导纳控制器。

Result: 在所有测试场景中可靠跟踪参考；参考生成器显著减少链式控制器中的重复参考处理代码；控制器实现专注于控制律，架构在仿真和真实机器人（Universal Robots和Franka Emika）上验证有效。

Conclusion: 提出的模块化架构成功将参考处理与控制律解耦，提高了ROS2控制器的代码复用性和开发效率，支持复杂控制器管道的构建和重用。
...
### [98] [Keyframe-based Dense Mapping with the Graph of View-Dependent Local Maps](https://arxiv.org/abs/2601.08520)
*Krzysztof Zielinski,Dominik Belter*

Main category: cs.RO

TL;DR: 提出基于关键帧的NDT地图构建系统，利用RGB-D传感器数据更新局部NDT地图，通过2D视角相关结构存储单元，支持回环检测后的全局地图校正


<details>
  <summary>Details</summary>
Motivation: 现有地图构建方法未能充分利用RGB-D相机的特性和不确定性模型，需要一种能够根据相机距离自然调整精度、支持局部地图合并与全局校正的映射系统

Method: 使用关键帧和RGB-D传感器数据更新局部NDT地图，将NDT单元存储在2D视角相关结构中，局部地图存储在姿态图中支持回环检测后的全局校正，并提出局部地图合并与过滤流程构建全局地图

Result: 与Octomap和NDT-OM方法进行比较，展示了所提映射方法的实际应用效果

Conclusion: 提出的关键帧NDT映射系统能够更好地利用RGB-D相机特性，实现距离相机越近精度越高的自然表示，支持有效的局部地图合并和全局地图校正
...
### [99] [QP-Based Control of an Underactuated Aerial Manipulator under Constraints](https://arxiv.org/abs/2601.08523)
*Nesserine Laribi,Mohammed Rida Mokhtari,Abdelaziz Benallegue,Abdelhafid El-Hadri,Mehdi Benallegue*

Main category: cs.RO

TL;DR: 本文提出了一种针对欠驱动空中机械臂的约束感知控制框架，能够在考虑安全和可行性约束的同时实现精确的末端执行器轨迹跟踪。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够处理欠驱动空中机械臂复杂控制问题的框架，同时显式考虑安全约束、执行器限制和系统约束，提高在实际操作条件下的鲁棒性和可靠性。

Method: 将控制问题表述为二次规划问题，计算动态一致的广义加速度，同时考虑欠驱动性、执行器边界和系统约束。在扭矩层面加入基于被动性的积分作用以增强对扰动、建模不确定性和稳态误差的鲁棒性。

Result: 通过高保真物理仿真验证了方法的有效性，包括参数扰动、粘性关节摩擦以及真实传感和状态估计效应。结果显示在真实操作条件下实现了精确跟踪、平滑控制输入和可靠的约束满足。

Conclusion: 提出的约束感知控制框架能够有效处理欠驱动空中机械臂的复杂控制问题，在满足多种约束的同时实现精确的轨迹跟踪，具有良好的鲁棒性和实际应用价值。
...
### [100] [VLingNav: Embodied Navigation with Adaptive Reasoning and Visual-Assisted Linguistic Memory](https://arxiv.org/abs/2601.08665)
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在具身导航中主要依赖从观察到动作的被动映射，缺乏显式推理能力和持久记忆，难以处理复杂、长视野的导航任务。需要一种能够结合人类认知双过程理论的方法，实现快速直觉执行和慢速深思规划的灵活切换。

Method: 1. 引入自适应思维链机制，动态触发显式推理；2. 开发视觉辅助语言记忆模块，构建跨模态语义记忆；3. 构建Nav-AdaCoT-2.9M数据集，包含自适应CoT标注；4. 采用在线专家引导的强化学习阶段，超越纯模仿学习。

Result: VLingNav在广泛的具身导航基准测试中实现了最先进的性能。特别值得注意的是，该模型能够以零样本方式迁移到真实机器人平台，执行各种导航任务，并展现出强大的跨领域和跨任务泛化能力。

Conclusion: VLingNav通过语言驱动认知方法成功解决了VLA模型在具身导航中的推理和记忆限制问题，实现了自适应推理和持久记忆，为复杂长视野导航任务提供了有效解决方案，并展示了向真实世界应用的强大迁移能力。
...
### [101] [Real-Time Localization Framework for Autonomous Basketball Robots](https://arxiv.org/abs/2601.08713)
*Naren Medarametla,Sreejon Mondal*

Main category: cs.RO

TL;DR: 本文提出了一种混合定位算法，结合经典技术与基于学习的方法，仅利用球场地面视觉数据实现篮球场上的自主定位


<details>
  <summary>Details</summary>
Motivation: 定位是自主机器人的基本能力，对于Robocon 2025比赛，准确可靠的定位对于提高射击精度、避免与其他机器人碰撞以及高效导航比赛场地至关重要

Method: 提出混合定位算法，集成经典定位技术与基于学习的视觉方法，仅使用球场地面视觉数据进行定位

Result: 未在摘要中明确说明具体实验结果

Conclusion: 该方法旨在为Robocon 2025比赛中的机器人提供准确可靠的定位能力，以提升比赛表现
...
### [102] [Angled Aortic Cannulation Reduces Cerebral Embolic Transport and Adverse Wall Loading During Cardiopulmonary Bypass: A Patient-Specific Hemodynamic Study](https://arxiv.org/abs/2601.07881)
*Nafis M. Arefin,Bryan C. Good*

Main category: physics.med-ph

TL;DR: 该研究通过计算流体动力学模拟发现，与垂直插管相比，30度角度的主动脉插管能显著减少体外循环期间栓子进入主动脉分支的风险，特别是在老年患者中效果最明显。


<details>
  <summary>Details</summary>
Motivation: 体外循环期间，插管引起的血流扰动会影响栓子运输和主动脉壁负荷，导致栓塞性卒中。本研究旨在量化不同插管方向如何影响栓子分布和动脉粥样硬化斑块破裂风险，为临床实践提供依据。

Method: 采用经过验证的计算流体动力学和拉格朗日粒子追踪框架，分析四个患者特异性主动脉模型（儿科、青少年、成人和老年）。评估两种临床相关插管方向（垂直90度和角度30度），在不同血液粘度（1.5-3.5 cP）和栓子大小（0.5-2.5 mm）条件下，量化主动脉分支出口百分比、壁压和壁剪切应力。

Result: 30度角度插管比垂直插管减少18-50%的栓子进入主动脉分支，老年模型减少最多。垂直插管产生集中喷射冲击，导致后壁压力增加24%，局部壁剪切应力增加高达8倍。儿科解剖对粘度变化最敏感，粘度降低和栓子增大分别增加分支出口8.4%和25-44%。

Conclusion: 角度插管通过沿主动脉曲率重新分配喷射动量，减弱喷射冲击，降低局部压力和壁应力，限制栓子向脑循环输送。这些发现为采用角度插管方向减少体外循环并发症提供了机制性、患者特异性的证据支持。
...
### [103] [Percentile-based probabilistic optimization for systematic and random uncertainties in radiation therapy](https://arxiv.org/abs/2601.08644)
*Albin Fredriksson,Erik Engwall,Jenneke de Jong,Johan Sundström*

Main category: physics.med-ph

TL;DR: 提出了一种基于百分位数优化的概率规划框架，用于放射治疗中几何不确定性的管理，通过显式控制临床目标实现概率来改善治疗效果。


<details>
  <summary>Details</summary>
Motivation: 放射治疗中的几何不确定性会影响治疗质量，传统的边缘扩展和鲁棒优化方法只能隐式控制临床目标实现概率，需要一种能显式控制目标实现概率的方法。

Method: 开发了基于百分位数优化的概率规划框架，显式建模系统性和随机性不确定性，采用基于固定剂量集插值的场景剂量近似方法，实现治疗过程中高效模拟。

Result: 在前列腺VMAT病例中，相比边缘规划，概率优化在保持靶区覆盖的同时改善了危及器官保护，OAR目标实现概率平均提高13.3个百分点，90百分位数OAR剂量平均降低3.5Gy。在脑部PBS质子治疗病例中，靶区最小剂量通过概率显著改善（D95从22%提高到88%），脑干最大剂量通过概率从30%提高到70%。

Conclusion: 概率优化能够提供显式且可解释的目标实现概率控制，结合完整治疗过程建模和高效近似剂量计算，相比传统规划方法在光子和质子治疗中都能改善靶区覆盖与危及器官保护之间的权衡。
