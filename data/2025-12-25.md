...
TL;DR: VL4Gaze是首个大规模基准测试，用于评估和训练视觉语言模型在注视理解方面的能力，包含48.9万个自动生成的问答对，涵盖四个互补任务，结果显示当前VLM在注视理解方面表现不佳，但通过VL4Gaze训练能显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 人类注视为理解视觉场景中的注意力、意图和社交互动提供了重要线索，但当前视觉语言模型在注视理解方面研究不足，缺乏系统性的评估基准，不清楚通用视觉语言预训练是否能自然产生注视理解能力。

Method: 提出VL4Gaze基准测试，包含48.9万个自动生成的问答对，覆盖12.4万张图像，将注视理解统一为视觉问答问题，通过四个互补任务：注视对象描述、注视方向描述、注视点定位和模糊问题识别。

Result: 评估商业和开源VLM在上下文学习和微调设置下的表现，结果显示即使大规模VLM也难以可靠推断注视语义和空间定位，而使用VL4Gaze训练能在所有任务上带来显著且一致的改进。

Conclusion: 注视理解能力不会自然从通用视觉语言预训练中涌现，需要针对性的多任务监督，VL4Gaze基准测试为开发VLM的注视理解能力提供了重要支持，将发布数据集和代码以促进该方向的研究。
...
TL;DR: 本文提出了OccuFly，首个基于相机的真实世界空中语义场景补全基准数据集，针对无人机应用场景，包含不同高度和季节的数据，并提出了基于相机模态的LiDAR-free数据生成框架。


<details>
  <summary>Details</summary>
Motivation: 语义场景补全在移动机器人3D感知中至关重要，但现有研究主要集中于地面自动驾驶领域，空中场景如自主飞行仍未被充分探索。此外，LiDAR作为主要数据采集方式对无人机存在诸多限制，包括飞行法规、重量能耗约束以及高空视角点云稀疏性问题。

Method: 提出了OccuFly基准数据集，在50m、40m、30m高度采集春夏秋冬四季数据，覆盖城市、工业和农村场景，提供22个语义类别。提出了基于相机模态的LiDAR-free数据生成框架，利用传统3D重建技术，通过将标注的2D掩码提升到重建点云中实现标签自动转移，大幅减少手动3D标注工作量。

Result: 建立了首个真实世界相机空中SSC基准，数据格式遵循现有研究规范便于集成。在OccuFly上对现有最先进方法进行了基准测试，并突出了高空视角特有的挑战。

Conclusion: OccuFly为整体空中3D场景理解提供了全面的视觉基准，解决了无人机领域语义场景补全研究的空白，通过相机模态的数据生成框架克服了LiDAR在无人机应用中的限制。
...
TL;DR: NullBUS：一种多模态混合监督框架，通过可学习的空嵌入和存在掩码处理缺失的文本提示，在有无提示的情况下都能进行乳腺超声分割，在混合提示可用性下达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 乳腺超声分割对计算机辅助诊断和治疗规划至关重要。虽然提示方法在有文本或空间提示时能提高分割性能，但许多公共BUS数据集缺乏可靠的元数据或报告，限制了训练到小型多模态子集并降低了鲁棒性。

Method: 提出NullBUS多模态混合监督框架，从有提示和无提示的图像中学习。引入可空提示，通过可学习的空嵌入和存在掩码实现，当元数据缺失时回退到仅图像证据，当文本存在时使用文本。

Result: 在三个公共BUS数据集的统一池上评估，NullBUS实现了0.8568的平均IoU和0.9103的平均Dice，在混合提示可用性下展示了最先进的性能。

Conclusion: NullBUS通过可空提示有效处理缺失的文本元数据，在混合监督设置下实现了稳健的乳腺超声分割，为缺乏可靠元数据的实际应用场景提供了解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前用于量化细胞形态的模型通常只针对单一显微镜成像类型训练，导致模型无法跨研究重复使用，因为技术规格不匹配或目标实验条件超出分布范围。
...
### [5] [NeRV360: Neural Representation for 360-Degree Videos with a Viewport Decoder](https://arxiv.org/abs/2512.20871)
...
<details>
  <summary>Details</summary>
Motivation: 现有的隐式神经表示方法（如NeRV）应用于高分辨率360度视频时存在内存占用高、解码速度慢的问题，难以实现实时应用。

Method: NeRV360将视口提取集成到解码过程中，并引入时空仿射变换模块，根据视点和时间进行条件解码，仅解码用户选择的视口而非整个全景帧。

Result: 在6K分辨率视频上，NeRV360相比代表性先前工作HNeRV，内存消耗降低7倍，解码速度提升2.5倍，同时在客观指标上提供更好的图像质量。

Conclusion: NeRV360通过仅解码用户选择的视口，有效解决了360度视频应用中隐式神经表示的内存和速度瓶颈，为实时应用提供了可行方案。
...
Result: 在DocVQA数据集子集上的实验显示：自适应预处理将每图像推理时间减少50%以上，降低平均完整生成时间，视觉标记数量相比基线流水线持续减少55%以上。

Conclusion: 输入感知预处理是提高视觉语言模型部署效率的有效且轻量级策略。实现代码已开源在FastVLM仓库的分支中。
...
TL;DR: ALIVE是一个本地部署的交互式视频引擎，将传统讲座视频转化为实时互动学习体验，通过神经化身、内容感知检索和多模态交互实现隐私保护的学习支持。


<details>
  <summary>Details</summary>
Motivation: 传统讲座视频缺乏实时澄清机制，学习者遇到困惑时需要外部搜索；现有交互式学习系统通常缺乏讲座意识、依赖云服务或未能整合检索与化身解释功能。

Method: ALIVE采用完全本地部署架构，包含：1) 通过ASR转录、LLM精炼和神经说话头合成生成化身讲座；2) 结合语义相似度和时间戳对齐的内容感知检索机制；3) 支持文本或语音提问、文本或化身回答的实时多模态交互。

Result: 在完整医学影像课程上演示系统，评估显示ALIVE提供准确、内容感知且引人入胜的实时支持，具有高检索准确性和可接受的延迟特性。
...
TL;DR: 本文提出了一种用于地下管道检测的3D GPR智能检测框架，通过多视图特征融合、改进的YOLO算法和三维空间特征匹配，显著提升了复杂场景下小尺度目标的识别精度。


<details>
  <summary>Details</summary>
Motivation: 针对3D GPR地下管道检测中存在的多视图特征相关性弱、小尺度目标识别精度低、复杂场景鲁棒性不足等问题，需要开发更有效的检测框架。

Method: 1. 基于B/C/D-Scan三视图联合分析策略，通过FDTD方法仿真与实际测量数据交叉验证建立三维管道三视图特征评估方法；2. 提出DCO-YOLO框架，在YOLOv11中集成DySample、CGLU和OutlookAttention跨维度相关机制；3. 提出3D-DIoU空间特征匹配算法，集成三维几何约束和中心距离惩罚项实现多视图标注自动关联。

Result: 在真实城市地下管道数据实验中，该方法在复杂多管道场景下达到准确率96.2%、召回率93.3%、平均精度96.7%，分别比基线模型提高2.0%、2.1%、0.9%。消融实验验证了动态特征增强模块的协同优化效果。

Conclusion: 本研究将深度学习优化策略与3D GPR物理特性相结合，为地下管道的智能识别与定位提供了高效可靠的新型技术框架，三视图融合策略解决了单视图检测的固有模糊性问题。
...
<details>
  <summary>Details</summary>
Motivation: 跨模态船舶重识别面临显著的模态差异挑战，主流方法依赖大规模配对数据集进行预训练和显式模态对齐。本文基于柏拉图表示假说，探索视觉基础模型在弥合模态差距方面的潜力，并针对现有参数高效微调方法在有限容量模型上表现不佳的问题，提出从权重空间优化转向特征空间优化的新思路。

Method: 提出领域表示注入（DRI）策略：1）保持视觉基础模型完全冻结以保留通用知识；2）设计轻量级可学习的偏移编码器从原始输入中提取富含模态和身份属性的领域特定表示；3）基于不同层中间特征的上下文信息，通过调制器自适应转换这些表示；4）通过加性融合将转换后的表示注入中间层，动态重塑特征分布以适应下游任务，而不改变预训练权重。

Result: 在HOSS-ReID数据集上，仅使用1.54M和7.05M参数分别达到57.9%和60.5%的mAP，实现了最先进的性能。该方法以最小的可训练参数取得了优越的实验结果。

Conclusion: 提出的领域表示注入（DRI）策略通过特征空间优化有效解决了跨模态船舶重识别中的模态差异问题，在保持视觉基础模型冻结的同时实现了参数高效微调，为跨模态任务提供了一种新的优化视角。
...
Method: 1) 创建首个全面的基准测试，评估VLM处理压缩图像的能力，涵盖多种图像编解码器和任务，包含超过100万张压缩图像；2) 分析性能差距来源：压缩过程中的信息损失和VLM的泛化失败；3) 提出通用VLM适配器来增强模型在现有编解码器压缩图像上的性能。
...
### [11] [DexAvatar: 3D Sign Language Reconstruction with Hand and Body Pose Priors](https://arxiv.org/abs/2512.21054)
*Kaustubh Kundu,Hrishav Bakul Barua,Lucy Robertson-Bell,Zhixi Cai,Kalin Stefanov*

Main category: cs.CV

TL;DR: DexAvatar是一个从单目手语视频重建生物力学精确的细粒度手部关节和身体运动的新框架，使用学习的3D手部和身体先验，在SGNify数据集上比现有方法提升35.11%


<details>
  <summary>Details</summary>
Motivation: 当前手语生成主要依赖数据驱动的生成方法，需要大量精确的2D和3D人体姿态数据。但大多数手语数据集基于视频，仅包含自动重建的2D关键点，缺乏准确的3D信息。现有从手语视频进行3D人体姿态估计的方法容易受到自遮挡、噪声和运动模糊的影响，导致重建质量差。

Method: DexAvatar是一个新颖的框架，通过学习的3D手部和身体先验，从野外单目手语视频重建生物力学准确的细粒度手部关节和身体运动。该方法利用先验知识来克服自遮挡、噪声和运动模糊等问题。

Result: 在SGNify运动捕捉数据集（该任务的唯一基准）上，DexAvatar在身体和手部姿态估计方面比现有最先进方法提升了35.11%的性能。

Conclusion: DexAvatar能够从单目手语视频中准确重建3D手部和身体运动，解决了现有方法在自遮挡、噪声和运动模糊下的性能限制，为手语生成提供了更高质量的3D姿态数据。
...
TL;DR: PanoGrounder是一个通用的3D视觉定位框架，通过将多模态全景表示与预训练的2D视觉语言模型结合，实现了强大的视觉语言推理能力。


<details>
  <summary>Details</summary>
Motivation: 传统监督模型依赖显式3D几何但泛化能力有限，主要受限于3D视觉语言数据集的稀缺性以及与现代视觉语言模型相比有限的推理能力。
...
Conclusion: PanoGrounder通过将多模态全景表示与预训练2D VLMs耦合，为3D视觉定位提供了强大的泛化框架，克服了传统方法的局限性。
...
TL;DR: 提出协作多智能体推理框架，通过语义规划与视觉合成的解耦，结合自校正验证机制和多样化假设生成，显著提升无模态补全的语义一致性和结构完整性，并引入新评估指标MAC-Score。


<details>
  <summary>Details</summary>
Motivation: 无模态补全任务在保持语义一致性和结构完整性方面面临挑战，现有渐进式方法存在推理不稳定和误差累积的固有局限。

Method: 提出协作多智能体推理框架，将语义规划与视觉合成解耦；包含自校正验证智能体（使用思维链推理修正分割）和多样化假设生成器（提供多种语义解释）；引入MAC-Score评估指标。

Result: 在多个数据集上显著优于现有最先进方法，新评估指标与人类判断和真实标注高度一致。

Conclusion: 通过协作多智能体推理框架，实现了语义规划与视觉合成的有效解耦，解决了无模态补全中的语义一致性和结构完整性问题，为评估提供了新标准。
...
Method: 提出真实中心包络建模（REM），通过特征级扰动自重建生成近真实样本，使用包络估计器与跨域一致性学习包围真实图像流形的边界

Result: 在八个基准评估中平均提升7.5%，在严重退化的RealChain基准上保持优异泛化能力，为真实世界条件下的合成图像检测奠定基础

Conclusion: REM通过从学习生成器伪影转向建模真实图像鲁棒分布，显著提升了真实世界条件下的检测鲁棒性和泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 现有神经激光雷达建图方法大多依赖密集隐式表示且未充分利用几何结构，而现有的体素引导方法难以实现实时性能，需要一种能同时利用几何结构和实现高效增量建图的解决方案
...
Conclusion: XGrid-Mapping通过混合表示框架有效解决了神经激光雷达建图中的效率和几何结构利用问题，实现了大规模增量建图的实时性能
...
TL;DR: 提出了一种用于增强胸部X光片中长尾肺部异常诊断的数据合成方法，利用大量正常X光片训练扩散模型，通过修复病变区域来增强罕见病变样本，结合大语言模型知识引导和渐进增量学习策略提升性能。


<details>
  <summary>Details</summary>
Motivation: 胸部X光片中长尾肺部异常（罕见病变）的诊断面临挑战，现有基于扩散的方法由于罕见病变样本稀缺，生成能力有限，导致诊断精度不足。需要一种能够有效增强罕见病变数据的方法。

Method: 1) 收集大量正常X光片训练扩散模型生成正常图像；2) 利用预训练扩散模型修复病变X光片中的头部病变区域，保留尾部病变作为增强训练数据；3) 引入大语言模型知识引导(LKG)模块和渐进增量学习(PIL)策略稳定修复微调过程。

Result: 在公开肺部数据集MIMIC和CheXpert上进行全面评估，证明该方法在性能上达到了新的基准水平。

Conclusion: 提出的数据合成管道能够有效增强长尾肺部异常诊断，通过利用大量正常X光片和创新的模型架构，显著提升了罕见病变的诊断精度。
...
TL;DR: 提出前馈多视角逆渲染框架，通过交替注意力机制直接预测材质和光照参数，并利用无标签真实视频进行一致性微调，实现多视角一致性和真实场景泛化。


<details>
  <summary>Details</summary>
Motivation: 现有单视角方法在多视角图像上忽略跨视角关系导致结果不一致，而多视角优化方法依赖缓慢的可微分渲染和逐场景优化，计算成本高且难以扩展。同时，现有合成数据集训练的模型难以泛化到真实世界场景。

Method: 提出前馈多视角逆渲染框架，通过交替注意力机制直接预测空间变化的反照率、金属度、粗糙度、漫反射着色和表面法线。采用基于一致性的微调策略，利用无标签真实世界视频增强多视角一致性和野外条件鲁棒性。

Result: 在基准数据集上的广泛实验表明，该方法在多视角一致性、材质和法线估计质量以及真实图像泛化方面达到最先进性能。
...
<details>
  <summary>Details</summary>
Motivation: 虽然自回归生成模型在NLP领域取得了革命性进展，但大多数视觉生成预训练方法仍依赖BERT风格的掩码建模，忽略了视频分析所需的时间信息。现有的少数自回归视觉预训练方法存在语义定位不准确和生成质量差的问题，导致语义理解能力不足。
...
Conclusion: NExT-Vid通过创新的自回归视觉生成预训练框架，成功解决了现有方法在语义定位和生成质量方面的不足，为视觉表示学习提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型依赖大规模标注数据且容易过拟合，现有数据增强方法缺乏结构感知能力，可能丢弃重要语义信息

Method: 提出粒球引导掩码(GBGM)方法，基于粒球计算进行结构感知，通过粗到细的分层掩码过程自适应保留语义丰富区域，抑制冗余区域

Result: 在多个基准测试中，分类准确率和掩码图像重建性能均获得一致提升，证明了方法的有效性和广泛适用性
...
TL;DR: FluencyVE是一种简单有效的一次性视频编辑方法，通过将Mamba时间序列模块集成到基于Stable Diffusion的视频编辑模型中，替代时间注意力层，在保持图像生成能力的同时减少计算负担。


<details>
  <summary>Details</summary>
Motivation: 大规模文本到图像扩散模型在图像生成和编辑方面取得了巨大成功，但将其扩展到视频编辑仍然具有挑战性。现有的视频编辑方法虽然通过添加时间注意力机制来适应视频任务，但仍存在时间不一致性和高计算开销的问题。

Method: 提出FluencyVE方法：1）将线性时间序列模块Mamba集成到基于预训练Stable Diffusion的视频编辑模型中，替代时间注意力层；2）使用低秩近似矩阵替换因果注意力中的查询和键权重矩阵；3）在训练期间使用加权平均技术更新注意力分数。

Result: 实验和分析表明，该方法在编辑真实世界视频的各种属性、主体和位置方面取得了有希望的结果，能够在保持文本到图像模型生成能力的同时有效减少计算负担。

Conclusion: FluencyVE是一种简单而有效的一次性视频编辑方法，通过集成Mamba模块和优化注意力机制，解决了现有视频编辑方法中的时间不一致性和高计算开销问题，为视频编辑任务提供了新的解决方案。
...
Method: 提出新颖高效的视频防御框架，包含相似性引导的参数共享机制以提高计算效率，以及多尺度双域注意力模块来联合优化空间-频率扰动，通过扰动3D信息获取过程来保护视频。

Result: 实验表明该框架具有强大的防御能力，相比最快基线实现47倍加速，同时保持高保真度，对缩放操作和最先进的净化攻击保持鲁棒性。

Conclusion: 该研究提出了首个针对3D场说话人脸生成方法的有效视频防御框架，通过创新的相似性引导参数共享和多尺度双域注意力设计，在保护隐私的同时保持了视频质量。
...
TL;DR: 韩国科技大学发布首个大规模全息图数据集KOREATECH-CGH，包含6000对RGB-D图像与复杂全息图对，分辨率覆盖256×256到2048×2048，并提出振幅投影技术提升大深度范围的全息图质量。


<details>
  <summary>Details</summary>
Motivation: 机器学习全息技术发展受限于高质量大规模全息数据集的缺乏，现有数据集规模小、质量有限，阻碍了ML-CGH系统的进一步发展。

Method: 1) 创建KOREATECH-CGH数据集：包含6000对RGB-D图像与复杂全息图，分辨率覆盖256×256到2048×2048，深度范围达到角谱方法的理论极限；2) 提出振幅投影技术：一种后处理方法，在保持相位不变的同时替换每个深度层的全息波场振幅分量，提升重建质量。

Result: 数据集显著提升全息图质量，达到27.01 dB PSNR和0.87 SSIM，比最新的优化轮廓掩模层方法分别高出2.03 dB和0.04 SSIM。通过在全息生成和超分辨率任务上的实验验证了数据集的实用性。

Conclusion: KOREATECH-CGH数据集填补了ML-CGH领域大规模高质量数据集的空白，振幅投影技术有效提升了大深度范围的全息图质量，为下一代ML-CGH系统的训练和评估提供了重要资源。
...
<details>
  <summary>Details</summary>
Motivation: 核范数作为秩函数的凸替代在低秩矩阵补全中应用广泛，但存在过度收缩奇异值的问题，导致次优解。需要更有效的非凸替代函数来更好地逼近秩函数。

Method: 提出新的重加权对数范数作为秩函数的非凸替代，比现有替代函数提供更接近的逼近。使用交替方向乘子法(ADMM)高效求解优化问题。

Result: 在图像修复实验中，该方法在视觉质量和量化指标上都优于最先进的低秩矩阵补全方法。

Conclusion: 提出的重加权对数范数作为秩函数的非凸替代是有效的，能够获得比核范数更好的低秩矩阵补全性能。
...
TL;DR: 提出了一种基于事件相机的光流引导6DoF物体姿态跟踪方法，通过2D-3D混合特征提取和光流引导的角点-边缘关联，实现精确的姿态跟踪。
...
Method: 采用2D-3D混合特征提取策略检测角点和边缘；通过最大化时空窗口内的事件关联概率搜索角点的光流；建立光流引导的角点-边缘关联；通过最小化角点与边缘距离迭代优化6DoF姿态。

Result: 在模拟和真实事件数据上的实验结果表明，该方法在准确性和鲁棒性方面优于现有的事件相机方法。

Conclusion: 提出的光流引导6DoF物体姿态跟踪方法有效利用了事件相机的优势，在复杂条件下实现了精确的姿态跟踪，为多媒体应用提供了新的解决方案。
...
Method: UniPath采用多流控制机制：1) 原始文本流；2) 高级语义流，使用可学习查询从冻结的病理MLLM中提取诊断语义标记和诊断感知属性包；3) 原型流，通过原型库实现组件级形态控制。同时构建了265万图像-文本语料库和6.8万精细标注子集。

Result: UniPath在病理图像生成任务上达到SOTA性能，Patho-FID为80.9（比第二名提升51%），细粒度语义控制达到真实图像的98.7%。建立了四层评估体系，并公开了数据集、源代码和预训练模型权重。

Conclusion: UniPath通过利用成熟的诊断理解能力实现可控的病理图像生成，解决了数据稀缺、语义控制和术语异质性等关键问题，为计算病理学提供了强大的生成工具。
...
<details>
  <summary>Details</summary>
Motivation: 解决多模态动作理解中有效利用模态互补性与保持模型效率的矛盾。现有方法要么计算开销大（晚融合），要么性能不佳（早融合）

Method: 提出分解与组合策略：分解策略将融合的多模态特征分解为单模态特征并与真实单模态特征对齐；组合策略整合多个单模态特征作为自监督指导增强多模态表示学习

Result: 在NTU RGB+D 60、NTU RGB+D 120和PKU-MMD II数据集上的实验表明，该方法在计算成本和模型性能间取得了优异平衡

Conclusion: Decomposition and Composition框架有效解决了多模态动作理解中效率与效果的权衡问题，为自监督多模态表示学习提供了新思路
...
TL;DR: 提出了一种利用非因果注意力捕捉完整表格结构的新型多任务模型，以及用于更快单元格内容推理的并行推理算法，在大型公共数据集上取得了优越性能。
...
Conclusion: 该方法在表格识别任务中表现出色，不仅提高了识别准确性，还显著减少了推理时间，为文档知识提取提供了更高效的解决方案。
...
Method: 开发了T2AV-Compass基准，包含500个通过分类学驱动流程构建的多样化复杂提示；引入双级评估框架，结合客观信号级指标（视频质量、音频质量、跨模态对齐）和主观MLLM-as-a-Judge协议（指令遵循和真实性评估）。

Result: 对11个代表性T2AV系统的广泛评估显示，即使最强的模型也远未达到人类级别的真实性和跨模态一致性，在音频真实性、细粒度同步、指令遵循等方面存在持续失败。

Conclusion: T2AV-Compass作为一个具有挑战性和诊断性的测试平台，突显了未来模型的显著改进空间，对推进文本到音视频生成技术的发展具有重要价值。
...
<details>
  <summary>Details</summary>
Motivation: 文档中的文本和公式是核心信息组成部分，现有视觉语言模型虽然能统一识别两者，但参数量大、计算需求高，限制了实际应用。需要开发轻量级但性能强大的统一识别模型。

Method: 1) 构建UniRec40M大规模数据集（4000万文本、公式及混合样本）；2) 针对层次结构变异性和文本公式语义纠缠问题，提出分层监督训练和语义解耦分词器；3) 建立涵盖中英文多领域多层次的综合评估基准。

Result: UniRec-0.1B在综合基准和公开基准测试中，性能优于通用视觉语言模型和领先的文档解析专家模型，同时实现2-9倍的速度提升。

Conclusion: UniRec-0.1B是一个高效且有效的轻量级统一识别模型，通过大规模数据集、分层监督训练和语义解耦分词器，成功解决了文本和公式的统一识别问题，在保持高性能的同时大幅提升了推理速度。
...
TL;DR: FreeInpaint是一种即插即用的免调优文本引导图像修复方法，通过在推理时直接优化扩散潜变量来提升生成图像的真实性
...
Conclusion: FreeInpaint是一种有效的免调优方法，能显著提升文本引导图像修复的提示对齐和视觉合理性
...
Result: 实验结果表明，现有VLMs无法有效回答领域特定的海洋问题，性能提升空间很大。模型在需要海洋专业知识的问题上表现不佳。

Conclusion: 提出了首个海洋VLM基准测试MarineEval，揭示了现有模型在海洋领域的局限性，为未来研究提供了评估框架和方向。
...
TL;DR: TGC-Net：基于CLIP的文本引导医学分割框架，通过参数高效的任务特定适配解决医学图像与临床文本对齐问题，在多个数据集上实现SOTA性能
...
Conclusion: TGC-Net通过参数高效的任务特定适配有效解决了CLIP在医学图像分割中的局限性，为文本引导医学分割提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 海洋视觉理解对于监测和保护海洋生态系统至关重要，但当前研究受到训练数据有限和缺乏系统化任务定义的阻碍，无法将海洋领域特定挑战与明确定义的计算机视觉任务对齐。

Method: 创建ORCA多模态基准数据集，包含精细的视觉和文本标注，捕捉不同海洋物种的形态学特征。在三个任务上评估18个最先进模型：目标检测（封闭集和开放词汇）、实例描述和视觉定位。

Result: 评估结果突出了海洋理解的关键挑战，包括物种多样性、形态重叠和特定领域需求，表明海洋视觉理解具有较高难度。

Conclusion: ORCA建立了一个全面的基准，为海洋领域研究提供标准化评估框架，将推动海洋视觉理解的方法学进展。
...
TL;DR: 提出了一种基于隐式神经表示的人类运动序列连续表示方法NAME，能够以任意帧率对输入运动序列进行插值、中间帧生成甚至外推。


<details>
  <summary>Details</summary>
Motivation: 物理运动本质上是连续的，更高的相机帧率通常能提高平滑度和时间一致性。目前缺乏能够以任意帧率对运动序列进行连续表示、插值和外推的方法。

Method: 提出NAME框架，基于隐式神经表示，采用分层时间编码机制从多时间尺度提取特征，并集成基于傅里叶变换的参数化激活函数到MLP解码器中，增强连续表示的表达能力。

Result: 在多个基准数据集上的广泛评估表明，该方法在表示复杂运动行为方面具有高精度，证明了其有效性和鲁棒性。

Conclusion: 首次探索了人类运动序列的连续表示，提出的NAME框架能够以任意帧率对运动序列进行插值、中间帧生成和外推，为运动分析提供了新的连续表示方法。
...
TL;DR: UltraShape 1.0是一个可扩展的3D扩散框架，采用两阶段生成流程：先生成粗略全局结构，再细化生成高质量几何细节。通过创新的数据处理管道和空间定位与几何细节解耦的扩散方法，在有限训练资源下实现高质量3D几何生成。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成方法在几何质量和细节保留方面存在不足，特别是使用公开数据集时数据质量参差不齐。需要开发一个既能处理低质量数据又能生成高保真几何的框架。

Method: 1. 两阶段生成流程：粗略全局结构生成 + 细节细化；2. 综合数据处理管道：包括新颖的水密处理方法和高质量数据过滤；3. 空间定位与几何细节解耦：使用基于体素的细化，在固定空间位置通过RoPE编码位置锚点，让扩散模型专注于局部几何细节合成。

Result: 模型在公开3D数据集上训练，在有限训练资源下实现了强大的几何质量。广泛评估表明，UltraShape 1.0在数据处理质量和几何生成方面与现有开源方法具有竞争力。

Conclusion: UltraShape 1.0提供了一个有效的3D扩散框架，通过创新的数据处理和生成策略，能够在资源有限的情况下实现高质量的3D几何生成，为未来研究提供了有价值的工具和模型。
...
TL;DR: VisRes Bench是一个用于评估视觉语言模型在无语言监督下的视觉推理能力的基准测试，包含三个复杂度级别，揭示了当前先进模型在感知扰动下表现接近随机，视觉推理能力有限。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在视觉问答和图像描述等任务上取得了显著进展，但尚不清楚这些模型是在进行真正的视觉推理还是主要依赖语言先验。需要设计一个基准来研究模型在自然场景中的视觉推理能力，而不依赖上下文语言监督。

Method: 提出了VisRes Bench基准，包含三个复杂度级别：Level 1测试感知补全和全局图像匹配（在模糊、纹理变化、遮挡、旋转等扰动下）；Level 2测试基于单一属性（如颜色、数量、方向）的规则推理；Level 3测试需要整合多个视觉属性的组合推理。使用超过19,000张受控任务图像进行分析。

Result: 研究发现，最先进的视觉语言模型在细微的感知扰动下表现接近随机水平，显示出有限的抽象能力，超越了模式识别但未能达到真正的视觉推理。VisRes Bench能够分离不同层次的推理能力。

Conclusion: VisRes Bench为多模态研究中推进抽象视觉推理提供了一个统一的框架，揭示了当前模型在视觉推理方面的局限性，为未来研究指明了方向。
...
TL;DR: EveryWear是一个基于日常可穿戴设备（智能手机、智能手表、耳机、智能眼镜）的轻量级人体运动捕捉系统，无需显式校准，通过多模态师生框架整合视觉和惯性信号，在真实世界数据集上训练，优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有的人体运动估计方法存在佩戴不便、硬件昂贵、校准繁琐等问题，阻碍了其在日常生活中的应用。需要一种更轻便、实用且无需显式校准的解决方案。

Method: 提出EveryWear系统，完全基于日常可穿戴设备（智能手机、智能手表、耳机、智能眼镜），使用多模态师生框架整合智能眼镜的视觉信号（一个前向和两个下向摄像头）与其他设备的惯性信号。创建了Ego-Elec数据集（9小时真实世界数据，56种日常活动，17个不同环境），直接在真实数据而非合成数据上训练，避免了模拟到真实的差距。

Result: 实验表明，该方法优于基线模型，验证了其在实际全身运动估计中的有效性。

Conclusion: EveryWear提供了一种轻量级、实用的人体运动捕捉方法，基于日常可穿戴设备，无需显式校准，通过整合视觉和惯性信号并在真实数据上训练，有效解决了现有方法的局限性。
...
TL;DR: 提出了一种任务无关的机制，让大型多模态模型能够自主发现和使用视觉推理标记，无需显式监督，从而在视觉中心任务上取得优异表现。
...
Method: 提出任务无关机制，训练LMMs自主发现和使用视觉推理标记，无需显式监督。这些标记能够全局关注并以任务自适应方式重新编码图像，使模型能够提取相关视觉信息而无需手工监督。

Result: 该方法在直接微调的基础上表现更优，在多种视觉中心任务上取得了最先进的结果，包括那些中间抽象难以指定的任务，同时也能泛化到多任务指令调优。

Conclusion: 通过让模型自主发现视觉推理标记，可以克服现有方法的限制，实现更好的视觉推理能力，特别是在难以指定中间抽象的任务上表现突出。
...
TL;DR: 本文提出了一种轻量级的两阶段检索方法，通过事件中心实体提取结合BM25过滤和BEiT-3重排序，在OpenEvents v1基准上取得了0.559的mAP，显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现实世界的图像-文本检索面临模糊或上下文相关的查询、语言变异性以及可扩展性需求等挑战，需要更有效的解决方案来处理复杂场景。

Method: 提出两阶段检索流水线：第一阶段使用基于显著实体的BM25进行高效候选过滤；第二阶段应用BEiT-3模型捕获深度多模态语义并重排序结果，利用事件中心实体提取融入时间上下文信号。

Result: 在OpenEvents v1基准测试中，该方法取得了0.559的平均精度均值，显著优于先前基线方法，证明了事件引导过滤与长文本视觉语言建模结合的有效性。

Conclusion: 结合事件引导过滤和长文本视觉语言建模的方法能够在复杂现实场景中实现准确高效的图像检索，为实际应用提供了有效的解决方案。
...
TL;DR: SegMo是一个基于文本生成3D人体动作的新框架，通过将文本描述和动作序列分解为语义连贯的片段，实现细粒度的文本-动作对齐，相比序列级对齐方法有显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有方法在文本描述和人体动作之间进行序列级对齐，忽略了模态内部的语义结构。文本描述和动作序列都可以自然分解为更小、语义连贯的片段，这些片段可以作为原子对齐单元实现更细粒度的对应关系。

Method: SegMo框架包含三个模块：(1)文本片段提取：将复杂文本描述分解为时间顺序的短语，每个短语代表一个简单的原子动作；(2)动作片段提取：将完整动作序列分割为对应的动作片段；(3)细粒度文本-动作对齐：通过对比学习对齐文本和动作片段。

Result: 在HumanML3D等两个广泛使用的数据集上，SegMo显著改进了强基线，在HumanML3D测试集上达到了0.553的TOP 1分数。此外，由于学习了文本和动作片段的共享嵌入空间，SegMo还可应用于动作定位和动作到文本检索等检索式任务。

Conclusion: 通过将文本和动作分解为语义片段并进行细粒度对齐，SegMo框架在文本条件人体动作生成任务上取得了显著改进，同时为相关检索任务提供了有效的共享表示空间。
...
<details>
  <summary>Details</summary>
Motivation: 电影制作中的"一镜到底"技术具有独特美学价值，但实际实现成本高昂且受现实约束。现有视频生成模型通常依赖简单的片段拼接，难以保持视觉平滑性和时间一致性，因此需要开发能够从多样化用户输入生成高质量一镜到底视频的框架。

Method: 1. 在DiT架构中集成轻量级中间条件机制，采用自适应调优策略利用基础训练数据，实现鲁棒的任意帧控制能力；2. 通过高质量数据集和视觉表达监督微调阶段提升视觉保真度和电影表达力，使用定制DPO方案解决主体运动合理性和过渡平滑性问题；3. 设计分段自回归推理策略，以内存高效方式生成长序列视频。

Result: 实验表明，该方法能够实现视觉震撼且无缝连贯的一镜到底效果，同时保持计算效率，使用户能够将碎片化的视觉材料转化为生动、连贯的一镜到底电影体验。

Conclusion: DreaMontage框架成功解决了生成高质量一镜到底视频的关键挑战，通过创新的技术组合实现了视觉平滑性、时间一致性和计算效率的平衡，为虚拟一镜到底视频制作提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 脑MRI异常检测面临两个主要挑战：标注异常病例稀缺和临床工作流中经常缺少关键成像模态。现有方法通常依赖固定模态配置，需要重复训练，或无法泛化到未见过的模态组合，限制了临床可扩展性。

Method: 1. 集成双通路DINOv2编码器与特征分布对齐机制，统计对齐不完整模态特征与完整模态表示；2. 引入内在正常原型提取器和INP引导解码器，仅重建正常解剖模式；3. 通过随机模态掩码和间接特征补全训练，使模型适应所有模态配置而无需重新训练。
...
TL;DR: 提出了一种新的图像序列生成方法，通过将生成过程分解为低分辨率序列生成和高分辨率单帧细化，避免了直接处理大型张量，实现了更高质量、更连贯且更高效的序列生成。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的图像序列生成方法通常将图像序列视为顺序堆叠帧的大型张量，这种方法存在效率低下和瓶颈问题。作者旨在设计一种更有效的图像序列数据建模方式。

Method: 将生成过程分解为两个阶段：首先在低分辨率下生成粗粒度序列（使用扩散变换器DiT的自注意力机制捕捉帧间相关性），然后对每个帧单独进行超分辨率处理以添加高分辨率细节。该方法无需修改架构即可将2D图像生成器扩展为低分辨率3D图像序列生成器。

Result: 相比现有图像序列生成模型，该方法实现了更优的合成质量和序列连贯性，能够生成任意长度的高保真序列，在推理时间和训练数据使用上效率更高。在多个数据集上始终优于最先进方法，推理速度至少快两倍。

Conclusion: 通过将图像序列生成分解为低分辨率序列生成和高分辨率单帧细化，该方法克服了当前最先进方法的局限性，实现了更高质量、更高效且更具泛化能力的图像序列生成。
...
TL;DR: 本文提出了一种新颖的多尺度信号处理方法，用于从表格掩码中检测表格边缘，通过高斯卷积和统计阈值处理提高表格结构分割的准确性。


<details>
  <summary>Details</summary>
Motivation: 从表格中提取结构化数据在文档图像分析中至关重要，但现有方法在低分辨率或噪声图像中准确识别表格段边界（行和列）仍面临挑战。基于掩码的边缘检测技术虽然对噪声条件更鲁棒，但现有方法直接将掩码应用于图像，导致噪声敏感性、分辨率损失或高计算成本。

Method: 提出一种多尺度信号处理方法：将行和列转换建模为一维信号，使用方差逐渐增加的高斯卷积进行处理，然后通过统计阈值处理抑制噪声同时保留稳定的结构边缘。检测到的信号峰值映射回图像坐标以获得准确的段边界。

Result: 实验结果表明，在PubLayNet-1M基准测试中，将所提方法应用于列边缘检测，使用TableNet与PyTesseract OCR时，Cell-Aware Segmentation Accuracy (CASA)从67%提高到76%。该方法通过零填充和缩放策略对分辨率变化具有鲁棒性。

Conclusion: 该方法能够产生优化的结构化表格输出，适用于下游分析，在表格结构提取任务中表现出优越的性能和鲁棒性。
...
Method: TICON使用单个共享编码器，通过掩码建模目标进行预训练，同时统一和上下文化来自不同切片级病理基础模型的表示。该方法能够处理来自任何切片级基础模型的嵌入，并为这些嵌入添加丰富的上下文信息。

Result: TICON上下文化嵌入在多种任务中显著提升性能，在切片级基准测试（HEST-Bench、THUNDER、CATCH）和切片级基准测试（Patho-Bench）上建立了新的最先进结果。此外，在TICON上预训练的聚合器仅使用11K WSIs就形成了切片级基础模型，性能优于使用多达350K WSIs预训练的最先进切片级基础模型。

Conclusion: TICON提供了一个统一的框架，能够为计算病理学中的任何应用生成上下文丰富的切片嵌入，显著提升各种任务的性能，同时通过高效的预训练方法在资源有限的情况下实现卓越的切片级表示学习。
...
TL;DR: 本文提出了一种文本引导的token剪枝框架，用于提高SAM2视频对象分割的推理效率，通过选择性减少时间传播前的token密度，实现了42.5%的推理加速和37.41%的GPU内存降低。


<details>
  <summary>Details</summary>
Motivation: SAM2等视觉基础模型在处理视频对象分割时面临高计算和内存成本的问题，因为它们在时间传播过程中处理所有视觉token，无论这些token是否与目标对象相关，这导致了二次内存注意力开销和可扩展性受限。

Method: 提出了一种文本引导的token剪枝框架，在视觉编码之后、基于内存的传播之前运行。该方法使用轻量级路由机制对token进行排序，整合了局部视觉上下文、从对象中心文本描述（用户提供或自动生成）中提取的语义相关性，以及有助于保留模糊或边界关键区域的不确定性线索。通过仅保留最具信息量的token进行下游处理，减少了冗余计算。

Result: 在多个具有挑战性的视频分割基准测试中，后编码器token剪枝方法实现了高达42.50%的推理加速和37.41%的GPU内存使用降低，同时保持了竞争力的J和F性能指标。

Conclusion: 早期token选择有潜力提高基于transformer的视频分割系统的可扩展性，适用于实时和资源受限的应用场景。文本引导的token剪枝为高效、提示感知的视频分割提供了一条实用且有效的途径。
...
TL;DR: Streamo是一个实时流媒体视频LLM，作为通用交互助手，能够执行广泛的流媒体视频任务，包括实时解说、动作理解、事件标注、时间事件定位和时间敏感问答。


<details>
  <summary>Details</summary>
Motivation: 现有在线视频模型主要专注于问答或字幕生成等狭窄任务，缺乏能够处理多种流媒体视频任务的通用实时助手，需要弥合离线视频感知模型与实时多模态助手之间的差距。

Method: 构建了Streamo-Instruct-465K大规模指令跟随数据集，涵盖多样时间上下文和多任务监督；通过简化管道进行端到端训练，实现异构流媒体任务的统一训练。

Result: Streamo展现出强大的时间推理能力、响应式交互能力以及在各种流媒体基准测试中的广泛泛化能力，实验证明其在多个流媒体任务上表现优异。
...
TL;DR: 研究发现视觉语言模型存在显著流行度偏见，在知名建筑上准确率比普通建筑高34%，表明模型依赖记忆而非泛化理解。为此创建了YearGuessr数据集，包含55,546个建筑图像，用于构建年份预测任务。


<details>
  <summary>Details</summary>
Motivation: 揭示当前最先进的视觉语言模型存在严重的流行度偏见，模型在知名建筑上表现优异，但在普通建筑上表现不佳，这表明模型更多依赖记忆而非真正的理解能力，需要系统性地量化这种偏见。

Method: 构建了YearGuessr数据集（55,546个建筑图像，来自157个国家），包含建造年份（1001-2024）、GPS数据和页面浏览量（作为流行度指标）。将建造年份预测任务构建为序数回归问题，并引入流行度感知的区间准确度指标来量化偏见。

Result: 评估了30多个模型（包括提出的YearCLIP模型），发现视觉语言模型在流行、被记忆的项目上表现出色，但在未被识别的主题上表现显著较差，确认了模型存在严重的流行度偏见。

Conclusion: 视觉语言模型存在严重的流行度偏见，依赖记忆而非泛化理解能力，这暴露了其推理能力的关键缺陷。YearGuessr数据集和提出的评估方法为系统研究这种偏见提供了基准。
...
Method: HiStream采用三轴压缩策略：1)空间压缩：先在低分辨率去噪，再利用缓存特征在高分辨率细化；2)时间压缩：分块处理策略配合固定大小的锚点缓存；3)时间步压缩：对后续缓存条件块应用更少的去噪步骤。

Result: 在1080p基准测试中，主要模型HiStream(i+ii)达到最先进的视觉质量，去噪速度比Wan2.1基线快76.2倍且质量损失可忽略；HiStream+(i+ii+iii)实现107.5倍加速，在速度和质量间提供良好权衡。
...
TL;DR: 本文提出Milaps框架，通过最小延迟问题建模来解决连续环境中的移动搜索任务，相比现有方法在解质量和运行时间之间取得了更好的平衡。


<details>
  <summary>Details</summary>
Motivation: 移动搜索任务中，全局路径优化在静态2D连续环境中研究不足，主要因为目标函数评估困难（环境连续性和运动/可见性约束的交互）。现有方法要么采用部分离散化，要么通过固定图上的最小延迟问题启发式近似目标函数。

Method: 提出Milaps框架，整合了新的辅助目标函数，并适配了旅行送货员问题的元启发式算法（因其在严格运行时约束下的优异表现）。最佳策略快速生成初步解，为感知配置分配静态权重，并通过元启发式优化全局成本。

Result: 在大规模新数据集上的评估显示，相比最先进的基线方法，Milaps在解质量和运行时间之间取得了更优的权衡。定性研究还展示了该框架在不同场景下的灵活性。

Conclusion: Milaps框架通过最小延迟问题建模，为连续环境中的移动搜索任务提供了有效的全局路径优化解决方案，在解质量和计算效率方面优于现有方法。
...
TL;DR: 提出一个通用视觉框架，用于自主拦截动态非合作目标，在三种移动平台上验证：无人机、地面漫游车和太空推进器测试台，仅使用单目摄像头和基准标记，无需全局信息。


<details>
  <summary>Details</summary>
Motivation: 解决异构移动系统在有限可观测性和无全局定位条件下的自主拦截问题，开发一个能适应不同机器人动力学、在部分视野、传感器丢失和目标遮挡等限制条件下工作的通用框架。

Method: 集成三个核心组件：(1) 扩展卡尔曼滤波器处理间歇测量下的相对位姿估计，(2) 历史条件运动预测器用于动态目标轨迹传播，(3) 实时求解约束凸规划的滚动时域规划器，确保时间高效且运动学可行的拦截路径。

Result: 在模拟和物理实验中表现出稳健性能，拦截误差低（在保持位置和场景完成时），在确定性和随机目标运动模式下成功率高，能在Jetson Orin、VOXL2和Raspberry Pi 5等嵌入式处理器上实时执行。

Conclusion: 该框架在无人机动态目标着陆、漫游车交会和跟随任务、以及航天器近距离操作等应用中展示了通用性、鲁棒性和计算效率，为异构移动系统的自主拦截问题提供了全面解决方案。
...
TL;DR: YCB-Handovers数据集收集了2771个人类-人类物品交接的运动数据，包含不同物体重量，旨在为机器人协作研究提供重量感知的运动规划参考。


<details>
  <summary>Details</summary>
Motivation: 当前人类-机器人协作研究中缺乏考虑物体重量影响的交接数据集，需要建立包含重量变化的人类交接行为数据集，以支持更直观、自适应的机器人运动规划。

Method: 基于YCB标准数据集，收集2771个人类-人类交接运动数据，涵盖广泛重量范围，分析重量对交接行为的影响，特别关注需要谨慎交接的物体。

Result: 创建了包含重量变化的YCB-Handovers数据集，提供了物体重量对人类交接运动影响的详细分析，揭示了重量变化对交接行为的影响模式。

Conclusion: YCB-Handovers数据集填补了机器人协作研究中重量感知交接数据的空白，为数据驱动的人类启发式模型和自适应机器人行为提供了重要资源。
...
TL;DR: 该论文提出了一种基于临界减速现象的系统安全监测方法，无需精确系统模型，通过动态韧性指标预测反馈系统的不稳定性，并在无人机上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 传统工程方法依赖精确系统模型计算安全操作指令，但当系统（可能受损）偏离模型时会失效。增量性损伤会使控制系统变得脆弱，这种脆弱性往往直到小扰动引发不稳定（即失控）时才被发现。

Method: 采用基于临界减速现象的韧性动态指标来监测系统向不稳定性的趋近。这种方法不依赖系统模型，利用复杂非线性系统接近临界点时出现的临界减速这一通用现象。

Result: 在无人机上验证了该方法的有效性。这种基于通用原理的方法表明，这些指标可适用于更广泛的受控系统，包括反应堆、飞机和自动驾驶汽车。

Conclusion: 该方法为实时早期预警系统和韧性系统设计探索提供了新途径，具有广泛的应用前景，可用于监测各类反馈系统的稳定性。
...
TL;DR: 该研究评估视觉语言模型在机器人运动理解方面的能力，通过视频描述任务测试模型对机器人任务自动描述和任务序列分割的能力，提出利用机器人运动数据提升VLM性能的方法。


<details>
  <summary>Details</summary>
Motivation: 从机器人未来发展角度，需要验证仅基于离线数据（如图像和语言）训练的基础模型是否能理解机器人运动。由于视觉语言模型训练数据中不包含机器人低级运动信息，理解包含轨迹信息的视频仍然是一个重大挑战。

Method: 提出通过视频描述任务评估VLM的两种能力：1）机器人任务自动描述；2）任务序列分割。方法利用图像描述和机器人轨迹数据生成多个"场景"描述，然后总结生成完整任务描述。同时通过比较图像描述文本嵌入的相似性进行子任务分割。在两种描述任务中，通过向VLM提供机器人运动数据（关节和末端执行器状态）来提升性能。

Result: 通过模拟器实验验证了所提方法的有效性，表明提供机器人运动数据可以改善视觉语言模型在机器人任务理解和分割方面的性能。

Conclusion: 该研究为评估基础模型在机器人运动理解方面的能力提供了框架，通过结合机器人运动数据，可以增强视觉语言模型在机器人任务描述和分割方面的表现，从而提高机器人模仿学习的效率。
...
### [55] [Stretchable and High-Precision Optical Tactile Sensor for Trajectory Tracking of Parallel Mechanisms](https://arxiv.org/abs/2512.20888)
*Yiding Nie,Dongliang Fan,Jiatai Huang,Chunyu Liu,Jian S. Dai*

Main category: cs.RO

TL;DR: 提出了一种基于连续光谱滤波原理的可拉伸触觉传感器，实现了超高空间分辨率，在拉伸和弯曲条件下仍能保持高线性响应，并展示了在平面并联机构中的实时精确轨迹跟踪应用。
...
Method: 开发了一种基于连续光谱滤波原理的可拉伸触觉传感器，该传感器具有设计可扩展性和交互鲁棒性，能够承受穿刺和切割等损伤。

Result: 传感器实现了高线性空间响应（0.996），即使在拉伸和弯曲条件下；获得了高连续空间分辨率（7 μm）和力分辨率（5 mN）；在平面并联机构中实现了实时精确轨迹跟踪（旋转分辨率：0.02°）。
...
### [56] [Certifiable Alignment of GNSS and Local Frames via Lagrangian Duality](https://arxiv.org/abs/2512.20931)
*Baoshan Song,Matthew Giamou,Penggao Yan,Chunxi Xia,Li-Ta Hsu*

Main category: cs.RO

TL;DR: 提出一种全局最优的GNSS帧对齐方法，通过凸松弛将原始非凸QCQP问题转化为可验证的优化问题，即使在只有2颗卫星和2D运动的情况下也能提供可验证的最优解。


<details>
  <summary>Details</summary>
Motivation: 现有GNSS帧对齐方法在卫星信号受限环境中表现不佳，依赖大量卫星或使用无法保证最优性的局部优化方法，需要一种能在GNSS信号退化环境下提供可验证最优解的方法。

Method: 将原始帧对齐问题建模为非凸QCQP问题，通过凸松弛转化为凹拉格朗日对偶问题，提供原始问题的下界成本，并进行松弛紧致性和可观测性分析以推导可验证最优性标准。

Result: 实验表明该方法在仅使用2颗卫星多普勒测量和2D车辆运动的情况下仍能提供可验证的最优解，而传统速度基VOBA方法和先进GVINS对齐技术可能失败或收敛到局部最优而不自知。

Conclusion: 提出了一种全局最优且可验证的GNSS帧对齐方法，填补了现有局部优化器无法验证结果正确性的空白，为机器人GNSS导航技术的发展提供了可靠工具。
...
### [57] [ETP-R1: Evolving Topological Planning with Reinforcement Fine-tuning for Vision-Language Navigation in Continuous Environments](https://arxiv.org/abs/2512.20940)
*Shuhao Ye,Sitong Mao,Yuxiang Cui,Xuan Yu,Shichao Zhai,Wen Chen,Shunbo Zhou,Rong Xiong,Yue Wang*

Main category: cs.RO

TL;DR: ETP-R1框架通过大规模数据预训练和强化微调，将图基VLN-CE模型的性能提升到新水平，在R2R-CE和RxR-CE基准测试中创下最佳表现。


<details>
  <summary>Details</summary>
Motivation: 当前基于图的VLN-CE方法虽然通过拓扑地图抽象提供了高效的结构化方法，但在利用大规模数据和先进训练范式方面落后于基于大视觉语言模型的方法。本文旨在弥合这一差距。

Method: 1. 使用Gemini API构建高质量、大规模预训练数据集，包含多样化的拓扑轨迹指令；2. 统一R2R和RxR任务数据进行联合预训练；3. 采用三阶段训练范式，首次将闭环在线强化微调应用于图基VLN-CE模型，使用GRPO算法。

Result: 该方法在R2R-CE和RxR-CE基准测试的所有主要指标上都达到了新的最先进性能。

Conclusion: ETP-R1框架成功地将大规模数据预训练和强化微调范式应用于图基VLN-CE模型，显著提升了性能，证明了这种方法的有效性。
...
### [58] [Multimodal Sensing for Robot-Assisted Sub-Tissue Feature Detection in Physiotherapy Palpation](https://arxiv.org/abs/2512.20992)
*Tian-Ao Ren,Jorge Garcia,Seongheon Hong,Jared Grinberg,Hojung Choi,Julia Di,Hao Li,Dmitry Grinberg,Mark R. Cutkosky*

Main category: cs.RO

TL;DR: 提出了一种集成了高分辨率视觉触觉成像和6轴力扭矩传感器的紧凑型多模态传感器，用于改善机器人触诊中软组织下表面特征的检测


<details>
  <summary>Details</summary>
Motivation: 机器人触诊依赖力传感，但在软组织环境中力信号变化大，无法可靠地揭示细微的下表面特征。需要更可靠的传感方法来检测软组织中的结构差异

Method: 开发了一种紧凑型多模态传感器，集成了高分辨率视觉触觉成像和6轴力扭矩传感器。在具有不同下表面肌腱几何形状的硅胶模型上进行实验

Result: 单独的力信号经常产生模糊响应，而触觉图像能清晰揭示存在、直径、深度、交叉和多重性等结构差异。准确的力跟踪对于维持安全、一致的接触仍然至关重要

Conclusion: 结合触觉和力传感模态能够实现稳健的下表面特征检测和受控的机器人触诊。多模态传感方法比单一力传感更有效
...
### [59] [Tracing Energy Flow: Learning Tactile-based Grasping Force Control to Prevent Slippage in Dynamic Object Interaction](https://arxiv.org/abs/2512.21043)
...
<details>
  <summary>Details</summary>
Motivation: 动态物体交互中调节抓握力以防止滑移是机器人操作的基本挑战，特别是在多滚动接触、物体属性未知、外部传感不可靠的情况下。人类仅凭触觉就能快速调节抓握力，受此启发，研究旨在让机器人手也能实现类似的触觉驱动抓握力控制

Method: 提出物理信息能量抽象，将物体建模为虚拟能量容器；手指施加功率与物体保留能量之间的不一致性提供了物理基础的滑移感知稳定性信号；基于此抽象，采用基于模型的学习和规划，从触觉传感中高效建模能量动力学，并进行实时抓握力优化

Result: 仿真和硬件实验表明，该方法能在几分钟内从零开始学习抓握力控制，有效减少滑移，延长不同运动-物体对的抓握持续时间，且不依赖外部传感或物体先验知识

Conclusion: 通过物理信息能量抽象和基于模型的学习规划，实现了机器人手快速学习触觉驱动抓握力控制的能力，为动态物体交互中的滑移减少提供了有效解决方案
...
### [60] [Quadrupped-Legged Robot Movement Plan Generation using Large Language Model](https://arxiv.org/abs/2512.21293)
*Muhtadin,Vincentius Gusti Putu A. B. M.,Ahmad Zaini,Mauridhi Hery Purnomo,I Ketut Eddy Purnama,Chastine Fatichah*

Main category: cs.RO

TL;DR: 本文提出了一种基于大语言模型的四足机器人自然语言控制框架，通过分布式架构将高级指令处理卸载到外部服务器，在真实室内环境中实现了超过90%的成功率。
...
Conclusion: 验证了基于卸载式LLM规划在真实世界环境中自主四足机器人部署的可行性，为降低机器人操作门槛提供了有效解决方案。
...
### [61] [Language-Guided Grasp Detection with Coarse-to-Fine Learning for Robotic Manipulation](https://arxiv.org/abs/2512.21065)
*Zebin Jiang,Tianle Jin,Xiangtong Yao,Alois Knoll,Hu Cao*

Main category: cs.RO

TL;DR: LGGD提出了一种从粗到细的语言引导抓取检测方法，通过分层跨模态融合和语言条件动态卷积头，实现了视觉语义对齐和任务指令自适应抓取预测，在多个数据集和真实机器人平台上表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前语言引导抓取方法通常采用浅层融合策略，导致语义基础薄弱，语言意图与视觉抓取推理之间的对齐不足。在非结构化、杂乱且语义多样的环境中，机器人需要更好地理解和执行自然语言指令。

Method: 提出LGGD方法：1）采用从粗到细的学习范式；2）利用CLIP视觉和文本嵌入构建分层跨模态融合管道，逐步注入语言线索到视觉特征重建过程；3）引入语言条件动态卷积头，基于句子级特征混合多个卷积专家；4）最终细化模块增强复杂场景中的抓取一致性和鲁棒性。

Result: 在OCID-VLG和Grasp-Anything++数据集上，LGGD超越了现有语言引导抓取方法，对未见过的物体和多样化语言查询表现出强大的泛化能力。在真实机器人平台上的部署验证了该方法在执行准确、指令条件抓取动作方面的实际有效性。

Conclusion: LGGD通过分层跨模态融合和语言自适应机制，实现了精细的视觉语义对齐，提高了抓取预测与任务指令的一致性，为机器人操作中的语言引导抓取提供了有效的解决方案。
...
### [62] [Global End-Effector Pose Control of an Underactuated Aerial Manipulator via Reinforcement Learning](https://arxiv.org/abs/2512.21085)
*Shlok Deshmukh,Javier Alonso-Mora,Sihao Sun*

Main category: cs.RO

TL;DR: 本文研究了一种基于差动机构的轻量级2自由度机械臂安装在四旋翼无人机上的系统，通过强化学习控制实现六自由度末端执行器位姿控制，在保持简单轻量设计的同时解决了欠驱动和外部扰动问题。


<details>
  <summary>Details</summary>
Motivation: 空中机械臂（机械臂与多旋翼无人机结合）面临严格的臂重和机械复杂度限制。轻量级2自由度机械臂设计虽然简化了结构并减少了有效载荷，但引入了欠驱动和对外部扰动敏感等挑战，特别是在操纵重负载和推压任务中。

Method: 采用强化学习方法，在仿真环境中训练近端策略优化（PPO）智能体，生成四旋翼加速度和机体角速度的前馈指令以及关节角度目标。这些指令分别由增量非线性动态反演（INDI）姿态控制器和PID关节控制器进行跟踪。

Result: 飞行实验展示了厘米级的位置精度和度级的姿态精度，在外部力扰动下表现出鲁棒性能。系统能够实现接触丰富的空中操纵任务。

Conclusion: 研究结果表明，基于学习的控制策略能够使简单轻量平台实现接触丰富的空中操纵，为轻量级空中机械臂系统提供了有效的解决方案。
...
### [63] [Robust and Efficient MuJoCo-based Model Predictive Control via Web of Affine Spaces Derivatives](https://arxiv.org/abs/2512.21109)
*Chen Liang,Daniel Rakita*

Main category: cs.RO

TL;DR: 本文介绍了在MuJoCo MPC（MJPC）中使用WASP导数作为有限差分法的替代方案，以提升实时控制性能，特别是在高自由度系统和复杂场景中。


<details>
  <summary>Details</summary>
Motivation: MJPC依赖有限差分法计算导数，这在时间敏感任务中成为瓶颈，尤其在高自由度系统或复杂场景中计算成本过高，需要更高效的导数计算方法。

Method: 将WASP（Web of Affine Spaces）导数集成到MJPC中作为有限差分法的替代方案。WASP通过重用先前相关导数计算的信息，加速和稳定新导数的计算，特别适合MPC的迭代细粒度更新。

Result: WASP在多种MJPC任务中表现优异：无缝集成到各任务中，提供稳定性能，与基于导数的规划器（如iLQG）配合使用时，相比有限差分法后端实现高达2倍加速。此外，基于WASP的MPC在评估任务中优于MJPC的随机采样规划器，提供更高效率和可靠性。

Conclusion: WASP导数在MJPC中特别有效，能够显著提升计算效率，作者发布了集成WASP导数的开源MJPC实现以支持采用和未来研究。
...
### [64] [Flocking phase transition and threat responses in bio-inspired autonomous drone swarms](https://arxiv.org/abs/2512.21196)
*Matthieu Verdoucq,Dari Trendafilov,Clément Sire,Ramón Escobedo,Guy Theraulaz,Gautier Hattenberger*

Main category: cs.RO

TL;DR: 无人机群体通过局部对齐和吸引两种简单交互规则，可实现多种集体运动相变，在临界区域表现出最佳响应性和恢复能力。


<details>
  <summary>Details</summary>
Motivation: 受动物群体集体运动启发，研究如何为自主无人机群设计简单有效的控制算法，使其能够像生物群体一样表现出灵活、稳定的集体行为。

Method: 提出基于生物启发的3D集群算法，每个无人机仅与少数有影响力的邻居交互，依赖局部对齐和吸引两种线索。通过系统调整这两种交互增益，绘制相图分析不同集体行为相变。

Result: 相图揭示了集群和编队之间的尖锐相变，临界区域表现出最高的敏感性、极化波动和重组能力。室外十架无人机实验结合校准飞行动力学模型仿真表明，在临界区域附近运行能增强对外部干扰的响应性。

Conclusion: 最小局部交互规则足以产生多种集体相，简单的增益调节为无人机群提供了调整稳定性、灵活性和恢复力的有效机制，在临界区域运行能实现快速集体转向、瞬态扩展和秒级高对齐恢复。
...
### [65] [Schrödinger's Navigator: Imagining an Ensemble of Futures for Zero-Shot Object Navigation](https://arxiv.org/abs/2512.21201)
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本物体导航方法在真实杂乱环境中表现不佳，特别是在存在严重遮挡、未知风险或动态移动目标时。需要一种能够处理这些挑战的导航框架。

Method: 受薛定谔思想实验启发，将未观测空间视为一组可能的未来世界。基于自中心视觉输入和候选轨迹，使用轨迹条件3D世界模型沿每条路径想象未来观测。将想象的3D观测融合到导航地图中，更新价值地图，指导策略选择能避免遮挡、减少不确定空间暴露并更好跟踪移动目标的轨迹。

Result: 在Go2四足机器人上的实验表明，在严重静态遮挡、未知风险和动态移动目标三种挑战场景中，Schrödinger's Navigator在自定位、物体定位和整体成功率方面均优于现有零样本物体导航基线方法。

Conclusion: 轨迹条件3D想象方法能够实现鲁棒的零样本物体导航，特别是在遮挡严重的环境中。该方法使智能体能够"看穿"遮挡并预测未观测区域的风险，而无需额外绕行或密集全局建图。
...
### [66] [Wireless Center of Pressure Feedback System for Humanoid Robot Balance Control using ESP32-C3](https://arxiv.org/abs/2512.21219)
*Muhtadin,Faris Rafi Pramana,Dion Hayu Fandiantoro,Moh Ismarintan Zazuli,Atar Fuady Babgei*

Main category: cs.RO

TL;DR: 本文提出了一种用于人形机器人（特别是舞蹈机器人）的无线嵌入式平衡系统，通过脚部集成的四个称重传感器和ESP32-C3微控制器实时估计压力中心，采用PID控制策略调整躯干、髋部和踝关节，在3度倾斜面上单腿抬起任务中实现了100%的平衡成功率。
...
Method: 设计了一个无线嵌入式平衡系统，包括集成了四个称重传感器的定制脚部单元和ESP32-C3微控制器，用于实时估计压力中心。CoP数据无线传输到主控制器，减少了29自由度VI-ROSE人形机器人的布线复杂性。采用PID控制策略，基于CoP反馈调整躯干、髋部和踝关节的滚动关节。

Result: 实验表征显示传感器具有高精度，平均测量误差为14.8克。在优化PID参数（Kp=0.10，Kd=0.005）后，提出的控制系统在3度倾斜面上进行单腿抬起任务时实现了100%的平衡成功率。

Conclusion: 这些结果验证了无线CoP反馈在增强人形机器人姿态稳定性方面的有效性，同时不损害其机械灵活性。
...
### [67] [Relative Localization System Design for SnailBot: A Modular Self-reconfigurable Robot](https://arxiv.org/abs/2512.21226)
*Shuhan Zhang,Tin Lun Lam*

Main category: cs.RO

TL;DR: 本文介绍了SnailBot模块化自重构机器人的相对定位系统设计，该系统融合ArUco标记识别、光流分析和IMU数据处理，实现协作任务的鲁棒相对定位。
...
Result: 实验验证显示系统在实时操作中有效，在动态场景下保持可靠性，相对定位准确。

Conclusion: 该系统展示了在模块化机器人系统中可扩展部署的潜力，为协作机器人任务提供了鲁棒的相对定位解决方案。
...
### [68] [UniTacHand: Unified Spatio-Tactile Representation for Human to Robotic Hand Skill Transfer](https://arxiv.org/abs/2512.21233)
*Chi Zhang,Penglin Cai,Haoqi Yuan,Chaoyi Xu,Zongqing Lu*

Main category: cs.RO

TL;DR: 提出UniTacHand统一表示方法，通过将人类手套触觉数据与机器人触觉数据对齐，实现零样本触觉策略从人类到机器人的迁移，提高数据效率和泛化能力。
...
Method: 1. 将人类手和机器人手的触觉信号投影到MANO手模型的形态一致的2D表面空间，统一异构数据结构；2. 引入对比学习方法，在统一潜在空间中对齐人类和机器人触觉表示，仅需10分钟配对数据训练。

Result: 实现了从人类到真实机器人的零样本触觉策略迁移，能泛化到预训练未见过的物体。混合人类和机器人演示数据的协同训练比仅使用机器人数据性能更好、数据效率更高。

Conclusion: UniTacHand为触觉灵巧手提供了通用、可扩展且数据高效的学习路径，通过统一表示对齐人类与机器人触觉信息，解决了触觉数据收集和策略迁移的挑战。
...
### [69] [RoboCade: Gamifying Robot Data Collection](https://arxiv.org/abs/2512.21235)
...
<details>
  <summary>Details</summary>
Motivation: 从人类演示中学习是训练自主机器人策略的主要方法，但收集演示数据集成本高昂，需要接触机器人且过程繁琐耗时，这限制了可用于训练策略的数据规模。作者希望通过游戏化数据收集体验来解决这一可扩展性挑战。

Method: 开发了游戏化远程遥操作平台RoboCade，在系统界面中嵌入视觉反馈、音效、目标可视化、进度条、排行榜和徽章等游戏化组件。提出了构建游戏化任务的原则，这些任务与有用的下游目标任务具有重叠结构。在三个操作任务（空间排列、扫描和插入）上实例化了RoboCade。
...
### [70] [Equivariant Multiscale Learned Invertible Reconstruction for Cone Beam CT: From Simulated to Real Data](https://arxiv.org/abs/2512.21180)
*Nikita Moriakov,Efstratios Gavves,Jonathan H. Mason,Carmen Seller-Oria,Jonas Teuwen,Jan-Jakob Sonke*

Main category: physics.med-ph

TL;DR: LIRE++是一种旋转等变多尺度可逆原始-对偶学习方案，用于快速、内存高效的CBCT重建，在合成和临床数据上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: CBCT图像质量低于传统CT是其应用的主要限制因素。深度学习重建方法有潜力，但应用于CBCT面临缺乏真实数据、内存限制和需要快速推理等挑战。

Method: 提出LIRE++：端到端旋转等变多尺度学习可逆原始-对偶方案，结合内存优化和多尺度重建实现快速训练和推理，使用快速准蒙特卡洛CBCT投影模拟器生成训练数据。

Result: 在合成数据上，LIRE++比替代深度学习基线平均提高1 dB的峰值信噪比；在真实临床数据上，相比当前最先进的混合深度学习和迭代方法，重建与规划CT之间的平均绝对误差减少了10个亨氏单位。

Conclusion: LIRE++通过旋转等变性和内存优化，在CBCT重建中实现了更好的图像质量和计算效率，为临床CBCT应用提供了有前景的解决方案。
...
### [71] [Pioneering Multimodal Emotion Recognition in the Era of Large Models: From Closed Sets to Open Vocabularies](https://arxiv.org/abs/2512.20938)
*Jing Han,Zhiqiang Gao,Shihao Gao,Jialing Liu,Hongyu Chen,Zixing Zhang,Björn W. Schuller*

Main category: cs.HC

TL;DR: 该研究首次对19个主流多模态大语言模型在开放词汇多模态情感识别任务上进行大规模基准测试，发现两阶段三模态融合方法效果最佳，视频是最关键模态，开源与闭源模型差距较小。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在多模态整合方面表现出色，但在细粒度情感理解方面的潜力尚未得到系统探索。开放词汇多模态情感识别虽是有前景的方向，但目前缺乏对MLLM在该任务上的全面评估。

Method: 在OV-MERD数据集上对19个主流MLLM进行大规模基准测试，包括通用模型、模态专门化模型和推理增强架构。系统分析模型推理能力、融合策略、上下文利用和提示设计。

Result: 两阶段三模态（音频、视频和文本）融合方法在MER-OV中表现最佳，视频是最关键模态。开源与闭源LLM之间的差距出乎意料地小。

Conclusion: 该研究为开放词汇和细粒度情感计算建立了重要基准，提供了实用指南，为更细致和可解释的情感AI系统铺平了道路。
...
### [72] [A Design Study Process Model for Medical Visualization](https://arxiv.org/abs/2512.21034)
*Mengjie Fan,Liang Zhou*

Main category: cs.HC

TL;DR: 本文提出了一种基于现有医学可视化研究和跨学科经验的新型医学可视化设计研究过程模型，强调利益相关者区分、分析逻辑阶段划分和任务类型分类，使可视化设计更具针对性、可推广性和可操作性。


<details>
  <summary>Details</summary>
Motivation: 医学可视化研究面临复杂多样的医学问题，需要系统化的设计过程模型来指导跨学科研究。现有模型在适应医学问题特性方面存在不足，需要更针对性的框架来提升可视化设计的有效性。

Method: 通过文献综述分析现有医学可视化工作，结合跨学科研究经验，构建新的设计研究过程模型。该模型强调：1）在设计前区分不同利益相关者和目标用户；2）根据分析逻辑或认知习惯划分设计阶段；3）将任务类型分为推理性或描述性，并进一步分为基于假设或无假设。模型还根据医学问题特点细化步骤并提供参考指导。

Result: 新模型应用于指导视觉分析方法设计，并重新分析了三个医学可视化相关工作。结果表明，该过程模型能为跨学科医学可视化研究提供系统的理论框架和实践指导，使可视化设计更具针对性、可推广性和可操作性。

Conclusion: 提出的医学可视化设计研究过程模型能有效适应医学问题的复杂性和多样性，为未来研究者提供参考框架。模型与现有模型有所区分，并包含反思和建议，有助于推动医学可视化领域的系统性研究。
...
### [73] [When LLMs fall short in Deductive Coding: Model Comparison and Human AI Collaboration Workflow Design](https://arxiv.org/abs/2512.21041)
...
<details>
  <summary>Details</summary>
Motivation: 随着生成式AI推动教育对话数据增长，自动化编码成为学习分析提高效率的有前景方向。但自动化编码可能因数据不平衡而难以捕捉关键但罕见的代码，而人工编码又耗时耗力。需要理解LLMs在理论驱动编码中的潜力以及人机协作如何支持大规模编码任务。

Method: 在两个数据集上比较小型Transformer分类器（如BERT）和LLMs的编码性能，特别关注对话代码中的头尾不平衡分布。设计并评估人机协作工作流。

Result: LLMs在演绎编码任务中并未超越BERT模型，表现出系统性错误和偏见。人机协作工作流提高了编码效率，同时保持了编码可靠性。

Conclusion: 研究揭示了LLMs的局限性（特别是在语义相似性和理论解释方面的困难）以及人类判断的不可或缺作用，同时展示了人机协作工作流在编码任务中的实际前景。
...
### [74] [Learning Factors in AI-Augmented Education: A Comparative Study of Middle and High School Students](https://arxiv.org/abs/2512.21246)
*Gaia Ebli,Bianca Raimondi,Maurizio Gabbrielli*

Main category: cs.HC

TL;DR: 研究探讨AI工具在中小学编程教育中，不同年龄段学生对学习体验、清晰度、舒适度和动机四个关键因素的感知差异及其相互关系。


<details>
  <summary>Details</summary>
Motivation: 现有AI教育研究多关注高等教育和传统教学环境，缺乏对中小学AI辅助学习中关键学习因素相互关系的研究，特别是不同年龄段学生感知结构的差异。

Method: 在真实课堂环境中，让学生使用AI工具进行编程学习，收集四个学习因素数据和学生感知。采用多方法定量分析，结合相关性分析和文本挖掘。

Result: 初中生各维度呈现强正相关，显示整体性评价模式；高中生各维度呈现弱相关或接近零相关，显示更分化的独立评估过程。

Conclusion: 感知维度在AI增强学习中起中介作用，发展阶段调节其相互依赖关系。为开发适应学习者发展水平和年龄特定维度结构的AI整合策略奠定基础。
