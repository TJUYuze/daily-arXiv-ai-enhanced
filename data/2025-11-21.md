...
TL;DR: 本文提出了SAM2S模型，通过构建最大的手术视频分割基准SA-SV，并引入多样记忆机制、时间语义学习和抗模糊学习，显著提升了手术视频分割的长期跟踪和零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决手术视频分割中存在的领域差距和长期跟踪限制问题，超越基于预定义类别的方法，提供更灵活的提示式分割能力。

Method: 构建SA-SV基准数据集，提出SAM2S模型包含：1) DiveMem多样记忆机制用于鲁棒长期跟踪；2) 时间语义学习用于器械理解；3) 抗模糊学习缓解多源数据集标注不一致问题。

Result: 在SA-SV上微调使SAM2性能提升12.99平均J&F，SAM2S进一步达到80.42平均J&F，比原始和微调SAM2分别提升17.10和4.11点，同时保持68 FPS实时推理和强零样本泛化能力。

Conclusion: SAM2S通过专门设计的手术视频分割增强方法，在保持实时性能的同时显著提升了分割精度和泛化能力，为计算机辅助手术提供了有效的解决方案。
...
### [2] [Adaptive Guided Upsampling for Low-light Image Enhancement](https://arxiv.org/abs/2511.16623)
*Angela Vivian Dcosta,Chunbo Song,Rafael Radkowski*

Main category: cs.CV

TL;DR: AGU是一种用于低光图像上采样的高效方法，通过多参数优化同时减少噪声和提高锐度，利用少量样本图像对学习低光与明亮图像特征关联，实现实时高质量图像渲染。


<details>
  <summary>Details</summary>
Motivation: 现有引导图像方法在处理低光图像时效果不佳，因为低光图像噪声高、亮度低，缺乏足够的特征用于引导传输，导致图像质量提升有限。

Method: 基于引导图像方法，通过多参数优化学习低光与明亮图像特征之间的关联，仅需少量样本图像对进行机器学习训练。

Result: 实验表明AGU在低光图像处理用例中优于现有最先进方法，能够使用低质量、低分辨率输入实时渲染高质量图像。
...
Method: 提出MLLM引导的语义对齐模块，通过多模态输入整合和语义对齐损失捕获跨模态语义关系；采用两阶段渐进训练策略和自合成流程。

Result: UniFit不仅支持多服装和模型到模型试穿等多种任务，还实现了最先进的性能表现。

Conclusion: UniFit通过语义对齐和高效训练策略，成功构建了一个能够灵活处理多样化复杂任务的通用虚拟试穿框架。
...
TL;DR: 该论文提出四种新颖扩展来改进基于模型的6D物体姿态估计方法，包括动态非均匀密集采样、多假设推理机制、迭代优化和遮挡增强训练，在遮挡场景下显著提升精度和推理速度。


<details>
  <summary>Details</summary>
Motivation: 解决在遮挡条件下现有6D物体姿态估计方法因多阶段流水线早期错误传播导致的性能下降问题，提升对未见物体的泛化能力。

Method: 提出四种扩展：动态非均匀密集采样策略聚焦可见区域计算；多假设推理机制保留多个置信度排序的姿态候选；迭代优化逐步提升精度；遮挡增强训练提升鲁棒性和泛化性。

Result: 在ICBIN数据集上精度提升超过5%，在BOP数据集基准上提升超过2%，同时推理速度提升约3倍。

Conclusion: 所提出的方法有效解决了遮挡条件下的姿态估计问题，显著提升了精度和效率，并提出了新的加权可见性评估指标以减少现有评估协议的偏差。
...
TL;DR: Box6D是一种针对仓库场景中存储箱的类别级6D姿态估计方法，通过快速二分搜索推断箱子尺寸，使用类别CAD模板而非实例特定模型，结合深度合理性过滤和早停策略，在保持竞争力的同时减少约76%推理时间。


<details>
  <summary>Details</summary>
Motivation: 解决仓库自动化、分拣等场景中，现有6D姿态估计方法在灵活性、准确性和实用性方面的不足：基于模型的方法需要精确CAD模型且泛化差；无模型方法在挑战条件下易失败；类别级方法过于通用而忽略环境和物体先验。

Method: 从单次RGB-D观测中，通过快速二分搜索推断箱子尺寸，使用类别CAD模板进行姿态估计，结合深度合理性过滤器和早停策略拒绝不合理假设。

Result: 在真实世界存储场景和公共基准测试中，Box6D实现了竞争性或更优的6D姿态精度，同时推理时间减少约76%。

Conclusion: Box6D方法在仓库存储箱场景中有效平衡了灵活性、准确性和计算效率，为工业应用提供了实用的6D姿态估计解决方案。
...
TL;DR: 本文提出了一种两阶段自改进范式，通过生成文本推理来弥合视觉语言模型在领域特定视频分类中的语义差距，无需新标注即可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在领域特定视频分类任务中表现不佳，特别是在数据有限的情况下，这源于稀疏领域数据无法弥合复杂时空内容与抽象分类标签之间的语义距离（理性差距）。

Method: 提出两阶段自改进范式：第一阶段提示VLMs为每个视频生成详细文本推理，强制模型表达领域特定逻辑，然后在这些自生成推理上微调VLM；第二阶段在任务标签上进行传统监督微调。

Result: 在多个数据集上的广泛实验表明，该方法显著优于直接监督微调，验证了自生成推理作为适应VLMs到领域特定视频分析的有效、标注高效范式。

Conclusion: 自生成推理是一种有效的、标注高效的范式，能够成功适应视觉语言模型到领域特定的视频分析任务。
...
TL;DR: 提出了MGLL框架，通过多粒度语言学习改进图像文本预训练，解决医学影像中多标签、多粒度对齐问题，在多个数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP等图像文本预训练方法主要关注单标签、单粒度对齐，但在医学影像等复杂领域中，图像通常对应多个高级标签和不同粒度的标注，需要更有效的多标签和跨粒度对齐方法。

Method: 提出MGLL对比学习框架，利用结构化多标签监督，整合不同粒度的文本描述，引入带点约束的软标签监督来增强对齐，使用平滑KL散度确保跨粒度一致性，作为即插即用模块保持计算效率。

Result: 在构建的大规模多粒度数据集上预训练，并在多个数据集上评估，MGLL在下游任务中优于其他最先进方法。

Conclusion: MGLL框架有效解决了医学影像中的多标签和跨粒度对齐问题，提升了视觉语言模型的性能，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 传统心脏超声依赖2D视频，而3D超声心动图具有更好的图像质量，但医生习惯使用2D视图。该方法旨在让医生在保持常规2D视图格式的同时，享受3D扫描的速度和可用性优势。

Method: 应用深度学习视图分类器和基于解剖标志的下游启发式算法，结合心脏病专家提供的启发式方法，从3D心脏超声体积中重建标准超声心动图视图。

Result: 在来自2家医院的1,600个视频中，三位心脏病专家的盲法评估显示准确率达96%。提取的2D视频保留了空间校准和诊断特征，能够检测心脏异常并生成临床级心脏解剖测量。

Conclusion: 该方法成功地从3D体积中提取出保留诊断特征的2D视频，使临床医生能够从3D体积中获得准确的真实世界解释，并发布了代码和29个3D超声心动图视频数据集。
...
Method: 引入动态交互发现模块生成基于主体的对象提示，以及语义分类头进行联合实体和谓词推理。从单一用户提示出发，分割跟踪主体，自主发现交互对象，预测三元组。

Result: 在OpenPVSG基准测试中，Click2Graph为人类引导的全景视频场景图生成建立了坚实基础，展示了如何将人类提示与全景定位和关系推理相结合。

Conclusion: Click2Graph通过结合视觉提示与空间、时间和语义理解，实现了可控且可解释的视频场景理解，为交互式全景视频场景图生成提供了有效解决方案。
...
Conclusion: InfoCLIP通过信息论方法成功实现了CLIP到分割任务的稳定知识迁移，提升了开放词汇语义分割的性能。
...
<details>
  <summary>Details</summary>
Motivation: 多任务学习存在破坏性任务干扰问题，导致联合训练模型性能不如单任务基线，限制了泛化能力。本文旨在通过改进多任务学习来提高乳腺超声肿瘤分割的泛化性能。

Method: 提出一种新颖的一致性正则化方法，包含可微分的BI-RADS启发形态特征，用于缓解分割和分类任务间的破坏性干扰。在BrEaST数据集上训练，在三个外部数据集上进行验证。

Result: 在UDIAT、BUSI、BUS-UCLM三个外部数据集上，提出的多任务方法相比基线在分割任务上取得了统计学显著改进（p<0.001），Dice系数分别为0.81 vs 0.59、0.66 vs 0.56、0.69 vs 0.49。在UDIAT数据集上达到了最先进的分割性能。
...
TL;DR: 提出了一种检测引导的生成式框架，通过预测分层类别和属性token来实现视觉语义理解，显著优于现有的基于相似性和多阶段分类的方法。


<details>
  <summary>Details</summary>
Motivation: 当前先进方法依赖全局相似性，难以捕捉细粒度类别差异和类别特定属性多样性，特别是在大规模电商场景中。

Method: 检测引导的生成式框架，提取精炼的ROI级特征，使用BART生成器以粗到细的顺序生成语义token，涵盖类别层次和属性-值对，支持属性条件识别。

Result: 在大型专有电商数据集和开源数据集上的实验表明，该方法显著优于现有相似性流水线和多阶段分类系统。

Conclusion: 该方法实现了更强的细粒度识别和更一致的统一推理能力。
...
TL;DR: 本文提出FADS方法，通过聚类采样构建人口统计学平衡的演示样本，改善多模态大语言模型在医学图像推理中的公平性，无需微调即可减少性别、种族和民族相关的偏见。
...
Method: 提出公平感知演示选择(FADS)方法，通过聚类采样构建人口统计学平衡且语义相关的演示样本，作为轻量级、无需调优的替代方案。

Result: 在多个医学影像基准测试中，FADS一致减少了性别、种族和民族相关的差异，同时保持强准确性。

Conclusion: 公平感知的上下文学习为公平医学图像推理提供了可扩展且数据高效的解决方案。
...
### [14] [LEGO-SLAM: Language-Embedded Gaussian Optimization SLAM](https://arxiv.org/abs/2511.16144)
...
<details>
  <summary>Details</summary>
Motivation: 现有3DGS SLAM系统缺乏开放词汇语义理解能力，无法满足高级机器人交互需求。存储高维语言特征需要过多内存和渲染开销，而静态模型方法缺乏对新环境的适应性。

Method: 提出场景自适应编码器-解码器，将高维语言嵌入蒸馏到紧凑的16维特征空间；采用语言引导的剪枝策略识别语义冗余；引入基于语言的闭环检测方法复用映射特征。

Result: 将每个高斯的存储内存减少，渲染加速，实现实时性能（15 FPS）；高斯数量减少超过60%同时保持渲染质量；在映射质量和跟踪精度方面达到竞争性表现。

Conclusion: LEGO-SLAM成功在3DGS SLAM系统中实现了实时开放词汇语义映射，解决了高维语言特征存储和渲染效率问题，为机器人高级交互提供了语义理解能力。
...
TL;DR: 该论文提出了一种用于单步图像超分辨率的混合秩（MoR）架构，通过将稀疏混合专家（MoE）集成到真实世界图像超分辨率中，解决了现有密集模型在捕捉复杂退化特征方面的局限性。
...
Method: 提出混合秩架构，将LoRA中的每个秩视为独立专家，采用细粒度专家划分策略；开发基于CLIP嵌入的退化估计模块计算相对退化分数；引入零专家槽和退化感知负载均衡损失，根据退化严重度动态调整活跃专家数量。

Result: 综合实验验证了该框架的有效性和最先进的性能表现。

Conclusion: 通过将稀疏MoE集成到Real-ISR中，提出的MoR架构能够灵活重组知识，同时保持常识特征，在真实世界图像超分辨率任务中取得了优异效果。
...
TL;DR: CuriGS是一个基于课程学习的稀疏视图3D重建框架，通过生成多扰动级别的伪视图（学生视图）来增强稀疏训练数据，结合深度相关性和协同正则化技术提升3D高斯溅射在稀疏视图下的重建质量。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射在稀疏视图设置下面临监督稀缺和过拟合问题，因为有限的视角覆盖导致训练不足。

Method: 提出课程引导框架，为每个真实视角（教师）生成多组不同扰动级别的学生视图，通过渐进式课程学习逐步解锁更高扰动级别，使用深度相关性和协同正则化进行正则化，并通过多信号指标评估学生视图质量。

Result: 在合成和真实稀疏视图场景中，CuriGS在渲染保真度和几何一致性方面均优于现有最先进基线方法。

Conclusion: CuriGS通过课程学习和伪视图增强策略，有效解决了3D高斯溅射在稀疏视图下的挑战，显著提升了重建质量。
...
<details>
  <summary>Details</summary>
Motivation: 传统卫星任务受限于空间分辨率，难以满足现代农业微地块管理的需求，而无人机虽然性能优越但成本较高。需要一种方法能够结合两者的优势。

Method: 使用大约84种玉米杂交品种在5个不同地点的复式地块收集的卫星-无人机图像对，训练模型学习不同传感模态之间的细粒度光谱-空间对应关系。
...
TL;DR: 本文提出了一种利用大型语言模型（LLMs）解决食品识别中领域偏移、长尾分布和细粒度分类挑战的框架。通过LLMs解析食品图像生成标题和成分，将文本和图像投影到共享嵌入空间进行对齐，最终实现多模态特征的识别。


<details>
  <summary>Details</summary>
Motivation: 解决食品识别中的三个主要挑战：训练数据（来自互联网）与真实环境用户拍摄图片之间的领域偏移问题、真实世界食品数据的长尾分布特性，以及不同类别食品间视觉差异细微难以区分的问题。
...
Conclusion: 通过LLMs辅助的多模态特征对齐框架，能够有效解决食品识别中的领域偏移、长尾分布和细粒度分类问题，在多个任务上取得优于专门方法的性能。
...
TL;DR: AMS-KV是一种针对视觉自回归模型中下一尺度预测的尺度自适应KV缓存策略，通过优先存储压缩尺度和局部尺度的KV，显著减少KV缓存使用达84.83%，降低自注意力延迟60.48%，并提升模型的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 下一尺度预测在视觉自回归模型中面临KV内存随尺度数量增长而过度膨胀的问题，严重限制了模型的可扩展性，而现有研究对此关注不足。

Method: 基于系统研究发现：(1)局部尺度注意力对生成质量重要；(2)压缩尺度内存分配稳定多尺度生成；(3)缓存需求层间尺度相似性较弱。提出AMS-KV策略，优先存储压缩和局部尺度KV，并通过尺度间相似性分析优化缓存利用。

Result: AMS-KV相比基线模型减少KV缓存使用84.83%，降低自注意力延迟60.48%。在基线VAR-d30模型批大小128出现内存不足时，AMS-KV能稳定扩展到批大小256并提升吞吐量。

Conclusion: AMS-KV通过尺度自适应KV缓存策略有效解决了视觉自回归模型中下一尺度预测的KV内存瓶颈，显著提升了模型的可扩展性和计算效率。
...
TL;DR: LiSTAR是一个直接在传感器原生几何上操作的生成世界模型，通过混合柱面-球面表示减少量化伪影，使用时空注意力与射线中心变换器捕捉复杂动态，并提出4D点云对齐体素布局和掩码生成框架，实现可控的高分辨率合成。
...
Method: 提出LiSTAR模型，采用混合柱面-球面表示保持数据保真度；使用时空注意力与射线中心变换器建模特征沿传感器射线的演化；提出4D点云对齐体素布局和掩码生成START框架，学习场景的紧凑标记化表示。
...
TL;DR: VideoSeg-R1是首个将强化学习引入视频推理分割的框架，采用解耦架构将任务分为参考图像分割和视频掩码传播，通过分层文本引导帧采样、推理模型和分割传播三阶段实现，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 传统视频推理分割方法依赖监督微调，限制了在分布外场景的泛化能力且缺乏显式推理过程，因此需要引入强化学习来提升泛化性和推理透明度。
...
Conclusion: VideoSeg-R1成功将强化学习引入视频推理分割，通过解耦架构和显式推理链实现了更好的泛化能力和性能表现。
...
TL;DR: SpectralTrain是一个通用的、架构无关的训练框架，通过结合课程学习和基于PCA的光谱降采样来提高高光谱图像分类的学习效率，在显著降低计算成本的同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像分类通常涉及大规模数据和计算密集型训练，限制了深度学习模型在实际遥感任务中的部署。需要一种能提高学习效率的通用训练框架。

Method: 将课程学习与基于主成分分析的光谱降采样相结合，逐步引入光谱复杂性同时保留关键信息，实现高效的光谱-空间模式学习。该框架独立于特定架构、优化器或损失函数。

Result: 在三个基准数据集上的实验表明，训练时间减少了2-7倍，精度损失较小，在云分类等气候相关遥感任务中显示出潜力。

Conclusion: SpectralTrain证明了训练策略优化是高光谱图像模型中架构设计的有效补充，为实际遥感应用提供了高效解决方案。
...
TL;DR: Rad-GS是一个基于4D雷达-相机融合的SLAM系统，使用3D高斯作为可微空间表示，适用于千米级户外环境。该系统结合雷达点云和多普勒信息进行动态物体掩码，利用未同步图像帧优化3D高斯表示，并通过八叉树结构和高斯基元管理策略降低噪声和内存消耗。


<details>
  <summary>Details</summary>
Motivation: 开发能够在千米级户外环境中实现稳健建图的SLAM系统，利用4D毫米波雷达的优势，解决传统基于相机或LiDAR方法在大规模环境中的局限性。

Method: 结合原始雷达点云与多普勒信息，通过几何增强的点云指导同步图像中的动态物体掩码；利用未同步图像帧全局优化3D高斯表示；采用全局八叉树结构和目标高斯基元管理策略。

Result: 在广泛实验和消融研究中，Rad-GS实现了与传统基于相机或LiDAR的3D高斯方法相当的性能，验证了使用4D毫米波雷达进行稳健户外建图的可行性。千米级真实世界重建验证了其大规模场景重建潜力。

Conclusion: Rad-GS证明了4D毫米波雷达在大型户外环境SLAM中的有效性，通过雷达-相机融合和3D高斯表示，实现了高质量的大规模场景重建，为雷达在SLAM系统中的应用提供了新的可能性。
...
TL;DR: 本文提出了T2T-VICL框架，探索视觉语言模型在不同视觉任务间的跨任务上下文学习能力，通过文本提示生成和选择机制，结合感知评分推理实现跨任务视觉上下文学习。


<details>
  <summary>Details</summary>
Motivation: 研究视觉语言模型在不同视觉任务之间进行上下文学习的潜力，探索当视觉提示和目标图像来自不同视觉任务时，视觉语言模型是否仍能实现有效的视觉上下文学习。

Method: 设计了生成和选择文本提示的机制来隐式描述不同低层次视觉任务之间的差异，构建首个跨任务视觉上下文学习数据集，并提出结合感知评分推理和传统评估指标的新型推理框架。

Result: 在九个跨任务场景中取得顶级结果，在另外十个场景中获得第二层级性能，成功突破了视觉语言模型中跨任务视觉上下文学习的边界。

Conclusion: T2T-VICL框架有效解锁了视觉语言模型在跨任务视觉上下文学习方面的潜力，证明了在不同视觉任务间进行上下文学习的可行性。
...
TL;DR: 本文提出了一种改进4D高斯泼溅的方法，通过椭圆误差聚类和分组4D高斯泼溅来解决动态场景重建中的像素对应模糊和动态区域密度不足问题，显著提升了时间一致性和渲染质量。
...
Method: 方法包含两个关键组件：1) 椭圆误差聚类和误差校正泼溅添加，用于精确定位动态区域并初始化拟合泼溅；2) 分组4D高斯泼溅，提高泼溅与动态对象映射的一致性。通过将渲染误差分类为缺失颜色和遮挡类型，并基于跨视图颜色一致性进行反向投影或前景分割的针对性校正。

Result: 在Neural 3D Video和Technicolor数据集上的评估表明，该方法显著改善了时间一致性，并达到了最先进的感知渲染质量，在Technicolor光场数据集上PSNR提高了0.39dB。可视化结果显示泼溅与动态对象的对齐度得到改善。

Conclusion: 该方法通过有效的误差识别和校正机制，成功解决了4D高斯泼溅在动态场景重建中的关键问题，实现了更好的时间一致性和渲染质量。
...
TL;DR: DCS-LDM是一种新的视觉生成范式，通过构建层次化、尺度无关的潜在空间，将信息复杂度与尺度解耦，支持在固定潜在表示下解码到任意分辨率和帧率，实现灵活的计算质量权衡。
...
Method: 构建层次化、尺度无关的潜在空间，通过多级token建模样本复杂度，支持在固定潜在表示下解码到任意分辨率和帧率，采用渐进式由粗到细的生成范式。

Result: 实验结果显示DCS-LDM在性能上与最先进方法相当，同时支持跨不同尺度和视觉质量的灵活生成。

Conclusion: DCS-LDM通过解耦信息复杂度与尺度，实现了灵活的计算质量权衡，在保持性能的同时支持多尺度生成。
...
<details>
  <summary>Details</summary>
Motivation: 现有QECV方法依赖已知QP参数，而实际场景中QP可能未知；当前盲方法仅捕获全局退化信息，缺乏空间细节；现有方法采用统一架构，忽略了不同压缩级别的计算需求差异。

Method: 提出预训练退化表示学习模块，从视频内容中解耦和提取高维多尺度退化表示；引入分层终止机制，根据压缩级别动态调整伪影减少阶段的数量。

Result: 在QP=22时，相比最先进的盲方法PSNR提升110%（从0.31dB到0.65dB）；分层终止机制在QP=22时的平均推理时间比QP=42减少一半。

Conclusion: 所提出的方法显著提升了盲QECV的性能和计算效率，解决了现有方法在退化表示和计算资源分配方面的局限性。
...
Result: 在ScanNet V2和SUN RGB-D数据集上的大量实验表明，SR3D有效弥合了训练-推理差距，在保持实时速度的同时，准确率显著优于先前方法。
...
Method: 提出HSRW-CD数据集（空间分辨率高于3米）和SSCP注意力模块，该模块包含多语义空间注意力(MSA)、结构关系感知全局注意力(SRGA)和通道自注意力(CSA)三个组件，能够充分挖掘深度特征的空间语义和结构信息。

Result: 在提出的HSRW-CD和Water-CD数据集上的大量实验验证了SSCP的有效性和泛化能力。

Conclusion: 提出的SSCP模块能够显著提升水体变化检测的判别能力，可作为即插即用模块集成到现有的WBCD模型中。
...
TL;DR: 本文提出推理引导嵌入（RGE）方法，通过将多模态大语言模型的生成推理能力融入嵌入过程，提升多模态表示质量。
...
Method: 提出RGE方法，首先让模型基于指令执行结构化推理生成，然后在推理展开后提取表示，并将生成推理过程与对比训练相结合。

Result: 在MMEB基准测试中，推理引导的条件化使多模态检索性能比非推理基线提高了4.9%。

Conclusion: 显式推理能有效增强嵌入质量，推理引导的嵌入方法可提升多模态表示性能。
...
TL;DR: 提出PPCL框架，通过可插拔剪枝和连续层蒸馏技术，在保持图像生成质量的同时，将DiT模型的参数量减少50%。


<details>
  <summary>Details</summary>
Motivation: DiT模型在图像生成方面表现出色，但参数量大导致计算成本高，难以在资源受限环境中部署。

Method: 使用线性探测机制和相似度指标一阶微分趋势分析识别冗余层区间，提出可插拔的师生交替蒸馏方案，在单一训练阶段集成深度和宽度剪枝。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于栅格认知地图的方法依赖离散化表示，限制了细粒度空间推理能力。需要克服这种局限性以实现更精确的空间理解。
...
Conclusion: 连续边界坐标表示能有效提升模型的空间计算能力，缓解自然语言描述空间关系时的模糊性，验证了方法的优越性。
...
TL;DR: Simba框架通过将点云补全中的点级变换回归重新定义为分布学习问题，结合对称先验和扩散模型的生成能力，解决了现有回归方法过拟合和噪声敏感性的问题。


<details>
  <summary>Details</summary>
Motivation: 点云补全任务面临同时保留输入细节和确保全局结构完整性的挑战。现有基于局部对称变换的回归方法存在过拟合和噪声敏感性的问题。

Method: 提出Simba框架，将点级变换回归重新定义为分布学习问题，结合对称先验和扩散模型，并采用基于Mamba的分层架构实现高保真上采样。

Result: 在PCN、ShapeNet和KITTI基准测试上的广泛实验验证了该方法的最先进性能。

Conclusion: Simba通过分布学习的方法有效解决了点云补全中的过拟合和噪声敏感性问题，实现了优异的性能。
...
TL;DR: 提出LNG-SWR方法，通过层间噪声注入学习频率偏置先验，结合选择性小波重构实现频率自适应，提升医学图像分割的鲁棒性，同时保持清洁精度。


<details>
  <summary>Details</summary>
Motivation: 临床部署需要分割模型在分布偏移和扰动下保持稳定。主流对抗训练方法存在清洁-鲁棒性权衡和高训练成本问题，限制了医学影像中的可扩展性和可维护性。

Method: 提出层间噪声引导的选择性小波重构(LNG-SWR)：训练时在多个层注入小噪声学习频率偏置先验；推理时应用先验引导的选择性小波重构，抑制噪声敏感频带、增强方向结构和形状线索，同时保持频谱一致性。
...
Conclusion: 提出的VTIA方法成功解决了现有方法在稳定性和可控性方面的不足，能够有效诱导视觉语言模型产生冗长输出，为模型部署效率评估提供了新视角。
...
TL;DR: EvoVLA是一个自监督的视觉语言动作框架，通过阶段对齐奖励、基于姿态的对象探索和长时程记忆三个组件，解决了VLA模型在多步任务中的阶段幻觉问题，在长时程操作任务中显著提升了成功率。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在多步长时程机器人操作中存在阶段幻觉问题，即智能体利用粗糙的评估信号来走捷径完成任务，而没有真正完成多步任务。

Method: 提出三个互补组件：阶段对齐奖励（使用三元组对比学习和Gemini生成的难负样本）、基于姿态的对象探索（基于相对对象-夹爪姿态而非原始像素）、长时程记忆（选择性上下文保留和门控融合）。

Result: 在Discoverse-L基准测试中，EvoVLA比最强基线（OpenVLA-OFT）平均任务成功率提高10.2个百分点，达到69.2%；样本效率提高1.5倍；阶段幻觉从38.5%降至14.8%。真实世界部署平均成功率达54.6%，比OpenVLA-OFT高11个百分点。

Conclusion: EvoVLA有效解决了VLA模型的阶段幻觉问题，实现了从仿真到真实世界的有效迁移和强泛化能力。
...
TL;DR: 本文提出RF-CLIP方法，通过模拟人类注意力分散-重新聚焦行为，过滤CLIP中导致注意力分散的无关标记，从而提升开放词汇语义分割的像素级多模态对齐能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法虽然利用CLIP的视觉-语言对齐能力取得了不错的效果，但很少从可解释性机制角度研究CLIP在密集预测任务中的性能边界。研究发现CLIP存在类似人类注意力分散的现象，将大量注意力资源从目标区域转移到无关标记上。

Method: 提出RF-CLIP（重新聚焦CLIP），这是一种无需训练的方法，通过识别和过滤维度特定的过度激活产生的无关标记，将注意力从分散标记重新引导回目标区域，从而细化CLIP的多模态对齐粒度。

Result: 在八个基准测试中达到了最先进的性能，同时保持了较高的推理效率。

Conclusion: 通过系统分析CLIP的内部机制并解决注意力分散问题，RF-CLIP有效提升了开放词汇语义分割的性能，为多模态密集预测任务提供了新的解决方案。
...
TL;DR: 本文提出了一种名为DSLGA的两阶段模型，用于解决无监督域自适应可见光-红外行人重识别问题，通过域共享学习策略和渐进对齐策略来处理域间和域内模态差异。


<details>
  <summary>Details</summary>
Motivation: 由于公共数据集与现实世界数据存在差异，现有的VI-ReID算法在实际应用中表现不佳。本文旨在研究无监督域自适应VI-ReID，将公共数据的知识迁移到现实数据中，同时不降低准确性且无需新样本标注。

Method: 设计了两阶段模型DSLGA：第一阶段使用域共享学习策略缓解域间模态差异导致的无效预训练；第二阶段使用渐进对齐策略通过簇到整体的对齐方式处理域内模态差异。

Result: 大量实验表明，该方法在各种设置下显著优于现有的VI-ReID域自适应方法，甚至优于一些监督方法。

Conclusion: DSLGA模型通过有效处理域间和域内模态差异，在无监督域自适应VI-ReID任务中取得了优异性能，为解决现实应用中的VI-ReID问题提供了有效方案。
...
Result: 在心脏、海马体和肺部上展示了有效性，实现了高几何精度、正确拓扑结构，即使在有限或噪声训练数据下也能保持稳健性能。
...
TL;DR: 本文提出了VLA-Fool框架，系统研究视觉-语言-动作模型在具身环境中的多模态对抗鲁棒性，涵盖白盒和黑盒攻击场景。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉-语言-动作模型在具身环境中表现出色，但其对抗鲁棒性尚未得到充分研究，特别是在真实多模态和黑盒条件下。现有研究主要关注单模态扰动，忽视了影响具身推理和决策的跨模态错位问题。

Method: VLA-Fool统一了三种多模态对抗攻击：基于梯度的文本扰动和提示操纵、视觉补丁和噪声扰动、以及破坏感知与指令语义对应的跨模态错位攻击。还开发了首个自动构建的语义引导提示框架。
...
TL;DR: 该论文提出了多模态交互欺骗评估(MIDA)任务和数据集，评估了12个先进MLLMs在识别社交欺骗方面的能力，发现现有模型存在显著性能差距，并设计了SoCoT推理管道和DSEM模块来提升性能。
...
Method: 引入MIDA任务和包含同步视频文本的多模态数据集；评估12个开源和闭源MLLMs；设计Social Chain-of-Thought推理管道和Dynamic Social Epistemic Memory模块。

Result: 基准测试显示显著的性能差距，即使是GPT-4o等强大模型也难以可靠地区分真假；新框架在该挑战性任务上实现了性能提升。

Conclusion: 现有MLLMs无法有效将语言与多模态社交线索相结合，缺乏建模他人知识、信念或意图的能力，需要新方法来构建更具感知力和可信度的AI系统。
...
TL;DR: 提出基于视觉门控生成变换器(VGGT)的快速时空场景理解框架，支持接近实时的辅助导航应用。通过滑动窗口处理图像流和对齐子地图来克服VGGT的高内存需求，利用VGGT跟踪头将2D语义实例掩码聚合为3D对象，并保持时间一致性和上下文推理能力。


<details>
  <summary>Details</summary>
Motivation: 开发高效的实时场景理解系统，特别针对辅助导航应用，需要解决VGGT模型的高内存需求问题，同时实现连续更新的3D场景表示。

Method: 使用滑动窗口处理图像流，对齐子地图来管理内存；利用VGGT跟踪头聚合2D语义实例掩码为3D对象；存储时间戳和实例级身份以实现时间一致性和环境变化检测。

Result: 在知名基准测试和专门设计的辅助导航数据集上评估，结果表明该框架适用于现实世界场景。

Conclusion: 提出的VGGT框架能够有效实现快速、时空一致性的场景理解，在辅助导航等应用中具有实际应用价值。
...
TL;DR: 本文提出了一种混合深度学习框架，结合CNN、ViT和GAN进行杂草检测，通过自监督对比预训练和数据增强实现高精度识别，准确率达99.33%，支持边缘设备实时部署。


<details>
  <summary>Details</summary>
Motivation: 精准农业中的杂草检测对选择性施用除草剂至关重要，现有方法在不同田间条件下鲁棒性不足，需要更稳健的检测框架来支持可持续农业管理。

Method: 采用混合深度学习框架，结合CNN、ViT和GNN提取局部、全局和关系特征；使用GAN进行数据增强平衡类别分布；通过自监督对比预训练从有限标注数据中学习更多特征。

Result: 在多基准数据集上获得99.33%的准确率、精确率、召回率和F1分数，模型具有高可解释性和适应性。

Conclusion: 该框架实现了高效的杂草检测，减少对除草剂的过度依赖，为精准农业提供了可扩展的可持续解决方案，支持边缘设备实时部署。
...
TL;DR: Texture3dgs是一个针对移动GPU优化的3D高斯泼溅(3DGS)实现，通过新颖的排序算法和内存优化，在移动设备上实现了显著的性能提升和内存使用减少。


<details>
  <summary>Details</summary>
Motivation: 受3DGS的高效性和移动设备部署优势（数据隐私、离线操作、快速响应）的启发，开发针对移动GPU优化的3D场景重建方法。

Method: 核心是专为2D内存优化的新颖排序算法，通过改进变量布局设计和其他优化来加速3DGS算法的各个步骤，特别针对移动GPU的二维纹理缓存进行优化。

Result: Texture3dgs在排序和整体3D场景重建方面分别实现了最高4.1倍和1.7倍的加速，同时内存使用减少了最高1.6倍。
...
<details>
  <summary>Details</summary>
Motivation: 视觉基础模型虽然在下游任务中表现出强大的泛化能力，但其表示通常被下采样14x/16x（如ViT），限制了在像素级应用中的直接使用。现有特征上采样方法依赖于数据集特定的重新训练或沉重的隐式优化，限制了可扩展性和泛化性。

Method: 通过简单的每图像优化学习一个结合空间和范围线索的各向异性高斯核，有效桥接高斯泼溅和联合双边上采样。学习到的核作为通用的边缘感知算子，可在架构和模态间无缝转移。
...
TL;DR: 本文提出将稀疏自编码器(SAE)重新理解为主题模型，并开发了SAE-TM框架，通过训练SAE学习可重用的主题原子，在文本和图像数据集上生成更连贯且多样化的主题。
...
Method: 将潜在狄利克雷分配扩展到嵌入空间，推导出SAE目标作为该模型下的最大后验估计器；提出SAE-TM框架：训练SAE学习主题原子，将其解释为下游数据的词分布，无需重新训练即可合并成任意数量的主题。

Result: SAE-TM在文本和图像数据集上比强基线方法产生更连贯的主题，同时保持多样性；成功分析了图像数据集的主题结构，并追踪了日本木版画中主题随时间的变化。

Conclusion: 该工作将SAE定位为跨模态大规模主题分析的有效工具，为SAE提供了理论依据和实际应用价值。
...
Result: NaTex在纹理一致性和对齐方面显著优于先前方法，并展现出强大的泛化能力，无需训练或仅需简单调优即可应用于材质生成、纹理优化、部件分割和纹理化等下游任务。
...
TL;DR: 该研究提出了一种基于师生架构的弱监督学习方法，结合关键帧选择和更新算法，用于无人机遥感的地形和地貌分类任务，仅使用30%的标注数据就能同时提升mIoU和时序一致性。
...
Result: 实验结果显示，仅使用30%标注数据就能同时提升mIoU和时序一致性，确保地形物体的稳定定位。

Conclusion: 提出的方法克服了传统时序一致性训练在航空任务中的不足，实现了有效的弱监督地形分类。
...
Method: 从预捕获的LiDAR点云渲染合成视图，建立实时帧与点云的2D-3D对应关系；使用神经渲染技术减少合成与真实图像间的域差距，改善特征匹配；提供两种实时变体：在线渲染匹配和预构建定位。

Result: 在ScanNet++数据集上展示了改进结果，优于现有SLAM流程，实现了无漂移的相机跟踪和正确的度量尺度。

Conclusion: 该方法能够实现基于LiDAR点云的实时、无漂移相机定位，解决了现有视觉定位方法的局限性。
...
TL;DR: CAMS提出了一种基于CLIP的组成零样本学习方法，通过门控交叉注意力机制和多空间解缠技术，从视觉特征中提取语义特征并实现属性和对象的语义解缠，在多个基准测试中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于CLIP的CZSL方法主要利用图像编码器获得的全局语义表示来解缠属性和对象，但这种表示能力有限，无法完全实现两者的解缠。
...
Conclusion: CAMS通过语义特征提取和多空间解缠技术，有效提高了对未见属性-对象组合的泛化能力，在组成零样本学习任务中表现出色。
...
TL;DR: 提出了一种几何引导的自监督环视深度估计方法，通过将多相机图像投影到共享圆柱体上，实现跨视图一致的深度预测。
...
Method: 使用多相机标定参数，将各图像的3D点投影到共享圆柱体上建立邻域关系，然后应用显式空间注意力机制跨图像聚合特征来预测最终深度图。

Result: 在DDAD和nuScenes数据集上的评估表明，该方法相比现有最优方法提高了跨图像深度估计的一致性和整体深度质量。

Conclusion: 所提出的几何引导方法能够有效实现跨视图一致的深度估计，为环视3D感知提供了更好的解决方案。
...
TL;DR: 提出两种结合Vision Transformer和Graph Neural Networks的图分割模型，用于提升腹腔镜胆囊切除术中肝脏解剖结构的识别精度，在遮挡、长距离依赖和精细几何结构分割方面表现优异。
...
Method: 提出两种分割模型：(1) 静态k近邻图结合GCNII实现稳定的长距离信息传播；(2) 动态可微分图生成器结合GAT支持自适应拓扑学习。在Endoscapes-Seg50和CholecSeg8k基准上评估。

Result: 相比最先进基线，mIoU提升7-8%，mDice提升6%，在薄、罕见和安全关键结构上产生解剖学一致的预测。

Conclusion: 图分割方法提升了手术场景分割的性能和解剖一致性，结合ViT全局上下文和图关系推理，提高了模型可解释性和可靠性，为更安全的腹腔镜和机器人辅助手术铺平道路。
...
<details>
  <summary>Details</summary>
Motivation: 现有小样本分割方法主要从支持图像中挖掘参考信息作为元指导，但由于视觉表示的类内差异，这些信息无法为未训练类别提供准确的分割指导。作者认为支持图像提供的参考可能并非必需，关键在于为训练和未训练类别提供无偏的元指导。

Method: 设计了语言驱动的属性泛化（LDAG）架构，包括多属性增强（MaE）模块和多模态属性对齐（MaA）模块。MaE通过大语言模型生成目标类别的多个详细属性描述，构建精炼的视觉-文本先验指导；MaA实现属性文本与视觉特征之间的跨模态交互。

Result: 实验表明，该方法明显优于现有方法，实现了新的最先进性能。

Conclusion: 通过利用语言描述构建无偏的支持表示，可以有效解决小样本分割中的类内差异问题，提升分割性能。
...
TL;DR: 提出了StreetView-Waste数据集，专注于城市垃圾容器检测、跟踪和溢出分割任务，并通过启发式方法和几何先验策略提升了基线模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有垃圾检测数据集缺乏对垃圾容器跟踪的标注，且在静态环境下采集，限制了在实际物流场景中的应用。需要专门针对垃圾容器监控的数据集。

Method: 构建包含垃圾和垃圾容器的城市场景数据集，支持三个评估任务：垃圾容器检测、跟踪和溢出分割。提出两种策略：基于启发式的垃圾容器跟踪改进方法和利用几何先验的模型无关分割框架。

Result: 微调的目标检测器在垃圾容器检测上表现良好，但基线跟踪方法在数量估计上困难；提出的启发式方法将平均绝对计数误差降低79.6%。几何感知策略在轻量级模型上将分割mAP@0.5提升27%。
...
TL;DR: VLA-Pruner是一种专门针对Vision-Language-Action模型的视觉令牌剪枝方法，通过双级重要性标准和自适应令牌选择策略，在保持计算效率的同时提升VLA模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有的VLM令牌剪枝方法仅基于语义显著性选择令牌，忽视了VLA模型的双系统特性（高级语义理解和低级动作执行），导致丢弃对动作生成至关重要的信息，显著降低VLA性能。

Method: 提出双级重要性标准：视觉语言预填充注意力用于语义级相关性，通过时间平滑估计的动作解码注意力用于动作级重要性。基于此标准设计自适应双级令牌选择策略。

Result: 实验表明VLA-Pruner在多种VLA架构和多样化机器人任务中实现了最先进的性能。

Conclusion: VLA-Pruner作为即插即用的VLA专用令牌剪枝方法，有效解决了VLA模型实时部署中的计算效率问题，同时保持了模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 由于3D训练数据有限，而2D数据集丰富，作者希望开发能够理解3D场景的多模态语言模型。

Method: 受立体主义画家启发，通过场景的中间多视角3D重建，为每个对象生成全方位视觉表示来描述3D场景。

Result: 在3D视觉问答和3D语言定位任务上的广泛实验表明，该方法优于之前的基于2D的VLM解决方案。

Conclusion: LLaVA³证明了仅使用多视角2D图像即可有效提升VLM的3D场景理解能力，无需额外3D数据或模型微调。
...
TL;DR: FastSurfer-CC是一个高效、全自动的胼胝体形态测量框架，能够自动识别矢状面切片、分割胼胝体和穹窿，定位前后连合以标准化头部位置，生成厚度剖面和细分区域，并提取8个形状指标进行统计分析。
...
Method: 开发了FastSurfer-CC框架，包括自动识别矢状面切片、分割胼胝体和穹窿、定位前后连合、生成厚度剖面和细分区域、提取形状指标等完整流程。

Result: FastSurfer-CC在各项任务中均优于现有专业工具，并且能够检测到当前最先进方法未能发现的亨廷顿病患者与健康对照之间的统计学显著差异。

Conclusion: FastSurfer-CC提供了一个高效、全自动的胼胝体形态测量解决方案，在性能和临床检测能力方面均优于现有工具。
...
TL;DR: 该论文研究了遮挡情况下的视频预测问题，提出通过结合点流（point-flow）和深度图（depth-maps）信息来改进视频预测模型在遮挡场景下的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管通用视频预测模型在标准场景中表现优异，但遮挡仍然是视频预测的内在挑战。作者假设提供明确的运动信息（通过点流）和几何结构信息（通过深度图）将有助于模型在遮挡和背景运动情况下表现更好。

Method: 使用标准的多对象潜在变换器架构进行未来帧预测，但修改该架构以融入深度和点流信息。在合成和真实世界数据集上进行受控评估，不仅使用基于外观的指标，还使用对象掩码上的Wasserstein距离来有效测量预测的运动分布。

Result: 研究发现，当预测模型辅助以点流和深度信息时，在遮挡场景下表现更好，并且相比不使用这些模态的模型，能预测更准确的背景运动。

Conclusion: 该研究首次系统性地关注遮挡视频预测问题，证明了结合点流和深度信息能显著提升视频预测模型在遮挡场景下的性能表现。
...
TL;DR: 本文提出了一种利用图像采集时间信息来改善乳腺DCE-MRI肿瘤分割的方法，通过FiLM层将时间信息融入模型特征中，提高了分割性能和模型泛化能力。
...
Method: 使用特征线性调制(FiLM)层将采集时间信息融入模型特征，利用完整的可变数量图像，在不同骨干架构上训练时间调制模型。
...
<details>
  <summary>Details</summary>
Motivation: 由于战斗运动的动态性和非结构化特性，以及记录环境的变化，开发鲁棒的数据集仍然是一个主要瓶颈。
...
Conclusion: 该数据集旨在加速拳击及相关领域的运动分析、自动化教练和性能评估方面的进展，特别支持低资源和无约束环境下的实时视觉动作识别研究。
...
<details>
  <summary>Details</summary>
Motivation: CLIP模型在否定或释义文本上的评估结果表现不一，因为否定会以最小的词汇变化根本改变含义，而释义可以创建具有相同含义但表达不同的文本，这对改进视觉语言模型的评估和对齐提出了重大挑战。

Method: 提出SemCLIP方法：评估释义和否定的组合，提出新的CLIP对比损失函数，应用LLM生成的包含原始、释义和否定文本描述的训练三元组到CLIP类训练模型中。

Result: SemCLIP将释义描述向原始图像嵌入移动，同时将否定描述在嵌入空间中推得更远。在CC-Neg基准测试中，原始对否定的图像检索准确率从68.1%提高到78.1%。在下游零样本分类任务中，SemCLIP在所有测试任务上都优于CLIP。
...
TL;DR: 提出一种用于体操裁判的鲁棒多摄像头体操运动员跟踪方法，通过结合体操领域知识解决摄像头数量有限和检测失败的问题。
...
Conclusion: 提出的多摄像头体操运动员跟踪方法通过结合领域知识和级联数据关联策略，有效解决了传统方法在摄像头受限和检测失败情况下的跟踪问题，在实际应用中表现出色。
...
Method: 实现Horn-Schunck算法的多分辨率版本，使用双线性插值和延拓技术来改进算法精度和收敛速度，同时分析局部和全局方法的性能。

Result: 多分辨率Horn-Schunck算法在估计帧间运动方面表现出改进的准确性和收敛性，特别是在不同图像条件下。

Conclusion: 结合多分辨率策略的Horn-Schunck算法能够有效提高光流计算的精度和效率，为运动估计提供了更可靠的解决方案。
...
TL;DR: 提出了一种新颖的两阶段检测框架，通过监督对比学习和k-NN分类器实现合成图像检测，在少样本学习下达到91.3%的平均检测准确率，显著优于现有方法。
...
Method: 第一阶段使用监督对比学习的视觉深度学习模型提取判别性嵌入，第二阶段采用k-NN分类器在嵌入空间进行少样本学习。

Result: 在每类仅150张图像的少样本学习下，平均检测准确率达到91.3%，比现有方法提高5.2个百分点；在源归属任务中，AUC和OSCR分别提高14.70%和4.27%。

Conclusion: 该框架为适应不断发展的生成AI环境提供了强大、可扩展的取证归属系统，无需进行详尽的重新训练。
...
TL;DR: EOGS++是基于3D高斯泼溅的卫星图像重建方法，直接处理原始高分辨率全色数据，通过光流技术嵌入束调整改进相机姿态估计，在重建质量和效率上优于原始EOGS和NeRF方法。


<details>
  <summary>Details</summary>
Motivation: 扩展地球观测高斯泼溅框架，开发专门针对卫星图像的新方法，避免外部预处理依赖，提高相机姿态估计精度。

Method: 直接在原始高分辨率全色数据上操作，利用光流技术在训练过程中嵌入束调整，引入早停和TSDF后处理等技术改进。

Result: 在IARPA 2016和DFC2019数据集上实现最先进的性能，建筑物平均MAE误差从1.33提升到1.19，优于原始EOGS和其他NeRF方法。

Conclusion: EOGS++在保持高斯泼溅计算优势的同时，显著提高了重建质量和几何精度，为卫星图像处理提供了高效解决方案。
...
TL;DR: 本文针对长尾分布下的2D目标检测问题，在LVISv1数据集上使用改进的Faster R-CNN架构和平衡组Softmax框架，并探索度量学习来提升稀有类别的检测性能，达到了24.5% mAP的新SOTA。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的目标检测面临长尾分布挑战，多数类别只有少量实例，导致检测模型偏向频繁类别，在稀有类别上性能下降。

Method: 采用两阶段Faster R-CNN架构，增强平衡组Softmax框架以缓解类别不平衡；探索度量学习生成特征嵌入，使类间分离、类内紧凑；推理时使用k-NN方法提升分类性能。

Result: 在LVISv1数据集上达到24.5%的平均精度均值(mAP)，超越了之前的24.0%基准，特别是在稀有类别上表现出色。

Conclusion: 所提出的方法有效提升了长尾目标检测性能，证明了平衡组Softmax和度量学习在解决类别不平衡问题中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 解决自然图像中3D物体重建的挑战，特别是在遮挡和场景杂乱的情况下，利用上下文视觉识别线索进行更准确的3D重建。

Method: 采用人机协同标注流程获取物体形状、纹理和姿态数据，结合合成预训练和真实世界对齐的多阶段训练框架，突破3D数据瓶颈。

Result: 在真实世界物体和场景的人类偏好测试中，相比近期工作获得显著提升，胜率至少为5:1。

Conclusion: SAM 3D在自然图像3D重建方面取得突破性进展，将发布代码、模型权重、在线演示和新的野外3D重建基准测试。
...
TL;DR: SurvAgent是首个用于多模态生存预测的分层思维链增强多智能体系统，通过两阶段方法解决现有病理智能体在生存预测中的局限性，在TCGA数据集上优于传统方法和医疗智能体。


<details>
  <summary>Details</summary>
Motivation: 现有生存分析方法缺乏临床所需的透明度，而当前病理智能体在生存预测中存在三个局限：无法整合多模态数据、无效的感兴趣区域探索、未能利用历史案例的经验学习。
...
Conclusion: SurvAgent通过分层思维链增强的多智能体系统，成功解决了多模态数据整合、区域探索和经验学习等关键问题，为临床可解释的生存预测提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D高斯扩散模型由于高斯基元数量庞大，导致去噪和后处理耗时严重，生成速度慢且沿采样轨迹的可扩展性有限。
...
TL;DR: 提出一种新颖的教师引导剪枝框架，将知识蒸馏与重要性分数估计紧密结合，实现一次性全局剪枝，减少迭代训练-剪枝-重训练的计算开销。
...
Method: 在重要性分数计算过程中利用教师模型的梯度信号，识别对任务性能和知识传递都关键的重要参数，实现一次性全局剪枝，剪枝后使用稀疏感知重训练恢复精度。
...
TL;DR: 本文对Cambrian-S的视频空间超感知研究提出批评，通过简单基线方法NoSense在VSR基准上达到95%准确率，并通过VSC-Repeat实验显示Cambrian-S方法可能利用基准中的捷径启发式而非真正的空间超感知能力。


<details>
  <summary>Details</summary>
Motivation: 批判性分析Cambrian-S在视频空间超感知方面的研究，验证其基准是否真正衡量空间认知能力，以及其推理方法是否依赖捷径而非稳健的空间超感知。

Method: 引入简单基线方法NoSense（仅使用词袋SigLIP模型），并在VSC基准上设计VSC-Repeat实验（将视频与自身连接1-5次），观察Cambrian-S性能变化。

Result: NoSense在4小时视频上达到95%准确率，几乎解决VSR基准；VSC-Repeat实验使Cambrian-S的平均相对准确率从42%降至0%，表明其依赖基准中的捷径（房间不被重复访问）。

Conclusion: 当前VSI-Super基准不能可靠衡量空间超感知能力，Cambrian-S的预测感知推理方法通过无意中利用基准捷径来提高性能，而非来自稳健的空间超感知。
...
TL;DR: PartUV是一个基于部件分割的UV展开方法，针对AI生成网格的噪声和不良条件问题，通过语义部件分解和几何启发式方法生成更少但部件对齐的图表，同时保持低失真。
...
Method: 基于PartField学习型部件分解方法，PartUV在自上而下的递归框架中结合高层语义部件分解和新型几何启发式方法，确保每个图表的失真低于用户指定阈值，同时最小化总图表数量。

Result: 在四个不同数据集上的评估显示，PartUV在图表数量和接缝长度方面优于现有工具和近期神经方法，达到可比较的失真度，在挑战性网格上具有高成功率，并支持部件特定的多瓦片打包等新应用。

Conclusion: PartUV提供了一个有效的部件基础UV展开解决方案，能够处理AI生成网格等挑战性情况，生成更少但更合理的图表布局。
...
<details>
  <summary>Details</summary>
Motivation: 随着3D动画需求增长，从文本描述生成高保真、可控的4D虚拟形象仍面临重大挑战。现有4D生成方法存在时间几何不一致、感知伪影、运动不规则、计算成本高和动态控制有限等根本性限制。

Method: 采用基于扩散的三平面重定位技术，使用自回归策略生成任意长度的4D序列，每个3D帧通过单一扩散过程合成。模型从大规模3D和运动数据集中显式学习3D结构和运动先验，支持骨架驱动的4D生成。

Result: 实验结果表明TriDiff-4D显著优于现有方法，通过消除优化过程将生成时间从数小时减少到数秒，同时大幅改善了复杂运动的生成质量，具有高保真外观和准确的3D几何形状。

Conclusion: TriDiff-4D在时间一致性、运动准确性、计算效率和视觉保真度方面表现出色，能够生成任意长度的4D序列，为4D虚拟形象生成提供了高效可靠的解决方案。
...
Result: 对六个最先进视频模型的评估显示在四个维度上存在明显差异，分析了视频模型与图像模型的比较、常见幻觉行为以及视频时长对帧链推理的影响。
...
TL;DR: 本文提出Video-Next-Event Prediction (VNEP)任务，将视频作为下一事件预测的回答模态，并开发了VANS模型，通过强化学习对齐视觉语言模型和视频扩散模型，实现从文本描述到视频生成的转变。


<details>
  <summary>Details</summary>
Motivation: 视频具有展示物理世界信息的独特能力，而现有语言模型在视频生成方面主要局限于娱乐应用。作者发现将视频作为下一事件预测的回答模态的未充分利用机会，能够提供更直观和定制化的答案。

Method: 提出VANS模型，使用强化学习（Joint-GRPO）将视觉语言模型与视频扩散模型对齐。视觉语言模型生成准确且易于可视化的字幕，视频扩散模型根据这些字幕和输入视觉上下文生成视频。

Result: 在程序和预测基准测试中，VANS在视频事件预测和可视化方面都达到了最先进的性能。

Conclusion: VNEP任务将视频作为新的回答模态，VANS模型通过强化学习成功实现了从文本描述到视频生成的转变，为程序学习和创意探索提供了更直观的解决方案。
...
TL;DR: 提出了一种让视觉语言模型根据任务难度自动切换快慢思维模式的方法，通过基于输出长度的思维模式标注和GRPO训练，实现高效推理。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型主要追求冗长的推理链，导致计算成本过高，而人类会根据问题复杂度自动调整思维速度，需要让模型具备类似的智能资源分配能力。
...
Conclusion: 提出的简单RL方法成功让视觉语言模型具备类似人类的快慢思维切换能力，在保持高性能的同时大幅提升推理效率。
...
TL;DR: 本文提出了Thinking-while-Generating (TwiG)框架，首次实现了在视觉生成过程中交织文本推理，使文本推理与视觉内容生成协同演进。


<details>
  <summary>Details</summary>
Motivation: 现有视觉生成方法仅在生成前（预规划）或生成后（后精炼）进行文本推理，缺乏在生成过程中实时的多模态交互。
...
### [79] [UniUltra: Interactive Parameter-Efficient SAM2 for Universal Ultrasound Segmentation](https://arxiv.org/abs/2511.15771)
...
TL;DR: UniUltra是一个用于通用超声分割的框架，通过上下文-边缘混合适配器和深度监督知识蒸馏技术，在保持参数效率的同时显著提升了SAM2在超声图像上的分割性能。


<details>
  <summary>Details</summary>
Motivation: SAM2在自然图像上表现出色，但在超声图像上性能显著下降，存在领域差异问题。需要解决如何高效适配SAM2到超声成像并保持参数效率，以及如何在资源受限的临床环境中有效部署的问题。

Method: 提出上下文-边缘混合适配器（CH-Adapter）增强细粒度感知能力，实现参数高效微调；开发深度监督知识蒸馏（DSKD）技术，将知识从大型图像编码器转移到超轻量编码器。

Result: UniUltra在广泛实验中优于最先进方法，具有优越的泛化能力。微调阶段仅使用SAM2参数的8.91%，最终压缩模型相比原始SAM2参数减少94.08%。

Conclusion: 该框架在保持竞争力的性能同时大幅减少计算需求，非常适合实际临床部署。
...
### [80] [Weakly Supervised Segmentation and Classification of Alpha-Synuclein Aggregates in Brightfield Midbrain Images](https://arxiv.org/abs/2511.16268)
*Erwan Dereure,Robin Louiset,Laura Parkkinen,David A Menassa,David Holcman*

Main category: eess.IV

TL;DR: 开发了一个基于弱监督分割的自动化图像处理流程，用于在帕金森病和中度路易体病的中脑组织全玻片图像中分割和分类α-突触核蛋白聚集物，能够区分路易体和神经突等主要形态，平衡准确率达到80%。


<details>
  <summary>Details</summary>
Motivation: 帕金森病与错误折叠的α-突触核蛋白聚集物（形成路易体和神经突）积累相关，这些聚集物用于病理诊断。深度学习自动分析免疫组织化学组织病理学图像为更好地理解这些聚集物的空间组织提供了有前景的工具。

Method: 基于弱监督分割开发自动化图像处理流程，使用ResNet50分类器，对免疫组织化学标记变异性具有鲁棒性，能够分割和分类全玻片图像中的α-突触核蛋白聚集物。

Result: 该方法能够区分主要聚集物形态，包括路易体和神经突，平衡准确率达到80%。

Conclusion: 该框架为大规模表征亮视野免疫组织化学组织中α-突触核蛋白聚集物的空间分布和异质性，以及研究其与周围细胞（如小胶质细胞和星形胶质细胞）之间了解甚少的关系铺平了道路。
...
### [82] [Gimballed Rotor Mechanism for Omnidirectional Quadrotors](https://arxiv.org/abs/2511.15909)
*J. Cristobal,A. Z. Zain Aldeen,M. Izadi,R. Faieghi*

Main category: cs.RO

TL;DR: 提出了一种万向节转子机构作为构建全向四旋翼的模块化高效解决方案，通过独立倾斜每个转子实现六自由度全驱动控制，无需对四旋翼中心结构进行重大改动。


<details>
  <summary>Details</summary>
Motivation: 传统四旋翼是欠驱动的，无法实现六自由度独立运动。现有全向四旋翼设计通常需要显著结构修改，本设计旨在提供轻量级且易于集成的解决方案。

Method: 在转子平台内集成伺服电机，实现每个转子的独立倾斜；开发了PX4自动驾驶仪中的新控制分配方案。

Result: 成功进行了飞行测试，验证了所提方法的有效性。

Conclusion: 万向节转子系统为构建全向四旋翼提供了一种模块化、高效且易于集成的解决方案。
...
### [83] [I've Changed My Mind: Robots Adapting to Changing Human Goals during Collaboration](https://arxiv.org/abs/2511.15914)
*Debasmita Ghose,Oz Gitelson,Ryan Jin,Grace Abawe,Marynel Vazquez,Brian Scassellati*

Main category: cs.RO

TL;DR: 提出了一种检测人类目标变化的方法，通过跟踪多个候选动作序列并与策略库验证其合理性。检测到目标变化后，机器人重新评估过去动作的信念，并使用Receding Horizon Planning树来选择既能协助人类又能揭示更新目标的行动。


<details>
  <summary>Details</summary>
Motivation: 在真实世界场景中，人类经常改变目标，这使得机器人在没有明确沟通的情况下难以适应。现有方法通常假设固定目标，将目标预测简化为一次性推理，无法有效处理目标变化的情况。

Method: 跟踪多个候选动作序列并验证其与策略库的合理性；检测到目标变化后，重新评估过去动作的信念；构建Receding Horizon Planning树来主动选择既能协助人类又能鼓励Differentiating Actions以揭示更新目标的行动。

Result: 在包含多达30个独特食谱的协作烹饪环境中进行评估，与三种可比的人类目标预测算法相比，该方法在所有基准测试中表现优异，能够在目标切换后快速收敛到正确目标，减少任务完成时间，提高协作效率。

Conclusion: 该方法能够有效检测人类目标变化并快速适应，显著改善了人机协作的效率，特别是在目标频繁变化的真实场景中。
...
### [84] [The Role of Consequential and Functional Sound in Human-Robot Interaction: Toward Audio Augmented Reality Interfaces](https://arxiv.org/abs/2511.15956)
*Aliyah Smith,Monroe Kennedy*

Main category: cs.RO

TL;DR: 研究探索了机器人声音对人类感知和行为的影响，包括操作声音和功能声音，特别研究了空间声音在定位和交接任务中的作用。
...
Method: 研究了Kinova Gen3机械臂的后果性声音和功能声音对人类感知的影响，包括通过定位和交接任务探索空间声音。

Result: Kinova Gen3机械臂的后果性声音没有负面影响感知；侧向空间定位非常准确，但正面定位准确性下降；空间声音能同时传达任务相关信息并促进温暖感、减少不适。

Conclusion: 功能和变革性听觉设计具有增强人机协作的潜力，为未来基于声音的交互策略提供了信息。
...
### [85] [PushingBots: Collaborative Pushing via Neural Accelerated Combinatorial Hybrid Optimization](https://arxiv.org/abs/2511.15995)
*Zili Tang,Ying Zhang,Meng Guo*

Main category: cs.RO

TL;DR: 本文提出了一种多机器人协作推动任意形状物体到目标位置的方法，解决了复杂环境中非抓取操作、混合接触模式切换和任务协调等挑战。


<details>
  <summary>Details</summary>
Motivation: 许多机器人没有配备机械手，许多物体不适合抓取操作（如大箱子和圆柱体）。在这些情况下，推动是一种简单有效的非抓取技能，但现有工作通常假设预定义的推动模式和固定形状物体。

Method: 基于组合混合优化的方法，包括三个主要组件：(I) 推动子任务的分解、排序和滚动分配给机器人子组；(II) 关键帧引导的混合搜索优化每个子任务的参数化推动模式序列；(III) 混合控制执行这些模式并在它们之间转换。还采用了基于扩散的加速器来预测关键帧和推动模式。

Result: 该框架在温和假设下是完备的，在不同数量机器人和一般形状物体下的效率和有效性在仿真和硬件实验中得到了广泛验证，并可推广到异构机器人、平面装配和6D推动。

Conclusion: 该方法能够有效解决多机器人系统在复杂环境中协作推动任意形状物体的任务协调和混合控制问题，具有较高的实用性和推广性。
...
### [86] [Semantic Glitch: Agency and Artistry in an Autonomous Pixel Cloud](https://arxiv.org/abs/2511.16048)
*Qing Zhang,Jing Huang,Mingyang Xu,Jun Rekimoto*

Main category: cs.RO

TL;DR: 本文提出了一种"低保真"的软体飞行机器人艺术装置"语义故障"，采用基于多模态大语言模型的语义导航方法，拒绝传统传感器，通过自然语言提示为机器人赋予生物启发式个性，创造具有叙事思维的机器人伴侣。


<details>
  <summary>Details</summary>
Motivation: 探索"低保真"方法的创意潜力，区别于主流机器人技术追求精度和完美性能的传统，旨在创造具有个性和叙事能力的机器人伴侣。

Method: 开发了一个新颖的自主管道，仅依赖多模态大语言模型的语义理解进行导航，拒绝LiDAR和SLAM等传统传感器；通过自然语言提示为机器人设计生物启发式个性，形成"叙事思维"。

Result: 通过13分钟自主飞行日志和后续研究统计验证了框架的鲁棒性，能够创造量化上不同的个性；观察到从地标导航到"计划到执行"差距等涌现行为，以及由于缺乏精确本体感觉而产生的不可预测但合理的行为特征。

Conclusion: 展示了一个低保真框架，用于创造不完美的机器人伴侣，其成功标准在于个性特征而非效率，为机器人艺术和叙事表达开辟了新途径。
...
### [87] [MagBotSim: Physics-Based Simulation and Reinforcement Learning Environments for Magnetic Robotics](https://arxiv.org/abs/2511.16158)
*Lara Bergmann,Cedric Grothues,Klaus Neumann*

Main category: cs.RO

TL;DR: 本文介绍了MagBotSim，一个基于物理的磁悬浮系统模拟器，将磁悬浮系统视为机器人集群，旨在通过整合运输和操作功能来革新工业自动化中的物料流。


<details>
  <summary>Details</summary>
Motivation: 磁悬浮系统在工业自动化中具有动态运输和操作的潜力，但尚未被充分开发。通过将运输和操作功能整合到协调的磁机器人集群中，可以显著提高制造系统的效率、适应性和紧凑性。

Method: 开发了MagBotSim（磁机器人模拟器），这是一个基于物理的磁悬浮系统模拟器，将磁悬浮系统框架化为机器人集群，为磁悬浮系统的智能算法开发提供支持。

Result: 提供了MagBotSim模拟器，包括文档、视频、实验和代码，为下一代由磁机器人驱动的制造系统奠定了基础。

Conclusion: 通过将磁悬浮系统视为机器人集群并提供专用模拟器，这项工作为下一代由磁机器人驱动的制造系统奠定了基础，有望实现更高的效率、适应性和紧凑性。
...
### [88] [FT-NCFM: An Influence-Aware Data Distillation Framework for Efficient VLA Models](https://arxiv.org/abs/2511.16233)
*Kewei Chen,Yayu Long,Shuai Li,Mingsheng Shang*

Main category: cs.RO

TL;DR: FT-NCFM是一个数据中心的生成式数据蒸馏框架，通过事实追踪引擎评估样本内在价值，然后合成模型无关、信息密集的可重用数据资产，在仅使用5%蒸馏核心集的情况下达到全数据集85-90%的性能，同时减少80%以上的训练时间。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言-动作模型的泛化能力受限于对海量、冗余且价值不均数据集的依赖，现有模型中心优化方法无法从根本上解决这一数据层面的挑战。

Method: 提出FT-NCFM框架，包含事实追踪引擎（结合因果归因和程序化对比验证来评估样本内在价值）和对抗性NCFM过程（合成模型无关、信息密集的可重用数据资产）。

Result: 在多个主流VLA基准测试中，仅使用5%蒸馏核心集训练的模型达到全数据集85-90%的成功率，同时减少80%以上的训练时间。
...
### [89] [How Robot Dogs See the Unseeable](https://arxiv.org/abs/2511.16262)
...
TL;DR: 该论文提出了一种基于动物侧向摆动行为的合成孔径感知方法，通过机器人执行侧向运动来创建宽合成孔径，有效消除遮挡物模糊，恢复背景清晰图像。


<details>
  <summary>Details</summary>
Motivation: 解决机器人视觉中因小孔径和大景深导致的遮挡问题，受动物侧向摆动行为启发，建立动物行为与合成孔径感知的正式联系。

Method: 让机器人执行侧向摆动运动，相机描述宽合成孔径，通过计算集成捕获的图像合成具有极浅景深的图像，模糊遮挡元素同时使背景清晰。

Result: 该方法能够实时恢复基本场景理解，并在多模态模型中实现高级视觉推理，对遮挡具有鲁棒性，计算效率高，可立即部署到任何移动机器人。

Conclusion: 通过侧向摆动进行合成孔径感知是复杂杂乱环境中实现高级场景理解的关键，成功桥接了动物行为与机器人技术。
...
### [90] [Funabot-Upper: McKibben Actuated Haptic Suit Inducing Kinesthetic Perceptions in Trunk, Shoulder, Elbow, and Wrist](https://arxiv.org/abs/2511.16265)
*Haru Fukatsu,Ryoji Yasuda,Yuki Funabora,Shinji Doki*

Main category: cs.RO

TL;DR: Funabot-Upper是一种可穿戴触觉套装，能够通过刺激关节和肌肉独立地诱导用户感知14种上半身运动，包括躯干、肩部、肘部和腕部动作，相比前代设计显著减少了感知混合问题。


<details>
  <summary>Details</summary>
Motivation: 现有可穿戴触觉设备主要局限于单个身体部位的验证，很少有设备能够将相同方法应用于多个身体部位。前代设计在肩部和肘部之间出现了感知混合问题，需要新的设计策略来解决这些问题。

Method: 建立了新的简化设计策略，开发了一种新型触觉套装，通过独立刺激关节和肌肉来诱导躯干、肩部、肘部和腕部的动觉感知。

Result: 实验证实Funabot-Upper成功诱导了多个关节的动觉感知，同时减少了前代设计中观察到的感知混合。新套装将识别准确率从前代Funabot-Suit的68.8%提高到94.6%。

Conclusion: 新设计策略有效解决了多部位触觉刺激中的感知混合问题，证明了该触觉套装在未来触觉应用中的优越性和潜力。
...
### [91] [Safe and Optimal Variable Impedance Control via Certified Reinforcement Learning](https://arxiv.org/abs/2511.16330)
*Shreyas Kumar,Ravi Prakash*

Main category: cs.RO

TL;DR: 提出C-GMS框架，通过数学定义的稳定增益流形采样，在强化学习中保证李雅普诺夫稳定性和执行器可行性，确保每个策略都是稳定且物理可实现的。


<details>
  <summary>Details</summary>
Motivation: 传统无模型强化学习在机器人控制中存在不稳定和不安全探索的风险，特别是阻抗增益随时间变化时。需要一种能保证稳定性和物理可行性的方法。

Method: 采用认证高斯流形采样(C-GMS)框架，将策略探索重新定义为从数学定义的稳定增益流形中采样，结合动态运动基元(DMPs)和变阻抗控制(VIC)。

Result: 在仿真和真实机器人上验证了有效性，确保即使在有界模型误差和部署时不确定性下也有界跟踪误差。

Conclusion: C-GMS为复杂环境中的可靠自主交互铺平了道路，通过构造保证稳定性和物理可行性，无需奖励惩罚或事后验证。
...
### [92] [Flow-Aided Flight Through Dynamic Clutters From Point To Motion](https://arxiv.org/abs/2511.16372)
*Bowen Xu,Zexuan Yan,Minghao Lu,Xiyu Fan,Yi Luo,Youshen Lin,Zhiqiang Chen,Yeke Chen,Qiyuan Qiao,Peng Lu*

Main category: cs.RO

TL;DR: 本文提出了一种基于强化学习的无人机动态避障系统，通过单激光雷达感知直接实现从点到运动的自主飞行，无需物体检测、跟踪和预测。


<details>
  <summary>Details</summary>
Motivation: 解决动态环境中高效感知环境动态和生成规避行为的挑战，避免传统方法中显式建模动态障碍物运动的耗时和不可靠问题。

Method: 使用深度感知距离图和点流作为环境变化感知表示，结合强化学习策略优化，通过相对运动调制距离场驱动规避行为。

Result: 所提系统在部署友好的传感仿真和无动力学模型加速控制下，表现出优异的成功率和适应性，仿真策略可在真实四旋翼上实现安全机动。

Conclusion: 通过环境变化感知表示和强化学习，实现了无需显式障碍物运动建模的高效动态避障系统，具有实际部署能力。
...
### [93] [Robot Metacognition: Decision Making with Confidence for Tool Invention](https://arxiv.org/abs/2511.16390)
*Ajith Anil Meera,Poppy Collis,Polina Arbuzova,Abián Torres,Paul F Kinghorn,Ricardo Sanz,Pablo Lanillos*

Main category: cs.RO

TL;DR: 本文提出了一种基于置信度的机器人元认知架构，通过让机器人评估自身决策的可靠性来提高在现实世界物理部署中的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前机器人缺乏真正的智能行为关键要素——反思自身认知过程和决策的能力。人类通过元认知（自我监控）来学习、决策和解决问题，例如评估执行任务的自信程度。

Method: 受神经科学启发，提出以置信度（对决策的二阶判断）为中心的机器人元认知架构，并在自主工具发明用例中演示。将置信度作为机器人决策方案中的元认知度量。

Result: 置信度感知的机器人能够评估其决策的可靠性，强调具身行动监控作为实现更明智决策的手段。

Conclusion: 机器人元认知架构通过置信度评估提高了决策的鲁棒性，并指出了机器人元认知的潜在应用和研究方向。
...
### [94] [Homogeneous Proportional-Integral-Derivative Controller in Mobile Robotic Manipulators](https://arxiv.org/abs/2511.16406)
*Luis Luna,Isaac Chairez,Andrey Polyakov*

Main category: cs.RO

TL;DR: 提出了一种新型的齐次PID控制策略，用于移动机器人机械臂的鲁棒协调运动控制，通过齐次控制理论增强系统稳定性和收敛性。


<details>
  <summary>Details</summary>
Motivation: 移动机器人机械臂由于非线性动力学、欠驱动特性以及基座与机械臂子系统之间的耦合，存在显著的控制挑战，需要开发能够处理动态不确定性和外部干扰的鲁棒控制方法。

Method: 设计了齐次PID控制结构，将传统PID增益推广为非线性、状态相关的函数，采用分级齐次性方法改进跟踪误差的收敛性，并使用Lyapunov方法进行稳定性分析。
...
### [95] [LAOF: Robust Latent Action Learning with Optical Flow Constraints](https://arxiv.org/abs/2511.16407)
*Xizhou Bu,Jiexi Lyu,Fulei Sun,Ruichen Yang,Zhiqiang Ma,Wei Li*

Main category: cs.RO

TL;DR: 本文提出LAOF框架，利用光流作为动作驱动信号来学习对干扰物具有鲁棒性的潜在动作表示，在标签稀缺条件下显著提升下游模仿学习和强化学习任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从大规模视频中学习潜在动作时容易受到动作无关干扰物的影响，而动作监督又受限于标签稀缺问题。光流能够自然抑制背景元素并强调运动物体，因此被用作动作驱动信号。

Method: 提出LAOF框架，这是一个伪监督框架，利用智能体的光流作为动作驱动信号来学习对干扰物具有鲁棒性的潜在动作表示。

Result: 实验结果表明，LAOF学习的潜在表示在下游模仿学习和强化学习任务中优于现有方法。光流约束显著稳定了训练过程，在极低标签比例条件下提高了表示质量，在10%标签比例下仍保持有效。
...
### [96] [From Prompts to Printable Models: Support-Effective 3D Generation via Offset Direct Preference Optimization](https://arxiv.org/abs/2511.16434)
*Chenming Wu,Xiaofan Li,Chengkai Dai*

Main category: cs.RO

TL;DR: SEG框架通过将支持结构优化集成到3D模型生成过程中，显著减少3D打印所需的支撑材料使用量


<details>
  <summary>Details</summary>
Motivation: 当前3D打印切片技术主要关注后处理优化，而非在设计阶段解决支撑结构效率问题，导致材料浪费和生产时间增加

Method: 提出SEG框架，将带偏移的直接偏好优化(ODPO)集成到3D生成流程中，在训练过程中融入支撑结构模拟，直接优化模型以减少支撑材料需求
...
### [97] [MiMo-Embodied: X-Embodied Foundation Model Technical Report](https://arxiv.org/abs/2511.16518)
...
### [98] [InternData-A1: Pioneering High-Fidelity Synthetic Data for Pre-training Generalist Policy](https://arxiv.org/abs/2511.16651)
...
TL;DR: 本文首次证明仅使用合成数据就能达到最强π数据集在VLA模型预训练中的性能，展示了大规模仿真的巨大价值。


<details>
  <summary>Details</summary>
Motivation: 探索真实数据和合成数据对视觉-语言-动作模型泛化能力的影响，当前VLA模型主要依赖大规模真实机器人预训练，而合成数据尚未展示出同等规模的能力。

Method: 构建包含63万条轨迹、7433小时的合成数据集InternData-A1，覆盖4种体现形式、18种技能、70项任务和227个场景，采用高度自主、完全解耦和组合式仿真流水线。

Result: 使用与π₀相同架构，仅用InternData-A1预训练的模型在49项仿真任务、5项真实世界任务和4项长序列灵巧任务上匹配官方π₀性能，并展现出惊人的零样本仿真到现实迁移能力。

Conclusion: 大规模合成数据能够替代真实数据实现同等性能的VLA模型预训练，为具身AI研究提供了可扩展的数据创建解决方案。
...
### [99] [Dexterity from Smart Lenses: Multi-Fingered Robot Manipulation with In-the-Wild Human Demonstrations](https://arxiv.org/abs/2511.16661)
...
TL;DR: AINA框架通过Aria Gen 2眼镜收集人类日常任务数据，学习多指机器人策略，无需机器人数据即可直接部署，解决了人机体现差异和从野外视频中提取运动线索的瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 目标是从人类在自然环境中执行日常任务的数据中学习多指机器人策略，减少对劳动密集型机器人数据收集的依赖，实现人类环境中通用机器人操作的重大进展。

Method: 使用轻便便携的Aria Gen 2眼镜收集数据，该设备配备高分辨率RGB相机、准确的3D头部和手部姿态估计，以及可用于场景深度估计的宽立体视图，学习基于3D点的多指手策略。

Result: 在九个日常操作任务中展示了结果，与先前的人到机器人策略学习方法相比表现更优，策略对背景变化具有鲁棒性，无需任何机器人数据即可直接部署。

Conclusion: AINA框架使从任何人、任何地点、任何环境收集的数据中学习多指机器人策略成为可能，为实现从人类视频学习机器人自主策略的梦想迈出了重要一步。
...
### [100] [A Crowdsourced Study of ChatBot Influence in Value-Driven Decision Making Scenarios](https://arxiv.org/abs/2511.15857)
*Anthony Wise,Xinyi Zhou,Martin Reimann,Anind Dey,Leilani Battle*

Main category: cs.HC

TL;DR: 研究表明，基于LLM的聊天机器人仅通过价值框架就能显著影响用户改变其预算决策，即使内容中立，框架本身也能产生说服效果，甚至在某些情况下引发反冲效应。


<details>
  <summary>Details</summary>
Motivation: 探索LLM聊天机器人是否仅通过价值框架（而非明显的偏见或错误信息）就能说服用户改变行为，特别是在政策决策领域。

Method: 通过众包研究，让336名参与者与中性或两种价值框架的聊天机器人互动，在决定调整美国国防开支时观察其预算选择变化。

Result: 暴露于价值框架聊天机器人的参与者相对于中性对照组显著改变了预算选择；当框架与个人价值观不一致时，部分参与者反而强化了原有偏好，显示出可复制的反冲效应。

Conclusion: 价值框架本身降低了LLM被用于操纵性用途的门槛，揭示了与明显偏见或错误信息不同的风险，并澄清了对抗错误信息时的风险。
...
### [101] [Panel-by-Panel Souls: A Performative Workflow for Expressive Faces in AI-Assisted Manga Creation](https://arxiv.org/abs/2511.16038)
*Qing Zhang,Jing Huang,Yifei Huang,Jun Rekimoto*

Main category: cs.HC

TL;DR: 提出了一个交互式系统，通过双混合管道解决文本到图像模型在渲染漫画面部表情时的模糊性问题。第一阶段结合地标自动检测和手动框架工具进行面部准备，第二阶段使用LivePortrait引擎映射表情，通过视频输入提供精细控制。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型在渲染漫画叙事所需的微妙面部表情方面存在困难，主要由于语言本身的模糊性。需要弥合艺术意图与AI执行之间的差距。

Method: 采用双混合管道：第一阶段结合地标自动检测和手动框架工具进行面部准备；第二阶段使用LivePortrait引擎，通过视频输入映射表情，提供精细控制。

Result: 案例研究表明，该集成工作流程可以简化创作过程，有效将叙事意图转化为视觉表达。

Conclusion: 这项工作为人类-AI协同创作提供了一个实用模型，为艺术家提供了更直接直观的方式为角色"注入灵魂"。主要贡献不是新的生成模型，而是弥合艺术意图与AI执行差距的新型交互工作流程。
...
### [102] [Heterogeneous Stroke: Using Unique Vibration Cues to Improve the Wrist-Worn Spatiotemporal Tactile Display](https://arxiv.org/abs/2511.16133)
...
<details>
  <summary>Details</summary>
Motivation: 由于手腕皮肤区域有限和空间敏锐度较低，腕戴式触觉显示器中相邻触觉器之间经常发生混淆，导致识别准确率低。此外，先前研究的准确率大多在特定姿势下测量，在现实生活中的自由手臂姿势下准确率会进一步下降。

Method: 提出Heterogeneous Stroke设计概念，为每个触觉器分配独特的振动触觉刺激，以减少触觉器之间的混淆。通过实现这一概念，在不同手臂姿势下传递字母数字字符。

Result: 通过Heterogeneous Stroke的实现，字母数字字符的传递准确率很高（26个字母为93.8%，10个数字为92.4%），且在不同手臂姿势下都能保持高准确率。

Conclusion: Heterogeneous Stroke设计概念能有效提高腕戴式触觉显示器上时空触觉模式的识别准确率，特别是在不同手臂姿势下仍能保持高准确率。
...
### [103] [Gaze Archive: Enhancing Human Memory through Active Visual Logging on Smart Glasses](https://arxiv.org/abs/2511.16214)
*Haoxin Ren,Feng Lu*

Main category: cs.HC

TL;DR: Gaze Archive是一种基于智能眼镜的视觉记忆增强系统，利用人眼注视作为自然注意力指示器，实现意图精准捕捉和无干扰交互。


<details>
  <summary>Details</summary>
Motivation: 解决信息过载导致的认知负担问题，传统视觉记忆增强方法要么费力干扰，要么无法与用户意图对齐。

Method: 开发GAHMA技术框架，通过注视实现紧凑且意图对齐的记忆编码，支持基于自然语言查询的直观记忆检索。

Result: 在GAVER数据集上的定量实验显示，GAHMA比非注视基线方法实现更精准的意图记录。用户研究表明其在感知轻松性、无干扰性和整体偏好方面具有优势。

Conclusion: Gaze Archive在意图精准性、易用性和无干扰性方面表现出色，具有实际部署的强潜力。
...
### [104] [Optimized User Experience for Labeling Systems for Predictive Maintenance Applications](https://arxiv.org/abs/2511.16236)
*Michelle Hallmann,Michael Stern,Francesco Vona,Ute Franke,Thomas Ostertag,Benjamin Schlueter,Jan-Niklas Voigt-Antons*

Main category: cs.HC

TL;DR: 本文介绍了一个用于德国农村地区列车和铁路基础设施监控与预测性维护系统的图形化标注用户界面的设计与实现，旨在通过成本效益高的无线监控系统和机器学习算法提高铁路运输的经济可行性和运营效率。


<details>
  <summary>Details</summary>
Motivation: 为了增强铁路运输的经济可行性和运营效率，项目采用经济高效的无线监控系统结合机器学习算法。成功的标注阶段对于训练监督式机器学习系统至关重要，因此需要设计用户友好的标注界面，能够优化整合到标注人员的日常工作流程中。

Method: 基于可用性启发式最佳实践设计了标注系统，并通过研究协议进行可用性和用户体验验证。系统结合经济实惠的传感器和机器学习算法，重点关注标注用户界面的优化设计。

Result: 标注系统已完成设计，研究协议已制定，待研究完成后将分享结果并进一步完善系统。

Conclusion: 这项工作具有降低维护成本和提高铁路运输服务可靠性的潜力，为学术文献贡献了有效标注用户界面的研究，并为工业界标注系统开发提供了实用见解。系统完成后将探索其在其他基础设施维护领域的可扩展性。
...
### [105] [Optimizing Predictive Maintenance: Enhanced AI and Backend Integration](https://arxiv.org/abs/2511.16239)
*Michael Stern,Michelle Hallmann,Francesco Vona,Ute Franke,Thomas Ostertag,Benjamin Schlueter,Jan-Niklas Voigt-Antons*

Main category: cs.HC

TL;DR: 提出了一种用于铁路运输的无线监测系统，集成传感器和机器学习技术，通过预测性维护提高铁路运输的可靠性和效率。


<details>
  <summary>Details</summary>
Motivation: 铁路运输的成功依赖于高效的维护以避免延误和故障，特别是在资源有限的农村地区。需要成本效益高的解决方案来应对这些挑战。

Method: 开发了安全的数据管理系统，在列车车厢和轨道段安装传感器收集结构和环境数据。采用稳健的后端基础设施进行安全数据传输、存储和分析，并与利益相关者合作设计系统。

Result: 系统支持预测性维护，能够在故障发生前识别潜在问题。讨论了传感器选择、数据处理协议和机器学习模型的设计原理。

Conclusion: 通过先进技术集成，该方法旨在提高铁路运输的可靠性和效率，为农村地区提供有效的维护解决方案。
...
### [106] [GazeInterpreter: Parsing Eye Gaze to Generate Eye-Body-Coordinated Narrations](https://arxiv.org/abs/2511.16245)
*Qing Chang,Zhiming Hu*

Main category: cs.HC

TL;DR: GazeInterpreter是一个基于大语言模型的新方法，通过解析眼动数据生成眼-体协调的行为叙述，结合符号化眼动解析、分层结构和自校正循环来理解人类行为。


<details>
  <summary>Details</summary>
Motivation: 现有工作主要关注身体行为，忽略了眼动及其与身体运动的协同作用在理解人类行为中的重要性。

Method: 1) 符号化眼动解析器将原始眼动信号转换为符号化眼动事件；2) 分层结构先用LLM生成语义级眼动叙述，然后在同一观察窗口内整合眼动和身体运动生成综合叙述；3) 自校正循环迭代优化模态匹配、时间连贯性和叙述完整性。

Result: 在Nymeria基准测试的文本驱动运动生成任务中验证了有效性，并在动作预测和行为总结等下游任务中实现了显著性能提升。

Conclusion: 解析眼动数据在理解人类行为方面具有巨大潜力，为人类行为理解开辟了新方向。
