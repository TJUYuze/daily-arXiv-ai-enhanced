...
Result: EfficientNet-B0表现优于DenseNet121：准确率84.6% vs 79.7%，F1分数0.8899 vs 0.8597，MCC 0.6849 vs 0.5852。两种模型的召回率均超过0.99，表明对肺炎检测具有高敏感性。可视化显示模型关注临床相关的肺部区域。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在复杂视频QA基准测试（如HD-EPIC VQA）中表现不佳，主要面临查询/选项模糊、长程时序推理能力差、输出非标准化等问题
...
Conclusion: 该研究强调了在要求严格的视频理解任务中，需要采用整体管道优化的方法，而非仅仅依赖单一模型改进
...
TL;DR: TuneCLIP是一种自监督微调框架，能够在无需从头训练的情况下提升现有CLIP模型在各种下游任务上的性能，避免了传统微调方法导致的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: CLIP作为多模态表示学习的基石，其性能提升通常需要从头训练数十亿样本，成本极高。本研究探索能否仅使用现有自监督数据集来提升开源CLIP模型在各种下游任务上的通用性能，而不是针对单一任务进行监督微调。

Method: TuneCLIP包含两个关键组件：1）预热阶段，通过恢复优化统计量来减少冷启动偏差（受理论分析启发）；2）微调阶段，优化新的对比损失函数以减轻对假阴性对的惩罚。

Result: 实验表明TuneCLIP在不同模型架构和规模上都能持续提升性能。特别地，它能将SigLIP（ViT-B/16）等领先开源模型的ImageNet及相关分布外基准提升高达+2.5%，在竞争激烈的DataComp基准上提升+1.2%，为高效的后预训练适应设立了新的强基线。

Conclusion: TuneCLIP提供了一种有效的自监督微调框架，能够在无需昂贵从头训练的情况下显著提升现有CLIP模型的通用性能，为多模态表示学习的后预训练适应提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于航拍图像的方法受限于采集频率低和不规则的时间安排，无法满足持续监测需求。需要一种能够利用全球可用的Sentinel-2数据，实现一致、频繁监测的解决方案。

Method: 采用生成式超分辨率框架，利用Sentinel-2的季节性合成图像，将10米分辨率的卫星图像超分辨率为0.5米冠层高度模型。

Result: 在美国西部22个EPA三级生态区域的评估中，VibrantSR对冠层高度≥2米的平均绝对误差为4.39米，优于Meta(4.83米)、LANDFIRE(5.96米)和ETH(7.05米)等卫星基准方法。

Conclusion: 虽然基于航拍的VibrantVS(2.71米MAE)精度更高，但VibrantSR能够在不需要昂贵且时间频率低的航拍数据的情况下，实现大洲尺度的可操作森林监测和碳核算。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学视觉语言模型在图像级文本任务上表现良好，但在3D医学影像中的细粒度视觉定位和体积空间推理方面仍面临挑战，需要统一的框架来整合这些能力。

Method: 提出MedVL-SAM2统一架构，整合图像级推理和像素级感知，采用SAM2体积分割模块。采用多阶段训练：首先在3D CT图像-文本对上预训练，然后在综合3D CT分割数据集上联合优化语言理解和分割目标。

Result: 模型在报告生成、视觉问答和多个3D分割任务上达到最先进性能，提供可靠的3D视觉定位、可控的交互式分割和鲁棒的跨模态推理。

Conclusion: MedVL-SAM2证明了在统一的3D医学视觉语言模型中，高级语义推理和精确的3D定位可以同时实现，为医学影像分析提供了多功能解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据驱动方法在分布外场景中分割性能下降，影响下游驾驶任务，需要提升可通行区域分割在OOD场景下的鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型虽然发展迅速，但在捕捉空间关系方面存在明显不足。这主要是因为CLIP风格的图像编码器训练方法将图像展平为1D补丁序列，丢弃了理解空间关系所必需的2D结构信息。这种空间意识的缺乏被认为是VLM设计中的一个缺失维度，也是机器人技术和具身AI等需要空间基础的应用的瓶颈。

Method: 作者通过两个主要方向来改进VLMs的空间推理能力：(1) 研究使用替代训练目标的图像编码器；(2) 探索2D位置编码方法。这些架构选择旨在恢复和增强模型对图像2D结构的理解能力。

Result: 实验结果表明，通过采用替代训练目标的图像编码器和2D位置编码，可以在多个基准测试上显著改善视觉语言模型的空间推理性能。

Conclusion: 空间关系理解是当前视觉语言模型的一个重要盲点，通过改进图像编码器的训练目标和引入2D位置编码，可以有效提升模型的空间推理能力，这对于需要空间基础的应用具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 现有的推理分割方法通常存在过度思考问题，生成冗长的推理链会干扰多模态大语言模型中的目标定位
...
Conclusion: DR²Seg框架有效解决了推理分割中的过度思考问题，无需额外思考监督就能同时提升推理效率和分割准确性
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注视频中事件、对象和场景存在的幻觉，而忽略了事件关系幻觉。视频大语言模型在处理事件间关系时存在严重问题，特别是在因果、时序和子事件关系方面。

Method: 提出了VERHallu基准，包含关系分类、问答和反事实问答三种任务类型，专门评估事件关系幻觉。同时提出了关键帧传播策略，在中间层重新分配帧级注意力以增强多事件理解。

Result: 实验表明当前最先进的VideoLLMs在密集事件关系推理方面表现不佳，过度依赖先验知识而未能充分利用帧级线索。提出的KFP策略能有效缓解事件关系幻觉，且不影响推理速度。

Conclusion: 事件关系幻觉是VideoLLMs中一个被忽视但重要的问题，需要专门的方法来解决。提出的VERHallu基准和KFP策略为解决这一问题提供了有效工具和方法。
...
<details>
  <summary>Details</summary>
Motivation: 水下成像面临波长相关光衰减、悬浮颗粒强散射、浑浊模糊和非均匀照明等挑战，传统相机难以获取真实运动信息。事件相机具有微秒分辨率和动态范围优势，但缺乏结合真实水下光学与准确光流的数据集限制了其在水下环境的研究进展

Method: 基于物理光线追踪渲染水下RGBD序列，通过现代视频到事件转换管道生成真实事件数据流，提供密集真值光流、深度和相机运动信息

Result: 建立了首个合成水下事件相机光流基准数据集，评估了最先进的基于学习和模型的光流预测方法，分析了水下光传输对事件形成和运动估计精度的影响

Conclusion: 该数据集为未来水下事件感知算法的开发和评估建立了新基准，公开了源代码和数据集
...
TL;DR: 提出DiCo框架，通过解耦概念表示实现文本到图像行人重识别的分层跨模态对齐，在多个数据集上达到SOTA性能并提升可解释性。
...
Result: 在CUHK-PEDES、ICFG-PEDES和RSTPReid数据集上的广泛实验表明，该框架达到了与最先进方法相竞争的性能，同时通过显式的槽位和块级表示增强了可解释性，实现更细粒度的检索结果。

Conclusion: DiCo框架通过分层解耦的概念表示有效解决了文本到图像行人重识别中的模态对齐和细粒度区分问题，在性能和可解释性方面都有显著提升。
...
Method: 提出RAG-3DSG：1）通过重拍引导的不确定性估计减少聚合噪声；2）通过可靠低不确定性对象支持对象级检索增强生成；3）动态下采样映射策略加速跨图像对象聚合

Result: 在Replica数据集上，RAG-3DSG显著提高了3D场景图生成中的节点标注准确性，同时将映射时间减少了三分之二
...
### [13] [See Less, Drive Better: Generalizable End-to-End Autonomous Driving via Foundation Models Stochastic Patch Selection](https://arxiv.org/abs/2601.10707)
*Amir Mallak,Erfan Aasi,Shiva Sreeram,Tsun-Hsuan Wang,Daniela Rus,Alaa Maalouf*

Main category: cs.CV

TL;DR: 提出随机补丁选择(SPS)方法，通过在训练时随机屏蔽部分补丁特征来提高端到端自动驾驶策略的OOD鲁棒性、泛化性和效率


<details>
  <summary>Details</summary>
Motivation: 基于基础模型提取的补丁对齐特征存在高度冗余性（90%方差仅需17/64个主成分），导致策略过拟合虚假相关性，损害OOD鲁棒性

Method: 提出随机补丁选择(SPS)：对每一帧随机屏蔽部分补丁描述符，同时保持剩余补丁的空间布局，让策略基于不同但完整的场景视图进行训练

Result: 在所有OOD场景中超越SOTA，平均提升6.2%，闭环模拟中最高提升20.4%，速度提升2.4倍，9个系统中8个超越先前SOTA，无需调优即可迁移到真实物理车辆

Conclusion: SPS通过强制策略学习对特定补丁不变的鲁棒特征，显著提高了端到端自动驾驶策略的OOD鲁棒性、泛化能力和效率
...
<details>
  <summary>Details</summary>
Motivation: 视频生成模型已展现出链式帧推理能力，能进行逐帧视觉推理，但在文本到图像生成领域的潜力尚未充分探索，因为T2I生成过程缺乏明确的视觉推理起点和可解释的中间状态
...
Conclusion: 视频模型在推进高质量文本到图像生成方面具有巨大潜力，CoF-T2I通过链式帧推理实现了显式、可解释的图像生成过程
...
TL;DR: 提出基于3D高斯泼溅的流引导几何对流框架，实现后印象派风格的3D风格迁移，通过提取2D绘画中的方向流场并反向传播到3D空间，使高斯基元形成流对齐的笔触，实现几何抽象而非表面纹理投影。


<details>
  <summary>Details</summary>
Motivation: 现有3D风格迁移方法大多将几何视为刚性基底进行表面纹理投影，这与后印象派艺术"在本质中寻求夸张"的原则相悖。后印象派强调几何抽象作为主要表达手段，因此需要开发能够真正再现后印象派风格化的方法。

Method: 提出流引导几何对流框架：1）基于投影的、无网格的流引导机制，将2D艺术运动传递到3D高斯几何中；2）亮度-结构解耦策略，将几何变形与颜色优化分离，减少激进结构抽象时的伪影；3）VLM作为评判者的评估框架，通过美学判断而非传统像素级指标评估艺术真实性。

Result: 该方法能够在无显式网格先验的情况下，使高斯基元形成符合场景拓扑的流对齐笔触，实现由绘画运动驱动的表达性结构变形，而非受光度约束。

Conclusion: 该框架成功实现了后印象派风格的3D风格迁移，将几何抽象作为主要表达手段，通过流引导机制将2D艺术运动转化为3D几何变形，为艺术风格化提供了更符合艺术原则的新方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的多模态学习方法严重依赖大规模人工标注数据集，这些数据集获取成本高且耗时，需要一种无需人工标注的自改进方法

Method: 建立问答者和解答者两个角色的协同进化循环：问答者通过对比直觉猜测与推理结果的双轨推理奖励学习生成高质量挑战性问题；解答者使用基于自身响应多数投票的伪标签进行优化；两者通过组相对策略优化(GRPO)迭代训练

Result: 在Qwen2.5-VL-7B-Instruct模型上，无需任何人工标注，V-Zero实现了视觉数学推理+1.7和通用视觉中心任务+2.6的性能提升

Conclusion: V-Zero展示了多模态系统自改进的潜力，为减少对昂贵人工标注数据的依赖提供了有效解决方案
...
TL;DR: InfoSculpt：基于信息瓶颈原则的广义类别发现新框架，通过双条件互信息目标在表示空间中分离类别定义信号与实例特定噪声
...
Conclusion: 从信息论视角重新构建GCD问题，InfoSculpt通过系统性地雕刻表示空间，在保留类别信息的同时丢弃噪声细节，为开放世界应用提供了更有效的解决方案
...
TL;DR: FlowAct-R1是一个实时交互式人形视频生成框架，基于MMDiT架构，支持任意时长视频流式合成，在480p分辨率下达到稳定25fps，首帧延迟仅约1.5秒。


<details>
  <summary>Details</summary>
Motivation: 现有视频合成方法在高保真合成和实时交互需求之间存在权衡问题，需要一种既能保持高质量又能实现低延迟响应的交互式人形视频生成方案。

Method: 基于MMDiT架构，采用分块扩散强制策略及其自强制变体，结合高效蒸馏和系统级优化，实现全身体细粒度控制。

Result: 在480p分辨率下达到稳定25fps，首帧延迟约1.5秒，实现了卓越的行为生动性和感知真实感，并在不同角色风格上保持鲁棒泛化能力。

Conclusion: FlowAct-R1成功解决了交互式人形视频生成中高保真与实时性的权衡问题，为自然流畅的交互体验提供了有效解决方案。
...
TL;DR: MathDoc是首个针对真实高中数学试卷文档级信息提取的基准测试，包含3,609个带有真实噪声的问题，专门评估模型在视觉噪声下的提取能力和拒绝不可识别输入的能力。
...
Method: 提出MathDoc基准，包含3,609个精心策划的真实高中数学试卷问题，包含真实噪声和明确不可识别样本。设计多维评估框架，涵盖题干准确性、视觉相似度和拒绝能力三个维度。

Result: 实验表明，包括Qwen3-VL和Gemini-2.5-Pro在内的SOTA多模态大语言模型在端到端提取方面表现良好，但在拒绝不可识别输入方面一致失败，反而产生自信但无效的输出。

Conclusion: 当前MLLMs在退化文档条件下存在关键缺陷，无法可靠拒绝不可识别输入。MathDoc为评估模型在真实世界噪声文档中的可靠性提供了重要基准，揭示了模型可靠性的关键差距。
...
TL;DR: 本文提出了一种新的视觉上下文学习框架，通过多组合协作融合方法，利用多个高质量提示生成互补的上下文表示分支，而非将多个提示压缩为单一表示，从而提升模型的推理能力和预测准确性。


<details>
  <summary>Details</summary>
Motivation: 当前主流的"检索-提示"方法通常只选择单一最佳视觉提示，丢弃了其他合适候选的宝贵上下文信息。即使最近的研究尝试融合前K个提示，也只是简单地将多个丰富信号压缩成一个，限制了模型的推理能力。需要一种更全面、协作的融合方法来充分释放这些多样化上下文的潜力。
...
Result: 在多种任务上的广泛实验，包括前景分割、单目标检测和图像着色，证明了该方法具有强大的跨任务泛化能力、有效的上下文融合能力，以及比现有方法产生更稳健和准确预测的能力。

Conclusion: 通过多组合协作融合方法，能够更好地利用视觉上下文学习中的多样化提示信息，显著提升模型的推理能力和预测性能，为视觉任务处理提供了更有效的框架。
...
TL;DR: 本文提出了一种端到端的视觉上下文学习框架，通过自适应融合模块聚合多个提示的关键信息，并引入排列特定的轻量级MLP来解耦布局先验，同时采用双向微调机制增强融合模块与修复模型之间的协作。
...
Method: 1）自适应融合模块：聚合多个提示的关键模式和标注信息，形成更精确的上下文提示；2）排列特定的轻量级MLP：解耦布局先验与核心模型，同时最小化对整体模型的影响；3）双向微调机制：交换查询和提示的角色，鼓励模型从融合上下文中重建原始提示，增强融合模块与修复模型之间的协作。
...
TL;DR: VQ-Seg提出了一种基于向量量化的半监督医学图像分割方法，通过量化扰动模块替代传统的dropout，解决了dropout率难以调优的问题，并在肺癌CT数据集上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有半监督医学图像分割方法通常使用dropout进行特征扰动，但dropout率是一个敏感且难以优化的超参数，容易导致次优的正则化效果。需要一种更可控的扰动方法。

Method: 提出VQ-Seg方法：1) 使用向量量化离散化特征空间；2) 设计量化扰动模块，通过打乱码书索引的空间位置实现可控扰动；3) 采用双分支架构，量化后特征空间同时用于图像重建和分割任务；4) 引入后量化特征适配器，融合基础模型的高层语义信息。

Result: 在包含828个CT扫描的肺癌数据集和其他公开基准测试上进行了广泛实验，结果表明该方法优于现有最先进方法。

Conclusion: VQ-Seg通过向量量化实现了更有效和可控的特征扰动，解决了传统dropout方法的局限性，在医学图像分割任务中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态潜在推理常依赖外部监督，忽略了内在视觉注意力动态。研究发现蒸馏过程中存在关键感知鸿沟：学生模型经常模仿教师的文本输出，却关注完全不同的视觉区域，实际上依赖语言先验而非基础感知

Method: 提出LaViT框架，通过强制学生自回归地重构教师的视觉语义和注意力轨迹来进行潜在视觉思维对齐，采用课程感知门控机制防止捷径学习
...
TL;DR: 该论文提出了视频异常推理(VAR)新任务，从描述性理解提升到结构化多阶段推理，并构建了包含8,641个视频和50,000+样本的大型数据集，提出了基于PerCoAct-CoT的标注框架和Vad-R1-Plus模型。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在视频异常检测与理解领域主要局限于异常定位或事后描述，缺乏显式推理过程、风险意识和决策导向的解释，需要从描述性理解提升到结构化推理。

Method: 1) 定义视频异常推理(VAR)新任务；2) 构建包含8,641个视频和50,000+样本的大型数据集；3) 采用Perception-Cognition-Action Chain-of-Thought (PerCoAct-CoT)结构化标注框架；4) 提出异常感知组相对策略优化方法；5) 开发端到端的Vad-R1-Plus模型支持自适应分层推理和风险感知决策。

Result: 提出的基准和方法有效提升了MLLMs在VAR任务上的推理能力，在开源和专有基线模型上都取得了优越性能，证明了结构化推理框架的有效性。

Conclusion: 视频异常推理(VAR)任务将视频异常分析从描述性理解提升到结构化多阶段推理，通过PerCoAct-CoT框架和Vad-R1-Plus模型实现了视觉感知、因果解释和风险感知决策的集成，为视频异常理解提供了新的研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖3D数据先验（泛化能力差），要么依赖2D生成先验（计算量大且容易产生身份幻觉）。作者发现这两种先验具有互补性，希望设计一个高效系统，实现高质量、可动画的化身合成，并具有强大的野外泛化能力。

Method: 1. 引入前馈Mesh2Gaussian先验模型（MGPM），实现高斯化身的快速初始化。2. 设计测试时生成适应阶段，利用真实和合成图像作为监督。3. 提出渲染引导的单步扩散增强器，基于高斯化身渲染恢复缺失的视觉细节，避免传统全扩散去噪策略的缓慢和幻觉问题。

Result: 实验表明，ELITE在视觉质量上优于先前工作，即使在挑战性表情下也能生成高质量化身，同时比2D生成先验方法快60倍。

Conclusion: ELITE通过结合3D数据先验和2D生成先验的优势，设计了一个高效的高斯头部化身合成系统，实现了高质量的化身生成和强大的野外泛化能力，同时显著提升了计算效率。
...
Method: 提出DepthDirector框架：1) 使用显式3D表示的深度视频作为相机控制引导；2) 设计视图-内容双流条件机制，将源视频和目标视角下渲染的变形深度序列注入预训练视频生成模型；3) 采用轻量级LoRA-based视频扩散适配器进行训练；4) 使用Unreal Engine 5构建大规模多相机同步数据集MultiCam-WarpData。

Result: 大量实验表明，DepthDirector在相机可控性和视觉质量方面均优于现有方法。该方法能够精确控制相机轨迹，同时保持视频内容的忠实再现。

Conclusion: DepthDirector通过深度视频引导和双流条件机制，有效解决了视频生成中的精确相机控制问题，充分利用了视频扩散模型的3D理解能力，实现了精确相机控制和一致内容生成的平衡。
...
TL;DR: 该论文提出了一种自动识别和突出显示幻灯片相关区域的方法，通过分析演讲者的叙述内容并与幻灯片中的文本或图形元素匹配，确保听众听到的内容与他们需要关注的内容之间更好的同步。


<details>
  <summary>Details</summary>
Motivation: 在观看演示时，听众需要同时关注演讲者的叙述和幻灯片内容，但识别幻灯片中的相关区域具有挑战性。当听众专注于幻灯片的某一部分时，演讲者可能已经进入新的句子，导致视觉追赶，造成听觉内容和重要视觉元素之间的脱节，特别是在快节奏或内容密集的演示中难以吸收关键细节。

Method: 引入一种自动识别和突出显示最相关幻灯片区域的方法，通过分析演讲者的叙述内容，并将其与幻灯片中的文本或图形元素进行匹配，探索不同的解决方案方法。

Result: 评估了不同方法的成功和失败案例，代码和数据集已在GitHub上公开可用，网址为：https://github.com/meghamariamkm2002/Slide_Highlight。

Conclusion: 分析多媒体文档正在成为无缝理解内容丰富视频（如教育视频和会议演讲）的关键需求，通过减少认知负担和提高理解能力，该方法有助于改善演示体验。
...
TL;DR: 提出分层精炼攻击（HRA），一种针对视觉语言预训练模型的多模态通用对抗攻击框架，通过样本级和优化级精炼实现高效通用攻击
...
Method: 1. 图像模态：将对抗样本解耦为干净图像和扰动，分别处理以更有效破坏跨模态对齐；引入ScMix增强策略多样化视觉上下文，增强UAP的全局和局部效用。2. 优化路径精炼：利用历史和估计未来梯度的时间层次结构避免局部极小值，稳定通用扰动学习。3. 文本模态：结合句内和句间重要性度量识别全局有影响力的词，作为通用文本扰动

Result: 在多种下游任务、VLP模型和数据集上的广泛实验证明了所提出的通用多模态攻击的优越性

Conclusion: HRA框架通过分层精炼策略实现了对VLP模型的高效通用对抗攻击，克服了传统样本特定攻击的计算瓶颈
...
TL;DR: ROMA是一个实时全模态助手，能够处理连续的音频-视频流输入，实现统一的反应式和主动式交互，在主动任务上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有的全模态大语言模型在流式音频-视频理解方面存在挑战，存在模态支持不完整或缺乏自主主动监控能力的问题，需要统一的实时全模态解决方案

Method: ROMA将连续输入处理为同步的多模态单元，对齐密集音频和离散视频帧；引入轻量级说话头解耦响应启动和生成；使用两阶段课程学习训练，包括流式格式适应和主动响应性优化

Result: 在12个基准测试中，ROMA在主动任务（警报、叙述）上达到最先进性能，在反应式任务（问答）上具有竞争力，验证了其在统一实时全模态理解中的鲁棒性

Conclusion: ROMA通过统一的实时全模态处理框架，成功解决了流式音频-视频理解中的模态不匹配和主动监控问题，为实时多模态交互提供了有效解决方案
...
TL;DR: 论文提出T2G范式，通过让LLM文本编码器推理并重写用户提示，然后使用重写后的提示作为扩散条件，从而超越传统的文本-像素映射，实现更智能的图像生成。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型仅将大语言模型用作文本编码器，未能利用其内在推理能力来推断文本提示应如何视觉化呈现。需要超越这种字面生成，实现基于推理的图像合成。

Method: 提出think-then-generate范式：1) 通过轻量级监督微调激活LLM编码器的推理-重写模式；2) 使用Dual-GRPO共同优化LLM编码器和扩散主干，LLM编码器通过图像基础奖励进行强化以推断和回忆世界知识，扩散主干则被推动生成语义一致且视觉连贯的图像。

Result: 在基于推理的图像生成和编辑基准测试中，在事实一致性、语义对齐和视觉真实性方面取得显著改进，WISE分数达到0.79，接近GPT-4水平。

Conclusion: T2G范式是迈向下一代具有推理、表达和演示能力的统一模型的有希望的一步，能够实现更智能、更符合语义的图像生成。
...
TL;DR: 本文提出了HPE-Bench基准测试和基于MLLM的统一框架，用于解决文本引导人体姿态编辑中的结构异常和生成伪影问题，通过对比LoRA调优和层敏感性分析实现姿态评估的最优特征层选择。
...
Method: 1. 引入HPE-Bench基准测试，包含来自17个最先进编辑模型的1700个标准化样本，提供真实性标签和多维质量评分；2. 提出基于层选择性多模态大语言模型（MLLM）的统一框架，采用对比LoRA调优和新型层敏感性分析（LSA）机制识别姿态评估的最优特征层。

Result: 该框架在真实性检测和多维质量回归方面均实现了优越性能，有效弥合了取证检测与质量评估之间的差距。

Conclusion: HPE-Bench基准测试和基于MLLM的统一框架为解决文本引导人体姿态编辑中的评估问题提供了有效解决方案，通过层敏感性分析机制实现了更准确的姿态一致性评估。
...
TL;DR: DiffCR是一种基于扩散模型的图像压缩框架，通过一致性先验精炼实现高效高保真图像重建，相比现有方法在比特率节省和解码速度上都有显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散先验的图像压缩方法存在采样过程缓慢和比特分配次优的问题，这源于碎片化的训练范式。需要开发一种既能保持视觉质量又能提高效率的压缩框架。
...
Conclusion: DiffCR通过一致性先验精炼有效解决了扩散基图像压缩中的效率问题，在保持高质量重建的同时大幅提升了压缩效率和速度，为实际应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在端到端OCR中的成功表明文本信息可以进行低损失压缩。先前工作将Transformer输入渲染为图像进行预填充，减少了token数量，但只在预填充阶段有效，无法在逐token推理时节省计算或内存成本。

Method: 提出VIST2 Transformer，将输入文本块与其视觉编码交错排列，在预测下一个文本token分布时仅依赖前文中的视觉token。将文本块渲染为草图图像，采用多阶段训练：从课程调度的光学语言建模预训练开始，然后进行模态交错指令微调。

Result: 在0.6B到8B不同规模的VIST2模型上进行广泛实验。以4倍压缩比，模型在长文本写作任务上显著优于基线，平均实现首token生成速度提升3倍，内存使用减少77%，FLOPS减少74%。

Conclusion: VIST2通过全局上下文压缩技术，在预填充和推理阶段都有效减少token数量，显著提升了长文本生成的效率和资源利用率，为文本压缩和高效推理提供了新思路。
...
TL;DR: 提出了一种缺失感知的多模态生存预测框架，整合CT、WSI和临床数据用于NSCLC生存预测，通过基础模型特征提取和缺失感知编码，实现中间融合并处理缺失模态


<details>
  <summary>Details</summary>
Motivation: NSCLC生存预测需要整合异质临床、影像和病理信息，但多模态深度学习面临小样本和模态缺失问题，现有方法常需丢弃患者或激进插补，限制了临床适用性

Method: 使用基础模型进行模态特定特征提取，采用缺失感知编码策略，提出中间融合架构，该架构对缺失模态具有鲁棒性，可充分利用所有可用数据而不丢弃患者

Result: 中间融合策略持续优于单模态基线以及早期和晚期融合，WSI与临床模态融合表现最佳（C-index 73.30），模态重要性分析显示模型能自动降低低信息模态（如CT）的权重

Conclusion: 提出的缺失感知多模态生存框架能有效处理自然不完整的模态数据，提高NSCLC生存预测性能，为临床决策提供更可靠的预后工具
...
<details>
  <summary>Details</summary>
Motivation: 荧光显微镜在活体生物样本分析中广泛应用，但记录结果常受噪声、时间变异性和信号振荡可视化不一致的限制，需要一种方法将多时间帧信息整合为高质量图像

Method: 提出独特计算框架，结合来自不同计算机视觉应用领域的可解释技术，将多时间分辨帧信息融合为单张高质量图像，保留原始视频生物内容

Result: 在111种配置和包含动态、异质、形态复杂的心脏细胞2D单层数据集上评估，框架能生成保留并增强单个显微镜帧质量和信息的复合图像，相比先前方法平均细胞计数增加44%
...
TL;DR: Lunar-G2R：首个从地形几何直接推断空间变化反射率模型的方法，通过U-Net和可微分渲染从数字高程模型预测月球表面的空间变化BRDF参数，无需多视角图像或专用硬件。
...
Method: 提出Lunar-G2R几何到反射率学习框架，使用U-Net架构结合可微分渲染，从月球数字高程模型直接预测空间变化的BRDF参数。通过最小化真实轨道图像与物理渲染图像之间的光度差异进行训练，无需多视角图像、受控照明或专用硬件。

Result: 在Tycho陨石坑地理保留区域的实验中，相比最先进的基线方法，光度误差减少38%，同时获得更高的PSNR和SSIM分数，以及更好的感知相似性，能够捕捉空间均匀模型无法呈现的精细尺度反射率变化。

Conclusion: 这是首个从地形几何直接推断空间变化反射率模型的方法，显著提高了月球表面渲染的光度真实感，为高保真渲染和基于视觉的导航提供了重要支持。
...
TL;DR: 本文提出SocioSeg数据集和SocioReasoner框架，通过视觉语言模型推理实现城市社会语义分割，解决了传统方法难以处理社会定义类别（如学校、公园）的问题。
...
Method: 提出了SocioReasoner视觉语言推理框架，通过跨模态识别和多阶段推理模拟人类识别和标注社会语义实体的过程。使用强化学习优化这个不可微分的过程，激发视觉语言模型的推理能力。同时构建了SocioSeg数据集，包含卫星图像、数字地图和分层结构的社会语义实体像素级标签。

Result: 实验表明该方法在现有最先进模型基础上取得了显著提升，并展现出强大的零样本泛化能力。数据集和代码已在GitHub上开源。

Conclusion: 通过视觉语言模型推理实现了城市社会语义分割，提出的SocioReasoner框架和SocioSeg数据集为解决社会定义类别的分割问题提供了有效方案，具有重要的实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（如CLIP）在微调时容易出现灾难性遗忘，现有方法主要关注减轻遗忘过程，但遗忘往往不可避免。作者提出在模型已经微调后恢复预训练知识的新范式

Method: 提出MERGETUNE方法，基于线性模式连接性（LMC）指导持续微调。通过继续微调可训练参数（如soft prompts或线性头），寻找同时连接零样本模型和微调模型的低损失路径模型，利用损失景观几何特性隐式合并两个解决方案

Result: MERGETUNE将CoOp的基类-新类泛化调和平均值提升+5.6%，无需增加参数。在DTD和EuroSAT数据集上首次超越CLIP性能。在鲁棒微调评估中，LMC合并模型超越集成基线且推理成本更低，与零样本模型集成时获得进一步增益和SOTA结果

Conclusion: 提出持续微调新范式，通过MERGETUNE方法有效恢复微调过程中丢失的预训练知识，提升模型泛化能力和鲁棒性，为视觉语言模型微调提供了新的解决方案
...
TL;DR: SatMap：一种在线矢量高精地图构建方法，通过融合卫星地图与多视角摄像头观测，直接预测矢量HD地图，显著提升自动驾驶系统的地图构建性能。


<details>
  <summary>Details</summary>
Motivation: 现有的车载摄像头方法存在深度感知有限和遮挡导致的精度下降问题，需要一种能克服这些限制的高精地图构建方案。

Method: 提出SatMap方法，整合卫星地图（提供鸟瞰视角的车道级语义和纹理信息作为全局先验）与多视角摄像头观测，直接预测矢量化的高精地图。

Result: 在nuScenes数据集上，相比纯摄像头基线提升34.8% mAP，相比摄像头-激光雷达融合基线提升8.5% mAP；在长距离和恶劣天气条件下也表现出优势。

Conclusion: SatMap通过融合卫星先验地图有效缓解了深度模糊和遮挡问题，为下游预测和规划模块提供了高质量的在线矢量高精地图。
...
TL;DR: FUSE-Bike平台与BikeActions数据集：首个从骑行者视角采集的VRU行为理解开放平台与多模态数据集，用于提升密集共享空间中的VRU意图预测
...
Result: 发布了首个从骑行者视角的VRU行为理解开放平台和数据集，建立了该任务的性能基准，并公开了完整数据集、数据管理工具、开放硬件设计和基准代码。

Conclusion: FUSE-Bike平台和BikeActions数据集填补了密集共享空间中VRU行为理解的研究空白，为未来VRU动作理解研究提供了重要资源，有望促进自动驾驶和移动机器人领域的安全发展。
...
TL;DR: 该研究使用深度学习模型FFA-Net和AECR-Net来增强烟雾和雾霾环境下模拟仪表的可读性，通过Unreal Engine生成合成数据集进行训练，取得了良好的去雾效果，为应急响应提供技术支持。
...
Method: 研究采用两种深度学习架构FFA-Net和AECR-Net来增强受雾霾和烟雾影响的仪表图像可见度。由于缺乏模拟仪表基准数据集，使用Unreal Engine生成了包含超过14,000张图像的合成数据集。采用80%训练、10%验证和10%测试的数据划分方案。

Result: 对于合成雾霾数据集，SSIM和PSNR指标分别达到约0.98和43dB，与最先进结果相当。AECR-Net相比FFA-Net表现更稳健。虽然合成烟雾数据集的结果较差，但训练模型仍取得有趣成果。烟雾图像增强更具挑战性，因为烟雾的不均匀性和高密度特性。

Conclusion: 深度学习架构可以显著提高烟雾和雾霾场景中模拟仪表图像的质量，增强后的输出图像可以成功进行后处理，实现仪表的自动自主读取，为应急响应提供有价值的技术支持。
...
TL;DR: 该论文提出了一种针对城市路边基础设施智能分析的领域自适应框架，通过结合开放词汇微调、LoRA适配和双模态RAG模块，显著提升了基础设施检测和属性识别的准确率。


<details>
  <summary>Details</summary>
Motivation: 通用视觉语言模型在处理城市路边基础设施的细粒度属性和工程标准时存在困难，导致在实际应用中性能不可靠，需要专门针对该领域的解决方案。

Method: 1) 在Grounding DINO上进行开放词汇微调，实现少监督下的资产定位；2) 在Qwen-VL上进行LoRA适配，用于深度语义属性推理；3) 引入双模态检索增强生成模块，动态检索权威行业标准和视觉示例以确保专业合规性。

Result: 在全面的城市路边场景数据集上评估，框架实现了58.9 mAP的检测性能和95.5%的属性识别准确率。

Conclusion: 该框架将通用视觉语言模型转化为专门的基础设施分析智能体，为智能基础设施监控提供了稳健的解决方案。
...
TL;DR: 论文提出WMReward方法，利用潜在世界模型（VJEPA-2）的物理先验作为奖励，在推理时对齐视频生成模型，显著提升生成视频的物理合理性。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的视频生成模型虽然能产生视觉上吸引人的内容，但经常违反基本物理原理，限制了其实用性。研究发现这种缺陷不仅源于预训练中物理理解的不足，还来自次优的推理策略。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶系统的预测和规划能力严重依赖基准测试，但现有基准测试缺乏密集交通场景，这限制了系统对复杂交通参与者交互的理解和建模能力。

Method: 与工业合作伙伴DeepScenario合作开发DeepUrban数据集，通过无人机在约100米高空采集高分辨率图像，提取3D交通对象，并补充详细的地图和场景信息。

Result: 将DeepUrban与nuScenes数据集结合使用时，车辆预测和规划的准确性显著提升，ADE/FDE指标分别提高了44.1%和44.3%。

Conclusion: DeepUrban数据集填补了密集城市交通场景的空白，能够有效提升自动驾驶系统的预测和规划能力，为更复杂的交互建模提供了重要支持。
...
TL;DR: 该论文提出了一种基于约当曲线定理的拓扑感知分割评估方法，用于评估分割掩码的结构连贯性，弥补传统评估指标在拓扑结构保持方面的不足。


<details>
  <summary>Details</summary>
Motivation: 传统分割评估指标（像素级、区域基或边界聚焦）难以捕捉分割的结构和拓扑连贯性。在医学影像或对象分割等实际应用中，边界小误差、空洞或碎片化预测可能导致高评分，但掩码无法保持对象的全局形状或连通性，这凸显了传统指标无法评估分割是否将图像划分为有意义的内外区域的局限性。

Method: 基于约当曲线定理并适应数字平面，提出了"约当可分割掩码"概念。通过数字拓扑和同调理论分析分割掩码，从掩码中提取4-曲线候选，使用贝蒂数验证其拓扑有效性。当候选形成数字4-曲线且β₀=β₁=1时（或其补集恰好分裂为两个8-连通分量），掩码被视为约当可分割。

Result: 该框架提供了一个数学严谨的无监督准则，用于评估分割掩码的结构连贯性。结合数字约当理论和同调不变量，为需要保持拓扑正确性的应用提供了有价值的替代评估方法。

Conclusion: 提出的拓扑感知分割评估方法弥补了传统指标的不足，特别适用于需要保持拓扑正确性的应用场景，为分割质量评估提供了新的数学理论基础。
...
TL;DR: 该论文提出了一种基于SHAP值的计算机视觉模型白盒攻击方法，通过降低输出置信度或诱导错误分类来破坏深度学习模型性能


<details>
  <summary>Details</summary>
Motivation: 对抗性规避攻击能够欺骗算法感知同时逃避人类感知，因为其对人眼不可察觉。这种攻击特别隐蔽，需要研究更有效的攻击方法来评估模型鲁棒性

Method: 利用SHAP值在推理阶段量化单个输入对输出的重要性，提出基于SHAP的白盒攻击方法，并与快速梯度符号方法进行比较

Result: 研究发现SHAP攻击在生成错误分类方面更加鲁棒，特别是在梯度隐藏场景下表现优于快速梯度符号方法

Conclusion: 基于SHAP值的白盒攻击方法为评估计算机视觉模型的鲁棒性提供了新工具，在梯度隐藏场景下具有更好的攻击效果
...
Method: 使用全自动流程：(1) 基于V-JEPA 2嵌入进行层次时间分割；(2) 生成组织为"标题树"的多级帧和片段标题；(3) 通过GPT-OSS-120B推理模型在多轮自我优化程序下聚合证据，输出结构化标注（简要/详细动作、执行者、简要/详细标题）。
...
TL;DR: RSATalker是首个利用3D高斯泼溅技术实现真实且社交感知的说话头生成框架，支持多轮对话，通过社交关系编码和三阶段训练实现高质量虚拟形象生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在明显局限：基于网格的3D方法能建模双人对话但缺乏真实纹理，基于大模型的2D方法产生自然外观但计算成本过高，而3DGS方法虽然高效真实但仅限于单人且忽略社交关系。需要一种既能高效渲染又能捕捉人际动态的解决方案。

Method: 首先从语音驱动基于网格的3D面部运动，然后将3D高斯绑定到网格面片上渲染高保真2D虚拟形象视频。提出社交感知模块，通过可学习查询机制将社交关系（血缘/非血缘、平等/不平等）编码为高层嵌入。设计三阶段训练范式，并构建带有社交关系标注的语音-网格-图像三元组数据集RSATalker。

Result: 大量实验表明，RSATalker在真实性和社交感知方面均达到最先进性能。代码和数据集将公开发布。

Conclusion: RSATalker是首个利用3DGS实现真实且社交感知的说话头生成框架，成功解决了现有方法在纹理质量、计算成本和社交关系建模方面的局限，为VR社交场景中的多轮对话提供了高效解决方案。
...
TL;DR: Molmo2是一个开源视频语言模型家族，在视频理解任务上达到开源模型最佳性能，并具备像素级指向和跟踪能力，超越了现有开源模型，在某些任务上甚至超过Gemini 3 Pro等专有模型。


<details>
  <summary>Details</summary>
Motivation: 当前最强的视频语言模型多为专有模型，开源模型要么依赖专有模型的合成数据，要么不公开训练数据和配方。开源社区缺乏改进视频语言模型的基础设施，且现有模型缺乏像素级定位能力。

Method: 收集了7个新视频数据集和2个多图像数据集，包括详细视频描述数据集、自由形式视频问答数据集、复杂查询对象跟踪数据集和创新视频指向数据集。采用高效打包和消息树编码方案，使用双向注意力机制和新的令牌权重策略。

Result: 8B模型在短视频、计数和字幕任务上优于同类开源模型，在长视频任务上具有竞争力。在视频定位任务上显著优于Qwen3-VL等开源模型（视频计数准确率35.5 vs 29.6），在某些任务上超过Gemini 3 Pro（视频指向F1分数38.4 vs 20.0，视频跟踪J&F分数56.2 vs 41.1）。

Conclusion: Molmo2为开源社区提供了无需依赖专有模型的高质量视频语言模型，填补了开源视频理解模型的空白，并在像素级定位能力上实现了突破。
...
TL;DR: CoMoVi：耦合3D人体动作与2D视频生成的协同生成框架，通过双分支扩散模型在单个去噪循环中同步生成动作与视频
...
Method: 提出CoMoVi框架：1）设计有效的2D人体动作表示，继承预训练视频扩散模型的强大先验；2）设计双分支扩散模型，通过相互特征交互和3D-2D交叉注意力耦合动作与视频生成；3）构建CoMoVi数据集，包含文本和动作标注的大规模真实世界人体视频

Result: 大量实验证明该方法在3D人体动作生成和视频生成任务中的有效性

Conclusion: 通过耦合3D人体动作与2D视频的生成过程，实现了两者在单个扩散去噪循环中的同步生成，为人体动作和视频生成提供了新的协同生成范式
...
TL;DR: CURVE是一个用于多文化和多语言视频推理的挑战性基准，包含18个全球地区的高质量人工标注，旨在解决现有基准中西方中心主义和英语主导的偏见问题。


<details>
  <summary>Details</summary>
Motivation: 当前视频模型基准主要基于西方中心数据和英语，存在显著评估偏见，需要建立能够评估多文化和多语言视频理解能力的基准。

Method: 1) 创建CURVE基准：包含18个全球地区文化视频的高质量人工标注，所有问题、答案和推理步骤均使用原生语言；2) 利用推理轨迹构建证据图；3) 提出基于证据图的迭代策略来识别细粒度推理错误。

Result: 当前最先进的视频大语言模型在CURVE上表现显著低于人类水平，主要错误源于对文化元素的视觉感知不足。

Conclusion: CURVE基准揭示了现有视频模型在多文化理解方面的局限性，为评估和改进视频模型的文化理解能力提供了重要工具。
...
TL;DR: 该研究通过数字化超过13,200个NEON地面甲虫标本，创建了多模态数据集，包括高分辨率图像和数字测量的鞘翅长度与宽度，为AI驱动的自动特征提取和物种识别提供基础。


<details>
  <summary>Details</summary>
Motivation: 全球特征数据库严重偏向脊椎动物和植物，限制了高多样性无脊椎动物（如地面甲虫）的全面生态分析。NEON收集了大量地面甲虫标本，但主要为物理收藏，限制了广泛的研究访问和大规模分析。

Method: 对来自美国大陆和夏威夷30个站点的13,200多个NEON地面甲虫标本进行高分辨率成像数字化，数字测量每个标本的鞘翅长度和宽度，并与手动测量结果进行验证。

Result: 数字特征提取达到亚毫米精度，确保生态和计算研究的可靠性。该数据集为AI驱动的自动物种识别和基于特征的研究提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型存在严重的视觉特征瓶颈，仅将视觉编码器输出连接到LLM输入，这种静态架构限制了LLM与分层视觉知识的全面对齐能力，无法准确整合局部细节与全局语义进行连贯推理。
...
Result: 在LLaVA-OneVision和LLaVA-1.5中集成CLI，在18个多样化基准测试中表现出显著的性能提升，证明了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖无监督先验（难以保持物体身份），要么使用过于严格的监督（限制内在属性的有意义变化）。需要一种既能保持物体身份又能实现有意义内在属性编辑的方法。

Method: 1) 使用宽松的训练目标：模型基于身份参考图像、描述目标内在属性的文本提示、背景图像和物体掩码进行条件生成。在推理时重用原始背景和掩码以限制外在变化。2) 引入视觉命名实体：细粒度的视觉身份类别，自动从大型公共图像数据集中提取，提供可扩展的身份保持监督。

Result: Alterbute在保持物体身份的内在属性编辑任务上优于现有方法。

Conclusion: 该方法通过结合宽松训练目标和视觉命名实体，实现了在保持物体身份的同时有效编辑内在属性的能力，为图像编辑提供了新的解决方案。
...
TL;DR: WildRayZer是一个自监督框架，用于解决动态环境下的新视角合成问题，通过分析-合成测试分离静态结构和动态区域，构建伪运动掩码来指导训练，并在新构建的动态数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 动态环境中的新视角合成面临挑战，因为相机和物体都在运动，破坏了静态NVS模型依赖的多视角一致性，导致重影、幻觉几何和不稳定的姿态估计等问题。

Method: 采用分析-合成测试方法：首先使用仅相机的静态渲染器解释刚性结构，然后从残差中识别瞬态区域。基于这些残差构建伪运动掩码，蒸馏出运动估计器，并用它来掩码输入token和门控损失梯度，使监督专注于跨视角背景补全。

Result: 实验表明，WildRayZer在瞬态区域去除和全帧NVS质量方面均优于基于优化的和前馈基线方法，仅需单次前馈推理。同时构建了Dynamic RealEstate10K数据集（15K动态序列）及其iPhone版本用于训练和评估。

Conclusion: WildRayZer成功解决了动态环境下的新视角合成问题，通过自监督方式分离静态结构和动态内容，在真实世界动态场景中实现了高质量的视图合成，为动态NVS提供了有效的解决方案。
...
### [56] [Who Owns My AI Twin? Data Ownership in a New World of Simulated Identities](https://arxiv.org/abs/2601.09877)
*Paulius Jurcys,Ashley Greenwald,Mark Fenwick,Valto Loikkanen,Sebastian Porsdam Mann,Brian D. Earp*

Main category: cs.HC

TL;DR: 本文主张自然人应被视为其AI双胞胎的道德和法律所有者，提出以人为中心的数据治理模式，强调个人主权和默认隐私原则，以应对AI双胞胎带来的法律伦理挑战。


<details>
  <summary>Details</summary>
Motivation: AI双胞胎作为包含个人知识、记忆、心理特征和行为模式的数字复制品，对数据治理和个人身份提出了新的法律和伦理挑战。现有法律框架优先考虑技术基础设施和平台控制，忽视了数据和个体自主权，存在结构性局限。

Method: 通过批判现有法律框架，提出以人为中心的数据治理模式，基于个人主权和默认隐私原则，重新构想AI驱动身份的社会契约。

Result: 论证了自然人应被认定为AI双胞胎的道德和法律所有者，AI双胞胎应被视为自我的亲密延伸而非专有技术制品。提出了能够增强个人能动性、促进公平数据管理、并使法律规范与AI双胞胎的社会技术现实更好对齐的治理方案。

Conclusion: 需要建立以人为中心的数据治理框架，承认个人对AI双胞胎的所有权，通过个人主权和默认隐私原则来应对AI双胞胎带来的法律伦理挑战，重新构想适应AI驱动身份的社会契约。
...
### [57] [LAMDA: Aiding Visual Exploration of Atomic Displacements in Molecular Dynamics Simulations](https://arxiv.org/abs/2601.09887)
*Rostyslav Hnatyshyn,Danny Perez,Gerik Scheuermann,Ross Maciejewski,Baldwin Nsonga*

Main category: cs.HC

TL;DR: LAMDA是一个视觉分析系统，用于帮助材料科学专家快速系统地探索原子模拟中的状态到状态转变，通过层次化分类转变并支持不同分辨率分析。


<details>
  <summary>Details</summary>
Motivation: 当前材料科学研究大量使用计算机模拟，产生海量原子尺度演化数据。识别原子位移的基本模式对于理解和预测物理性质演化至关重要，但可能的转变空间组合复杂性加上高通量模拟产生的海量数据，使得领域专家进行分析极其困难和耗时。现有视觉分析系统虽然擅长识别时间感兴趣区域，但将每个模拟时间步作为独立事件处理，不考虑时间步之间的原子位移行为。

Method: 提出LAMDA视觉分析系统，将转变进行层次化分类，为位移行为编目提供基础，并支持从非常宽泛的定性转变类别到非常狭窄的单位过程定义的不同分辨率分析。系统支持导航转变层次结构，可视化每个类别中不同转变之间的共性（通过表征局部原子环境的不变特征），并通过注释捕获用户输入来简化分析。

Result: 通过案例研究评估系统，并报告领域专家的发现。

Conclusion: LAMDA填补了现有系统的空白，使领域专家能够快速系统地探索状态到状态转变，为材料科学模拟数据的分析提供了有效的视觉分析工具。
...
### [58] [In-Browser Agents for Search Assistance](https://arxiv.org/abs/2601.09928)
*Saber Zerhoudi,Michael Granitzer*

Main category: cs.HC

TL;DR: 提出一个浏览器扩展，采用混合架构在客户端实现隐私保护的AI搜索辅助，结合自适应概率模型和小型语言模型，无需向外部服务传输用户数据。


<details>
  <summary>Details</summary>
Motivation: 当前AI搜索辅助需要用户将敏感浏览数据传输到外部服务，存在隐私风险和数据控制问题，需要在提供智能辅助的同时保护用户隐私。

Method: 开发浏览器扩展，采用完全在客户端运行的混合架构：1) 自适应概率模型从直接反馈中学习用户行为策略；2) 在浏览器中运行的小型语言模型，以概率模型为基础生成上下文感知建议。

Result: 通过为期三周的纵向用户研究（18名参与者）评估，结果显示这种隐私保护方法能有效适应用户个体行为，显著提高搜索效率。

Conclusion: 研究表明，在不损害用户隐私或数据控制的前提下，实现复杂的AI辅助是可行的，为隐私保护的智能搜索辅助提供了可行方案。
...
### [59] [Empowering Older Adults in Digital Technology Use with Foundation Models](https://arxiv.org/abs/2601.10018)
*Hasti Sharifi,Homaira Huda Shomee,Sourav Medya,Debaleena Chattopadhyay*

Main category: cs.HC

TL;DR: 研究探讨老年人技术支持的沟通障碍，使用AI重新表述查询来提升解决方案准确性，开发了首个老年人技术支持请求的合成数据集OATS。


<details>
  <summary>Details</summary>
Motivation: 老年人因技术术语不熟悉和认知变化，在寻求技术支持时难以清晰表达问题，导致沟通障碍。研究旨在探索AI方法缓解这些障碍，改善老年人数字应用使用体验。

Method: 1. 日记研究收集老年人异步技术查询；2. 反思性主题分析识别沟通障碍；3. 评估基础模型改写查询的效果；4. 两个对照实验：年轻人评估AI改写查询，老年人评估AI生成解决方案；5. 开发使用大语言模型生成OATS合成数据集的流程。

Result: 识别出四个关键沟通挑战：冗长、不完整、过度规范和规范不足。使用GPT-4o的提示链方法显著提高解决方案准确性（69% vs 46%）和谷歌搜索结果（69% vs 35%）。年轻人对AI改写查询的理解度更高（93.7% vs 65.8%）。老年人对回答情境问题的感知能力高（89.8%），遵循解决方案的能力强（94.7%）。OATS数据集显示出良好的保真度和表面效度。

Conclusion: 基础模型能有效解决老年人年龄相关的沟通障碍，提升技术支持效果。OATS数据集为开发更公平、更好地服务老年人群的AI系统提供了可扩展资源。
...
### [60] [Tables or Sankey Diagrams? Investigating User Interaction with Different Representations of Simulation Parameters](https://arxiv.org/abs/2601.10232)
...
TL;DR: 该研究评估了交互式桑基图相比传统电子表格界面在理解参数依赖关系方面的效果，发现桑基图能显著降低认知负荷和交互复杂度。


<details>
  <summary>Details</summary>
Motivation: 理解复杂参数依赖关系对于软件系统配置和维护至关重要，但传统的表格界面使工程师难以理解参数间的关联，导致工作流程低效、配置错误成本高和系统信任度降低。
...
Conclusion: 桑基图通过显式可视化参数关系，解决了软件可视化的核心挑战：帮助用户理解复杂系统配置而无需深入的工具特定知识。虽然通过CAE软件演示，但该研究对任何需要理解复杂参数关系的软件领域都有重要意义。
...
### [61] [Who Owns the Text? Design Patterns for Preserving Authorship in AI-Assisted Writing](https://arxiv.org/abs/2601.10236)
*Bohan Zhang,Chengke Bu,Paramveer S. Dhillon*

Main category: cs.HC

TL;DR: AI写作助手能降低认知负荷但会削弱作者所有权感，研究发现个性化风格能部分恢复所有权感，提出了五种保护作者身份的设计模式


<details>
  <summary>Details</summary>
Motivation: 研究AI写作助手在降低写作努力和提高流畅性时，如何影响作者的归属感和所有权感，探索设计选择如何缓解这种紧张关系

Method: 开发所有权感知的协同写作编辑器，提供按需句子级建议，测试两种设计选择：基于角色的指导和风格个性化。通过在线研究（N=176），参与者完成三项专业写作任务：无AI帮助的电子邮件、通用AI建议的提案、基于角色指导的求职信，其中一半参与者获得基于先前写作样本的个性化建议

Result: 在AI辅助任务中，心理所有权相对于无辅助写作下降约0.85-1.0分（7分量表），认知负荷降低约0.9分，质量评分总体相似。角色指导未能阻止所有权下降。风格个性化部分恢复所有权（约+0.43分）并增加AI文本融入率（+5个百分点）

Conclusion: 提出了五种设计模式来指导保护作者身份的写作工具：按需启动、微建议、声音锚定、受众支架和决策点来源，以平衡AI辅助与作者所有权之间的关系
...
### [62] [Developer Interaction Patterns with Proactive AI: A Five-Day Field Study](https://arxiv.org/abs/2601.10253)
*Nadine Kuo,Agnia Sergeyuk,Valerie Chen,Maliheh Izadi*

Main category: cs.HC

TL;DR: 研究通过实地实验发现，在IDE中主动提供AI代码质量建议时，开发者在工作流边界（如提交后）的接受率高达52%，而在任务中（如编辑被拒绝时）的拒绝率达62%。时机恰当的主动建议比被动建议节省一半以上的解读时间。


<details>
  <summary>Details</summary>
Motivation: 当前IDE中的AI编码工具通常需要手动提示和上下文管理，而能够预测开发者需求而无需显式调用的主动式AI助手仍未被充分探索。人类在日常工作中何时愿意接受这种主动AI帮助，是人机交互研究中一个未解决的问题。

Method: 进行了为期五天的实地研究，15名专业开发者使用集成在生产级IDE中的AI助手主动功能，该功能基于IDE内开发者活动提供代码质量建议。分析了5,732个交互点中的229次AI干预，了解主动建议在不同工作流阶段的接受情况、开发者体验及其感知影响。

Result: 发现人类对主动建议的接受度存在系统性模式：工作流边界（如提交后）的干预实现了52%的参与率，而任务中（如编辑被拒绝时）的干预有62%被拒绝。时机恰当的主动建议比被动建议的解读时间显著减少（45.4秒 vs 101.4秒，W = 109.00，r = 0.533，p = 0.0016），表明认知对齐增强。

Conclusion: 本研究为设计主动式编码助手提供了可操作的启示，包括如何安排干预时机、如何与开发者上下文对齐，以及在生产IDE中平衡AI自主性和用户控制。时机恰当的主动建议能显著减少认知负担，提高开发效率。
...
### [63] [Does Cognitive Load Affect Human Accuracy in Detecting Voice-Based Deepfakes?](https://arxiv.org/abs/2601.10383)
*Marcel Gohsen,Nicola Libera,Johannes Kiesel,Jan Ehlers,Benno Stein*

Main category: cs.HC

TL;DR: 研究探讨认知负荷对人类检测语音深度伪造能力的影响，发现低认知负荷不会损害检测能力，而同时暴露于次要刺激反而有助于检测任务。


<details>
  <summary>Details</summary>
Motivation: 深度伪造技术可能被滥用于恶意目的，如社交媒体上的虚假信息传播。现有研究大多在参与者专注于检测任务的条件下进行，未能反映社交媒体用户在认知负荷下的真实检测能力。

Method: 通过一项包含30名参与者的实证研究，调查认知负荷对人类检测语音深度伪造能力的影响。

Result: 研究结果表明，低认知负荷通常不会损害检测能力，同时暴露于次要刺激反而有助于人们在检测任务中的表现。

Conclusion: 在真实社交媒体环境中，认知负荷对人类检测深度伪造能力的影响比预期更复杂，低负荷不会损害检测，而次要刺激可能反而提高检测效果。
...
### [64] [LangLasso: Interactive Cluster Descriptions through LLM Explanation](https://arxiv.org/abs/2601.10458)
*Raphael Buchmüller,Dennis Collaris,Linhao Meng,Angelos Chatzimparmpas*

Main category: cs.HC

TL;DR: LangLasso使用大语言模型为聚类结果生成自然语言描述，提高非专家用户对聚类解释的可访问性


<details>
  <summary>Details</summary>
Motivation: 现有降维和聚类可视化方法虽然能揭示数据结构，但生成的坐标轴是复杂的非线性特征组合，缺乏语义可解释性，需要技术专业知识和大量人工努力

Method: 提出LangLasso方法，利用大语言模型为聚类生成交互式自然语言描述，补充现有可视化分析方法，使聚类解释对非专家用户更易理解

Result: 系统评估显示该方法生成的解释具有可靠性，为更广泛受众参与聚类解释提供了有效的第一步，工具已在线可用

Conclusion: LangLasso通过自然语言描述使聚类解释对非专家用户更易理解，并能整合数据集之外的外部上下文知识，扩展了聚类分析的可访问性
...
### [65] [AI Sycophancy: How Users Flag and Respond](https://arxiv.org/abs/2601.10467)
*Kazi Noshin,Syed Ishtiaque Ahmed,Sharifa Sultana*

Main category: cs.HC

TL;DR: 研究通过分析Reddit讨论，探索用户如何检测、缓解和感知LLM的谄媚行为，提出了ODR框架，发现谄媚行为的影响是情境依赖的而非普遍有害，特别对弱势群体具有情感支持价值。
...
Method: 通过分析Reddit讨论，开发了ODR框架（观察、检测、响应），研究用户对谄媚行为的体验。用户采用多种检测技术，包括跨平台比较和不一致性测试，并记录不同的缓解方法。

Result: 研究发现谄媚行为的影响是情境依赖的而非普遍有害。弱势群体（经历创伤、心理健康挑战或孤立的人）主动寻求并重视谄媚行为作为情感支持。用户对谄媚行为产生原因形成了技术和民间解释。

Conclusion: 研究挑战了"谄媚行为应被普遍消除"的假设，提出情境感知的AI设计，在肯定性互动的风险与益处之间取得平衡，并讨论了用户教育和透明度的影响。
...
### [66] [An Extension-Based Accessibility Framework for Making Blockly Accessible to Blind and Low-Vision Users](https://arxiv.org/abs/2601.10688)
*Rubel Hassan Mollik,Vamsi Krishna Kosuri,Hans Djalali,Stephanie Ludi,Aboubakar Mountapmbeme*

Main category: cs.HC

TL;DR: 提出基于扩展的可访问性框架(EAF)，使基于Blockly的积木编程环境能够为视障学习者提供无障碍支持，无需修改Blockly库本身。


<details>
  <summary>Details</summary>
Motivation: 当前Scratch、Code.org等积木编程环境在K-12计算机科学课程中广泛使用，但对盲人或视障学习者基本不可访问。现有解决方案需要修改Blockly库，难以在现有环境中应用，限制了采用。

Method: 提出基于扩展的可访问性框架(EAF)，采用模块化架构，可与现有Blockly环境无缝集成。包含三维层次导航模型（堆栈标签和积木编号）、基于模式的编辑防止意外修改，以及WAI-ARIA实现确保与外部屏幕阅读器兼容。

Result: 将EAF框架集成到两个积木编程环境中（覆盖177个测试用例），并与使用VoiceOver、JAWS和NVDA的4名参与者进行半结构化访谈。参与者报告相比默认Blockly键盘导航，空间方向更清晰，心智模型形成更容易。

Conclusion: EAF表明模块化架构可以在确保与现有积木编程环境兼容的同时，提供全面的可访问性支持。
...
### [67] [Interprofessional and Agile Development of Mobirobot: A Socially Assistive Robot for Pediatric Therapy Across Clinical and Therapeutic Settings](https://arxiv.org/abs/2601.09838)
...
TL;DR: Mobirobot是一个基于NAO平台的社会辅助机器人，通过个性化锻炼计划支持创伤、骨折或抑郁障碍儿童的康复活动，采用敏捷、以人为本的开发方法，在医院环境中进行迭代设计和部署。


<details>
  <summary>Details</summary>
Motivation: 社会辅助机器人在儿科临床环境中具有增强治疗参与度的潜力，但成功实施需要技术稳健性和情境敏感的共同设计解决方案。本研究旨在开发一个支持儿童康复活动的机器人系统。

Method: 采用敏捷、以人为本的开发方法，涉及多学科临床团队和最终用户共同开发。基于NAO平台，具有简单设置、可适应锻炼程序、交互指导、激励对话和图形用户界面等功能。在真实儿科外科和精神病学环境中进行早期整合。

Result: 在医院环境中部署后识别了关键设计需求和可用性约束，利益相关者反馈改进了交互设计、运动能力和技术配置。正在进行可行性研究，评估接受度、可用性和感知治疗效果，数据收集包括问卷、行为观察和员工-患者访谈。

Conclusion: Mobirobot展示了多专业、利益相关者主导的开发如何产生适合动态住院环境的社会辅助系统。早期发现强调了情境整合、稳健性和最小侵入设计的重要性。虽然存在传感器限制和患者招募等挑战，但该平台为进一步研究和临床应用提供了有希望的基础。
...
### [68] [How Human Motion Prediction Quality Shapes Social Robot Navigation Performance in Constrained Spaces](https://arxiv.org/abs/2601.09856)
*Andrew Stratton,Phani Teja Singamaneni,Pranav Goyal,Rachid Alami,Christoforos Mavrogiannis*

Main category: cs.RO

TL;DR: 研究探讨了在受限动态环境中，人类运动预测质量对机器人导航性能及人类体验的影响，通过用户研究发现传统评估指标不可靠，人类合作假设在受限环境中失效，机器人效率提升常以牺牲人类效率为代价。


<details>
  <summary>Details</summary>
Motivation: 为了让移动机器人更好地融入仓库、医院、制造工厂和家庭等人类环境，需要研究机器人在动态和空间受限环境中的导航问题。这要求机器人具备对人类运动行为的建模能力，但人类运动预测面临行为随机性、用户偏好差异和数据稀缺等挑战。

Method: 设计了机器人导航场景，涉及机器人在受限工作空间中与两名人类受试者互动。在两个不同世界区域的地点进行了用户研究（N=80），使用了两种不同的机器人平台，系统性地调查了人类运动预测质量对机器人导航性能、人类生产效率和印象的影响。

Result: 关键发现包括：1）广泛采用的平均位移误差（ADE）不是机器人导航性能和人类印象的可靠预测指标；2）在受限环境中，常见的人类合作假设失效，用户通常不回应机器人的合作行为，导致性能下降；3）机器人导航效率的提升往往以牺牲人类效率和舒适度为代价。

Conclusion: 在动态和空间受限环境中，机器人导航设计需要重新考虑评估指标和交互假设。传统的人类运动预测评估方法不足以反映实际导航性能，人类-机器人交互中的合作假设在受限环境中不成立，需要在机器人效率和人类体验之间寻求更好的平衡。
...
### [69] [SyncTwin: Fast Digital Twin Construction and Synchronization for Safe Robotic Grasping](https://arxiv.org/abs/2601.09920)
...
Conclusion: SyncTwin框架通过统一的数字孪生同步方法，为动态和遮挡环境下的机器人抓取提供了鲁棒且安全的解决方案。
...
### [70] [In-the-Wild Compliant Manipulation with UMI-FT](https://arxiv.org/abs/2601.09988)
...
<details>
  <summary>Details</summary>
Motivation: 商业力/扭矩传感器成本高、体积大且易碎，限制了大规模、力感知策略学习。需要一种能够收集手指级力数据的方法来训练柔顺操作策略。

Method: 开发UMI-FT手持数据收集平台，在每个手指上安装紧凑的六轴力/扭矩传感器，收集手指级力/扭矩测量数据以及RGB、深度和姿态数据。使用这些多模态数据训练自适应柔顺策略，预测位置目标、抓握力和刚度，在标准柔顺控制器上执行。
...
### [71] [CoCoPlan: Adaptive Coordination and Communication for Multi-robot Systems in Dynamic and Unknown Environments](https://arxiv.org/abs/2601.10116)
*Xintong Zhang,Junfeng Chen,Yuxiao Zhu,Bing Luo,Meng Guo*

Main category: cs.RO

TL;DR: CoCoPlan是一个多机器人系统框架，通过联合优化协作任务规划和团队间歇通信，在有限通信条件下提升动态时空任务分配的协调效率。


<details>
  <summary>Details</summary>
Motivation: 多机器人系统通过协调协作可大幅提升效率，但实际中全时通信很少可用，交互仅限于近距离交换。现有方法要么维持全时连接、依赖固定调度，要么采用成对协议，都无法在有限通信条件下有效适应动态时空任务分布，导致协调效果不佳。
...
### [72] [Terrain-Adaptive Mobile 3D Printing with Hierarchical Control](https://arxiv.org/abs/2601.10208)
...
Method: 开发了一个闭环感知-学习-执行系统，整合AI驱动的扰动预测、多模态传感器融合（IMU、视觉、深度传感器）和三层控制架构（路径规划、预测性底盘-机械臂协调、精密硬件执行）

Result: 在具有坡度和表面不规则性的户外地形实验中，实现了亚厘米级打印精度，同时保持平台完全移动性

Conclusion: AI与硬件的紧密集成为非结构化环境中的自主建造奠定了实用基础，实现了移动性与精度的平衡
...
### [73] [A Unified Framework for Kinematic Simulation of Rigid Foldable Structures](https://arxiv.org/abs/2601.10225)
*Dongwook Kwak,Geonhee Cho,Jiook Chung,Jinkyu Yang*

Main category: cs.RO

TL;DR: 提出了一种自动化方法，为任意刚性可折叠结构生成Pfaffian约束矩阵，统一分析各种折纸启发的结构


<details>
  <summary>Details</summary>
Motivation: 随着折纸启发结构从薄板发展到厚板、剪纸和多层实现，需要统一的运动学分析方法。目前缺乏能够整合这些结构环约束的通用方法。

Method: 从最小扩展数据模式出发，构建面-铰链图，提取捕获所有约束的最小环基，通过螺旋理论组装编码耦合旋转和平移环闭合的速度级约束矩阵。

Result: 该框架能够计算和可视化各种刚性可折叠结构的展开和折叠运动，同时消除了繁琐且容易出错的约束计算过程。

Conclusion: 提出了一种自动化、统一的刚性可折叠结构运动学分析方法，为复杂折纸启发结构的设计和分析提供了有效工具。
...
### [74] [The impact of tactile sensor configurations on grasp learning efficiency -- a comparative evaluation in simulation](https://arxiv.org/abs/2601.10268)
*Eszter Birtalan,Miklós Koller*

Main category: cs.RO

TL;DR: 研究通过模拟评估了6种不同触觉传感器配置（密度和布局）对强化学习性能的影响，发现特定配置在两个实验设置中均表现最佳，为机器人手设计提供指导。


<details>
  <summary>Details</summary>
Motivation: 触觉传感器在机器人领域应用日益重要，能提供接触面信息改善抓握稳定性，但目前机器人手设计中传感器配置差异很大且占用大量空间，需要系统评估不同配置对学习性能的影响。

Method: 使用双实验设置系统进行模拟评估，测试6种不同密度和布局的触觉传感器配置，确保结果不依赖于特定物理模拟器、机器人手模型或机器学习算法，通过强化学习评估性能。

Result: 结果显示存在设置特定的效果和跨6种传感器化模拟的通用效果，识别出一种配置在两个实验设置中始终表现最佳，为机器人手设计提供实证依据。

Conclusion: 该研究为未来机器人手（包括假肢）设计提供了有价值的指导，表明特定传感器配置能优化强化学习性能，有助于改善抓握稳定性和功能性。
...
### [75] [CHORAL: Traversal-Aware Planning for Safe and Efficient Heterogeneous Multi-Robot Routing](https://arxiv.org/abs/2601.10340)
...
Method: 从侦察飞行开始，使用开放词汇视觉模型构建度量-语义地图，识别需要近距离检查的区域，并为每个平台计算能力感知路径，然后整合到异构车辆路径规划中，联合分配检查任务并计算机器人轨迹。

Result: 在仿真和真实检查任务（使用三个机器人平台）中的实验表明，该方法通过明确考虑每个平台的导航能力，能够规划出更安全、更高效的路径。

Conclusion: 提出的CHORAL框架有效解决了异构机器人协调问题，将语义感知地图与能力感知路径规划相结合，提高了监测任务的性能和可行性，并开源以支持可重复性和多样化机器人团队的部署。
...
### [76] [FastStair: Learning to Run Up Stairs with Humanoid Robots](https://arxiv.org/abs/2601.10365)
*Yan Liu,Tao Yu,Haolin Song,Hongbo Zhu,Nianzong Hu,Yuzhi Hao,Xiuyong Yao,Xizhe Zang,Hua Chen,Jie Zhao*

Main category: cs.RO

TL;DR: FastStair是一个结合模型规划与强化学习的框架，通过多阶段训练实现人形机器人快速稳定爬楼梯，在竞赛中获胜并达到1.65m/s的爬升速度。


<details>
  <summary>Details</summary>
Motivation: 人形机器人爬楼梯面临敏捷性与稳定性的双重挑战：纯强化学习容易产生不安全行为，而模型规划又过于保守限制速度。需要结合两者的优势实现既快速又稳定的楼梯攀爬。

Method: 提出多阶段学习框架：1) 将并行模型立足点规划器集成到RL训练循环中，引导探索并预训练安全基础策略；2) 通过微调创建速度专家策略；3) 使用LoRA集成实现全速度范围的平滑操作。

Result: 在Oli人形机器人上实现稳定爬楼梯，最高速度达1.65m/s，12秒内完成33级螺旋楼梯（每级17cm）。该方案在广州塔机器人爬楼梯竞赛中获得冠军。

Conclusion: FastStair成功结合了模型规划的结构化安全性与强化学习的动态能力，实现了人形机器人高速稳定的楼梯攀爬，为动态运动控制提供了有效框架。
...
### [77] [Online identification of nonlinear time-varying systems with uncertain information](https://arxiv.org/abs/2601.10379)
...
Method: 提出贝叶斯回归符号学习框架，将在线符号发现建模为统一的概率状态空间模型，引入稀疏马蹄先验，将模型选择转化为贝叶斯推断任务，实现系统识别和不确定性量化；推导带遗忘因子的在线递归算法，建立保证后验分布适定性的精确递归条件

Result: 框架实现了可解释的概率预测和在线学习；提供了严格的收敛性分析，证明在持续激励条件下参数估计的收敛性；案例研究验证了框架的有效性
...
### [78] [Cell Behavior Video Classification Challenge, a benchmark for computer vision methods in time-lapse microscopy](https://arxiv.org/abs/2601.10250)
...
TL;DR: 细胞行为视频分类挑战赛（CBVCC）评估了35种方法，比较了基于跟踪特征分类、端到端深度学习架构和特征集成三种方法在显微镜视频细胞行为分类中的表现。


<details>
  <summary>Details</summary>
Motivation: 显微镜视频中复杂细胞行为的分类对于理解和量化生物过程动态至关重要，但这是计算机视觉的前沿领域，需要有效建模无刚性边界物体的形状和运动、从整个图像序列提取层次时空特征，并处理视场中的多个物体。

Method: 组织了细胞行为视频分类挑战赛（CBVCC），基准测试了35种方法，基于三种主要方法：1）基于跟踪衍生特征的分类；2）端到端深度学习架构直接从整个视频序列学习时空特征，无需显式细胞跟踪；3）将跟踪衍生特征与图像衍生特征集成。

Result: 挑战赛评估了35种不同方法的性能，讨论了参与者的结果，并比较了每种方法的潜力和局限性。

Conclusion: 该研究为促进计算机视觉方法在细胞动力学研究中的发展提供了基础，通过系统比较不同方法在细胞行为视频分类任务中的表现，为未来方法开发提供了重要参考。
...
### [79] [An effective interactive brain cytoarchitectonic parcellation framework using pretrained foundation model](https://arxiv.org/abs/2601.10412)
*Shiqi Zhang,Fang Xu,Pengcheng Zhou*

Main category: eess.IV

TL;DR: 提出基于DINOv3视觉Transformer的交互式细胞构筑分区框架，通过多层级特征融合、轻量解码器和实时用户引导训练，实现高效的人机协作脑区分割


<details>
  <summary>Details</summary>
Motivation: 细胞构筑分区是脑结构解剖学分析的基础，但现有深度学习方法的性能受限于训练标签稀缺以及染色和成像条件的变异性，需要更高效的解决方案

Method: 结合多层级DINOv3特征融合、轻量分割解码器和基于稀疏涂鸦的实时用户引导训练，构建交互式细胞构筑分区框架

Result: 相比从头训练nnU-Net，基于DINOv3的迁移学习显著提升性能；DINOv3提取的特征显示清晰的解剖对应关系，方法在稀疏标签下具有实用价值

Conclusion: 基础模型驱动的交互式分割方法具有可扩展和高效的细胞构筑分区潜力，为大规模脑区分析提供了新途径
